...
kefir_bool_t kefir_ast_function_specifier_is_inline(kefir_ast_function_specifier_t);

#define KEFIR_AST_TYPE_SAME(type1, type2) ((type1) == (type2) || (type1)->ops.same((type1), (type2)))
#define KEFIR_AST_TYPE_COMPATIBLE(type_traits, type1, type2)                   \
    ((type1) == (type2) || KEFIR_AST_TYPE_SAME((type1), (kefir_ast_zero_unqualified_type(type2))) || \
...
typedef enum kefir_ast_type_data_model_classification {
    ...
    KEFIR_AST_TYPE_DATA_MODEL_INT128,
    KEFIR_AST_TYPE_DATA_MODEL_BITINT,
    KEFIR_AST_TYPE_DATA_MODEL_FLOAT,
    KEFIR_AST_TYPE_DATA_MODEL_DOUBLE,
    KEFIR_AST_TYPE_DATA_MODEL_LONG_DOUBLE,
    KEFIR_AST_TYPE_DATA_MODEL_DECIMAL32,
    KEFIR_AST_TYPE_DATA_MODEL_DECIMAL64,
    KEFIR_AST_TYPE_DATA_MODEL_DECIMAL128,
...
kefir_result_t kefir_ast_type_apply_qualification(struct kefir_mem *, struct kefir_ast_type_bundle *,
                                                  kefir_c_language_standard_version_t, const struct kefir_ast_type *,
                                                  const struct kefir_ast_type_qualification *,
                                                  const struct kefir_ast_type **);
const struct kefir_ast_type *kefir_ast_type_traits_quarter_integer_type(const struct kefir_ast_type_traits *,
                                                                        kefir_bool_t);
const struct kefir_ast_type *kefir_ast_type_traits_half_integer_type(const struct kefir_ast_type_traits *,
                                                                     kefir_bool_t);
const struct kefir_ast_type *kefir_ast_type_traits_single_integer_type(const struct kefir_ast_type_traits *,
                                                                       kefir_bool_t);
const struct kefir_ast_type *kefir_ast_type_traits_double_integer_type(const struct kefir_ast_type_traits *,
                                                                       kefir_bool_t);
