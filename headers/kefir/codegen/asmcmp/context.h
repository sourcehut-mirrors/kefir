...
typedef enum kefir_asmcmp_virtual_register_type {
    ...
    KEFIR_ASMCMP_VIRTUAL_REGISTER_SPILL_SPACE,
    KEFIR_ASMCMP_VIRTUAL_REGISTER_LOCAL_VARIABLE,
    KEFIR_ASMCMP_VIRTUAL_REGISTER_PAIR,
    KEFIR_ASMCMP_VIRTUAL_REGISTER_IMMEDIATE_INTEGER,
...
#define KEFIR_ASMCMP_MAKE_INT(_value)                                      \
    ((struct kefir_asmcmp_value) {.type = KEFIR_ASMCMP_VALUE_TYPE_INTEGER, \
                                  .int_immediate = (_value),               \
                                  .immediate_variant = KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT})
...
#define KEFIR_ASMCMP_MAKE_INDIRECT_LOCAL_AREA(_var, _offset, _variant)                         \
    ((struct kefir_asmcmp_value) {.type = KEFIR_ASMCMP_VALUE_TYPE_INDIRECT,                    \
                                  .indirect = {.type = KEFIR_ASMCMP_INDIRECT_LOCAL_AREA_BASIS, \
                                               .base.local_variable_id = (_var),               \
                                               .offset = (_offset),                            \
                                               .variant = (_variant)}})
...
typedef struct kefir_asmcmp_virtual_register {
    ...
    union {
        ...
        struct {
            ...
            kefir_asmcmp_virtual_register_index_t virtual_registers[2];
        } pair;
    } parameters;
...
kefir_result_t kefir_asmcmp_virtual_register_new_direct_spill_space_allocation(struct kefir_mem *,
                                                                               ...
                                                                               kefir_asmcmp_virtual_register_index_t *);
kefir_result_t kefir_asmcmp_virtual_register_new_spill_space(struct kefir_mem *, struct kefir_asmcmp_context *,
                                                             kefir_size_t, kefir_size_t,
                                                             kefir_asmcmp_virtual_register_index_t *);
...
kefir_result_t kefir_asmcmp_virtual_register_new_pair(struct kefir_mem *, struct kefir_asmcmp_context *,
                                                      ...
                                                      kefir_asmcmp_virtual_register_index_t,
                                                      kefir_asmcmp_virtual_register_index_t *);
kefir_result_t kefir_asmcmp_virtual_register_new_immediate_integer(struct kefir_mem *, struct kefir_asmcmp_context *,
                                                                   kefir_int64_t,
                                                                   kefir_asmcmp_virtual_register_index_t *);
