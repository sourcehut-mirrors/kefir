...
typedef kefir_int32_t kefir_lexer_char_t;
#define KEFIR_LEXER_SOURCE_CURSOR_EOF ((kefir_lexer_char_t) -1)
...
typedef struct kefir_lexer_source_cursor {
    ...
    kefir_lexer_char_t carriage_return_char;
    kefir_lexer_char_t newline_char;
...
kefir_result_t kefir_lexer_source_cursor_init(struct kefir_lexer_source_cursor *, const char *, kefir_size_t,
                                              const char *);
kefir_lexer_char_t kefir_lexer_source_cursor_at(const struct kefir_lexer_source_cursor *, kefir_size_t);
kefir_result_t kefir_lexer_source_cursor_next(struct kefir_lexer_source_cursor *, kefir_size_t);
kefir_result_t kefir_lexer_source_cursor_skip(struct kefir_lexer_source_cursor *, kefir_size_t);
...
kefir_result_t kefir_lexer_cursor_match_string(const struct kefir_lexer_source_cursor *, const char *);
