...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    struct kefir_ir_function_decl *decl =
        kefir_ir_module_new_function_declaration(mem, &module, "fact", func_params, false, func_returns);
    ...
    REQUIRE_OK(kefir_irbuilder_type_append(mem, locals, KEFIR_IR_TYPE_INT64, 0, 0));
    ...
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "1", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_VALUE,
        &(struct kefir_ir_inline_assembly_parameter_constraints) {.general_purpose_register = true}, decl_params,
        func_params, 0, 1, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "2", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_WRITE_LOCATION,
        &(struct kefir_ir_inline_assembly_parameter_constraints) {.general_purpose_register = true}, decl_result,
        func_returns, 0, 0, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_clobber(mem, &module.symbols, inline_asm1, "rcx"));
    REQUIRE_OK(kefir_ir_inline_assembly_add_clobber(mem, &module.symbols, inline_asm1, "rdx"));
