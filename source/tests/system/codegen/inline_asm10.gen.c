...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    struct kefir_ir_function_decl *decl =
        kefir_ir_module_new_function_declaration(mem, &module, "add", func_params, false, func_returns);
    REQUIRE(decl != NULL, KEFIR_INTERNAL_ERROR);
    struct kefir_ir_function *func = kefir_ir_module_new_function(mem, &module, decl, KEFIR_ID_NONE, 1024);
    REQUIRE(func != NULL, KEFIR_INTERNAL_ERROR);

    REQUIRE_OK(kefir_irbuilder_type_append(mem, decl_params, KEFIR_IR_TYPE_WORD, 0, 0));
    REQUIRE_OK(kefir_irbuilder_type_append(mem, decl_params, KEFIR_IR_TYPE_WORD, 0, 0));
    ...
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        ...
        &(struct kefir_ir_inline_assembly_parameter_constraints) { .general_purpose_register = true }, decl_params, func_params, 2, 0, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "3", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_STORE,
        &(struct kefir_ir_inline_assembly_parameter_constraints) { .memory_location = true }, decl_params, func_params, 1, 1, NULL));
