...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module,
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry1, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "a"), NULL));
    ...
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &entry4->declaration.specifiers,
                                                          kefir_ast_type_specifier_char(mem, NULL)));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry4, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, NULL),
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry5, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "d"),
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry6, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "e"),
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 13))));
