...
static kefir_result_t define_conditional_function(struct kefir_mem *mem, struct function *func,
    ...
    struct kefir_ast_compound_statement *outer_body = kefir_ast_new_compound_statement_noarena(mem);

    struct kefir_ast_declaration *declarationJ = kefir_ast_new_single_declaration_noarena(
    ...
    struct kefir_ast_node_base *inner_body = KEFIR_AST_NODE_BASE(kefir_ast_new_expression_statement_noarena(
        mem,
    ...
    struct kefir_ast_node_base *inner_loop =
        KEFIR_AST_NODE_BASE(kefir_ast_new_do_while_statement_noarena(mem, inner_condition, inner_body));
    REQUIRE_OK(kefir_ast_compound_statement_append(mem, outer_body, inner_loop));

    struct kefir_ast_node_base *outer_tail = KEFIR_AST_NODE_BASE(kefir_ast_new_expression_statement_noarena(
        mem, KEFIR_AST_NODE_BASE(kefir_ast_new_unary_operation_noarena(
                 mem, KEFIR_AST_OPERATION_PREFIX_INCREMENT,
                 KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, context_manager->current->symbols, "i"))))));
    REQUIRE_OK(kefir_ast_compound_statement_append(mem, outer_body, outer_tail));

    struct kefir_ast_node_base *outer_loop =
        KEFIR_AST_NODE_BASE(kefir_ast_new_do_while_statement_noarena(mem, outer_condition, KEFIR_AST_NODE_BASE(outer_body)));
    REQUIRE_OK(kefir_ast_compound_statement_append(mem, compound0, outer_loop));
