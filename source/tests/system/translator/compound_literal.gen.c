...
static kefir_result_t define_compound_literal_function(struct kefir_mem *mem, struct function *func,
    ...
    struct kefir_ast_structure_specifier *specifier0 = kefir_ast_structure_specifier_init_noarena(mem, NULL, NULL, true);
    struct kefir_ast_structure_declaration_entry *entryU1 = kefir_ast_structure_declaration_entry_alloc_noarena(mem);
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entryU1, kefir_ast_declarator_identifier_noarena(mem, context_manager->current->symbols, "fp64"), NULL));
    REQUIRE_OK(kefir_ast_structure_specifier_append_entry(mem, specifier0, entryU1));

    struct kefir_ast_structure_declaration_entry *entryU2 = kefir_ast_structure_declaration_entry_alloc_noarena(mem);
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        ...
        kefir_ast_declarator_array_noarena(mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
                                   ...
                                   kefir_ast_declarator_identifier_noarena(mem, context_manager->current->symbols, "fp32")),
        NULL));
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        ...
        kefir_ast_declarator_array_noarena(mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
                                   ...
                                   kefir_ast_declarator_identifier_noarena(mem, context_manager->current->symbols, "string")),
        NULL));
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry2, kefir_ast_declarator_identifier_noarena(mem, context_manager->current->symbols, "length"), NULL));
    ...
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(
        ...
        kefir_ast_alignment_specifier(mem, NULL, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 16)))));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry3, kefir_ast_declarator_identifier_noarena(mem, context_manager->current->symbols, "padding"), NULL));
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        ...
        kefir_ast_declarator_pointer_noarena(mem,
                                     kefir_ast_declarator_identifier_noarena(mem, context_manager->current->symbols, "ptr")),
        NULL));
    ...
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &entry5->declaration.specifiers,
                                                          kefir_ast_type_specifier_union(mem, NULL, specifier0)));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry5,
        kefir_ast_declarator_array_noarena(mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
                                   KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 4)),
                                   kefir_ast_declarator_identifier_noarena(mem, context_manager->current->symbols, "floats")),
        NULL));
    ...
    REQUIRE_OK(kefir_ast_initializer_list_append(
        ...
        kefir_ast_new_initializer_member_designation_noarena(mem, context_manager->current->symbols, "padding", NULL),
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 1010)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &compound->initializer->list, NULL,
        kefir_ast_new_expression_initializer_noarena(
            mem, KEFIR_AST_NODE_BASE(KEFIR_AST_MAKE_STRING_LITERAL_MULTIBYTE(mem, STRING)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &compound->initializer->list,
        kefir_ast_new_initializer_member_designation_noarena(mem, context_manager->current->symbols, "length", NULL),
        kefir_ast_new_expression_initializer_noarena(mem,
                                             KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, strlen(STRING))))));

    struct kefir_ast_initializer *list1 = kefir_ast_new_list_initializer_noarena(mem);
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &list1->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_double_noarena(mem, 4.0013)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &list1->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_double_noarena(mem, 19.88263)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &list1->list,
        kefir_ast_new_initializer_index_designation_noarena(
            ...
            kefir_ast_new_initializer_member_designation_noarena(
                ...
                kefir_ast_new_initializer_index_designation_noarena(
                    mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 2)), NULL))),
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_float_noarena(mem, 1.02f)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &list1->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_float_noarena(mem, 2.04f)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &list1->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_double_noarena(mem, 1e5)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &compound->initializer->list,
        kefir_ast_new_initializer_member_designation_noarena(mem, context_manager->current->symbols, "floats", NULL), list1));
