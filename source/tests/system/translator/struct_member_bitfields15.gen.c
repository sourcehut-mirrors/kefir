...
static kefir_result_t define_sum_function(struct kefir_mem *mem, struct function *func,
    ...
    struct kefir_ast_type_name *type_name1 =
        kefir_ast_new_type_name_noarena(mem, kefir_ast_declarator_identifier_noarena(mem, context_manager->current->symbols, NULL));
    ...
    struct kefir_ast_node_base *add5 = KEFIR_AST_NODE_BASE(kefir_ast_new_binary_operation_noarena(
        ...
        KEFIR_AST_NODE_BASE(kefir_ast_new_struct_indirect_member_noarena(
            ...
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, context_manager->current->symbols, "param")), "e")),
        KEFIR_AST_NODE_BASE(kefir_ast_new_struct_indirect_member_noarena(
            mem, context_manager->current->symbols,
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, context_manager->current->symbols, "param")), "f"))));

    struct kefir_ast_node_base *add4 = KEFIR_AST_NODE_BASE(kefir_ast_new_binary_operation_noarena(
        mem, KEFIR_AST_OPERATION_ADD,
        KEFIR_AST_NODE_BASE(kefir_ast_new_struct_indirect_member_noarena(
            mem, context_manager->current->symbols,
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, context_manager->current->symbols, "param")), "d")),
        add5));
    ...
    struct kefir_ast_node_base *add3 = KEFIR_AST_NODE_BASE(kefir_ast_new_binary_operation_noarena(
        ...
        KEFIR_AST_NODE_BASE(kefir_ast_new_struct_indirect_member_noarena(
            mem, context_manager->current->symbols,
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, context_manager->current->symbols, "param")), "c")),
        add4));
    ...
    struct kefir_ast_node_base *add2 = KEFIR_AST_NODE_BASE(kefir_ast_new_binary_operation_noarena(
        ...
        KEFIR_AST_NODE_BASE(kefir_ast_new_struct_indirect_member_noarena(
            mem, context_manager->current->symbols,
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, context_manager->current->symbols, "param")), "b")),
        add3));
    ...
    struct kefir_ast_node_base *add1 = KEFIR_AST_NODE_BASE(kefir_ast_new_binary_operation_noarena(
        ...
        KEFIR_AST_NODE_BASE(kefir_ast_new_struct_indirect_member_noarena(
            mem, context_manager->current->symbols,
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, context_manager->current->symbols, "param")), "a")),
        add2));
...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module,
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry1, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "a"),
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 33))));
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry2, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "b"),
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 8))));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry2, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, NULL),
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry3, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "c"),
    ...
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &entry4->declaration.specifiers,
                                                          kefir_ast_type_specifier_long(mem, NULL)));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry4, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "d"),
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 17))));
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry5, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "e"),
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 22))));
    ...
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &entry6->declaration.specifiers,
                                                          kefir_ast_type_specifier_char(mem, NULL)));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry6, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, NULL),
    ...
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &entry7->declaration.specifiers,
                                                          kefir_ast_type_specifier_long(mem, NULL)));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry7, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "f"),
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 39))));
