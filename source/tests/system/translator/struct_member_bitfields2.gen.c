...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module,
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry1, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "f2"),
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 5))));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry1, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, NULL),
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 0))));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry1, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "f3"),
    ...
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &entry2->declaration.specifiers,
                                                          kefir_ast_type_specifier_signed(mem)));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry2, kefir_ast_declarator_identifier_noarena(mem, context_manager.current->symbols, "f4"),
