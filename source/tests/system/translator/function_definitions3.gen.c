...
static struct kefir_ast_function_definition *define_sum_function(struct kefir_mem *mem,
    ...
    REQUIRE(kefir_ast_declarator_specifier_list_append(mem, &function1_param1->specifiers,
                                                       kefir_ast_type_specifier_struct(mem, NULL, specifier1)) == KEFIR_OK,
            NULL);
    REQUIRE(
            ...
            KEFIR_AST_NODE_BASE(kefir_ast_new_expression_statement_noarena(
                mem, KEFIR_AST_NODE_BASE(kefir_ast_new_simple_assignment_noarena(
                         ...
                         KEFIR_AST_NODE_BASE(kefir_ast_new_struct_indirect_member_noarena(
                             mem, context->symbols,
                             KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, context->symbols, "value")), "z")),
                         KEFIR_AST_NODE_BASE(kefir_ast_new_binary_operation_noarena(
                             mem, KEFIR_AST_OPERATION_ADD,
                             KEFIR_AST_NODE_BASE(kefir_ast_new_struct_indirect_member_noarena(
                                 mem, context->symbols,
                                 KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, context->symbols, "value")), "x")),
                             KEFIR_AST_NODE_BASE(kefir_ast_new_struct_indirect_member_noarena(
                                 mem, context->symbols,
