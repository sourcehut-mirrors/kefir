...
DEFINE_CASE(ast_enum_declaration1, "AST Declarations - enum declaration #1") {
    ...
    struct kefir_ast_enum_specifier *specifier =
        kefir_ast_enum_specifier_init_noarena(&kft_mem, &symbols, "test123", false, NULL);
    ...
    ASSERT_NOK(kefir_ast_enum_specifier_append(&kft_mem, specifier, &symbols, NULL, NULL, NULL));
    ASSERT_NOK(kefir_ast_enum_specifier_append(&kft_mem, specifier, &symbols, "A", NULL, NULL));

    struct kefir_ast_node_base *value1 = KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(&kft_mem, 1));
    ASSERT_NOK(kefir_ast_enum_specifier_append(&kft_mem, specifier, &symbols, "A", value1, NULL));
...
DEFINE_CASE(ast_enum_declaration2, "AST Declarations - enum declaration #2") {
    ...
    struct kefir_ast_enum_specifier *specifier =
        kefir_ast_enum_specifier_init_noarena(&kft_mem, &symbols, "some_enum", true, NULL);
    ASSERT(specifier != NULL);
    ...
    ASSERT_NOK(kefir_ast_enum_specifier_append(&kft_mem, specifier, &symbols, NULL, NULL, NULL));
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier, &symbols, "field1", NULL, NULL));
    ...
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier, &symbols, "field4", NULL, NULL));
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier, &symbols, "field5",
                                              KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(&kft_mem, 20)), NULL));
