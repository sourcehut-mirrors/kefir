...
DEFINE_CASE(ast_decimal_types1, "AST Type analysis - decimal types #1") {
    ...
    ASSERT(
        KEFIR_AST_TYPE_SAME(KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type128, type128), type128));
...
DEFINE_CASE(ast_decimal_types2, "AST Type analysis - decimal types #2") {
    ...
    ASSERT(KEFIR_AST_TYPE_SAME(kefir_ast_type_decimal32(),
                               kefir_ast_type_common_arithmetic(
                                   type_traits, kefir_ast_type_decimal32(), (struct kefir_ast_bitfield_properties) {0},
    ...
    ASSERT(KEFIR_AST_TYPE_SAME(kefir_ast_type_decimal64(),
                               kefir_ast_type_common_arithmetic(
                                   type_traits, kefir_ast_type_decimal64(), (struct kefir_ast_bitfield_properties) {0},
                                   kefir_ast_type_decimal32(), (struct kefir_ast_bitfield_properties) {0})));
    ...
    ASSERT(KEFIR_AST_TYPE_SAME(kefir_ast_type_decimal128(),
                               kefir_ast_type_common_arithmetic(
                                   type_traits, kefir_ast_type_decimal128(), (struct kefir_ast_bitfield_properties) {0},
                                   kefir_ast_type_decimal32(), (struct kefir_ast_bitfield_properties) {0})));
    ...
    for (kefir_size_t i = 0; i < sizeof(int_types) / sizeof(int_types[0]); i++) {
        ASSERT(KEFIR_AST_TYPE_SAME(
            ...
            kefir_ast_type_common_arithmetic(type_traits, kefir_ast_type_decimal32(),
        ...
        ASSERT(KEFIR_AST_TYPE_SAME(
            ...
            kefir_ast_type_common_arithmetic(type_traits, kefir_ast_type_decimal64(),
                                             (struct kefir_ast_bitfield_properties) {0}, int_types[i],
                                             (struct kefir_ast_bitfield_properties) {0})));
        ASSERT(KEFIR_AST_TYPE_SAME(
            kefir_ast_type_decimal64(),
        ...
        ASSERT(KEFIR_AST_TYPE_SAME(
            ...
            kefir_ast_type_common_arithmetic(type_traits, kefir_ast_type_decimal128(),
                                             (struct kefir_ast_bitfield_properties) {0}, int_types[i],
                                             (struct kefir_ast_bitfield_properties) {0})));
        ASSERT(KEFIR_AST_TYPE_SAME(
            kefir_ast_type_decimal128(),
    ...
    const struct kefir_ast_type *float_types[] = {
        kefir_ast_type_float(),         kefir_ast_type_double(),         kefir_ast_type_long_double(),
        kefir_ast_type_complex_float(), kefir_ast_type_complex_double(), kefir_ast_type_complex_long_double()};
    for (kefir_size_t i = 0; i < sizeof(float_types) / sizeof(float_types[0]); i++) {
        ASSERT(kefir_ast_type_common_arithmetic(type_traits, kefir_ast_type_decimal32(),
                                                (struct kefir_ast_bitfield_properties) {0}, float_types[i],
        ...
        ASSERT(kefir_ast_type_common_arithmetic(type_traits, kefir_ast_type_decimal64(),
                                                (struct kefir_ast_bitfield_properties) {0}, float_types[i],
                                                (struct kefir_ast_bitfield_properties) {0}) == NULL);
        ASSERT(kefir_ast_type_common_arithmetic(type_traits, float_types[i], (struct kefir_ast_bitfield_properties) {0},
                                                kefir_ast_type_decimal64(),
        ...
        ASSERT(kefir_ast_type_common_arithmetic(type_traits, kefir_ast_type_decimal128(),
                                                (struct kefir_ast_bitfield_properties) {0}, float_types[i],
                                                (struct kefir_ast_bitfield_properties) {0}) == NULL);
        ASSERT(kefir_ast_type_common_arithmetic(type_traits, float_types[i], (struct kefir_ast_bitfield_properties) {0},
                                                kefir_ast_type_decimal128(),
