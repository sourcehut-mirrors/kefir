...
DEFINE_CASE(ast_structure_declaration1, "AST Declarations - structure declarations #1") {
    ...
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(
        &kft_mem, entry1, kefir_ast_declarator_identifier_noarena(&kft_mem, &symbols, "field1"), NULL));
...
DEFINE_CASE(ast_structure_declaration2, "AST Declarations - structure declarations #2") {
    ...
    struct kefir_ast_structure_specifier *specifier1 =
        kefir_ast_structure_specifier_init_noarena(&kft_mem, &symbols, "struct_one", true);
    ASSERT(specifier1 != NULL);
    ...
    do {
        ...
        ASSERT_OK(kefir_ast_structure_declaration_entry_append(
            &kft_mem, entry1, kefir_ast_declarator_identifier_noarena(&kft_mem, &symbols, "a"), NULL));
    ...
    do {
        ...
        ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &entry2->declaration.specifiers,
                                                             kefir_ast_type_specifier_long(&kft_mem, NULL)));
        ASSERT_OK(kefir_ast_structure_declaration_entry_append(
            &kft_mem, entry2, kefir_ast_declarator_identifier_noarena(&kft_mem, &symbols, "b"), NULL));
        ASSERT_OK(kefir_ast_structure_declaration_entry_append(
            &kft_mem, entry2, kefir_ast_declarator_identifier_noarena(&kft_mem, &symbols, "c"),
    ...
    do {
        ...
        ASSERT_OK(kefir_ast_structure_declaration_entry_append(
            ...
            kefir_ast_declarator_array_noarena(&kft_mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
                                       ...
                                       kefir_ast_declarator_identifier_noarena(&kft_mem, &symbols, "last")),
            NULL));
    ...
    do {
        ...
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(entry_declarator->bitwidth))->type == KEFIR_AST_INT_CONSTANT);
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(entry_declarator->bitwidth))->value.integer == 10);
    ...
    do {
        ...
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(entry->static_assertion->condition))->type ==
               KEFIR_AST_BOOL_CONSTANT);
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(entry->static_assertion->condition))->value.boolean);
    } while (0);
    ...
    do {
        ...
        ASSERT(kefir_ast_type_qualifier_list_iter(&entry_declarator->declarator->array->type_qualifiers, &qual_iter, NULL) == KEFIR_ITERATOR_END);
