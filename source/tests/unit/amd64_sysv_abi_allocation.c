...
DEFINE_CASE(amd64_sysv_abi_allocation_test1, "AMD64 System V ABI - parameter allocation") {
    ...
    ASSERT_OK(kefir_amd64_sysv_parameter_classify(&kft_mem, &type, &layout.layout, &allocation));
    struct kefir_amd64_sysv_parameter_location location = {0};
    ASSERT_OK(kefir_amd64_sysv_parameter_allocate(&kft_mem, &type, &layout.layout, &allocation, &location));
...
DEFINE_CASE(amd64_sysv_abi_allocation_test6, "AMD64 System V ABI - parameter allocation #6") {
    ...
    ASSERT_OK(kefir_amd64_sysv_parameter_classify(&kft_mem, &type2, &layout.layout, &allocation));
    struct kefir_amd64_sysv_parameter_location location = {0};
    ASSERT_OK(kefir_amd64_sysv_parameter_allocate(&kft_mem, &type2, &layout.layout, &allocation, &location));
