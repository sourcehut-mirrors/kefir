...
DEFINE_CASE(ast_translator_flow_control_point1, "AST Translator - flow control points #1") {
    ...
    ASSERT_OK(kefir_irblock_appendu64(&block, KEFIR_IR_OPCODE_JUMP, 0));
    ASSERT_OK(kefir_irblock_appendu64_2(&block, KEFIR_IR_OPCODE_BRANCH, 0, KEFIR_IR_BRANCH_CONDITION_8BIT));
    ...
    ASSERT(kefir_irblock_at(&block, 0)->opcode == KEFIR_IR_OPCODE_JUMP);
    ASSERT(kefir_irblock_at(&block, 0)->arg.u64 == 0);
    ASSERT(kefir_irblock_at(&block, 1)->opcode == KEFIR_IR_OPCODE_BRANCH);
    ASSERT(kefir_irblock_at(&block, 1)->arg.u64_2[0] == 0);
    ASSERT(kefir_irblock_at(&block, 1)->arg.u64_2[1] == KEFIR_IR_BRANCH_CONDITION_8BIT);

    ASSERT_OK(kefir_irblock_appendu64(&block, KEFIR_IR_OPCODE_NOP, 0));
    ...
    ASSERT(kefir_irblock_at(&block, 1)->arg.u64_2[1] == KEFIR_IR_BRANCH_CONDITION_8BIT);
    ASSERT(kefir_irblock_at(&block, 2)->opcode == KEFIR_IR_OPCODE_NOP);
    ASSERT(kefir_irblock_at(&block, 2)->arg.u64 == 0);

    ASSERT_OK(kefir_irblock_appendu64_2(&block, KEFIR_IR_OPCODE_BRANCH, 0, KEFIR_IR_BRANCH_CONDITION_64BIT));
    ASSERT_OK(kefir_irblock_appendu64(&block, KEFIR_IR_OPCODE_JUMP, 0));
    ...
    ASSERT(kefir_irblock_at(&block, 2)->opcode == KEFIR_IR_OPCODE_NOP);
    ASSERT(kefir_irblock_at(&block, 2)->arg.u64 == 0);
    ASSERT(kefir_irblock_at(&block, 3)->opcode == KEFIR_IR_OPCODE_BRANCH);
    ASSERT(kefir_irblock_at(&block, 3)->arg.u64_2[0] == 2);
    ASSERT(kefir_irblock_at(&block, 3)->arg.u64_2[1] == KEFIR_IR_BRANCH_CONDITION_64BIT);
    ASSERT(kefir_irblock_at(&block, 4)->opcode == KEFIR_IR_OPCODE_JUMP);
