...
#define ASSERT_DATA_ALLOC(vector, index, _size, _alignment, offset)                                                  \
    do {                                                                                                             \
        ASSIGN_DECL_CAST(struct kefir_abi_sysv_amd64_typeentry_layout *, param, kefir_vector_at((vector), (index))); \
...
DEFINE_CASE(amd64_sysv_abi_data_test1, "AMD64 System V ABI - multiple scalars") {
    ...
    struct kefir_abi_sysv_amd64_type_layout vector;
    ...
    ASSERT_OK(kefir_abi_sysv_amd64_type_layout(&type, &kft_mem, &vector));
    ASSERT_DATA_ALLOC(&vector.layout, 0, 8, 8, 0);
    ASSERT_DATA_ALLOC(&vector.layout, 1, 1, 1, 8);
    ASSERT_DATA_ALLOC(&vector.layout, 2, 4, 4, 12);
    ASSERT_OK(kefir_abi_sysv_amd64_type_layout_free(&kft_mem, &vector));
...
DEFINE_CASE(amd64_sysv_abi_data_test8, "AMD64 System V ABI - eightbyte allocation") {
    struct kefir_ir_type type;
    struct kefir_abi_sysv_amd64_type_layout vector;
    struct kefir_abi_sysv_amd64_qwords qwords;
    ...
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_INT8, 0, 0));
    ASSERT_OK(kefir_abi_sysv_amd64_type_layout(&type, &kft_mem, &vector));
    ASSERT_OK(kefir_abi_sysv_amd64_qwords_count(&type, &vector.layout, &qword_count));
    ASSERT_OK(kefir_abi_sysv_amd64_qwords_alloc(&qwords, &kft_mem, qword_count));
    ASSERT_OK(kefir_abi_sysv_amd64_qwords_next(&qwords, KEFIR_AMD64_SYSV_PARAM_INTEGER, 1, 1, &ref));
    ASSERT(ref.qword->index == 0 && ref.offset == 0);
    ASSERT_OK(kefir_abi_sysv_amd64_qwords_next(&qwords, KEFIR_AMD64_SYSV_PARAM_INTEGER, 1, 1, &ref));
    ASSERT(ref.qword->index == 0 && ref.offset == 1);
    ASSERT_OK(kefir_abi_sysv_amd64_qwords_next(&qwords, KEFIR_AMD64_SYSV_PARAM_INTEGER, 4, 4, &ref));
    ASSERT(ref.qword->index == 0 && ref.offset == 4);
    ASSERT_OK(kefir_abi_sysv_amd64_qwords_next(&qwords, KEFIR_AMD64_SYSV_PARAM_INTEGER, 2, 2, &ref));
    ASSERT(ref.qword->index == 1 && ref.offset == 0);
    ASSERT_OK(kefir_abi_sysv_amd64_qwords_next(&qwords, KEFIR_AMD64_SYSV_PARAM_INTEGER, 8, 8, &ref));
    ...
    ASSERT_OK(kefir_abi_sysv_amd64_qwords_free(&qwords, &kft_mem));
    ASSERT_OK(kefir_abi_sysv_amd64_type_layout_free(&kft_mem, &vector));
    ASSERT_OK(kefir_ir_type_free(&kft_mem, &type));
}
END_CASE
...
DEFINE_CASE(amd64_sysv_abi_data_test9, "AMD64 System V ABI - external structure definitions") {
    ...
    ASSERT_OK(kefir_abi_sysv_amd64_type_layout(&type2, &kft_mem, &vector));
