...
DEFINE_CASE(ast_type_scope1, "AST Declaration scoping - tagged type scoping") {
    ...
    ASSERT(scoped_id->type_tag.type->tag == KEFIR_AST_TYPE_STRUCTURE);
    ASSERT(strcmp(scoped_id->type_tag.type->structure_type.identifier, "struct1") == 0);
    ASSERT(!scoped_id->type_tag.type->structure_type.complete);
    ...
    ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_TYPE_TAG);
    ASSERT(scoped_id->type_tag.type->tag == KEFIR_AST_TYPE_STRUCTURE);
    ASSERT(strcmp(scoped_id->type_tag.type->structure_type.identifier, "struct2") == 0);
    ASSERT(!scoped_id->type_tag.type->structure_type.complete);
    ...
    ASSERT(strcmp(scoped_id->type_tag.type->structure_type.identifier, "struct1") == 0);
    ASSERT(scoped_id->type_tag.type->structure_type.complete);
    ASSERT(&scoped_id->type_tag.type->structure_type == struct_type1);
...
DEFINE_CASE(ast_type_scope2, "AST Declaration scoping - tagged type scoping #2") {
    ...
    ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_TYPE_TAG);
    ASSERT(scoped_id->type_tag.type->tag == KEFIR_AST_TYPE_STRUCTURE);
    ASSERT(strcmp(scoped_id->type_tag.type->structure_type.identifier, "type1") == 0);
    ASSERT(scoped_id->type_tag.type->structure_type.complete);
    ASSERT(&scoped_id->type_tag.type->structure_type == struct_type1);
    ...
    ASSERT_OK(kefir_ast_local_context_resolve_scoped_tag_identifier(&context, "type1", &scoped_id));
    ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_TYPE_TAG);
    ASSERT(scoped_id->type_tag.type->tag == KEFIR_AST_TYPE_UNION);
    ASSERT(strcmp(scoped_id->type_tag.type->structure_type.identifier, "type1") == 0);
    ASSERT(!scoped_id->type_tag.type->structure_type.complete);
    ...
    ASSERT(strcmp(scoped_id->type_tag.type->structure_type.identifier, "type1") == 0);
    ASSERT(scoped_id->type_tag.type->structure_type.complete);
    ASSERT(&scoped_id->type_tag.type->structure_type == union_type1);
    ...
    ASSERT_OK(kefir_ast_local_context_resolve_scoped_tag_identifier(&context, "type1", &scoped_id));
    ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_TYPE_TAG);
    ASSERT(scoped_id->type_tag.type->tag == KEFIR_AST_TYPE_ENUMERATION);
    ASSERT(strcmp(scoped_id->type_tag.type->enumeration_type.identifier, "type1") == 0);
    ASSERT(!scoped_id->type_tag.type->enumeration_type.complete);
    ASSERT(KEFIR_AST_TYPE_SAME(scoped_id->type_tag.type->enumeration_type.underlying_type,
                               type_traits->underlying_enumeration_type));
    ...
    ASSERT(strcmp(scoped_id->type_tag.type->enumeration_type.identifier, "type1") == 0);
    ASSERT(scoped_id->type_tag.type->enumeration_type.complete);
    ASSERT(&scoped_id->type_tag.type->enumeration_type == enum_type1);
...
DEFINE_CASE(ast_type_scope3, "AST Declaration scoping - tagged type scoping #3") {
    ...
    do {
        ...
        ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_TYPE_TAG);
        ASSERT(scoped_id->type_tag.type->tag == KEFIR_AST_TYPE_UNION);
        ASSERT(strcmp(scoped_id->type_tag.type->structure_type.identifier, "union1") == 0);
        ASSERT(!scoped_id->type_tag.type->structure_type.complete);
        ...
        do {
            ...
            ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_TYPE_TAG);
            ASSERT(scoped_id->type_tag.type->tag == KEFIR_AST_TYPE_ENUMERATION);
            ASSERT(strcmp(scoped_id->type_tag.type->enumeration_type.identifier, "enum1") == 0);
            ASSERT(!scoped_id->type_tag.type->enumeration_type.complete);
            ASSERT(KEFIR_AST_TYPE_SAME(scoped_id->type_tag.type->enumeration_type.underlying_type,
                                       type_traits->underlying_enumeration_type));
            ...
            do {
                ...
                ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_TYPE_TAG);
                ASSERT(scoped_id->type_tag.type->tag == KEFIR_AST_TYPE_STRUCTURE);
                ASSERT(strcmp(scoped_id->type_tag.type->structure_type.identifier, "enum1") == 0);
                ASSERT(!scoped_id->type_tag.type->structure_type.complete);
                ...
                ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_TYPE_TAG);
                ASSERT(scoped_id->type_tag.type->tag == KEFIR_AST_TYPE_ENUMERATION);
                ASSERT(strcmp(scoped_id->type_tag.type->enumeration_type.identifier, "union1") == 0);
                ASSERT(!scoped_id->type_tag.type->enumeration_type.complete);
...
DEFINE_CASE(ast_ordinary_constant_scope1, "AST ordinary scope - constant scoping #1") {
    ...
    ASSERT_OK(kefir_ast_global_context_define_constant(&kft_mem, &global_context, "c3",
                                                       &KEFIR_AST_CONSTANT_EXPRESSION_INT_VALUE(3),
                                                       type_traits->underlying_enumeration_type, NULL, NULL, NULL));
    ...
    ASSERT_OK(kefir_ast_global_context_define_constant(&kft_mem, &global_context, "c4",
                                                       &KEFIR_AST_CONSTANT_EXPRESSION_INT_VALUE(40),
                                                       type_traits->underlying_enumeration_type, NULL, NULL, NULL));
    ...
    ASSERT(scoped_id->enum_constant.value.integer == 1);
    ...
    do {
        ...
        ASSERT_OK(kefir_ast_local_context_define_constant(&kft_mem, &context, "c2",
                                                          &KEFIR_AST_CONSTANT_EXPRESSION_INT_VALUE(20),
                                                          type_traits->underlying_enumeration_type, NULL, NULL, NULL));
        ASSERT_OK(kefir_ast_local_context_define_constant(&kft_mem, &context, "c5",
                                                          &KEFIR_AST_CONSTANT_EXPRESSION_INT_VALUE(50),
                                                          type_traits->underlying_enumeration_type, NULL, NULL, NULL));

        ASSERT_OK(kefir_ast_local_context_resolve_scoped_ordinary_identifier(&context, "c1", &scoped_id));
        ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_ENUM_CONSTANT);
        ASSERT(scoped_id->enum_constant.value.integer == 10);
        ASSERT(KEFIR_AST_TYPE_SAME(scoped_id->enum_constant.type, type_traits->underlying_enumeration_type));

        ASSERT_OK(kefir_ast_local_context_resolve_scoped_ordinary_identifier(&context, "c2", &scoped_id));
        ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_ENUM_CONSTANT);
        ASSERT(scoped_id->enum_constant.value.integer == 20);
        ASSERT(KEFIR_AST_TYPE_SAME(scoped_id->enum_constant.type, type_traits->underlying_enumeration_type));
        ...
        ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_ENUM_CONSTANT);
        ASSERT(scoped_id->enum_constant.value.integer == 50);
        ASSERT(KEFIR_AST_TYPE_SAME(scoped_id->enum_constant.type, type_traits->underlying_enumeration_type));
        ...
        do {
            ...
            ASSERT_OK(kefir_ast_local_context_resolve_scoped_ordinary_identifier(&context, "c5", &scoped_id));
            ASSERT(scoped_id->klass == KEFIR_AST_SCOPE_IDENTIFIER_ENUM_CONSTANT);
            ASSERT(scoped_id->enum_constant.value.integer == 500);
            ASSERT(KEFIR_AST_TYPE_SAME(scoped_id->enum_constant.type, type_traits->underlying_enumeration_type));
