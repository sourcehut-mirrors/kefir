...
DEFINE_CASE(ast_nodes_generic_selections, "AST nodes - generic selections") {
    ...
    struct kefir_ast_generic_selection *selection1 =
        kefir_ast_new_generic_selection(&kft_mem, NULL, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(&kft_mem, 5)));
    ASSERT(selection1 != NULL);
    ASSERT(selection1->base.klass->type == KEFIR_AST_GENERIC_SELECTION);
    ASSERT(KEFIR_AST_NODE_SELF(KEFIR_AST_NODE_BASE(selection1)) == selection1);
    ASSERT(selection1->control != NULL);
    ASSERT(selection1->control->klass->type == KEFIR_AST_CONSTANT);
    ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(selection1->control))->type == KEFIR_AST_INT_CONSTANT);
    ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(selection1->control))->value.integer == 5);
    ...
    ASSERT_ASSOC(selection1, 0, type_name1, {
        ...
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(assoc->expr))->type == KEFIR_AST_BOOL_CONSTANT);
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(assoc->expr))->value.boolean == true);
    });
    ASSERT(selection1->default_assoc == NULL);
    ...
    ASSERT_OK(kefir_ast_generic_selection_append(
        &kft_mem, selection1, type_name3, KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(&kft_mem, &symbols, "true"))));
    ...
    ASSERT_ASSOC(selection1, 1, type_name3, {
        ...
        ASSERT(((struct kefir_ast_identifier *) KEFIR_AST_NODE_SELF(assoc->expr))->identifier != NULL);
        ASSERT(strcmp(((struct kefir_ast_identifier *) KEFIR_AST_NODE_SELF(assoc->expr))->identifier, "true") == 0);
    });
    ASSERT(selection1->default_assoc == NULL);
    ...
    ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(selection1->default_assoc))->type == KEFIR_AST_CHAR_CONSTANT);
    ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(selection1->default_assoc))->value.character == 'H');

    struct kefir_ast_constant *cnst2 = kefir_ast_new_constant_char_noarena(&kft_mem, 'H');
...
#define ASSERT_CAST(_mem, _type_name, _expr, _checker)                                                     \
    do {                                                                                                   \
        struct kefir_ast_cast_operator *cast = kefir_ast_new_cast_operator_noarena((_mem), (_type_name), (_expr)); \
        ASSERT(cast != NULL);                                                                              \
        ASSERT(cast->base.klass->type == KEFIR_AST_CAST_OPERATOR);                                         \
        ASSERT(KEFIR_AST_NODE_SELF(KEFIR_AST_NODE_BASE(cast)) == cast);                                                                   \
...
DEFINE_CASE(ast_nodes_cast_operators, "AST nodes - cast operators") {
    ...
    ASSERT_CAST(&kft_mem, type_name1, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_uint_noarena(&kft_mem, 100)), {
        ASSERT(cast->expr->klass->type == KEFIR_AST_CONSTANT);
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(cast->expr))->type == KEFIR_AST_UINT_CONSTANT);
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(cast->expr))->value.uinteger == 100);
    });
    ...
    ASSERT_CAST(&kft_mem, type_name2, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_long_long_noarena(&kft_mem, 1234)), {
        ASSERT(cast->expr->klass->type == KEFIR_AST_CONSTANT);
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(cast->expr))->type == KEFIR_AST_LONG_LONG_CONSTANT);
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(cast->expr))->value.long_long == 1234);
    });

    struct kefir_ast_type_name *type_name3 = kefir_ast_new_type_name_noarena(
        &kft_mem, kefir_ast_declarator_pointer_noarena(&kft_mem, kefir_ast_declarator_identifier_noarena(&kft_mem, NULL, NULL)));
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &type_name3->type_decl.specifiers,
                                                         kefir_ast_type_specifier_void(&kft_mem, NULL)));

    ASSERT_CAST(&kft_mem, type_name3, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_long_noarena(&kft_mem, 0)), {
        ASSERT(cast->expr->klass->type == KEFIR_AST_CONSTANT);
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(cast->expr))->type == KEFIR_AST_LONG_CONSTANT);
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(cast->expr))->value.long_integer == 0);
    });
    ...
    ASSERT_CAST(
        &kft_mem, type_name4,
        KEFIR_AST_NODE_BASE(
            kefir_ast_new_unary_operation_noarena(&kft_mem, KEFIR_AST_OPERATION_ADDRESS,
                                          KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(&kft_mem, &symbols, "x")))),
        {
            ASSERT(cast->expr->klass->type == KEFIR_AST_UNARY_OPERATION);
            ASSERT(((struct kefir_ast_unary_operation *) KEFIR_AST_NODE_SELF(cast->expr))->type == KEFIR_AST_OPERATION_ADDRESS);
            ASSERT(((struct kefir_ast_unary_operation *) KEFIR_AST_NODE_SELF(cast->expr))->arg->klass->type == KEFIR_AST_IDENTIFIER);
        });
...
#define ASSERT_CONDITIONAL(_mem, _condition, _expr1, _expr2, _checker)                    \
    do {                                                                                  \
        struct kefir_ast_conditional_operator *oper =                                     \
            kefir_ast_new_conditional_operator_noarena((_mem), (_condition), (_expr1), (_expr2)); \
        ASSERT(oper != NULL);                                                             \
        ASSERT(oper->base.klass->type == KEFIR_AST_CONDITIONAL_OPERATION);                \
        ASSERT(KEFIR_AST_NODE_SELF(KEFIR_AST_NODE_BASE(oper)) == oper);                                                  \
...
DEFINE_CASE(ast_nodes_conditional_operators, "AST nodes - conditional operators") {
    ...
    ASSERT_CONDITIONAL(
        ...
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_uint_noarena(&kft_mem, 2)), {
            ASSERT(oper->condition->klass->type == KEFIR_AST_CONSTANT);
            ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(oper->condition))->type == KEFIR_AST_BOOL_CONSTANT);
            ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(oper->condition))->value.boolean);

            ASSERT(oper->expr1->klass->type == KEFIR_AST_CONSTANT);
            ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(oper->expr1))->type == KEFIR_AST_INT_CONSTANT);
            ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(oper->expr1))->value.integer == -1);

            ASSERT(oper->expr2->klass->type == KEFIR_AST_CONSTANT);
            ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(oper->expr2))->type == KEFIR_AST_UINT_CONSTANT);
            ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(oper->expr2))->value.uinteger == 2);
        });

    ASSERT_CONDITIONAL(
        &kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(&kft_mem, &symbols, "x")),
        KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(&kft_mem, &symbols, "y")),
        KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(&kft_mem, &symbols, "z")), {
            ASSERT(oper->condition->klass->type == KEFIR_AST_IDENTIFIER);
            ASSERT(strcmp(((struct kefir_ast_identifier *) KEFIR_AST_NODE_SELF(oper->condition))->identifier, "x") == 0);

            ASSERT(oper->expr1->klass->type == KEFIR_AST_IDENTIFIER);
            ASSERT(strcmp(((struct kefir_ast_identifier *) KEFIR_AST_NODE_SELF(oper->expr1))->identifier, "y") == 0);

            ASSERT(oper->expr2->klass->type == KEFIR_AST_IDENTIFIER);
            ASSERT(strcmp(((struct kefir_ast_identifier *) KEFIR_AST_NODE_SELF(oper->expr2))->identifier, "z") == 0);
        });
...
DEFINE_CASE(ast_nodes_assignment_operators, "AST nodes - assignment operators") {
    ...
    for (kefir_size_t i = 0; i < OPERATIONS_LENGTH; i++) {
        struct kefir_ast_assignment_operator *oper = kefir_ast_new_compound_assignment_noarena(
            &kft_mem, OPERATIONS[i], KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(&kft_mem, &symbols, "x")),
        ...
        ASSERT(strcmp(((struct kefir_ast_identifier *) KEFIR_AST_NODE_SELF(oper->target))->identifier, "x") == 0);
        ASSERT(oper->value->klass->type == KEFIR_AST_CONSTANT);
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(oper->value))->type == KEFIR_AST_INT_CONSTANT);
        ASSERT(((struct kefir_ast_constant *) KEFIR_AST_NODE_SELF(oper->value))->value.integer == 100);
        ASSERT_OK(KEFIR_AST_NODE_FREE(&kft_mem, KEFIR_AST_NODE_BASE(oper)));
    }

    struct kefir_ast_assignment_operator *oper = kefir_ast_new_simple_assignment_noarena(
        &kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(&kft_mem, &symbols, "x")),
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(&kft_mem, 100)));
    ASSERT(oper != NULL);
    ASSERT(oper->base.klass->type == KEFIR_AST_ASSIGNMENT_OPERATOR);
...
DEFINE_CASE(ast_nodes_comma_operators, "AST nodes - comma operators") {
    ...
    struct kefir_ast_comma_operator *comma = kefir_ast_new_comma_operator_noarena(&kft_mem);
    ASSERT(comma != NULL);
    ASSERT(comma->base.klass->type == KEFIR_AST_COMMA_OPERATOR);
    ASSERT(KEFIR_AST_NODE_SELF(KEFIR_AST_NODE_BASE(comma)) == comma);

    for (kefir_size_t i = 0; i < 10; i++) {
        ASSERT_OK(
            kefir_ast_comma_append(&kft_mem, comma, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(&kft_mem, i))));
        ...
        ASSIGN_DECL_CAST(struct kefir_ast_constant *, cnst, KEFIR_AST_NODE_SELF(expr));
...
DEFINE_CASE(ast_nodes_compound_literals, "AST nodes - compound literals") {
    ...
    struct kefir_ast_compound_literal *compound = kefir_ast_new_compound_literal_noarena(&kft_mem, type_name1);
    ASSERT(compound != NULL);
    ASSERT(compound->base.klass->type == KEFIR_AST_COMPOUND_LITERAL);
    ASSERT(KEFIR_AST_NODE_SELF(KEFIR_AST_NODE_BASE(compound)) == compound);
    ...
    ASSERT_OK(kefir_ast_initializer_list_append(
        ...
        kefir_ast_new_initializer_member_designation_noarena(
            ...
            kefir_ast_new_initializer_index_designation_noarena(
                &kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_long_long_noarena(&kft_mem, 100)),
                kefir_ast_new_initializer_member_designation_noarena(&kft_mem, &symbols, "world", NULL))),
        kefir_ast_new_expression_initializer_noarena(&kft_mem,
                                             KEFIR_AST_NODE_BASE(kefir_ast_new_constant_bool_noarena(&kft_mem, true)))));
    ASSERT(compound->initializer->list.entries_length == 1);

    struct kefir_ast_initializer *initializer1 = kefir_ast_new_list_initializer_noarena(&kft_mem);
    ASSERT_OK(kefir_ast_initializer_list_append(
    ...
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(
        &kft_mem, entry1, kefir_ast_declarator_identifier_noarena(&kft_mem, &symbols, "x"), NULL));
    ASSERT_OK(kefir_ast_structure_specifier_append_entry(&kft_mem, specifier1, entry1));

    struct kefir_ast_structure_declaration_entry *entry2 = kefir_ast_structure_declaration_entry_alloc_noarena(&kft_mem);
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &entry2->declaration.specifiers,
                                                         kefir_ast_type_specifier_void(&kft_mem, NULL)));
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(
        &kft_mem, entry2,
        kefir_ast_declarator_pointer_noarena(&kft_mem, kefir_ast_declarator_identifier_noarena(&kft_mem, &symbols, "y")), NULL));
    ASSERT_OK(kefir_ast_structure_specifier_append_entry(&kft_mem, specifier1, entry2));

    struct kefir_ast_structure_declaration_entry *entry3 = kefir_ast_structure_declaration_entry_alloc_noarena(&kft_mem);
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &entry3->declaration.specifiers,
                                                         kefir_ast_type_specifier_boolean(&kft_mem, NULL)));
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(
        &kft_mem, entry3, kefir_ast_declarator_identifier_noarena(&kft_mem, &symbols, "z"), NULL));
    ...
    compound = kefir_ast_new_compound_literal_noarena(&kft_mem, type_name2);
    ASSERT(compound != NULL);
    ASSERT(compound->base.klass->type == KEFIR_AST_COMPOUND_LITERAL);
    ASSERT(KEFIR_AST_NODE_SELF(KEFIR_AST_NODE_BASE(compound)) == compound);
    ASSERT(compound->initializer->list.entries_length == 0);
    ...
    ASSERT_OK(kefir_ast_initializer_list_append(
        &kft_mem, &compound->initializer->list, NULL,
        kefir_ast_new_expression_initializer_noarena(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(&kft_mem, 5)))));
...
DEFINE_CASE(ast_nodes_init_declarators1, "AST nodes - declarations #1") {
    ...
    struct kefir_ast_declaration *decl1_list =
        kefir_ast_new_single_declaration_noarena(&kft_mem, kefir_ast_declarator_identifier_noarena(&kft_mem, NULL, NULL), NULL, &decl1);
    ASSERT(decl1 != NULL);
    ASSERT(decl1->declarator->klass == KEFIR_AST_DECLARATOR_IDENTIFIER);
    ASSERT(decl1->declarator->identifier.identifier == NULL);
    ASSERT(decl1->declarator->identifier.asm_label == NULL);
    struct kefir_ast_declarator_specifier_list_iterator iter;
    ASSERT(kefir_ast_declarator_specifier_list_iter(&decl1_list->specifiers, &iter, NULL) == KEFIR_ITERATOR_END);
