...
DEFINE_CASE(ast_node_analysis_function_calls2, "AST node analysis - function calls #2") {
    ...
    ASSERT_FUNCTION_CALL(&kft_mem, context, "func1", kefir_ast_type_signed_short(), {
        ASSERT_OK(kefir_ast_function_call_append(&kft_mem, call1,
                                                 KEFIR_AST_NODE_BASE(kefir_ast_new_constant_long_noarena(&kft_mem, 0))));
        ASSERT_OK(kefir_ast_function_call_append(&kft_mem, call1,
    ...
    ASSERT_FUNCTION_CALL(&kft_mem, context, "func1", kefir_ast_type_signed_short(), {
        ASSERT_OK(kefir_ast_function_call_append(&kft_mem, call1,
                                                 KEFIR_AST_NODE_BASE(kefir_ast_new_constant_double_noarena(&kft_mem, 0.0f))));
        ASSERT_OK(kefir_ast_function_call_append(&kft_mem, call1,
...
#define ASSERT_COMPOUND_LITERAL(_mem, _context, _type, _init, _result_type, _constant)                    \
    do {                                                                                                  \
        struct kefir_ast_compound_literal *compound = kefir_ast_new_compound_literal_noarena(                     \
        ...
        ASSERT(KEFIR_AST_NODE_IS_CONSTANT_EXPRESSION(KEFIR_AST_NODE_BASE(compound)) == (_constant));            \
...
DEFINE_CASE(ast_node_analysis_compound_literal2, "AST node analysis - compound literals #2") {
    ...
    ASSERT_OK(kefir_ast_local_context_declare_external(
        ...
        kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_char(), 10, NULL), NULL, NULL, NULL, NULL));
    ...
    ASSERT_COMPOUND_LITERAL(&kft_mem, context, type_name1, {},
                            kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_char(), 0, NULL),
                            false);
    ...
    ASSERT_COMPOUND_LITERAL(
        ...
        kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_char(), 14, NULL), false);
...
DEFINE_CASE(ast_node_analysis_compound_literal3, "AST node analysis - compound literals #3") {
    ...
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(
        &kft_mem, entry3,
        kefir_ast_declarator_pointer_noarena(&kft_mem, kefir_ast_declarator_identifier_noarena(&kft_mem, context->symbols, "field3")),
        NULL));
    ASSERT_OK(kefir_ast_structure_specifier_append_entry(&kft_mem, specifier1, entry3));

    struct kefir_ast_structure_specifier *specifier2 = kefir_ast_structure_specifier_init_noarena(&kft_mem, NULL, NULL, true);
    ...
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &entry4->declaration.specifiers,
                                                         kefir_ast_type_specifier_float(&kft_mem, NULL)));
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(
        &kft_mem, entry4, kefir_ast_declarator_identifier_noarena(&kft_mem, context->symbols, "x"), NULL));
    ...
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(
        ...
        kefir_ast_declarator_array_noarena(&kft_mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
                                   ...
                                   kefir_ast_declarator_identifier_noarena(&kft_mem, context->symbols, "y")),
        NULL));
    ASSERT_OK(kefir_ast_structure_specifier_append_entry(&kft_mem, specifier2, entry5));

    struct kefir_ast_type_name *type_name1 =
        kefir_ast_new_type_name_noarena(&kft_mem, kefir_ast_declarator_identifier_noarena(&kft_mem, NULL, NULL));
    struct kefir_ast_declarator_specifier *struct_spec2 = kefir_ast_type_specifier_struct(&kft_mem, NULL, specifier2);
    ...
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, structure2, "y",
                                          kefir_ast_type_array(&kft_mem, context->type_bundle, type1, 2, NULL), NULL));
    ...
    ASSERT_COMPOUND_LITERAL(&kft_mem, context, type_name2, {},
                            kefir_ast_type_array(&kft_mem, context->type_bundle, type2, 0, NULL), false);
...
#define ASSERT_COMPOUND_LITERAL_TEMP(_mem, _context, _type_name, _init, _storage)                           \
    do {                                                                                                    \
        struct kefir_ast_compound_literal *compound = kefir_ast_new_compound_literal_noarena((_mem), (_type_name)); \
        ASSERT(compound != NULL);                                                                           \
        _init ASSERT_OK(kefir_ast_analyze_node((_mem), (_context), KEFIR_AST_NODE_BASE(compound)));         \
...
DEFINE_CASE(ast_node_analysis_compound_literal4, "AST node analysis - compound literals temporaries") {
    ...
    ASSERT_COMPOUND_LITERAL_TEMP(
        ...
        KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_AUTO);
    ASSERT_COMPOUND_LITERAL_TEMP(
        &kft_mem, context, (struct kefir_ast_type_name *) KEFIR_AST_NODE_REF(KEFIR_AST_NODE_BASE(type_name8)),
        {
            ASSERT_OK(kefir_ast_initializer_list_append(
                &kft_mem, &compound->initializer->list, NULL,
                kefir_ast_new_expression_initializer_noarena(
    ...
    ASSERT_COMPOUND_LITERAL_TEMP(
        ...
        (struct kefir_ast_type_name *) KEFIR_AST_NODE_REF(KEFIR_AST_NODE_BASE(type_name11)),
        {
            ASSERT_OK(kefir_ast_initializer_list_append(
                &kft_mem, &compound->initializer->list, NULL,
                kefir_ast_new_expression_initializer_noarena(
