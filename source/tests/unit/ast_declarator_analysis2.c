...
DEFINE_CASE(ast_declarator_analysis7, "AST declarator analysis - struct declarators #1") {
    ...
    ASSERT_IDENTIFIER_TYPE(
        ...
        kefir_ast_storage_class_specifier_auto(&kft_mem, NULL), kefir_ast_type_qualifier_const(&kft_mem, NULL),
    ...
    struct kefir_ast_structure_specifier *specifier1 =
        kefir_ast_structure_specifier_init_noarena(&kft_mem, context->symbols, "struct2", true);
    ...
    struct kefir_ast_structure_specifier *specifier2 =
        kefir_ast_structure_specifier_init_noarena(&kft_mem, context->symbols, "struct3", true);
    ...
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &entry4->declaration.specifiers,
                                                         kefir_ast_type_specifier_struct(&kft_mem, NULL, specifier2)));
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(
    ...
    ASSERT_OK(kefir_ast_struct_type_bitfield(&kft_mem, context->symbols, struct_type3, "c",
                                             kefir_ast_type_signed_long_long(), NULL, 1));
    ...
    ASSERT_IDENTIFIER_TYPE(&kft_mem, context,
                           ...
                           4, kefir_ast_type_qualifier_volatile(&kft_mem, NULL),
...
DEFINE_CASE(ast_declarator_analysis8, "AST declarator analysis - struct declarators #2") {
    ...
    struct kefir_ast_structure_specifier *specifier1 =
        kefir_ast_structure_specifier_init_noarena(&kft_mem, context->symbols, "struct1", true);
    ASSERT(specifier1 != NULL);
    ...
    struct kefir_ast_structure_declaration_entry *entry4 = kefir_ast_structure_declaration_entry_alloc_noarena(&kft_mem);
    struct kefir_ast_structure_specifier *specifier2 =
        kefir_ast_structure_specifier_init_noarena(&kft_mem, context->symbols, NULL, true);
    ASSERT(specifier2 != NULL);
    struct kefir_ast_structure_declaration_entry *entry2_1 = kefir_ast_structure_declaration_entry_alloc_noarena(&kft_mem);
    ...
    ASSERT_IDENTIFIER_TYPE(
        ...
        kefir_ast_type_specifier_union(&kft_mem, NULL, specifier1), kefir_ast_storage_class_specifier_static(&kft_mem, NULL),
        kefir_ast_alignment_specifier(&kft_mem, NULL, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(&kft_mem, 32))));
...
DEFINE_CASE(ast_declarator_analysis9, "AST declarator analysis - enum declarators") {
    ...
    ASSERT_IDENTIFIER_TYPE(&kft_mem, context, type1, KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_EXTERN,
                           KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 2, kefir_ast_type_specifier_enum(&kft_mem, NULL, specifier1),
                           kefir_ast_storage_class_specifier_extern(&kft_mem, NULL));
    ...
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier2, context->symbols, "CONST1",
                                              KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(&kft_mem, 200)), NULL));
    ...
    ASSERT_OK(kefir_ast_enumeration_type_constant(&kft_mem, context->symbols, enum_type2, "B", 1));
    ASSERT_OK(kefir_ast_enumeration_type_constant(&kft_mem, context->symbols, enum_type2, "C", 2));
    ASSERT_OK(kefir_ast_enumeration_type_constant_auto(&kft_mem, context->symbols, enum_type2, "D"));
    ASSERT_OK(kefir_ast_enumeration_type_constant(&kft_mem, context->symbols, enum_type2, "CONST1", 200));
    ...
    ASSERT_IDENTIFIER_TYPE(
        ...
        kefir_ast_storage_class_specifier_thread_local(&kft_mem, NULL), kefir_ast_type_specifier_enum(&kft_mem, NULL, specifier2),
        kefir_ast_type_qualifier_volatile(&kft_mem, NULL), kefir_ast_storage_class_specifier_static(&kft_mem, NULL));

    struct kefir_ast_enum_specifier *specifier3 =
        kefir_ast_enum_specifier_init_noarena(&kft_mem, context->symbols, NULL, true, NULL);
    ASSERT_OK(kefir_ast_enum_specifier_append(
        ...
        KEFIR_AST_NODE_BASE(kefir_ast_new_unary_operation_noarena(
            &kft_mem, KEFIR_AST_OPERATION_NEGATE, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(&kft_mem, -1)))),
        NULL));
    ...
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier3, context->symbols, "TEN",
                                              KEFIR_AST_NODE_BASE(kefir_ast_new_constant_long_long_noarena(&kft_mem, 10)),
                                              NULL));

    struct kefir_ast_enum_type *enum_type3 = NULL;
    const struct kefir_ast_type *type3 =
        kefir_ast_type_enumeration(&kft_mem, context->type_bundle, NULL, kefir_ast_type_unsigned_int(), &enum_type3);
    ASSERT_OK(kefir_ast_enumeration_type_constant(&kft_mem, context->symbols, enum_type3, "ONE", 1));
    ...
    ASSERT_OK(kefir_ast_enumeration_type_constant(&kft_mem, context->symbols, enum_type3, "TEN", 10));
    ...
    ASSERT_IDENTIFIER_TYPE(
        ...
        kefir_ast_type_qualifier_const(&kft_mem, NULL), kefir_ast_type_specifier_enum(&kft_mem, NULL, specifier3),
