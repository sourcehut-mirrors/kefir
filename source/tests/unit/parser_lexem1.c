...
#define ASSERT_KEYWORD(_keyword)                                \
    ...
    ASSERT_KEYWORD(KEFIR_KEYWORD_ASM);
    ASSERT_KEYWORD(KEFIR_KEYWORD_ATTRIBUTE);
    ASSERT_KEYWORD(KEFIR_KEYWORD_TYPEOF);
    ASSERT_KEYWORD(KEFIR_KEYWORD_TYPEOF_UNQUAL);
    ASSERT_KEYWORD(KEFIR_KEYWORD_AUTO_TYPE);
...
#define ASSERT_CONSTANT(_fn, _type, _field, _value)    \
    ...
    for (kefir_int64_t i = -10; i < 10; i++) {
        ...
        ASSERT(LONG_DOUBLE_EQUALS(token.constant.large->long_double, ((kefir_long_double_t) i) / 10, LONG_DOUBLE_EPSILON));
        ASSERT_OK(kefir_token_free(&kft_mem, &token));
        ...
        for (double j = -10.0; j < 10.0; j += 0.1) {
            ...
            ASSERT(DOUBLE_EQUALS(token.constant.large->complex_float64.real, ((kefir_float64_t) i) / 10, DOUBLE_EPSILON));
            ASSERT(DOUBLE_EQUALS(token.constant.large->complex_float64.imaginary, (kefir_float64_t) j, DOUBLE_EPSILON));
            ASSERT_OK(kefir_token_free(&kft_mem, &token));
            ...
            ASSERT(LONG_DOUBLE_EQUALS(token.constant.large->complex_long_double.real, ((kefir_long_double_t) i) / 10,
                                      LONG_DOUBLE_EPSILON));
            ASSERT(LONG_DOUBLE_EQUALS(token.constant.large->complex_long_double.imaginary, (kefir_long_double_t) j,
                                      LONG_DOUBLE_EPSILON));
            ASSERT_OK(kefir_token_free(&kft_mem, &token));
        }
    }
