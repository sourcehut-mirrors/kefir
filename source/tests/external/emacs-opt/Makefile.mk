...
$(KEFIR_EXTERNAL_TEST_EMACS_OPT_SOURCE_DIR)/src/emacs: $(KEFIR_EXTERNAL_TEST_EMACS_OPT_SOURCE_DIR)/Makefile
	@echo "Building emacs-opt $(KEFIR_EXTERNAL_TEST_EMACS_OPT_VERSION)..."
	@cd "$(KEFIR_EXTERNAL_TEST_EMACS_OPT_SOURCE_DIR)" && \
		LD_LIBRARY_PATH="$(realpath $(LIB_DIR))$(if $(LD_LIBRARY_PATH),:$(LD_LIBRARY_PATH))" \
		KEFIR_RTINC="$(realpath $(HEADERS_DIR))/kefir/runtime" \
...
$(KEFIR_EXTERNAL_TEST_EMACS_OPT_DIR)/tests.log: $(KEFIR_EXTERNAL_TEST_EMACS_OPT_SOURCE_DIR)/src/emacs
