...
$(KEFIR_EXTERNAL_TEST_PYTHON_DIR)/tests.log: $(KEFIR_EXTERNAL_TEST_PYTHON_SOURCE_DIR)/python
	...
	@cd "$(KEFIR_EXTERNAL_TEST_PYTHON_SOURCE_DIR)" && \
		...
		CC="$(realpath $(KEFIR_EXE))" \
		LC_ALL=C.UTF-8 \
		bash -c "ulimit -s unlimited && ./python -m test -x test_gdb test_epoll test_external_inspection test_perf_profiler test_selectors test_ssl test_poplib" | tee "../tests.log.tmp"
	@mv "$@.tmp" "$@"
