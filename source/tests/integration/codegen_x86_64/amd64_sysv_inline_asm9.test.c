...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "p1", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_STORE,
        &(struct kefir_ir_inline_assembly_parameter_constraints) { .general_purpose_register = true }, decl_params, func_params, 0, 0, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "p2", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_STORE,
        &(struct kefir_ir_inline_assembly_parameter_constraints) { .general_purpose_register = true }, decl_params, func_params, 0, 0, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "p3", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_READ,
        &(struct kefir_ir_inline_assembly_parameter_constraints) { .general_purpose_register = true }, decl_params, func_params, 0, 0, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "p4", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_READ,
        &(struct kefir_ir_inline_assembly_parameter_constraints) { .general_purpose_register = true }, decl_params, func_params, 0, 1, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "p5", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_READ,
        &(struct kefir_ir_inline_assembly_parameter_constraints) { .general_purpose_register = true }, decl_params, func_params, 0, 2, NULL));

    REQUIRE_OK(kefir_ir_inline_assembly_add_jump_target(mem, &module.symbols, inline_asm1, "1", "func1", 5, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_jump_target(mem, &module.symbols, inline_asm1, "2", "func2", 6, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_jump_target(mem, &module.symbols, inline_asm1, "3", "func3", 7, NULL));
