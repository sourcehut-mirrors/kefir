...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    REQUIRE_OK(kefir_irbuilder_type_append(mem, decl_result, KEFIR_IR_TYPE_INT64, 0, 0));
    ...
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "p1", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_OUTPUT,
        &(struct kefir_ir_inline_assembly_parameter_constraints) {.general_purpose_register = true}, decl_params,
        func_params, 0, 0, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "p2", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_OUTPUT,
        &(struct kefir_ir_inline_assembly_parameter_constraints) {.general_purpose_register = true}, decl_params,
        func_params, 0, 0, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "p3", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_DIRECT_INPUT,
        &(struct kefir_ir_inline_assembly_parameter_constraints) {.general_purpose_register = true}, decl_params,
        func_params, 0, 0, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "p4", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_DIRECT_INPUT,
        &(struct kefir_ir_inline_assembly_parameter_constraints) {.general_purpose_register = true}, decl_params,
        func_params, 0, 1, NULL));
    REQUIRE_OK(kefir_ir_inline_assembly_add_parameter(
        mem, &module.symbols, inline_asm1, "p5", KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_DIRECT_INPUT,
        &(struct kefir_ir_inline_assembly_parameter_constraints) {.general_purpose_register = true}, decl_params,
