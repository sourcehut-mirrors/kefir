...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    struct kefir_ir_inline_assembly *inline_asm2 = kefir_ir_module_new_inline_assembly(mem, &module,
                                                                                       ...
                                                                                       "mov %rcx, $%=",
                                                                                       &id2);
    struct kefir_ir_inline_assembly *inline_asm3 = kefir_ir_module_new_inline_assembly(mem, &module,
                                                                                       "mov %rax, test%=\n"
                                                                                       "mov %rbx, temp%=\n"
                                                                                       "mov %rcx, $%=",
                                                                                       &id3);
    ...
    REQUIRE_OK(kefir_irbuilder_block_appendu64(mem, &func->body, KEFIR_IR_OPCODE_INLINE_ASSEMBLY, id2));
