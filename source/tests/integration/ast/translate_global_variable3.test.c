...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry1, kefir_ast_declarator_identifier_noarena(mem, global_context.context.symbols, "a"), NULL));
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry2, kefir_ast_declarator_identifier_noarena(mem, global_context.context.symbols, "b"), NULL));
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry3,
        kefir_ast_declarator_pointer_noarena(mem, kefir_ast_declarator_identifier_noarena(mem, global_context.context.symbols, "c")),
        NULL));
    REQUIRE_OK(kefir_ast_structure_specifier_append_entry(mem, specifier1, entry3));

    struct kefir_ast_structure_declaration_entry *entry4 = kefir_ast_structure_declaration_entry_alloc_noarena(mem);
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &entry4->declaration.specifiers,
                                                          kefir_ast_type_qualifier_const(mem, NULL)));
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &entry4->declaration.specifiers,
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        ...
        kefir_ast_declarator_array_noarena(mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
                                   ...
                                   kefir_ast_declarator_identifier_noarena(mem, global_context.context.symbols, "d")),
        NULL));
    REQUIRE_OK(kefir_ast_structure_specifier_append_entry(mem, specifier1, entry4));

    struct kefir_ast_structure_declaration_entry *entry5 = kefir_ast_structure_declaration_entry_alloc_noarena(mem);
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry5, kefir_ast_declarator_identifier_noarena(mem, global_context.context.symbols, "e"), NULL));
    ...
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init1->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 11)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init1->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_float_noarena(mem, 4.35f)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init1->list, NULL,
        kefir_ast_new_expression_initializer_noarena(
            ...
            KEFIR_AST_NODE_BASE(kefir_ast_new_unary_operation_noarena(
                ...
                KEFIR_AST_NODE_BASE(kefir_ast_new_struct_member_noarena(
                    ...
                    KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, global_context.context.symbols, "structure")),
                    "b")))))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init1->list,
        kefir_ast_new_initializer_index_designation_noarena(
            mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 2)),
            kefir_ast_new_initializer_member_designation_noarena(mem, global_context.context.symbols, "d", NULL)),
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char_noarena(mem, 'a')))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init1->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char_noarena(mem, 'b')))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init1->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char_noarena(mem, 'c')))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init1->list,
        kefir_ast_new_initializer_index_designation_noarena(
            mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 0)),
            kefir_ast_new_initializer_member_designation_noarena(mem, global_context.context.symbols, "d", NULL)),
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char_noarena(mem, '0')))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init1->list, kefir_ast_new_initializer_member_designation_noarena(mem, global_context.context.symbols, "e", NULL),
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_unary_operation_noarena(
    ...
    struct kefir_ast_declaration *decl1 = kefir_ast_new_single_declaration_noarena(
        mem, kefir_ast_declarator_identifier_noarena(mem, global_context.context.symbols, "structure"), init1, NULL);
    ...
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init2->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 1)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init2->list, NULL,
        kefir_ast_new_expression_initializer_noarena(
            mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_cast_operator_noarena(
                ...
                KEFIR_AST_NODE_BASE(kefir_ast_new_unary_operation_noarena(
                    ...
                    KEFIR_AST_NODE_BASE(kefir_ast_new_array_subscript_noarena(
                        mem,
                        KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, global_context.context.symbols, "array1")),
                        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 1)))))))))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init2->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_float_noarena(mem, 3.52f)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init2->list,
        kefir_ast_new_initializer_index_designation_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 7)), NULL),
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, -1)))));
    ...
    struct kefir_ast_declaration *decl2 = kefir_ast_new_single_declaration_noarena(
        ...
        kefir_ast_declarator_array_noarena(mem, KEFIR_AST_DECLARATOR_ARRAY_UNBOUNDED, NULL,
                                   kefir_ast_declarator_identifier_noarena(mem, global_context.context.symbols, "array1")),
        init2, NULL);
    ...
    REQUIRE_OK(kefir_ast_initializer_list_append(
        ...
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 10)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init3->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_float_noarena(mem, 20.02)))));
    REQUIRE_OK(kefir_ast_initializer_list_append(
        mem, &init3->list, NULL,
        kefir_ast_new_expression_initializer_noarena(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_double_noarena(mem, 20.02)))));
    ...
    struct kefir_ast_declaration *decl3 = kefir_ast_new_single_declaration_noarena(
        ...
        kefir_ast_declarator_array_noarena(mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
                                   ...
                                   kefir_ast_declarator_identifier_noarena(mem, global_context.context.symbols, "array2")),
        init3, NULL);
    REQUIRE_OK(
        kefir_ast_declarator_specifier_list_append(mem, &decl3->specifiers, kefir_ast_type_specifier_float(mem, NULL)));
