...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    FUNC2("goto1", {
        ...
        struct kefir_ast_conditional_statement *cond1 = kefir_ast_new_conditional_statement_noarena(
            ...
            KEFIR_AST_NODE_BASE(kefir_ast_new_goto_statement_noarena(mem, context->symbols, "end")), NULL);

        struct kefir_ast_goto_statement *goto1 = kefir_ast_new_goto_statement_noarena(mem, context->symbols, "begin");
        ...
        REQUIRE_OK(kefir_ast_compound_statement_append(mem, compound1, KEFIR_AST_NODE_BASE(goto1)));
    ...
    FUNC2("goto2", {
        struct kefir_ast_labeled_statement *label1 = kefir_ast_new_labeled_statement_noarena(
            mem, context->symbols, "begin",
            KEFIR_AST_NODE_BASE(kefir_ast_new_goto_statement_noarena(mem, context->symbols, "loop")));

        struct kefir_ast_labeled_statement *label2 = kefir_ast_new_labeled_statement_noarena(
            mem, context->symbols, "loop",
            KEFIR_AST_NODE_BASE(kefir_ast_new_goto_statement_noarena(mem, context->symbols, "end")));
        ...
        struct kefir_ast_labeled_statement *label3 = kefir_ast_new_labeled_statement_noarena(
            mem, context->symbols, "end",
            KEFIR_AST_NODE_BASE(kefir_ast_new_goto_statement_noarena(mem, context->symbols, "begin")));
