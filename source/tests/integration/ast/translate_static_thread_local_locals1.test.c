...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    struct kefir_ast_declarator *function1_decl =
        kefir_ast_declarator_function_noarena(mem, kefir_ast_declarator_identifier_noarena(mem, global_context.context.symbols, "fn"));

    struct kefir_ast_declaration *decl1 = kefir_ast_new_single_declaration_noarena(
        mem, kefir_ast_declarator_identifier_noarena(mem, global_context.context.symbols, "value"),
    ...
    struct kefir_ast_declaration *decl2 = kefir_ast_new_single_declaration_noarena(
        mem, kefir_ast_declarator_identifier_noarena(mem, global_context.context.symbols, "increment"),
        kefir_ast_new_expression_initializer(mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int_noarena(mem, 1))), NULL);
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &decl2->specifiers, kefir_ast_type_specifier_int(mem, NULL)));

    struct kefir_ast_compound_statement *function1_body = kefir_ast_new_compound_statement_noarena(mem);
    REQUIRE_OK(kefir_ast_compound_statement_append(mem, function1_body, KEFIR_AST_NODE_BASE(decl1)));
    REQUIRE_OK(kefir_ast_compound_statement_append(mem, function1_body, KEFIR_AST_NODE_BASE(decl2)));
    REQUIRE_OK(kefir_ast_compound_statement_append(
        ...
        KEFIR_AST_NODE_BASE(kefir_ast_new_expression_statement_noarena(
            ...
            KEFIR_AST_NODE_BASE(kefir_ast_new_compound_assignment_noarena(
                ...
                KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, global_context.context.symbols, "increment"))))))));
    REQUIRE_OK(kefir_ast_compound_statement_append(
        mem, function1_body,
        KEFIR_AST_NODE_BASE(kefir_ast_new_return_statement_noarena(
            mem, KEFIR_AST_NODE_BASE(kefir_ast_new_identifier_noarena(mem, global_context.context.symbols, "value"))))));
    ...
    REQUIRE_OK(
        kefir_ast_declarator_specifier_list_append(mem, &function1->specifiers, kefir_ast_type_specifier_int(mem, NULL)));
