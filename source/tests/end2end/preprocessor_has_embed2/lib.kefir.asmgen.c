...
__has_embed("file1.txt") __has_embed("filex.txt") __has_embed(<file2.txt>) __has_embed(<somedir / file2.txt>)
    ...
    __has_embed("file1.txt" limit

                (0)) __has_embed("file1.txt" limit(1)) __has_embed("file1.txt" limit /* HELLO WORLD */ (~0ull))

        XXX

    __has_embed("empty.txt") __has_embed("empty.txt" limit(100)) __has_embed("empty.txt" limit

                                                                             /* TEST */ (100) prefix(1))

        XXX

    __has_embed(__FILE__ limit(10) prefix /*TEST2*/ (X) suffix(Y) if_empty(Z))
        __has_embed(__FILE__ limit(10) prefix /*TEST3*/ (X) suffix(Y) if_empty
