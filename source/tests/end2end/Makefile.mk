...
KEFIR_END2END_SELECTIVE_VALGRIND=no

ifeq ($(PLATFORM),linux)
ifneq ($(KEFIR_TEST_USE_MUSL),yes)
KEFIR_END2END_TEST_LIBS+=-latomic
endif
endif
ifeq ($(PLATFORM),freebsd)
KEFIR_END2END_TEST_LIBS+=-lstdthreads -lcompiler_rt
endif
...
.SECONDEXPANSION:
$(KEFIR_END2END_BIN_PATH)/%.test: $$(KEFIR_END2END_TEST_HOST_OBJECT_FILES) $$(KEFIR_END2END_TEST_KEFIR_OBJECT_FILES)
	@mkdir -p $(shell dirname "$@")
	@echo "Linking $@"
	@$(CC) $(TEST_CFLAGS) $(SANITIZER_FLAGS) $(KEFIR_END2END_TEST_HOST_OBJECT_FILES) $(KEFIR_END2END_TEST_KEFIR_OBJECT_FILES) -o $@ $(KEFIR_END2END_TEST_LIBS) -lm $(LDFLAGS)
...
KEFIR_END2END_ALL_MAKEFILES=$(wildcard $(SOURCE_DIR)/tests/end2end/*/Makefile.mk)
...
KEFIR_END2END_EXCLUDE_TESTS:=
include $(KEFIR_END2END_ALL_MAKEFILES)
...
ifeq ($(KEFIR_TEST_USE_MUSL),yes)
KEFIR_END2END_ALL_RUN_TEST_SOURCES := $(filter-out $(foreach S,$(KEFIR_END2END_ALL_RUN_TEST_SOURCES),$(if $(findstring atomic,$S),$S)),$(KEFIR_END2END_ALL_RUN_TEST_SOURCES))
endif
KEFIR_END2END_ALL_RUN_TEST_SOURCES := $(filter-out $(KEFIR_END2END_EXCLUDE_TESTS),$(KEFIR_END2END_ALL_RUN_TEST_SOURCES))
