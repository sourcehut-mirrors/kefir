...
static kefir_result_t parser_identifier_rule(struct kefir_mem *mem, struct kefir_parser *parser,
    ...
    UNUSED(payload);
    ASSIGN_DECL_CAST(struct parser_extension_payload *, extension_payload, parser->extension_payload);
...
static kefir_result_t on_parser_init(struct kefir_mem *mem, struct kefir_parser *parser) {
    ...
    REQUIRE(extension_payload != NULL,
            KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate parser extension payload"));
...
static kefir_result_t analyze_extension_node(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    REQUIRE(ext_node->klass == &pi_ext_node_class,
...
static kefir_result_t evaluate_constant_extension_node(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    REQUIRE(node != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid AST node"));
    REQUIRE(value != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to constant expression value"));
...
static kefir_result_t translate_extension_node(struct kefir_mem *mem, struct kefir_ast_translator_context *context,
    ...
    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDF64(builder, KEFIR_IR_OPCODE_FLOAT64_CONST, M_PI));

    return KEFIR_OK;
}

const struct kefir_parser_extensions parser_extensions = {.on_init = on_parser_init, .on_free = on_parser_free};
...
const struct kefir_ast_translator_context_extensions translator_extensions = {.translate_extension_node =
                                                                                  translate_extension_node};

const struct kefir_compiler_extensions extensions = {
    .parser = &parser_extensions, .analyzer = &analyzer_extensions, .translator = &translator_extensions};

kefir_result_t kefir_compiler_extension_entry(struct kefir_mem *mem,
                                              const struct kefir_compiler_runner_configuration *config,
                                              const struct kefir_compiler_extensions **extensions_ptr) {
    UNUSED(mem);
    UNUSED(config);
    REQUIRE(extensions_ptr != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to compiler extensions"));
