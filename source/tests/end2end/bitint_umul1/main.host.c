...
#define MASK(_x, _y) ((_x) & ((1ull << (_y)) - 1))
    ...
    struct S2 s2 = mul5((struct S2) {{2, 0}}, (struct S2) {{0ull, (1ull << 55) - 1}});
    assert(s2.arr[0] == 0);
    assert(MASK(s2.arr[1], 56) == 0xfffffffffffffeull);
    ...
    for (unsigned long i = 0; i < 512; i += 4) {
        for (unsigned long j = 0; j < 512; j += 4) {
            ...
            struct S2 s2 = mul5((struct S2) {{i, 0}}, (struct S2) {{j, 0}});
            assert(s2.arr[0] == i * j);
            assert(MASK(s2.arr[1], 56) == 0);

            struct S5 s5 = mul6((struct S5) {{i, 0, 0, 0, 0}}, (struct S5) {{j, 0, 0, 0, 0}});
