...
int main(void) {
    ...
    for (long x = 0; x < 4096; x++) {
        assert(postinc_i8() == (char) x);
        assert(i8 == (char) (x + 1));

        assert(postinc_i16() == (short) x);
        assert(i16 == (short) (x + 1));

        assert(postinc_i32() == (int) x);
        assert(i32 == (int) (x + 1));

        assert(postinc_i64() == (long) x);
        assert(i64 == (long) (x + 1));

        assert(postinc_ptr() == BASE_PTR + x);
        assert(ptr == BASE_PTR + x + 1);

        assert(fabs(postinc_f32() - (float) x) < EPSILON_F);
    ...
    for (long x = 4096; x > 0; x--) {
        assert(postdec_i8() == (char) x);
        assert(i8 == (char) (x - 1));

        assert(postdec_i16() == (short) x);
        assert(i16 == (short) (x - 1));

        assert(postdec_i32() == (int) x);
        assert(i32 == (int) (x - 1));

        assert(postdec_i64() == (long) x);
        assert(i64 == (long) (x - 1));

        assert(postdec_ptr() == BASE_PTR + x);
        assert(ptr == BASE_PTR + x - 1);

        assert(fabs(postdec_f32() - (float) x) < EPSILON_F);
        assert(fabs(f32 - (float) (x - 1)) < EPSILON_F);
