...
int main(void) {
    ...
    assert(!add2((1ull << 48), (1ull << 59), &res));
    ...
    assert(add3((struct S2) {{0, 0}}, (struct S2) {{~0ull, ~0ull}}, &res));
    assert(res == ~0ull);
    assert(!add3((struct S2) {{0, 1}}, (struct S2) {{~0ull, ~0ull}}, &res));
    assert(res == ~0ull);
    assert(!add3((struct S2) {{1ull << 63, 0}}, (struct S2) {{~0ull, ~0ull}}, &res));
    assert(res == (1ull << 63) - 1);
    assert(add3((struct S2) {{~0ull, 0}}, (struct S2) {{1, 0}}, &res));
    assert(res == 0);
    assert(add3((struct S2) {{2, 1}}, (struct S2) {{~0ull, ~0ull}}, &res));
    assert(res == 1);
    return EXIT_SUCCESS;
}
