...
int main(void) {
    ...
    for (long i = -128; i < 128; i++) {
        for (long j = -4096; j < 4096; j++) {
            ...
            i8 = i;
            assert(add8r(&i8, j) == (char) (i + j));
            assert(i8 == (char) (i + j));

            i16 = i;
            assert(add16r(&i16, j) == (short) (i + j));
            assert(i16 == (short) (i + j));

            i32 = i;
            assert(add32r(&i32, j) == (int) (i + j));
            assert(i32 == (int) (i + j));

            i64 = i;
            assert(add64r(&i64, j) == (i + j));
            assert(i64 == (i + j));

            f32 = i / 10.0f;
            assert(fabs(addf32r(&f32, j) - (i / 10.0f + j)) < 1e-3);
            assert(fabs(f32 - (i / 10.0f + j)) < 1e-3);

            f64 = i / 10.0f;
            assert(fabs(addf64r(&f64, j) - (i / 10.0f + j)) < 1e-3);
            assert(fabs(f64 - (i / 10.0f + j)) < 1e-3);
        }
    }
#endif
