...
#define MASK(_x, _y) ((_x) & ((1ull << (_y)) - 1))
    for (unsigned long i = 0; i < 4096; i++) {
        ...
        assert(MASK(predec1(&i8), 6) == MASK(i - 1, 6));
        assert(MASK(i8, 6) == MASK(i - 1, 6));
        assert(MASK(predec2(&i16), 14) == MASK(i - 1, 14));
        assert(MASK(i16, 14) == MASK(i - 1, 14));
        assert(MASK(predec3(&i32), 29) == MASK(i - 1, 29));
        assert(MASK(i32, 29) == MASK(i - 1, 29));
        assert(MASK(predec4(&i64), 58) == MASK(i - 1, 58));
        assert(MASK(i64, 58) == MASK(i - 1, 58));

        struct S2 s2_orig = {{i, 0}};
        struct S2 s2 = predec5(&s2_orig);
        if (i >= 1) {
            assert(s2.arr[0] == i - 1);
            ...
            assert(MASK(s2.arr[1], 55) == MASK(~0ull, 55));
        ...
        if (i >= 1) {
            ...
            assert(MASK(s5.arr[4], 55) == MASK(~0ull, 55));
