...
BOOTSTRAP_CFLAGS += '-DKEFIR_BUILD_TIMESTAMP=$(KEFIR_BUILD_TIMESTAMP)'
endif
...
KEFIR_LIB_SOURCE := $(wildcard \
	...
	$(SOURCE)/codegen/amd64/pipeline/*.c \
	$(SOURCE)/codegen/amd64/dwarf/*.c \
	$(SOURCE)/codegen/asmcmp/*.c \
	$(SOURCE)/codegen/target-ir/*.c \
	$(SOURCE)/codegen/target-ir/amd64/*.c \
	$(SOURCE)/codegen/target-ir/transform/*.c \
	$(SOURCE)/codegen/target-ir/amd64/transform/*.c \
	$(SOURCE)/codegen/target-ir/amd64/transform/peephole/*.c \
	$(SOURCE)/compiler/*.c \
	$(SOURCE)/ir/*.c \
	$(SOURCE)/lexer/*.c \
...
KEFIR_DRIVER_SOURCE += $(SOURCE)/cc1/cc1.c
KEFIR_DRIVER_SOURCE += $(SOURCE)/cc1/options.c
...
BIN_HEADERS_INCDIR=$(HEADERS)
BIN_HEADERS_CC=$(BOOTSTRAP_CC)
include source/binary_headers.mk

$(BOOTSTRAP)/libkefir.so: $(KEFIR_LIB_ASM_FILES)
	@mkdir -p $(shell dirname "$@")
	@echo "Linking $@"
	@KEFIR_AS=$(KEFIR_AS) KEFIR_LD=$(KEFIR_LD) $(BOOTSTRAP_CC) $(BOOTSTRAP_CFLAGS) -shared -Wl,--build-id=none -Wl,--discard-all -Wl,-soname=libkefir.so $^ -o $@ -lm

ifeq ($(USE_SHARED),yes)
$(BOOTSTRAP)/kefir: $(KEFIR_DRIVER_ASM_FILES) $(BOOTSTRAP)/libkefir.so
	@echo "Linking $@"
	@KEFIR_AS=$(KEFIR_AS) KEFIR_LD=$(KEFIR_LD) $(BOOTSTRAP_CC) -pie $(BOOTSTRAP_CFLAGS) $(KEFIR_DRIVER_ASM_FILES) -Wl,--build-id=none -Wl,--discard-all -o $@ -L$(BOOTSTRAP) -lkefir -lm
else
$(BOOTSTRAP)/kefir: $(KEFIR_ASM_FILES)
	@echo "Linking $@"
	@KEFIR_AS=$(KEFIR_AS) KEFIR_LD=$(KEFIR_LD) $(BOOTSTRAP_CC) $(BOOTSTRAP_CFLAGS) $(KEFIR_ASM_FILES) -Wl,--build-id=none -Wl,--discard-all -o $@ -lm
endif
