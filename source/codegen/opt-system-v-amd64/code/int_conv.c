...
DEFINE_TRANSLATOR(int_conv) {
    ...
    REQUIRE_OK(kefir_codegen_opt_amd64_sysv_storage_acquire(
        ...
        source_allocation, &source_handle, NULL, NULL));
    ...
    switch (instr->operation.opcode) {
        case KEFIR_OPT_OPCODE_INT64_TRUNCATE_1BIT:
            REQUIRE_OK(kefir_asm_amd64_xasmgen_register8(result_handle.location.reg, &result_variant_reg));
        ...
        case KEFIR_OPT_OPCODE_INT64_SIGN_EXTEND_8BITS:
            REQUIRE_OK(kefir_asm_amd64_xasmgen_register8(source_handle.location.reg, &source_variant_reg));
        ...
        case KEFIR_OPT_OPCODE_INT64_SIGN_EXTEND_16BITS:
            REQUIRE_OK(kefir_asm_amd64_xasmgen_register16(source_handle.location.reg, &source_variant_reg));
            REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_MOVSX(&codegen->xasmgen,
                                                       kefir_asm_amd64_xasmgen_operand_reg(result_handle.location.reg),
                                                       kefir_asm_amd64_xasmgen_operand_reg(source_variant_reg)));
            break;
        ...
        case KEFIR_OPT_OPCODE_INT64_SIGN_EXTEND_32BITS:
            REQUIRE_OK(kefir_asm_amd64_xasmgen_register32(source_handle.location.reg, &source_variant_reg));
            REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_MOVSX(&codegen->xasmgen,
                                                       kefir_asm_amd64_xasmgen_operand_reg(result_handle.location.reg),
                                                       kefir_asm_amd64_xasmgen_operand_reg(source_variant_reg)));
            break;
        ...
        case KEFIR_OPT_OPCODE_INT64_ZERO_EXTEND_32BITS:
            REQUIRE_OK(kefir_asm_amd64_xasmgen_register32(source_handle.location.reg, &source_variant_reg));
            REQUIRE_OK(kefir_asm_amd64_xasmgen_register32(result_handle.location.reg, &result_variant_reg));
