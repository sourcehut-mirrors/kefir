...
static kefir_result_t allocator_init_impl(struct kefir_mem *mem,
    ...
    for (kefir_size_t instr_idx = 0; instr_idx < allocator->analysis->linearization_length; instr_idx++) {
        ...
        instr_allocation->constraint.allocation =
            KEFIR_CODEGEN_OPT_LINEAR_REGISTER_ALLOCATOR_CONSTRAINT_REGISTER_OR_STORAGE;
        instr_allocation->constraint.hint.hint = KEFIR_CODEGEN_OPT_LINEAR_REGISTER_ALLOCATOR_CONSTRAINT_HINT_NONE;
...
kefir_result_t kefir_codegen_opt_linear_register_allocator_set_constraint(
    ...
    const struct kefir_codegen_opt_linear_register_allocator_constraint *constraint) {
    REQUIRE(allocator != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid linear register allocator"));
    REQUIRE(constraint != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid linear register allocator constraint"));
...
static kefir_result_t deallocate(struct kefir_codegen_opt_linear_register_allocator *allocator,
    ...
    switch (allocation->constraint.type) {
        case KEFIR_CODEGEN_OPT_LINEAR_REGISTER_ALLOCATOR_CONSTRAINT_GENERAL_PURPOSE:
            REQUIRE_OK(
                kefir_codegen_opt_virtual_register_allocator_deallocte(&allocator->general_purpose, allocation->vreg));
            break;

        case KEFIR_CODEGEN_OPT_LINEAR_REGISTER_ALLOCATOR_CONSTRAINT_FLOATING_POINT:
            REQUIRE_OK(
                kefir_codegen_opt_virtual_register_allocator_deallocte(&allocator->floating_point, allocation->vreg));
            break;
    }
    return KEFIR_OK;
}
...
static kefir_result_t allocator_run_impl(struct kefir_mem *mem,
    ...
    for (kefir_size_t instr_idx = 0; instr_idx < allocator->analysis->linearization_length; instr_idx++) {
        ...
        // Deallocate assignments that are not alive anymore
        for (const struct kefir_list_entry *iter = kefir_list_head(alive); iter != NULL;) {
            ...
            const struct kefir_opt_instruction_liveness_interval *prev_liveness =
                &allocator->analysis->liveness
                     .intervals[allocator->analysis->instructions[prev_instr_ref].linear_position];
            const struct kefir_list_entry *next_iter = iter->next;
        ...
        // Fill in conflicting requirements and hints for the next allocation
        ...
        for (res = kefir_graph_edge_iter(&allocator->allocation, &edge_iter,
                                         (kefir_graph_node_id_t) instr_props->instr_ref, &conflict_edge);
            ...
            ASSIGN_DECL_CAST(const struct kefir_codegen_opt_linear_register_allocation *, conflict_allocation,
                             node->value);
            ...
            switch (conflict_allocation->constraint.hint.hint) {
                case KEFIR_CODEGEN_OPT_LINEAR_REGISTER_ALLOCATOR_CONSTRAINT_HINT_REGISTER:
                    REQUIRE_OK(kefir_hashtreeset_add(
                        mem, conflict_hints, (kefir_hashtreeset_entry_t) conflict_allocation->constraint.hint.vreg));
                    break;

                case KEFIR_CODEGEN_OPT_LINEAR_REGISTER_ALLOCATOR_CONSTRAINT_HINT_SAME_AS: {
                    struct kefir_graph_node *node = NULL;
                    REQUIRE_OK(kefir_graph_node(&allocator->allocation,
                                                (kefir_graph_node_id_t) conflict_allocation->constraint.hint.instr_ref,
                                                &node));
                    ASSIGN_DECL_CAST(const struct kefir_codegen_opt_linear_register_allocation *, ref_allocation,
                                     node->value);
                    if (ref_allocation->done) {
                        REQUIRE_OK(kefir_hashtreeset_add(mem, conflict_hints,
                                                         (kefir_hashtreeset_entry_t) ref_allocation->vreg));
                    }
                } break;

                case KEFIR_CODEGEN_OPT_LINEAR_REGISTER_ALLOCATOR_CONSTRAINT_HINT_NONE:
        ...
        // Attempt allocation according to the hint
        if (current_allocation->constraint.hint.hint ==
            KEFIR_CODEGEN_OPT_LINEAR_REGISTER_ALLOCATOR_CONSTRAINT_HINT_REGISTER) {
            kefir_bool_t success = false;
            REQUIRE_OK(
                allocate_vreg(allocator, current_allocation, current_allocation->constraint.hint.vreg, &success));

            if (success) {
                continue;
            }
        } else if (current_allocation->constraint.hint.hint ==
                   KEFIR_CODEGEN_OPT_LINEAR_REGISTER_ALLOCATOR_CONSTRAINT_HINT_SAME_AS) {
            kefir_bool_t success = false;
            struct kefir_graph_node *node = NULL;
            REQUIRE_OK(kefir_graph_node(&allocator->allocation,
                                        (kefir_graph_node_id_t) current_allocation->constraint.hint.instr_ref, &node));
            ASSIGN_DECL_CAST(const struct kefir_codegen_opt_linear_register_allocation *, ref_allocation, node->value);
            if (ref_allocation->done) {
                REQUIRE_OK(allocate_vreg(allocator, current_allocation, ref_allocation->vreg, &success));
            }
        ...
        switch (current_allocation->constraint.allocation) {
            case KEFIR_CODEGEN_OPT_LINEAR_REGISTER_ALLOCATOR_CONSTRAINT_REGISTER:
                res = kefir_codegen_opt_virtual_register_allocator_allocate_register(
                    vreg_allocator, &current_allocation->vreg, NULL, NULL);
            ...
            case KEFIR_CODEGEN_OPT_LINEAR_REGISTER_ALLOCATOR_CONSTRAINT_REGISTER_OR_STORAGE:
                REQUIRE_OK(kefir_codegen_opt_virtual_register_allocator_allocate_any(mem, vreg_allocator,
                                                                                     &current_allocation->vreg));
                current_allocation->done = true;
                break;
