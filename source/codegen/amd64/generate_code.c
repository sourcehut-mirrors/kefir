...
static kefir_result_t build_operand(const struct kefir_asmcmp_amd64 *target,
                                    ...
                                    const struct kefir_codegen_configuration *codegen_config,
    ...
    switch (value->type) {
        ...
        case KEFIR_ASMCMP_VALUE_TYPE_INTERNAL_LABEL:
            snprintf(arg_state->buf, BUFLEN, KEFIR_AMD64_LABEL, codegen_config->symbol_prefix, target->function_name, value->internal_label);
        ...
        case KEFIR_ASMCMP_VALUE_TYPE_INDIRECT: {
            ...
            switch (value->indirect.type) {
                ...
                case KEFIR_ASMCMP_INDIRECT_INTERNAL_LABEL_BASIS:
                    snprintf(arg_state->buf, BUFLEN, KEFIR_AMD64_LABEL, codegen_config->symbol_prefix, target->function_name,
    ...
    for (res = kefir_asmcmp_inline_assembly_fragment_iter(&target->context, inline_asm_idx, &iter);
        ...
        switch (iter.fragment->type) {
            ...
            case KEFIR_ASMCMP_INLINE_ASSEMBLY_FRAGMENT_VALUE:
                REQUIRE_CHAIN(&res, build_operand(target, stack_frame, &iter.fragment->value, &arg_state, codegen_config,
...
static kefir_result_t generate_instr(struct kefir_mem *mem, struct kefir_amd64_xasmgen *xasmgen,
    ...
    for (kefir_asmcmp_label_index_t label = kefir_asmcmp_context_instr_label_head(&target->context, index);
        ...
        REQUIRE_OK(KEFIR_AMD64_XASMGEN_LABEL(xasmgen, KEFIR_AMD64_LABEL, codegen_config->symbol_prefix, target->function_name, label));
...
#define DEF_OPCODE1(_opcode, _mnemonic, _variant, _flag, _op1)                                                         \
    case KEFIR_ASMCMP_AMD64_OPCODE(_opcode):                                                                           \
        REQUIRE_OK(                                                                                                    \
            build_operand(target, stack_frame, &instr->args[0], &arg_state[0], codegen_config, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT)); \
...
#define DEF_OPCODE2_(_opcode, _mnemonic, _variant, _flag, _op1, _op2)                                                  \
    case KEFIR_ASMCMP_AMD64_OPCODE(_opcode):                                                                           \
        REQUIRE_OK(                                                                                                    \
            build_operand(target, stack_frame, &instr->args[0], &arg_state[0], codegen_config, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT)); \
        REQUIRE_OK(                                                                                                    \
            build_operand(target, stack_frame, &instr->args[1], &arg_state[1], codegen_config, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT)); \
...
#define DEF_OPCODE2(_opcode, _mnemonic, _variant, _flag, _op1, _op2) \
    DEF_OPCODE2_##_variant(_opcode, _mnemonic, _variant, _flag, _op1, _op2)
#define DEF_OPCODE3(_opcode, _mnemonic, _variant, _flag, _op1, _op2, _op3)                                             \
    case KEFIR_ASMCMP_AMD64_OPCODE(_opcode):                                                                           \
        ...
        REQUIRE_OK(                                                                                                    \
            build_operand(target, stack_frame, &instr->args[1], &arg_state[1], codegen_config, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT)); \
        REQUIRE_OK(                                                                                                    \
            build_operand(target, stack_frame, &instr->args[2], &arg_state[2], codegen_config, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT)); \
...
#undef DEF_OPCODE3
        ...
        case KEFIR_ASMCMP_AMD64_OPCODE(virtual_register_link):
            if (!same_operands(&instr->args[0], &instr->args[1])) {
                REQUIRE_OK(build_operand(target, stack_frame, &instr->args[0], &arg_state[0], codegen_config,
                                         KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT));
                REQUIRE_OK(build_operand(target, stack_frame, &instr->args[1], &arg_state[1], codegen_config,
                                         KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT));
...
kefir_result_t kefir_asmcmp_amd64_generate_code(struct kefir_mem *mem, struct kefir_amd64_xasmgen *xasmgen,
    ...
    REQUIRE(codegen_config != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid codege configuration"));
    ...
    for (kefir_asmcmp_instruction_index_t idx = kefir_asmcmp_context_instr_head(&target->context);
        ...
        REQUIRE_OK(generate_instr(mem, xasmgen, target, stack_frame, codegen_config, idx));
    }
