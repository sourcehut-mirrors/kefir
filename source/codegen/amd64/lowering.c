...
static kefir_result_t lower_instruction(struct kefir_mem *mem, struct kefir_codegen_amd64_module *codegen_module,
    ...
    switch (instr->operation.opcode) {
        ...
        case KEFIR_OPT_OPCODE_BITINT_ADD: {
            ...
            if (bitwidth <= QWORD_BITS) {
                if (bitwidth <= 8) {
                    REQUIRE_OK(kefir_opt_code_builder_int8_add(mem, &func->code, block_id, arg1_ref, arg2_ref,
                                                               replacement_ref));
                } else if (bitwidth <= 16) {
                    REQUIRE_OK(kefir_opt_code_builder_int16_add(mem, &func->code, block_id, arg1_ref, arg2_ref,
                                                                replacement_ref));
                } else if (bitwidth <= 32) {
                    REQUIRE_OK(kefir_opt_code_builder_int32_add(mem, &func->code, block_id, arg1_ref, arg2_ref,
                                                                replacement_ref));
                } else if (bitwidth <= QWORD_BITS) {
                    REQUIRE_OK(kefir_opt_code_builder_int64_add(mem, &func->code, block_id, arg1_ref, arg2_ref,
                                                                replacement_ref));
                }
            } else {
                const kefir_size_t qwords = (bitwidth + QWORD_BITS - 1) / QWORD_BITS;
                ...
                kefir_id_t func_decl_id = KEFIR_ID_NONE;
                REQUIRE_OK(get_bigint_add_function_decl_id(mem, codegen_module, module, param, &func_decl_id));
