...
struct virtual_block_data {
    ...
    struct kefir_hashset virtual_registers;
};

static kefir_result_t virtual_block_free(struct kefir_mem *mem, struct kefir_hashtable *table, kefir_hashtable_key_t key,
    ...
    REQUIRE_OK(kefir_hashset_free(mem, &data->virtual_registers));
    KEFIR_FREE(mem, data);
    return KEFIR_OK;
}
...
struct do_allocation_state {
    ...
    struct kefir_hashset active_registers;
...
#define VREG_UNDEFINED_LOCAL_LIFETIME ((kefir_uint64_t) - 1ll)
#define VREG_LOCAL_LIFETIME(_begin, _end) ((((kefir_uint64_t) (_begin)) << 32) | (kefir_uint32_t) (_end))
#define VREG_LOCAL_LIFETIME_BEGIN(_lifetime) (((kefir_uint64_t) (_lifetime)) >> 32)
#define VREG_LOCAL_LIFETIME_END(_lifetime) ((kefir_uint64_t) (kefir_uint32_t) (_lifetime))
...
static kefir_result_t add_virtual_register_to_block(struct kefir_mem *mem, const struct kefir_asmcmp_context *context,
    ...
    if (asmcmp_vreg->type == KEFIR_ASMCMP_VIRTUAL_REGISTER_PAIR) {
        REQUIRE_OK(add_virtual_register_to_block(
            mem, context, block_data, asmcmp_vreg->parameters.pair.virtual_registers[0], linear_index, vreg));
        REQUIRE_OK(add_virtual_register_to_block(
            mem, context, block_data, asmcmp_vreg->parameters.pair.virtual_registers[1], linear_index, vreg));
    }
    REQUIRE_OK(kefir_hashset_add(mem, &block_data->virtual_registers, (kefir_hashset_key_t) vreg_idx));

    kefir_bool_t leaf_block = true;
    for (; block_data != NULL; block_data = block_data->parent, leaf_block = false) {
        kefir_hashtable_value_t *value_ptr;
        kefir_result_t res =
            kefir_hashtable_at_mut(&vreg->virtual_blocks, (kefir_hashtree_key_t) block_data->virtual_block_id, &value_ptr);
        if (res != KEFIR_NOT_FOUND) {
            REQUIRE_OK(res);
            if (leaf_block) {
                const kefir_size_t begin = VREG_LOCAL_LIFETIME_BEGIN(*value_ptr);
                const kefir_size_t end = VREG_LOCAL_LIFETIME_END(*value_ptr);
                *value_ptr = VREG_LOCAL_LIFETIME(MIN(begin, linear_index), MAX(end, linear_index));
            }
        } else if (leaf_block) {
            const kefir_hashtable_value_t local_range = VREG_LOCAL_LIFETIME(linear_index, linear_index);
            REQUIRE_OK(kefir_hashtable_insert(mem, &vreg->virtual_blocks,
                                             (kefir_hashtable_key_t) block_data->virtual_block_id, local_range));
        } else {
            REQUIRE_OK(kefir_hashtable_insert(mem, &vreg->virtual_blocks,
                                             (kefir_hashtable_key_t) block_data->virtual_block_id,
                                             (kefir_hashtable_value_t) VREG_UNDEFINED_LOCAL_LIFETIME));
        }
    }
    return KEFIR_OK;
}

static kefir_result_t mark_vreg_lifetime(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *target,
                                         struct kefir_codegen_amd64_xregalloc *xregalloc,
                                         kefir_codegen_amd64_xregalloc_virtual_block_id_t virtual_block_id,
                                         kefir_asmcmp_virtual_register_index_t vreg_idx, kefir_size_t linear_index) {
    const struct kefir_asmcmp_virtual_register *asmcmp_vreg;
    REQUIRE_OK(kefir_asmcmp_virtual_register_get(&target->context, vreg_idx, &asmcmp_vreg));
    if (asmcmp_vreg->type == KEFIR_ASMCMP_VIRTUAL_REGISTER_PAIR) {
        REQUIRE_OK(mark_vreg_lifetime(mem, target, xregalloc, virtual_block_id,
                                      asmcmp_vreg->parameters.pair.virtual_registers[0], linear_index));
        REQUIRE_OK(mark_vreg_lifetime(mem, target, xregalloc, virtual_block_id,
                                      asmcmp_vreg->parameters.pair.virtual_registers[1], linear_index));
    }

    struct kefir_codegen_amd64_xregalloc_virtual_register *vreg = &xregalloc->virtual_registers[vreg_idx];
    if (vreg->global_lifetime.begin == KEFIR_CODEGEN_AMD64_XREGALLOC_UNDEFINED) {
        vreg->global_lifetime.begin = linear_index;
        vreg->global_lifetime.end = linear_index;
    } else {
        vreg->global_lifetime.begin = MIN(vreg->global_lifetime.begin, linear_index);
        vreg->global_lifetime.end = MAX(vreg->global_lifetime.end, linear_index);
    }

    kefir_hashtable_value_t *value_ptr;
    kefir_result_t res = kefir_hashtable_at_mut(&vreg->virtual_blocks, (kefir_hashtable_key_t) virtual_block_id, &value_ptr);
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
        const kefir_size_t begin = VREG_LOCAL_LIFETIME_BEGIN(*value_ptr);
        const kefir_size_t end = VREG_LOCAL_LIFETIME_END(*value_ptr);
        *value_ptr = VREG_LOCAL_LIFETIME(MIN(begin, linear_index), MAX(end, linear_index));
        ...
        const kefir_hashtable_value_t local_range = VREG_LOCAL_LIFETIME(linear_index, linear_index);
        REQUIRE_OK(
            kefir_hashtable_insert(mem, &vreg->virtual_blocks, (kefir_hashtable_key_t) virtual_block_id, local_range));
    }
    return KEFIR_OK;
}
...
static kefir_result_t scan_virtual_register(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *target,
    ...
    switch (value->type) {
        ...
        case KEFIR_ASMCMP_VALUE_TYPE_VIRTUAL_REGISTER: {
            ...
            REQUIRE_OK(mark_vreg_lifetime(mem, target, xregalloc, block_data->virtual_block_id, value->vreg.index,
                                          linear_index));
            REQUIRE_OK(add_virtual_register_to_block(mem, &target->context, block_data, value->vreg.index, linear_index,
                                                     vreg));
        } break;
        ...
        case KEFIR_ASMCMP_VALUE_TYPE_INDIRECT:
            switch (value->indirect.type) {
                case KEFIR_ASMCMP_INDIRECT_VIRTUAL_BASIS: {
                    ...
                    REQUIRE_OK(mark_vreg_lifetime(mem, target, xregalloc, block_data->virtual_block_id,
                                                  value->indirect.base.vreg, linear_index));
                    REQUIRE_OK(add_virtual_register_to_block(mem, &target->context, block_data,
                                                             value->indirect.base.vreg, linear_index, vreg));
                } break;
        ...
        case KEFIR_ASMCMP_VALUE_TYPE_STASH_INDEX: {
            ...
            struct kefir_codegen_amd64_xregalloc_virtual_register *vreg = &xregalloc->virtual_registers[vreg_idx];
            REQUIRE_OK(
                mark_vreg_lifetime(mem, target, xregalloc, block_data->virtual_block_id, vreg_idx, linear_index));
            REQUIRE_OK(add_virtual_register_to_block(mem, &target->context, block_data, vreg_idx, linear_index, vreg));
        } break;
    }
    return KEFIR_OK;
}
...
static kefir_result_t push_virtual_block(struct kefir_mem *mem, struct kefir_codegen_amd64_xregalloc *xregalloc,
                                         ...
                                         kefir_codegen_amd64_xregalloc_virtual_block_id_t virtual_block_id) {
    REQUIRE(!kefir_hashtable_has(&xregalloc->virtual_blocks, (kefir_hashtable_key_t) virtual_block_id),
    ...
    kefir_result_t res = kefir_hashset_init(&data->virtual_registers, &kefir_hashtable_uint_ops);
    REQUIRE_CHAIN(&res, kefir_hashtable_insert(mem, &xregalloc->virtual_blocks, (kefir_hashtable_key_t) virtual_block_id,
                                              (kefir_hashtable_value_t) data));
...
static kefir_result_t propagate_virtual_register_local_lifetime(
    struct virtual_block_data *virtual_block, struct kefir_codegen_amd64_xregalloc_virtual_register *vreg) {
    kefir_hashtable_value_t base_value;
    REQUIRE_OK(kefir_hashtable_at(&vreg->virtual_blocks, (kefir_hashtree_key_t) virtual_block->virtual_block_id, &base_value));
    kefir_size_t begin = VREG_LOCAL_LIFETIME_BEGIN(base_value);
    kefir_size_t end = VREG_LOCAL_LIFETIME_END(base_value);

    for (struct virtual_block_data *virtual_block_iter = virtual_block; virtual_block_iter != NULL;
         virtual_block_iter = virtual_block_iter->parent) {
        kefir_hashtable_value_t *value_ptr;
        kefir_result_t res = kefir_hashtable_at_mut(&vreg->virtual_blocks,
                                               (kefir_hashtree_key_t) virtual_block_iter->virtual_block_id, &value_ptr);
        if (res == KEFIR_NOT_FOUND) {
            continue;
        }
        REQUIRE_OK(res);
        if (*value_ptr != VREG_UNDEFINED_LOCAL_LIFETIME) {
            begin = MIN(VREG_LOCAL_LIFETIME_BEGIN(*value_ptr), begin);
            end = MAX(VREG_LOCAL_LIFETIME_END(*value_ptr), end);
            *value_ptr = VREG_LOCAL_LIFETIME(begin, end);
        }
    }
    return KEFIR_OK;
}

static kefir_result_t propagate_virtual_register_local_lifetimes(struct kefir_codegen_amd64_xregalloc *xregalloc,
                                                                 kefir_asmcmp_virtual_register_index_t vreg_idx) {
    struct kefir_codegen_amd64_xregalloc_virtual_register *vreg = &xregalloc->virtual_registers[vreg_idx];
    ...
    kefir_hashtable_value_t iter_value;
    for (res = kefir_hashtable_iter(&vreg->virtual_blocks, &iter, &iter_key, &iter_value); res == KEFIR_OK;
         res = kefir_hashtable_next(&iter, &iter_key, &iter_value)) {
        ASSIGN_DECL_CAST(kefir_codegen_amd64_xregalloc_virtual_block_id_t, virtual_block_id, iter_key)
        if (iter_value != VREG_UNDEFINED_LOCAL_LIFETIME) {
            kefir_hashtable_value_t value;
            REQUIRE_OK(kefir_hashtable_at(&xregalloc->virtual_blocks, (kefir_hashtable_key_t) virtual_block_id, &value));
            ASSIGN_DECL_CAST(struct virtual_block_data *, virtual_block, value);
            REQUIRE_OK(propagate_virtual_register_local_lifetime(virtual_block, vreg));
        }
    }
...
static kefir_result_t scan_code(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *code,
    ...
    for (const struct kefir_hashtree_node *node = kefir_hashtree_iter(&state->preserve_vreg_locations, &iter);
        ...
        for (res = kefir_codegen_amd64_xregalloc_block_iter(xregalloc, block_id, &iter2, &vreg_idx); res == KEFIR_OK;
            ...
            struct kefir_codegen_amd64_xregalloc_virtual_register *vreg = &xregalloc->virtual_registers[vreg_idx];
            if (vreg->global_lifetime.begin <= linear_index && linear_index <= vreg->global_lifetime.end) {
                vreg->global_lifetime.begin = 0;
                vreg->global_lifetime.end = xregalloc->linear_code_length;
                REQUIRE_OK(add_virtual_register_to_block(mem, &code->context, state->base_virtual_block, vreg_idx,
                                                         linear_index, vreg));
            }
        }
        if (res != KEFIR_ITERATOR_END) {
    ...
    for (kefir_size_t vreg_idx = 0; vreg_idx < xregalloc->virtual_register_length; vreg_idx++) {
        REQUIRE_OK(propagate_virtual_register_local_lifetimes(xregalloc, vreg_idx));
    }
    return KEFIR_OK;
}
...
static kefir_result_t build_virtual_register_interference(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *code,
    ...
    struct kefir_hashtable_iterator iter;
    kefir_hashtable_key_t iter_key;
    for (res = kefir_hashtable_iter(&xregalloc->virtual_blocks, &iter, &iter_key, NULL);
        res == KEFIR_OK;
...
static kefir_result_t build_virtual_register_allocation_order(struct kefir_mem *mem,
    ...
    for (kefir_size_t i = 0; i < xregalloc->virtual_register_length; i++) {
        ...
        const kefir_bool_t whole_function_lifetime =
            vreg->global_lifetime.begin == 0 && vreg->global_lifetime.end == xregalloc->linear_code_length &&
            kefir_hashset_has(&state->base_virtual_block->virtual_registers, (kefir_hashset_key_t) i);
        const kefir_uint64_t lifetime_duration = vreg->global_lifetime.end - vreg->global_lifetime.begin;
        ...
        const kefir_uint32_t lower_half = vreg->global_lifetime.begin & mask;
...
static kefir_result_t add_active_virtual_register(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *code,
    ...
    switch (interfere_vreg->allocation.type) {
            ...
            if (preallocation != NULL) {
                switch (preallocation->type) {
                    case KEFIR_ASMCMP_AMD64_REGISTER_PREALLOCATION_REQUIREMENT: {
                        ...
                        REQUIRE_OK(
                            kefir_hashset_add(mem, &state->active_registers, (kefir_hashset_key_t) reg));
                    } break;
        ...
        case KEFIR_CODEGEN_AMD64_VIRTUAL_REGISTER_ALLOCATION_REGISTER:
            REQUIRE_OK(kefir_hashset_add(mem, &state->active_registers,
                                             (kefir_hashset_key_t) interfere_vreg->allocation.direct_reg));
            break;
...
static kefir_result_t build_active_virtual_registers(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *code,
                                                     ...
                                                     struct do_allocation_state *state,
                                                     struct kefir_codegen_amd64_xregalloc_virtual_register *vreg) {
    REQUIRE_OK(kefir_hashset_clear(mem, &state->active_registers));
    ...
    kefir_result_t res;
    for (res = kefir_hashset_iter(&vreg->interference, &iter, &iter_key); res == KEFIR_OK;
         res = kefir_hashset_next(&iter, &iter_key)) {
        ASSIGN_DECL_CAST(kefir_asmcmp_virtual_register_index_t, interfere_vreg_idx, iter_key);
...
static kefir_result_t do_allocate_register(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *code,
    ...
    if (preallocation != NULL && preallocation->type == KEFIR_ASMCMP_AMD64_REGISTER_PREALLOCATION_REQUIREMENT) {
        ASSIGN_DECL_CAST(kefir_asm_amd64_xasmgen_register_t, reg, preallocation->reg);
        REQUIRE_OK(kefir_asm_amd64_xasmgen_register_widest(reg, &reg));
        REQUIRE(!kefir_hashset_has(&state->active_registers, (kefir_hashset_key_t) reg),
        ...
        REQUIRE_OK(kefir_hashset_add(mem, &state->active_registers, (kefir_hashset_key_t) reg));
        return KEFIR_OK;
    }
    ...
    if (preallocation != NULL && preallocation->type == KEFIR_ASMCMP_AMD64_REGISTER_PREALLOCATION_HINT) {
        ASSIGN_DECL_CAST(kefir_asm_amd64_xasmgen_register_t, reg, preallocation->reg);
        REQUIRE_OK(kefir_asm_amd64_xasmgen_register_widest(reg, &reg));
        if (!kefir_hashset_has(&state->active_registers, (kefir_hashset_key_t) reg)) {
    ...
    if (preallocation != NULL && preallocation->type == KEFIR_ASMCMP_AMD64_REGISTER_PREALLOCATION_SAME_AS &&
        !kefir_hashset_has(&vreg->interference, preallocation->vreg)) {
        const struct kefir_codegen_amd64_xregalloc_virtual_register *other_vreg =
            &xregalloc->virtual_registers[preallocation->vreg];
...
#define ALLOC_REG(_reg)                                                                                       \
    do {                                                                                                      \
        ...
        REQUIRE_OK(kefir_hashset_add(mem, &state->active_registers, (kefir_hashset_key_t) (_reg))); \
        return KEFIR_OK;                                                                                      \
    } while (0)
    for (kefir_size_t i = 0; i < available_registers_length; i++) {
...
static kefir_result_t do_vreg_allocation(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *code,
    ...
    REQUIRE_OK(kefir_hashset_merge(mem, &xregalloc->used_registers, &state->active_registers));
...
static kefir_result_t do_stash_allocation(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *code,
    ...
    for (res = kefir_hashset_iter(&vreg->interference, &iter, &iter_key); res == KEFIR_OK;
        ...
        if (interfere_vreg->global_lifetime.begin > liveness_linear_idx ||
            interfere_vreg->global_lifetime.end <= liveness_linear_idx ||
            interfere_vreg->global_lifetime.begin > activation_linear_idx ||
            interfere_vreg->global_lifetime.end <= activation_linear_idx) {
            continue;
        }
...
static kefir_result_t do_allocation_impl(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *code,
    ...
    kefir_result_t res;
    struct kefir_hashset_iterator used_reg_iter;
    kefir_hashset_key_t used_reg_iter_key;
    for (res = kefir_hashset_iter(&xregalloc->used_registers, &used_reg_iter, &used_reg_iter_key); res == KEFIR_OK;
         res = kefir_hashset_next(&used_reg_iter, &used_reg_iter_key)) {
...
kefir_result_t kefir_codegen_amd64_xregalloc_run(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *code,
    ...
    for (kefir_size_t i = 0; i < xregalloc->virtual_register_length; i++) {
        ...
        xregalloc->virtual_registers[i].global_lifetime.begin = KEFIR_CODEGEN_AMD64_XREGALLOC_UNDEFINED;
        xregalloc->virtual_registers[i].global_lifetime.end = KEFIR_CODEGEN_AMD64_XREGALLOC_UNDEFINED;
        REQUIRE_OK(kefir_hashset_init(&xregalloc->virtual_registers[i].interference, &kefir_hashtable_uint_ops));
        REQUIRE_OK(kefir_hashtable_init(&xregalloc->virtual_registers[i].virtual_blocks, &kefir_hashtable_uint_ops));
    }
    ...
    REQUIRE_OK(kefir_hashset_init(&state.active_registers, &kefir_hashtable_uint_ops));
    ...
    kefir_hashset_free(mem, &state.active_registers);
...
kefir_result_t kefir_codegen_amd64_xregalloc_lifetime_of(
    ...
    for (struct virtual_block_data *virtual_block_iter = virtual_block; virtual_block_iter != NULL;
         virtual_block_iter = virtual_block_iter->parent) {
        res = kefir_hashtable_at_mut(&xregalloc->virtual_registers[vreg_idx].virtual_blocks,
                                (kefir_hashtable_key_t) virtual_block_iter->virtual_block_id, &value_ptr);
        if (res == KEFIR_NOT_FOUND) {
            continue;
        }
        REQUIRE_OK(res);
        if (*value_ptr != (kefir_hashtree_value_t) VREG_UNDEFINED_LOCAL_LIFETIME) {
            virtual_block = virtual_block_iter;
            topmost_nonempty_value_ptr = value_ptr;
        }
    }
...
kefir_result_t kefir_codegen_amd64_xregalloc_exists_in_block(const struct kefir_codegen_amd64_xregalloc *xregalloc,
    ...
    kefir_hashtable_value_t value;
    kefir_result_t res = kefir_hashtable_at(&xregalloc->virtual_blocks, (kefir_hashtable_key_t) block_id, &value);
    if (res == KEFIR_NOT_FOUND) {
    ...
    for (; !exists && virtual_block != NULL; virtual_block = virtual_block->parent) {
        if (kefir_hashset_has(&virtual_block->virtual_registers, (kefir_hashset_key_t) vreg_idx)) {
            exists = true;
        }
    }
    *exists_flag = exists;
...
kefir_bool_t kefir_codegen_amd64_xregalloc_has_used_register(const struct kefir_codegen_amd64_xregalloc *xregalloc,
    ...
    return kefir_hashset_has(&xregalloc->used_registers, reg);
}
...
kefir_result_t kefir_codegen_amd64_xregalloc_block_iter(
    ...
    while (iter->virtual_block != NULL) {
        kefir_result_t res = kefir_hashset_iter(&iter->virtual_block->virtual_registers, &iter->iter, &entry);
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
