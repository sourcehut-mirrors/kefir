...
struct devirtualize_state {
    ...
    struct {
        kefir_uint8_t *physical_regs;
        kefir_uint8_t *required_physical_regs;
        kefir_size_t num_of_regs;
        struct kefir_hashtable virtual_regs;
...
static kefir_result_t mark_alive_virtual_reg(struct kefir_mem *mem, struct devirtualize_state *state,
    ...
    if (linear_position >= lifetime_begin && linear_position <= lifetime_end) {
        REQUIRE_OK(kefir_hashtable_insert_or_update(mem, &state->alive.virtual_regs, (kefir_hashtable_key_t) vreg_idx, (kefir_hashtable_value_t) 0));
    }
...
static kefir_result_t collect_live_vregs(struct kefir_mem *mem, struct devirtualize_state *state,
    ...
    REQUIRE_OK(kefir_hashtable_clear(&state->alive.virtual_regs));
    ...
    switch (reg_alloc->type) {
        ...
        case KEFIR_CODEGEN_AMD64_VIRTUAL_REGISTER_ALLOCATION_REGISTER: {
            ...
            if (preallocation != NULL && preallocation->type == KEFIR_ASMCMP_AMD64_REGISTER_PREALLOCATION_REQUIREMENT) {
                state->alive.required_physical_regs[reg_alloc->direct_reg] = 1;
            }
        } break;
        ...
        case KEFIR_CODEGEN_AMD64_VIRTUAL_REGISTER_ALLOCATION_PAIR: {
            const struct kefir_asmcmp_virtual_register *asmcmp_vreg;
            REQUIRE_OK(kefir_asmcmp_virtual_register_get(&state->target->context, vreg_idx, &asmcmp_vreg));
            REQUIRE_OK(rebuild_alive_physical_reg(mem, state, asmcmp_vreg->parameters.pair.virtual_registers[0]));
            REQUIRE_OK(rebuild_alive_physical_reg(mem, state, asmcmp_vreg->parameters.pair.virtual_registers[1]));
        } break;
...
static kefir_result_t rebuild_alive_physical_regs(struct kefir_mem *mem, struct devirtualize_state *state) {
    if (state->alive.num_of_regs > 0) {
        memset(state->alive.physical_regs, 0, sizeof(kefir_uint8_t) * state->alive.num_of_regs);
        memset(state->alive.required_physical_regs, 0, sizeof(kefir_uint8_t) * state->alive.num_of_regs);
    }
    ...
    kefir_result_t res;
    for (res = kefir_hashtable_iter(&state->alive.virtual_regs, &iter, &iter_key, NULL); res == KEFIR_OK;
         res = kefir_hashtable_next(&iter, &iter_key, NULL)) {
        ASSIGN_DECL_CAST(kefir_asmcmp_virtual_register_index_t, vreg_idx, iter_key);
...
static kefir_result_t obtain_temporary_register(struct kefir_mem *mem, struct devirtualize_state *state,
    ...
    for (kefir_size_t i = 0; i < length; i++) {
        ...
        if (!kefir_hashtreeset_has(&state->current_instr_physical_regs, (kefir_hashtreeset_entry_t) candidate) &&
            (candidate >= state->alive.num_of_regs || !state->alive.physical_regs[candidate]) &&
    ...
    for (kefir_size_t i = 0; i < length; i++) {
        ...
        if (!kefir_hashtreeset_has(&state->current_instr_physical_regs, (kefir_hashtreeset_entry_t) candidate) &&
            !kefir_hashtree_has(&state->evicted_physical_regs, (kefir_hashtree_key_t) candidate) &&
            (candidate >= state->alive.num_of_regs || !state->alive.required_physical_regs[candidate])) {
    ...
    switch (value->type) {
        ...
        case KEFIR_ASMCMP_VALUE_TYPE_VIRTUAL_REGISTER:
            ...
            switch (reg_alloc->type) {
                ...
                case KEFIR_CODEGEN_AMD64_VIRTUAL_REGISTER_ALLOCATION_PAIR:
                    return KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Unable to devirtualize pair of virtual registers");
                ...
                case KEFIR_CODEGEN_AMD64_VIRTUAL_REGISTER_ALLOCATION_LOCAL_VARIABLE: {
                    ...
                    if (offset >= KEFIR_INT16_MIN && offset <= KEFIR_INT16_MAX) {
                        struct kefir_asmcmp_value arg = KEFIR_ASMCMP_MAKE_INDIRECT_LOCAL_AREA(
                            vreg->parameters.local_variable.identifier, offset, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT);
                        REQUIRE_OK(devirtualize_value(mem, state, position, &arg));
                        REQUIRE_OK(kefir_asmcmp_amd64_lea(
                            mem, state->target, kefir_asmcmp_context_instr_prev(&state->target->context, position),
                            &KEFIR_ASMCMP_MAKE_PHREG(phreg), &arg, &new_position));
                    } else if (offset >= KEFIR_INT32_MIN && offset <= KEFIR_INT32_MAX) {
                        struct kefir_asmcmp_value arg = KEFIR_ASMCMP_MAKE_INDIRECT_LOCAL_AREA(
                            vreg->parameters.local_variable.identifier, 0, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT);
                        REQUIRE_OK(devirtualize_value(mem, state, position, &arg));
                        REQUIRE_OK(kefir_asmcmp_amd64_lea(
                            mem, state->target, kefir_asmcmp_context_instr_prev(&state->target->context, position),
                            &KEFIR_ASMCMP_MAKE_PHREG(phreg), &arg, &new_position));
            ...
            switch (value->indirect.type) {
                ...
                case KEFIR_ASMCMP_INDIRECT_VIRTUAL_BASIS:
                    ...
                    switch (reg_alloc->type) {
                        ...
                        case KEFIR_CODEGEN_AMD64_VIRTUAL_REGISTER_ALLOCATION_PAIR:
                            return KEFIR_SET_ERROR(KEFIR_INVALID_STATE,
                                                   "Unable to devirtualize pair of virtual registers");
                        ...
                        case KEFIR_CODEGEN_AMD64_VIRTUAL_REGISTER_ALLOCATION_LOCAL_VARIABLE: {
                            ...
                            const kefir_int64_t offset =
                                vreg->parameters.local_variable.offset + value->indirect.offset;
                            if (offset >= KEFIR_INT16_MIN && offset <= KEFIR_INT16_MAX) {
                                *value = KEFIR_ASMCMP_MAKE_INDIRECT_LOCAL_AREA(
                                    vreg->parameters.local_variable.identifier, offset, value->indirect.variant);
                                REQUIRE_OK(devirtualize_value(mem, state, position, value));
                            } else if (offset >= KEFIR_INT32_MIN && offset <= KEFIR_INT32_MAX) {
                                REQUIRE_OK(
                                    obtain_temporary_register(mem, state, position, &phreg, TEMPORARY_REGISTER_GP));
                                struct kefir_asmcmp_value arg =
                                    KEFIR_ASMCMP_MAKE_INDIRECT_LOCAL_AREA(vreg->parameters.local_variable.identifier, 0,
                        ...
                        case KEFIR_CODEGEN_AMD64_VIRTUAL_REGISTER_ALLOCATION_MEMORY_POINTER:
                            ...
                            *value = KEFIR_ASMCMP_MAKE_INDIRECT_PHYSICAL(
                                vreg->parameters.memory.base_reg,
                                vreg->parameters.memory.offset + value->indirect.offset, value->indirect.variant);
                            break;
                    }
                    break;
            }
                ...
                if (!DEVIRT_HAS_FLAG(op_flags, KEFIR_AMD64_INSTRDB_GP_REGISTER)) {
                    ...
                    if (DEVIRT_HAS_FLAG(op_flags, KEFIR_AMD64_INSTRDB_WRITE)) {
                        if (DEVIRT_HAS_FLAG(op_flags, KEFIR_AMD64_INSTRDB_XMM_REGISTER_SINGLE)) {
                            kefir_asm_amd64_xasmgen_register_t reg32;
                            REQUIRE_OK(kefir_asm_amd64_xasmgen_register32(original.phreg, &reg32));
                            REQUIRE_OK(kefir_asmcmp_amd64_movd(mem, state->target, instr_idx,
...
static kefir_result_t activate_stash(struct kefir_mem *mem, struct devirtualize_state *state,
    ...
    if (liveness_instr_idx != KEFIR_ASMCMP_INDEX_NONE) {
        REQUIRE_OK(
            kefir_codegen_amd64_xregalloc_linear_position_of(state->xregalloc, liveness_instr_idx, &liveness_idx));
    }
    ...
    for (res = kefir_hashtable_iter(&state->alive.virtual_regs, &iter, &iter_key, NULL); res == KEFIR_OK;
        ...
        REQUIRE_CHAIN(&res, kefir_asmcmp_register_stash_has_virtual(&state->target->context, stash_idx, vreg_idx,
                                                                    &contains_vreg));
        if (res == KEFIR_NOT_FOUND) {
            continue;
        }
        ...
        if (((liveness_instr_idx != KEFIR_ASMCMP_INDEX_NONE &&
              !(exists_in_block && liveness_begin != KEFIR_CODEGEN_AMD64_XREGALLOC_UNDEFINED &&
                liveness_begin <= liveness_idx && liveness_end > liveness_idx))) ||
            !contains_phreg || !contains_vreg) {
            continue;
        }
    ...
    if (reg_alloc1->type == KEFIR_CODEGEN_AMD64_VIRTUAL_REGISTER_ALLOCATION_PAIR &&
        ...
        switch (vreg->parameters.pair.type) {
            case KEFIR_ASMCMP_VIRTUAL_REGISTER_PAIR_FLOAT_SINGLE:
                ...
                link_part_instr[0].args[1] = KEFIR_ASMCMP_MAKE_INDIRECT_SPILL(reg_alloc2->spill_area.index, 0,
                ...
                link_part_instr[1].args[1] = KEFIR_ASMCMP_MAKE_INDIRECT_SPILL(
        ...
        REQUIRE_OK(devirtualize_instr2(
            mem, state, instr_idx, &link_part_instr[0], original_instr, tail_idx, KEFIR_AMD64_INSTRDB_NONE,
            KEFIR_AMD64_INSTRDB_WRITE | KEFIR_AMD64_INSTRDB_GP_REGISTER_MEMORY |
                KEFIR_AMD64_INSTRDB_XMM_REGISTER_MEMORY_FULL,
            KEFIR_AMD64_INSTRDB_READ | KEFIR_AMD64_INSTRDB_GP_REGISTER_MEMORY |
                KEFIR_AMD64_INSTRDB_XMM_REGISTER_MEMORY_FULL | KEFIR_AMD64_INSTRDB_IMMEDIATE));
        REQUIRE_OK(devirtualize_instr2(
            mem, state, instr_idx, &link_part_instr[1], original_instr, tail_idx, KEFIR_AMD64_INSTRDB_NONE,
            KEFIR_AMD64_INSTRDB_WRITE | KEFIR_AMD64_INSTRDB_GP_REGISTER_MEMORY |
                KEFIR_AMD64_INSTRDB_XMM_REGISTER_MEMORY_FULL,
            KEFIR_AMD64_INSTRDB_READ | KEFIR_AMD64_INSTRDB_GP_REGISTER_MEMORY |
                KEFIR_AMD64_INSTRDB_XMM_REGISTER_MEMORY_FULL | KEFIR_AMD64_INSTRDB_IMMEDIATE));

        REQUIRE_OK(kefir_asmcmp_context_instr_insert_after(mem, &state->target->context, *tail_idx, &link_part_instr[0],
                                                           tail_idx));
        REQUIRE_OK(kefir_asmcmp_context_instr_insert_after(mem, &state->target->context, *tail_idx, &link_part_instr[1],
                                                           tail_idx));
        instr->opcode = KEFIR_ASMCMP_AMD64_OPCODE(noop);
        ...
        link_part_instr[1].args[1] = KEFIR_ASMCMP_MAKE_VREG(vreg2->parameters.pair.virtual_registers[1]);
        link_part_instr[1].args[2].type = KEFIR_ASMCMP_VALUE_TYPE_NONE;
        REQUIRE_OK(link_virtual_registers(mem, state, instr_idx, &link_part_instr[0], original_instr, tail_idx));
        REQUIRE_OK(link_virtual_registers(mem, state, instr_idx, &link_part_instr[1], original_instr, tail_idx));
...
static kefir_result_t devirtualize_impl(struct kefir_mem *mem, struct devirtualize_state *state) {
    ...
    for (kefir_asmcmp_instruction_index_t idx = kefir_asmcmp_context_instr_head(&state->target->context);
        ...
        kefir_result_t res =
            kefir_codegen_amd64_xregalloc_linear_position_of(state->xregalloc, idx, &instr_linear_position);
        if (res == KEFIR_NOT_FOUND) {
            continue;
        } else {
            REQUIRE_OK(res);
...
#undef DEF_OPCODE3
            ...
            case KEFIR_ASMCMP_AMD64_OPCODE(virtual_register_link):
                REQUIRE_OK(
                    do_link_virtual_registers(mem, state, idx, &devirtualized_instr, &original_instr, &tail_idx));
                break;
...
kefir_result_t kefir_codegen_amd64_devirtualize(struct kefir_mem *mem, struct kefir_asmcmp_amd64 *target,
    ...
    REQUIRE_OK(kefir_hashtable_init(&state.alive.virtual_regs, &kefir_hashtable_uint_ops));
    ...
    kefir_hashtable_free(mem, &state.alive.virtual_regs);
    kefir_hashtreeset_free(mem, &state.current_instr_physical_regs);
    KEFIR_FREE(mem, state.alive.physical_regs);
    KEFIR_FREE(mem, state.alive.required_physical_regs);
    return res;
}
