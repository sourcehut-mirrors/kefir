...
static kefir_result_t calculate_offsets(struct kefir_codegen_amd64_stack_frame *frame) {
    ...
    frame->offsets.top_of_frame = PAD_NEGATIVE(frame->offsets.spill_area, frame->sizes.total_alignment);
...
kefir_result_t kefir_codegen_amd64_stack_frame_register_epilogue(struct kefir_codegen_amd64_stack_frame *frame) {
    REQUIRE(frame != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid amd64 stack frame"));
...
kefir_result_t kefir_codegen_amd64_stack_frame_fused_epilogue(kefir_abi_amd64_variant_t abi_variant,
    ...
    // Costs estimate the length of prologue as generated by routine below
    kefir_size_t epilogue_cost =
        used_callee_preserved + frame->requirements.mxcsr_save + frame->requirements.x87_control_word_save;
    ...
    kefir_size_t total_epilogue_cost = epilogue_cost * frame->epilogues;

    ASSIGN_PTR(fused, total_epilogue_cost > 24 && epilogue_cost > 4);
    return KEFIR_OK;
}
...
kefir_result_t kefir_codegen_amd64_stack_frame_epilogue(
    ...
    struct kefir_codegen_amd64_stack_frame_fused_epilogue *fused_epilogue,
    const struct kefir_codegen_amd64_stack_frame *frame) {
    REQUIRE(xasmgen != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid amd64 assembly generator"));
    REQUIRE(frame != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid amd64 stack frame"));
    ...
    if (fused_epilogue != NULL && fuse_epilogue) {
        if (!fused_epilogue->generated) {
            ...
            REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_JMP(
                xasmgen, kefir_asm_amd64_xasmgen_operand_label(&operands[0], KEFIR_AMD64_XASMGEN_SYMBOL_ABSOLUTE,
                                                               fused_epilogue->label)));
            return KEFIR_OK;
        }
    }
    ...
    if (fused_epilogue != NULL) {
        REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_RET(xasmgen));
    }
    return KEFIR_OK;
}
