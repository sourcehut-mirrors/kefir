...
kefir_result_t kefir_codegen_amd64_dwarf_collect_code_fragments(struct kefir_mem *mem,
                                                                struct kefir_codegen_amd64_function *codegen_function,
                                                                kefir_opt_code_debug_info_code_ref_t begin_ref,
                                                                kefir_opt_code_debug_info_code_ref_t end_ref,
                                                                struct kefir_hashtree *fragment_tree) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(codegen_function != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid amd64 codegen function"));
    REQUIRE(fragment_tree != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid amd64 dwarf fragment tree"));
    ...
    for (kefir_opt_code_debug_info_code_ref_t code_ref = begin_ref; code_ref <= end_ref; code_ref++) {
        ...
        for (res = kefir_hashset_iter(&code_reference->instructions, &iter, &iter_key); res == KEFIR_OK;
            ...
            for (res = kefir_asmcmp_code_map_fragment_iter(&codegen_function->code.context.debug_info.code_map,
                                                           instr_ref, &iter, &fragment);
                 res == KEFIR_OK; res = kefir_asmcmp_code_map_fragment_next(&iter, &fragment)) {
...
kefir_result_t kefir_codegen_amd64_dwarf_generate_range_list_coalesce(
    ...
    REQUIRE_OK(
        kefir_codegen_amd64_dwarf_collect_code_fragments(mem, codegen_function, begin_ref, end_ref, fragment_tree));
...
kefir_result_t kefir_codegen_amd64_dwarf_generate_range_list(struct kefir_mem *mem,
                                                             struct kefir_codegen_amd64_function *codegen_function,
                                                             kefir_opt_code_debug_info_code_ref_t begin_ref,
                                                             kefir_opt_code_debug_info_code_ref_t end_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ...
    kefir_result_t res = kefir_codegen_amd64_dwarf_generate_range_list_coalesce(mem, codegen_function, begin_ref,
    ...
    for (struct kefir_hashtree_node *node = kefir_hashtree_iter(&fragment_tree, &tree_iter);
        ...
        REQUIRE_CHAIN(&res,
                      KEFIR_AMD64_DWARF_BYTE(&codegen_function->codegen->xasmgen, KEFIR_DWARF(DW_RLE_offset_pair)));
        REQUIRE_CHAIN(
            &res,
                ...
                kefir_asm_amd64_xasmgen_operand_subtract(
                    ...
                    kefir_asm_amd64_xasmgen_operand_label(
                        ...
                        kefir_asm_amd64_xasmgen_helpers_format(
                            ...
                            codegen_function->codegen->symbol_prefix, ir_identifier->symbol, begin_label)),
                    kefir_asm_amd64_xasmgen_operand_label(
            ...
            KEFIR_AMD64_XASMGEN_DATA(
                ...
                kefir_asm_amd64_xasmgen_operand_subtract(
                    ...
                    kefir_asm_amd64_xasmgen_operand_label(
                        ...
                        kefir_asm_amd64_xasmgen_helpers_format(
                            &codegen_function->codegen->xasmgen_helpers, KEFIR_AMD64_LABEL,
                            codegen_function->codegen->symbol_prefix, ir_identifier->symbol, end_label)),
                    kefir_asm_amd64_xasmgen_operand_label(
                        &codegen_function->codegen->xasmgen_helpers.operands[2], KEFIR_AMD64_XASMGEN_SYMBOL_ABSOLUTE,
                        kefir_asm_amd64_xasmgen_helpers_format(&xasmgen_helpers2, KEFIR_AMD64_TEXT_SECTION_BEGIN,
                                                               codegen_function->codegen->symbol_prefix)))));
