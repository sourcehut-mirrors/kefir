...
static kefir_result_t generate_label_info(struct kefir_mem *mem, struct kefir_codegen_amd64_dwarf_context *context,
    ...
    kefir_result_t res = kefir_opt_code_debug_info_code_reference(&codegen_function->generic.function->debug_info,
                                                                  attr->code_index, &code_reference);
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
        struct kefir_hashset_iterator iter;
        kefir_hashset_key_t iter_key;
        for (res = kefir_hashset_iter(&code_reference->instructions, &iter, &iter_key);
             res == KEFIR_OK && begin_label == KEFIR_ASMCMP_INDEX_NONE; res = kefir_hashset_next(&iter, &iter_key)) {
    ...
    REQUIRE_OK(KEFIR_AMD64_DWARF_ENTRY_INFO(&codegen_function->codegen->xasmgen, label_entry_id,
                                            context->abbrev.entries.label, codegen_function->codegen->symbol_prefix));
    REQUIRE_OK(kefir_ir_debug_entry_get_attribute(&codegen_function->generic.module->ir_module->debug_info.entries, entry_id,
    ...
    REQUIRE_OK(kefir_ir_debug_entry_has_attribute(&codegen_function->generic.module->ir_module->debug_info.entries, entry_id,
...
kefir_result_t kefir_codegen_amd64_dwarf_generate_label(struct kefir_mem *mem,
    ...
    KEFIR_DWARF_GENERATOR_SECTION(context->section, KEFIR_DWARF_GENERATOR_SECTION_INFO) {
        REQUIRE_OK(generate_label_info(mem, context, codegen_function, entry_id, dwarf_entry_id));
    }
