...
static kefir_result_t generate_parameter_abbrev(struct kefir_mem *mem,
    ...
    REQUIRE_OK(KEFIR_AMD64_DWARF_ENTRY_ABBREV(
        &codegen_function->codegen->xasmgen, context->abbrev.entries.subprogram_parameter,
        KEFIR_DWARF(DW_TAG_formal_parameter), KEFIR_DWARF(DW_CHILDREN_no), codegen_function->codegen->symbol_prefix));
...
static kefir_result_t generate_anonymous_parameter_abbrev(struct kefir_mem *mem,
    ...
    REQUIRE_OK(KEFIR_AMD64_DWARF_ENTRY_ABBREV(
        &codegen_function->codegen->xasmgen, context->abbrev.entries.anonymous_subprogram_parameter,
        KEFIR_DWARF(DW_TAG_formal_parameter), KEFIR_DWARF(DW_CHILDREN_no), codegen_function->codegen->symbol_prefix));
    REQUIRE_OK(KEFIR_AMD64_DWARF_ATTRIBUTE_ABBREV(&codegen_function->codegen->xasmgen, KEFIR_DWARF(DW_AT_type),
                                                  KEFIR_DWARF(DW_FORM_ref4)));
    REQUIRE_OK(KEFIR_AMD64_DWARF_ENTRY_ABBREV_END(&codegen_function->codegen->xasmgen));
...
static kefir_result_t generate_parameter_with_location_abbrev(struct kefir_mem *mem,
    ...
    REQUIRE_OK(KEFIR_AMD64_DWARF_ENTRY_ABBREV(
        &codegen_function->codegen->xasmgen, context->abbrev.entries.subprogram_parameter_location,
        KEFIR_DWARF(DW_TAG_formal_parameter), KEFIR_DWARF(DW_CHILDREN_no), codegen_function->codegen->symbol_prefix));
    REQUIRE_OK(KEFIR_AMD64_DWARF_ATTRIBUTE_ABBREV(&codegen_function->codegen->xasmgen, KEFIR_DWARF(DW_AT_name),
                                                  KEFIR_DWARF(DW_FORM_strp)));
    REQUIRE_OK(KEFIR_AMD64_DWARF_ATTRIBUTE_ABBREV(&codegen_function->codegen->xasmgen, KEFIR_DWARF(DW_AT_type),
...
static kefir_result_t generate_anonymous_parameter_with_location_abbrev(
    ...
    REQUIRE_OK(KEFIR_AMD64_DWARF_ENTRY_ABBREV(
        &codegen_function->codegen->xasmgen, context->abbrev.entries.anonymous_subprogram_parameter_location,
        KEFIR_DWARF(DW_TAG_formal_parameter), KEFIR_DWARF(DW_CHILDREN_no), codegen_function->codegen->symbol_prefix));
    REQUIRE_OK(KEFIR_AMD64_DWARF_ATTRIBUTE_ABBREV(&codegen_function->codegen->xasmgen, KEFIR_DWARF(DW_AT_type),
                                                  KEFIR_DWARF(DW_FORM_ref4)));
    REQUIRE_OK(KEFIR_AMD64_DWARF_ATTRIBUTE_ABBREV(&codegen_function->codegen->xasmgen, KEFIR_DWARF(DW_AT_location),
        ...
        kefir_asm_amd64_xasmgen_operand_subtract(
            ...
            kefir_asm_amd64_xasmgen_operand_label(
                ...
                kefir_asm_amd64_xasmgen_helpers_format(&codegen_function->codegen->xasmgen_helpers,
                                                       ...
                                                       codegen_function->codegen->symbol_prefix, parameter_type_id)),
            kefir_asm_amd64_xasmgen_operand_label(
                &codegen_function->codegen->xasmgen_helpers.operands[2], KEFIR_AMD64_XASMGEN_SYMBOL_ABSOLUTE,
                kefir_asm_amd64_xasmgen_helpers_format(&xasmgen_helpers2, KEFIR_AMD64_DWARF_DEBUG_INFO_SECTION,
                                                       codegen_function->codegen->symbol_prefix)))));
...
static kefir_result_t generate_function_parameter_loclists(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_codegen_amd64_dwarf_generate_instruction_location(mem, codegen_function,
                                                                       kefir_codegen_amd64_function_translator_get_parameter(&codegen_function->translator, attr->parameter)));
...
kefir_result_t kefir_codegen_amd64_dwarf_generate_function_parameter(
    ...
    kefir_bool_t has_type, has_name,
        has_location =
            kefir_codegen_amd64_function_translator_get_parameter(&codegen_function->translator, attr->parameter) != KEFIR_ID_NONE;
