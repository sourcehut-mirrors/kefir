...
#include "kefir/codegen/amd64/lowering_symbols.h"
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(atomic_load)(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[0], &ptr_vreg));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(atomic_load_complex)(
    ...
    REQUIRE_OK(kefir_codegen_amd64_function_call_preserve_regs(mem, function, NULL, NULL));
    ...
    REQUIRE_OK(kefir_asmcmp_virtual_register_new_spill_space(mem, &function->code.context, total_size_qwords,
                                                             total_alignment_qwords, &target_vreg));
    REQUIRE_OK(kefir_asmcmp_amd64_produce_virtual_register(
        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context), target_vreg, NULL));
    REQUIRE_OK(
        kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[0], &source_ptr_vreg));
    REQUIRE_OK(kefir_asmcmp_amd64_link_virtual_registers(mem, &function->code,
                                                         kefir_asmcmp_context_instr_tail(&function->code.context),
    ...
    REQUIRE_OK(kefir_codegen_amd64_do_call_direct(mem, function, atomic_memory_copy_fn_name, &call_idx));

    switch (instruction->operation.opcode) {
        case KEFIR_OPT_OPCODE_ATOMIC_LOAD_COMPLEX_FLOAT32: {
            kefir_asmcmp_virtual_register_index_t real_vreg, imag_vreg, result_vreg;
            REQUIRE_OK(kefir_asmcmp_virtual_register_new(mem, &function->code.context,
                                                         KEFIR_ASMCMP_VIRTUAL_REGISTER_FLOATING_POINT, &real_vreg));
            REQUIRE_OK(kefir_asmcmp_virtual_register_new(mem, &function->code.context,
                                                         KEFIR_ASMCMP_VIRTUAL_REGISTER_FLOATING_POINT, &imag_vreg));
            REQUIRE_OK(kefir_asmcmp_virtual_register_new_pair(mem, &function->code.context,
                                                              ...
                                                              real_vreg, imag_vreg, &result_vreg));

            REQUIRE_OK(kefir_asmcmp_amd64_movd(
                mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                &KEFIR_ASMCMP_MAKE_VREG(real_vreg),
                &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(target_vreg, 0, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT), NULL));
            REQUIRE_OK(
                kefir_asmcmp_amd64_movd(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                        &KEFIR_ASMCMP_MAKE_VREG(imag_vreg),
                                        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(target_vreg, KEFIR_AMD64_ABI_QWORD / 2,
        ...
        case KEFIR_OPT_OPCODE_ATOMIC_LOAD_COMPLEX_FLOAT64: {
            ...
            REQUIRE_OK(kefir_asmcmp_amd64_movq(
                ...
                &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(target_vreg, 0, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT), NULL));
            REQUIRE_OK(
                kefir_asmcmp_amd64_movq(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                        &KEFIR_ASMCMP_MAKE_VREG(imag_vreg),
                                        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(target_vreg, KEFIR_AMD64_ABI_QWORD,
                                                                            KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT),
                                        NULL));
            REQUIRE_OK(kefir_codegen_amd64_function_translator_assign_vreg(mem, &function->translator, instruction->id, result_vreg));
        } break;
        ...
        case KEFIR_OPT_OPCODE_ATOMIC_LOAD_COMPLEX_LONG_DOUBLE:
            REQUIRE_OK(kefir_codegen_amd64_function_translator_assign_vreg(mem, &function->translator, instruction->id, target_vreg));
            break;
...
static kefir_result_t size_of_memory_operand(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    const struct kefir_ir_type *ir_type =
        kefir_ir_module_get_named_type(function->generic.module->ir_module, instruction->operation.parameters.type.type_id);
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(atomic_copy_memory)(
    ...
    REQUIRE_OK(
        kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[0], &target_ptr_vreg));
    REQUIRE_OK(
        kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[1], &source_ptr_vreg));
    REQUIRE_OK(kefir_asmcmp_amd64_link_virtual_registers(mem, &function->code,
                                                         kefir_asmcmp_context_instr_tail(&function->code.context),
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(atomic_load_long_double)(
    ...
    kefir_asmcmp_instruction_index_t call_idx;
    REQUIRE_OK(kefir_codegen_amd64_do_call_direct(mem, function, LIBATOMIC_LOAD, &call_idx));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(atomic_store_complex_float)(
    struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    const struct kefir_opt_instruction *instruction) {
    REQUIRE_OK(kefir_codegen_amd64_function_call_preserve_regs(mem, function, NULL, NULL));
    ...
    REQUIRE_OK(
        kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[0], &target_ptr_vreg));
    REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[1], &source_vreg));
    REQUIRE_OK(kefir_asmcmp_virtual_register_pair_at(&function->code.context, source_vreg, 0, &source_real_vreg));
    REQUIRE_OK(kefir_asmcmp_virtual_register_pair_at(&function->code.context, source_vreg, 1, &source_imag_vreg));
    REQUIRE_OK(kefir_asmcmp_amd64_link_virtual_registers(mem, &function->code,
                                                         kefir_asmcmp_context_instr_tail(&function->code.context),
                                                         target_ptr_placement_vreg, target_ptr_vreg, NULL));

    kefir_size_t value_qwords_size;
    switch (instruction->operation.opcode) {
        case KEFIR_OPT_OPCODE_ATOMIC_STORE_COMPLEX_FLOAT32:
            value_qwords_size = kefir_abi_amd64_complex_float_qword_size(function->codegen->abi_variant);
            REQUIRE_OK(kefir_asmcmp_virtual_register_new_spill_space(
                mem, &function->code.context, kefir_abi_amd64_complex_float_qword_size(function->codegen->abi_variant),
                kefir_abi_amd64_complex_float_qword_alignment(function->codegen->abi_variant), &tmp_spill_area));
            REQUIRE_OK(kefir_asmcmp_amd64_produce_virtual_register(
                mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context), tmp_spill_area, NULL));
            REQUIRE_OK(kefir_asmcmp_amd64_movd(
                mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(tmp_spill_area, 0, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT),
                &KEFIR_ASMCMP_MAKE_VREG(source_real_vreg), NULL));
            REQUIRE_OK(
                kefir_asmcmp_amd64_movd(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(tmp_spill_area, KEFIR_AMD64_ABI_QWORD / 2,
                                                                            KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT),
                                        &KEFIR_ASMCMP_MAKE_VREG(source_imag_vreg), NULL));
            break;

        case KEFIR_OPT_OPCODE_ATOMIC_STORE_COMPLEX_FLOAT64:
            value_qwords_size = kefir_abi_amd64_complex_double_qword_size(function->codegen->abi_variant);
            REQUIRE_OK(kefir_asmcmp_virtual_register_new_spill_space(
                mem, &function->code.context, kefir_abi_amd64_complex_double_qword_size(function->codegen->abi_variant),
                kefir_abi_amd64_complex_double_qword_alignment(function->codegen->abi_variant), &tmp_spill_area));
            REQUIRE_OK(kefir_asmcmp_amd64_produce_virtual_register(
                mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context), tmp_spill_area, NULL));
            REQUIRE_OK(kefir_asmcmp_amd64_movq(
                mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
            ...
            REQUIRE_OK(
                kefir_asmcmp_amd64_movq(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(tmp_spill_area, KEFIR_AMD64_ABI_QWORD,
                                                                            KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT),
                                        &KEFIR_ASMCMP_MAKE_VREG(source_imag_vreg), NULL));
            break;
    ...
    kefir_asmcmp_instruction_index_t call_idx;
    REQUIRE_OK(kefir_codegen_amd64_do_call_direct(mem, function, LIBATOMIC_STORE, &call_idx));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(atomic_store_complex_long_double)(
    struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    const struct kefir_opt_instruction *instruction) {
    REQUIRE_OK(kefir_codegen_amd64_function_call_preserve_regs(mem, function, NULL, NULL));
    ...
    kefir_asm_amd64_xasmgen_register_t size_phreg, target_ptr_phreg, source_ptr_phreg, memorder_phreg;
    REQUIRE_OK(kefir_abi_amd64_general_purpose_parameter_register(function->codegen->abi_variant, 0, &size_phreg));
    const char *atomic_memory_copy_fn_name = LIBATOMIC_STORE;
    REQUIRE_OK(
        kefir_abi_amd64_general_purpose_parameter_register(function->codegen->abi_variant, 1, &target_ptr_phreg));
    REQUIRE_OK(
        kefir_abi_amd64_general_purpose_parameter_register(function->codegen->abi_variant, 2, &source_ptr_phreg));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(atomic_compare_exchange)(
    ...
    REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[1],
                                                    &expected_value_vreg));
    REQUIRE_OK(
        kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[2], &desired_value_vreg));
    ...
    REQUIRE_OK(kefir_opt_instruction_get_sole_use(&function->generic.function->code, instruction->id, &use_instr_ref));

    const struct kefir_opt_instruction *use_instr = NULL;
    if (use_instr_ref != KEFIR_ID_NONE) {
        REQUIRE_OK(kefir_opt_code_container_instr(&function->generic.function->code, use_instr_ref, &use_instr));
    }
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(atomic_compare_exchange_complex_float)(
    struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    const struct kefir_opt_instruction *instruction) {
    REQUIRE_OK(kefir_codegen_amd64_function_call_preserve_regs(mem, function, NULL, NULL));
    ...
    switch (instruction->operation.opcode) {
        case KEFIR_OPT_OPCODE_ATOMIC_CMPXCHG_COMPLEX_FLOAT32:
            total_size =
                kefir_abi_amd64_complex_float_qword_size(function->codegen->abi_variant) * KEFIR_AMD64_ABI_QWORD;
            total_alignment =
                kefir_abi_amd64_complex_float_qword_alignment(function->codegen->abi_variant) * KEFIR_AMD64_ABI_QWORD;
            break;

        case KEFIR_OPT_OPCODE_ATOMIC_CMPXCHG_COMPLEX_FLOAT64:
            total_size =
                kefir_abi_amd64_complex_double_qword_size(function->codegen->abi_variant) * KEFIR_AMD64_ABI_QWORD;
            total_alignment =
                kefir_abi_amd64_complex_double_qword_alignment(function->codegen->abi_variant) * KEFIR_AMD64_ABI_QWORD;
            break;
    ...
    kefir_asmcmp_virtual_register_index_t size_placement_vreg, ptr_vreg, ptr_placement_vreg, expected_value_vreg,
        expected_value_location_vreg, expected_value_placement_vreg, desired_value_location_vreg, desired_value_vreg,
    ...
    REQUIRE_OK(kefir_asmcmp_virtual_register_pair_at(&function->code.context, expected_value_vreg, 0, &real_vreg));
    REQUIRE_OK(kefir_asmcmp_virtual_register_pair_at(&function->code.context, expected_value_vreg, 1, &imag_vreg));
    switch (instruction->operation.opcode) {
        case KEFIR_OPT_OPCODE_ATOMIC_CMPXCHG_COMPLEX_FLOAT32:
            REQUIRE_OK(
                kefir_asmcmp_amd64_movd(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(expected_value_location_vreg, 0,
                                                                            KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT),
                                        &KEFIR_ASMCMP_MAKE_VREG(real_vreg), NULL));
            REQUIRE_OK(kefir_asmcmp_amd64_movd(
                mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(expected_value_location_vreg, KEFIR_AMD64_ABI_QWORD / 2,
                                                    KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT),
                &KEFIR_ASMCMP_MAKE_VREG(imag_vreg), NULL));
            break;
    ...
    REQUIRE_OK(kefir_asmcmp_virtual_register_pair_at(&function->code.context, desired_value_vreg, 0, &real_vreg));
    REQUIRE_OK(kefir_asmcmp_virtual_register_pair_at(&function->code.context, desired_value_vreg, 1, &imag_vreg));
    switch (instruction->operation.opcode) {
        case KEFIR_OPT_OPCODE_ATOMIC_CMPXCHG_COMPLEX_FLOAT32:
            REQUIRE_OK(
                kefir_asmcmp_amd64_movd(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(desired_value_location_vreg, 0,
                                                                            KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT),
                                        &KEFIR_ASMCMP_MAKE_VREG(real_vreg), NULL));
            REQUIRE_OK(kefir_asmcmp_amd64_movd(
                mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(desired_value_location_vreg, KEFIR_AMD64_ABI_QWORD / 2,
                                                    KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT),
                &KEFIR_ASMCMP_MAKE_VREG(imag_vreg), NULL));
            break;
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_link_virtual_registers(
        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context), desired_value_placement_vreg,
        desired_value_location_vreg, NULL));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(fenv_update)(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[0], &fenv_vreg));
    REQUIRE_OK(kefir_asmcmp_virtual_register_new(mem, &function->code.context,
                                                 KEFIR_ASMCMP_VIRTUAL_REGISTER_GENERAL_PURPOSE, &exception_vreg));
    REQUIRE_OK(kefir_asmcmp_virtual_register_new_spill_space(mem, &function->code.context, 1, 1, &tmp_vreg));
    REQUIRE_OK(kefir_asmcmp_virtual_register_new(mem, &function->code.context,
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_mov(
        ...
        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(tmp_vreg, 0, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT), NULL));
    REQUIRE_OK(kefir_asmcmp_amd64_produce_virtual_register(
        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context), exception_vreg, NULL));
