...
static kefir_result_t vararg_start_impl(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    REQUIRE(function->generic.function->ir_func->declaration->vararg,
    ...
    REQUIRE_OK(
        kefir_asmcmp_amd64_mov(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                               ...
                               &KEFIR_ASMCMP_MAKE_INT(KEFIR_AMD64_ABI_QWORD * reqs.general_purpose_regs), NULL));

    REQUIRE_OK(kefir_asmcmp_amd64_mov(
        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(arg_vreg, 4, KEFIR_ASMCMP_OPERAND_VARIANT_32BIT),
        &KEFIR_ASMCMP_MAKE_INT(KEFIR_AMD64_ABI_QWORD * kefir_abi_amd64_num_of_general_purpose_parameter_registers(
    ...
    if (function->translator.vararg_area == KEFIR_ASMCMP_INDEX_NONE) {
        REQUIRE_OK(kefir_asmcmp_virtual_register_new_spill_space(
            ...
            2, &function->translator.vararg_area));
        REQUIRE_OK(
            kefir_codegen_amd64_function_translator_add_virtual_register_alive_at_exit_point(mem, &function->translator, function->translator.vararg_area));
    }
    REQUIRE_OK(kefir_asmcmp_amd64_lea(
        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
        &KEFIR_ASMCMP_MAKE_VREG64(tmp_vreg),
        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(function->translator.vararg_area, 0, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT), NULL));
...
static kefir_result_t vararg_copy_impl(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[0], &source_vreg));
    REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[1], &target_vreg));
    REQUIRE_OK(kefir_asmcmp_virtual_register_new(mem, &function->code.context,
                                                 KEFIR_ASMCMP_VIRTUAL_REGISTER_GENERAL_PURPOSE, &tmp_vreg));
...
static kefir_result_t vararg_visit_integer(const struct kefir_ir_type *type, kefir_size_t index,
    ...
    REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&param->function->translator, param->instruction->operation.parameters.refs[0],
                                                    &valist_vreg));

    REQUIRE_OK(vararg_load_int(param->mem, param->function, valist_vreg, result_vreg));

    REQUIRE_OK(
        kefir_codegen_amd64_function_translator_assign_vreg(param->mem, &param->function->translator, param->instruction->id, result_vreg));
...
static kefir_result_t vararg_visit_bitint(const struct kefir_ir_type *type, kefir_size_t index,
    ...
    if (typeentry->param <= KEFIR_AMD64_ABI_QWORD * 8) {
        ...
        REQUIRE_OK(kefir_asmcmp_amd64_add(
            param->mem, &param->function->code, kefir_asmcmp_context_instr_tail(&param->function->code.context),
            &KEFIR_ASMCMP_MAKE_VREG64(tmp_vreg), &KEFIR_ASMCMP_MAKE_INT(qwords * KEFIR_AMD64_ABI_QWORD), NULL));

        REQUIRE_OK(kefir_asmcmp_amd64_cmp(
            param->mem, &param->function->code, kefir_asmcmp_context_instr_tail(&param->function->code.context),
            &KEFIR_ASMCMP_MAKE_VREG64(tmp_vreg),
            &KEFIR_ASMCMP_MAKE_INT(
        ...
        REQUIRE_OK(kefir_asmcmp_amd64_add(
            param->mem, &param->function->code, kefir_asmcmp_context_instr_tail(&param->function->code.context),
            &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(valist_vreg, 0, KEFIR_ASMCMP_OPERAND_VARIANT_32BIT),
            &KEFIR_ASMCMP_MAKE_INT(qwords * KEFIR_AMD64_ABI_QWORD), NULL));
...
static kefir_result_t vararg_visit_int128(const struct kefir_ir_type *type, kefir_size_t index,
    ...
    REQUIRE_OK(kefir_asmcmp_virtual_register_new(param->mem, &param->function->code.context,
                                                 ...
                                                 &result_lower_half_vreg));
    REQUIRE_OK(kefir_asmcmp_virtual_register_new(param->mem, &param->function->code.context,
                                                 KEFIR_ASMCMP_VIRTUAL_REGISTER_GENERAL_PURPOSE,
                                                 &result_upper_half_vreg));
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_add(param->mem, &param->function->code,
                                      kefir_asmcmp_context_instr_tail(&param->function->code.context),
                                      &KEFIR_ASMCMP_MAKE_VREG64(tmp_vreg), &KEFIR_ASMCMP_MAKE_VREG64(tmp2_vreg), NULL));
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_mov(
        param->mem, &param->function->code, kefir_asmcmp_context_instr_tail(&param->function->code.context),
        &KEFIR_ASMCMP_MAKE_VREG64(tmp2_vreg),
        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(tmp_vreg, 0, KEFIR_ASMCMP_OPERAND_VARIANT_64BIT), NULL));

    REQUIRE_OK(kefir_asmcmp_amd64_mov(
        param->mem, &param->function->code, kefir_asmcmp_context_instr_tail(&param->function->code.context),
        &KEFIR_ASMCMP_MAKE_VREG64(result_lower_half_vreg), &KEFIR_ASMCMP_MAKE_VREG64(tmp2_vreg), NULL));

    REQUIRE_OK(kefir_asmcmp_amd64_mov(
        param->mem, &param->function->code, kefir_asmcmp_context_instr_tail(&param->function->code.context),
        &KEFIR_ASMCMP_MAKE_VREG64(tmp2_vreg),
        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(tmp_vreg, KEFIR_AMD64_ABI_QWORD, KEFIR_ASMCMP_OPERAND_VARIANT_64BIT),
        NULL));

    REQUIRE_OK(kefir_asmcmp_amd64_mov(
        param->mem, &param->function->code, kefir_asmcmp_context_instr_tail(&param->function->code.context),
        &KEFIR_ASMCMP_MAKE_VREG64(result_upper_half_vreg), &KEFIR_ASMCMP_MAKE_VREG64(tmp2_vreg), NULL));
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_jmp(param->mem, &param->function->code,
                                      kefir_asmcmp_context_instr_tail(&param->function->code.context),
                                      &KEFIR_ASMCMP_MAKE_INTERNAL_LABEL(no_overflow_label), NULL));
    REQUIRE_OK(kefir_asmcmp_context_bind_label_after_tail(param->mem, &param->function->code.context, overflow_label));
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_lea(
        param->mem, &param->function->code, kefir_asmcmp_context_instr_tail(&param->function->code.context),
        &KEFIR_ASMCMP_MAKE_VREG64(tmp2_vreg),
        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(tmp_vreg, KEFIR_AMD64_ABI_QWORD * 2, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT),
        NULL));
...
static kefir_result_t vararg_load_sse(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    if (sse_size > KEFIR_AMD64_ABI_QWORD) {
        ...
        REQUIRE_OK(
            kefir_asmcmp_amd64_and(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_lea(
        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
        &KEFIR_ASMCMP_MAKE_VREG(tmp_vreg),
        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(tmp_vreg, sse_size, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT), NULL));
    REQUIRE_OK(kefir_asmcmp_amd64_mov(
        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
        &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(valist_vreg, KEFIR_AMD64_ABI_QWORD, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT),
    ...
    if (sse_size == KEFIR_AMD64_ABI_QWORD) {
        ...
        REQUIRE_OK(kefir_asmcmp_amd64_movdqu(
            mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
            &KEFIR_ASMCMP_MAKE_VREG(result_vreg),
            &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(tmp_vreg, -sse_size, KEFIR_ASMCMP_OPERAND_VARIANT_128BIT), NULL));
    }
...
static kefir_result_t vararg_load_long_double(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    // Load from overflow area
    REQUIRE_OK(kefir_codegen_amd64_function_translator_assign_vreg(mem, &function->translator, instr_ref, result_vreg));
...
static kefir_result_t vararg_visit_complex_float32(const struct kefir_ir_type *type, kefir_size_t index,
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_shufps(param->mem, &param->function->code,
                                         ...
                                         &KEFIR_ASMCMP_MAKE_VREG(result_imag_vreg), &KEFIR_ASMCMP_MAKE_INT(1), NULL));
...
static kefir_result_t vararg_visit_memory_aggregate(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_asmcmp_virtual_register_new(mem, &function->code.context,
                                                 KEFIR_ASMCMP_VIRTUAL_REGISTER_GENERAL_PURPOSE, &tmp_vreg));
    REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[0], &valist_vreg));
...
static kefir_result_t vararg_register_aggregate_check(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_add(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                      &KEFIR_ASMCMP_MAKE_VREG64(tmp_vreg),
                                      &KEFIR_ASMCMP_MAKE_INT(required_integers * KEFIR_AMD64_ABI_QWORD), NULL));

    REQUIRE_OK(kefir_asmcmp_amd64_cmp(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                      &KEFIR_ASMCMP_MAKE_VREG64(tmp_vreg),
                                      &KEFIR_ASMCMP_MAKE_INT(kefir_abi_amd64_num_of_general_purpose_parameter_registers(
                                                                 function->codegen->abi_variant) *
                                                             KEFIR_AMD64_ABI_QWORD),
                                      NULL));
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_add(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                      &KEFIR_ASMCMP_MAKE_VREG64(tmp_vreg),
                                      &KEFIR_ASMCMP_MAKE_INT(required_sse * 2 * KEFIR_AMD64_ABI_QWORD), NULL));
...
static kefir_result_t vararg_register_aggregate_load(
    ...
    if (integer_qwords > 0) {
        ...
        REQUIRE_OK(kefir_asmcmp_amd64_add(
            mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
            &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(valist_vreg, 0, KEFIR_ASMCMP_OPERAND_VARIANT_32BIT),
            &KEFIR_ASMCMP_MAKE_INT(integer_offset * KEFIR_AMD64_ABI_QWORD), NULL));
    }
    ...
    if (sse_qwords > 0) {
            ...
            switch (subparam.location) {
                ...
                case KEFIR_ABI_AMD64_FUNCTION_PARAMETER_LOCATION_SSE_REGISTER:
                    if (i + 1 < length) {
                        ...
                        REQUIRE_OK(kefir_abi_amd64_function_parameter_multireg_at(parameter, i + 1, &next_subparam));
                        ...
                        if (next_subparam.location == KEFIR_ABI_AMD64_FUNCTION_PARAMETER_LOCATION_SSEUP_REGISTER) {
                            REQUIRE_OK(kefir_asmcmp_amd64_mov(
                                mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                &KEFIR_ASMCMP_MAKE_VREG64(tmp2_vreg),
                                &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(
                                    tmp_vreg, sse_offset * 2 * KEFIR_AMD64_ABI_QWORD + KEFIR_AMD64_ABI_QWORD,
                                    KEFIR_ASMCMP_OPERAND_VARIANT_64BIT),
                                NULL));
                            ...
                            REQUIRE_OK(kefir_asmcmp_amd64_mov(
                                mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(result_vreg,
                                                                    i * KEFIR_AMD64_ABI_QWORD + KEFIR_AMD64_ABI_QWORD,
                                                                    KEFIR_ASMCMP_OPERAND_VARIANT_64BIT),
                                &KEFIR_ASMCMP_MAKE_VREG64(tmp2_vreg), NULL));
                            i++;
                        }
        ...
        REQUIRE_OK(kefir_asmcmp_amd64_add(
            mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
            &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(valist_vreg, 4, KEFIR_ASMCMP_OPERAND_VARIANT_32BIT),
            &KEFIR_ASMCMP_MAKE_INT(sse_offset * 2 * KEFIR_AMD64_ABI_QWORD), NULL));
    }
...
static kefir_result_t vararg_visit_register_aggregate(struct kefir_mem *mem,
    ...
    REQUIRE_OK(
        kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[1], &allocation_vreg));
...
static kefir_result_t vararg_get_impl(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    struct kefir_ir_type *type = kefir_ir_module_get_named_type(function->generic.module->ir_module, type_id);
