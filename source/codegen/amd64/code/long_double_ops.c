...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(long_double_binary_op)(
    ...
    REQUIRE_OK(kefir_codegen_amd64_function_vreg_of(function, instruction->operation.parameters.refs[2], &result_vreg));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(long_double_neg)(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_codegen_amd64_function_vreg_of(function, instruction->operation.parameters.refs[0], &arg1_vreg));
    REQUIRE_OK(kefir_codegen_amd64_function_vreg_of(function, instruction->operation.parameters.refs[1], &result_vreg));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(uint_to_long_double)(
    ...
    const char *symbolic_nosign_label;
    ...
    if (function->codegen->config->position_independent_code) {
        REQUIRE_OK(kefir_asmcmp_amd64_fadd(mem, &function->code,
                                           ...
                                           &KEFIR_ASMCMP_MAKE_RIP_INDIRECT(KEFIR_AMD64_CONSTANT_UINT_TO_LONG_DOUBLE,
                                                                           KEFIR_ASMCMP_OPERAND_VARIANT_FP_SINGLE),
                                           NULL));
    } else {
        REQUIRE_OK(kefir_asmcmp_amd64_fadd(mem, &function->code,
                                           kefir_asmcmp_context_instr_tail(&function->code.context),
                                           &KEFIR_ASMCMP_MAKE_INDIRECT_LABEL(KEFIR_AMD64_CONSTANT_UINT_TO_LONG_DOUBLE,
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(float32_to_long_double)(
    ...
    REQUIRE_OK(
        kefir_asmcmp_virtual_register_new_indirect_spill_space_allocation(mem, &function->code.context, 1, &tmp_vreg));
    REQUIRE_OK(kefir_codegen_amd64_function_vreg_of(function, instruction->operation.parameters.refs[0], &arg1_vreg));
    REQUIRE_OK(kefir_codegen_amd64_function_vreg_of(function, instruction->operation.parameters.refs[1], &result_vreg));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(long_double_trunc1)(
    struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    const struct kefir_opt_instruction *instruction) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(function != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid codegen amd64 function"));
    ...
    const char *symbolic_label = KEFIR_AMD64_RUNTIME_LONG_DOUBLE_TRUNC_1BIT;
    if (function->codegen->config->position_independent_code) {
        REQUIRE_OK(kefir_asmcmp_format(mem, &function->code.context, &symbolic_label, KEFIR_AMD64_PLT,
