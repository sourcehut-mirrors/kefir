...
kefir_result_t kefir_codegen_amd64_function_long_double_to_int(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_produce_virtual_register(
        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context), original_cw_vreg, NULL));
    REQUIRE_OK(kefir_asmcmp_amd64_produce_virtual_register(
        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context), new_cw_vreg, NULL));
    REQUIRE_OK(kefir_asmcmp_amd64_produce_virtual_register(
    ...
    REQUIRE_OK(kefir_codegen_amd64_function_translator_assign_vreg(mem, &function->translator, result_ref, result_vreg));
    return KEFIR_OK;
}
            ...
            kefir_asmcmp_amd64_fld(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                   &KEFIR_ASMCMP_MAKE_INDIRECT_EXTERNAL_LABEL(
                                       ...
                                       kefir_asm_amd64_xasmgen_helpers_format(&function->codegen->xasmgen_helpers,
                                                                              KEFIR_AMD64_CONSTANT_LONG_DOUBLE_TO_UINT,
                                                                              function->codegen->symbol_prefix),
                                       0, KEFIR_ASMCMP_OPERAND_VARIANT_FP_SINGLE),
                                   NULL));
    }
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(int_to_long_double)(
    ...
    REQUIRE_OK(kefir_codegen_amd64_function_int_to_long_double(mem, function, arg_vreg, instruction->id));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(uint_to_long_double)(
    ...
    kefir_asmcmp_virtual_register_index_t arg_vreg;
    REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instruction->operation.parameters.refs[0], &arg_vreg));
    REQUIRE_OK(kefir_codegen_amd64_function_uint_to_long_double(mem, function, arg_vreg, instruction->id));
