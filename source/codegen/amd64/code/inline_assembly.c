...
typedef struct inline_assembly_parameter_allocation_entry {
    ...
    kefir_bool_t direct_input;
    kefir_bool_t direct_output;
...
struct inline_assembly_context {
    ...
    struct kefir_list available_gp_registers;
    struct kefir_list available_sse_registers;
    ...
    struct kefir_hashtree cloberred_vregs;
    kefir_size_t x87_stack_index;
};
...
static kefir_result_t init_available_regs(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    for (kefir_size_t i = 0;
        ...
        if (kefir_hashtree_has(&context->cloberred_vregs, (kefir_hashtree_key_t) candidate)) {
            continue;
        }

        REQUIRE_OK(kefir_list_insert_after(mem, &context->available_gp_registers,
                                           kefir_list_tail(&context->available_gp_registers),
    ...
    for (kefir_size_t i = 0; i < kefir_abi_amd64_num_of_caller_preserved_sse_registers(function->codegen->abi_variant);
         i++) {
        kefir_asm_amd64_xasmgen_register_t candidate;
        REQUIRE_OK(kefir_abi_amd64_get_caller_preserved_sse_register(function->codegen->abi_variant, i, &candidate));

        if (kefir_hashtree_has(&context->cloberred_vregs, (kefir_hashtree_key_t) candidate)) {
            continue;
        }

        REQUIRE_OK(kefir_list_insert_after(mem, &context->available_sse_registers,
                                           kefir_list_tail(&context->available_sse_registers),
                                           (void *) (kefir_uptr_t) candidate));
    }
    return KEFIR_OK;
}
...
static kefir_result_t obtain_available_gp_register(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    *reg =
    ...
    REQUIRE_OK(
        kefir_list_pop(mem, &context->available_gp_registers, kefir_list_head(&context->available_gp_registers)));
...
static kefir_result_t init_explicitly_allocated_regs(struct kefir_mem *mem, struct inline_assembly_context *context) {
    for (const struct kefir_list_entry *iter = kefir_list_head(&context->ir_inline_assembly->parameter_list);
        ...
        if (ir_asm_param->constraints.explicit_register != NULL && !ir_asm_param->constraints.x87_stack) {
            ...
            if (kefir_asm_amd64_xasmgen_register_is_floating_point(phreg)) {
                for (struct kefir_list_entry *iter = kefir_list_head(&context->available_sse_registers); iter != NULL;
                     iter = iter->next) {
...
static kefir_result_t allocate_parameters(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(&context->ir_inline_assembly->parameter_list);
        ...
        kefir_bool_t direct_output = false;
        switch (ir_asm_param->klass) {
            ...
            case KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_READ_LOCATION:
                REQUIRE_OK(evaluate_parameter_type(mem, ir_asm_param->location_type.type,
                                                   ir_asm_param->location_type.index, &param_type, &param_size));
                break;

            case KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_VALUE_WRITE_LOCATION:
                REQUIRE_OK(evaluate_parameter_type(mem, ir_asm_param->location_type.type,
                                                   ir_asm_param->location_type.index, &param_type, &param_size));
                REQUIRE_OK(evaluate_parameter_type(mem, ir_asm_param->value_type.type, ir_asm_param->value_type.index,
        ...
        entry->direct_input = direct_input;
        entry->direct_output = direct_output;
        if (ir_asm_param->klass == KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_VALUE_WRITE_LOCATION ||
            ir_asm_param->klass == KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_VALUE) {
...
kefir_result_t match_vreg_to_size(kefir_asmcmp_virtual_register_index_t in, kefir_size_t sz, kefir_bool_t high_half,
                                  struct kefir_asmcmp_value *value) {
    if (sz <= 1) {
        if (high_half) {
            *value = KEFIR_ASMCMP_MAKE_VREG8_HIGH(in);
        } else {
            *value = KEFIR_ASMCMP_MAKE_VREG8(in);
        }
    } else if (sz <= 2) {
        REQUIRE(!high_half, KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Unexpected high-half flag for x86 word register"));
        *value = KEFIR_ASMCMP_MAKE_VREG16(in);
    } else if (sz <= 4) {
        REQUIRE(!high_half, KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Unexpected high-half flag for x86 dword register"));
        *value = KEFIR_ASMCMP_MAKE_VREG32(in);
    } else if (sz <= 8) {
        REQUIRE(!high_half, KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Unexpected high-half flag for x86 qword register"));
...
static kefir_result_t read_input(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
                                 struct inline_assembly_context *context,
                                 const struct kefir_ir_inline_assembly_parameter *ir_asm_param,
    ...
    *has_read = false;
    switch (ir_asm_param->klass) {
        case KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_VALUE:
        case KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_VALUE_WRITE_LOCATION:
            param_type = kefir_ir_type_at(ir_asm_param->value_type.type, ir_asm_param->value_type.index);
            break;
        ...
        case KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_READ_WRITE_LOCATION:
        case KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_IMMEDIATE:
            param_type = kefir_ir_type_at(ir_asm_param->location_type.type, ir_asm_param->location_type.index);
            break;
    }
                ...
                case INLINE_ASSEMBLY_PARAMETER_ALLOCATION_GP_REGISTER:
                    if (entry->direct_input) {
                        ...
                        *has_read = true;
                                                      ...
                                                      (kefir_hashtreeset_entry_t) entry->allocation_vreg)) {
                        REQUIRE_OK(match_vreg_to_size(entry->allocation_vreg, entry->parameter_props.size, false,
                                                      &vreg_variant_value));
        ...
        case KEFIR_IR_TYPE_COMPLEX_FLOAT32: {
            ...
            if (entry->direct_input) {
                ...
                REQUIRE_OK(kefir_asmcmp_amd64_shl(
                    mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                    &KEFIR_ASMCMP_MAKE_VREG64(packed_vreg), &KEFIR_ASMCMP_MAKE_INT(32), NULL));
                REQUIRE_OK(kefir_asmcmp_amd64_or(
                    mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                    &KEFIR_ASMCMP_MAKE_VREG64(packed_vreg), &KEFIR_ASMCMP_MAKE_VREG64(tmp_vreg), NULL));
            }
            ...
            switch (entry->allocation_type) {
                case INLINE_ASSEMBLY_PARAMETER_ALLOCATION_GP_REGISTER:
                    if (entry->direct_input) {
                        return KEFIR_SET_ERROR(
                            KEFIR_INVALID_STATE,
                            "Unable to read complex floating-point value as a direct inline assembly parameter");
                    } else if (!kefir_hashtreeset_has(&context->directly_read_vregs,
                                                      (kefir_hashtreeset_entry_t) entry->allocation_vreg)) {
                        REQUIRE_OK(kefir_asmcmp_amd64_movdqu(
                            mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                ...
                case INLINE_ASSEMBLY_PARAMETER_ALLOCATION_SSE_REGISTER:
                    if (entry->direct_input) {
                        ...
                        REQUIRE_OK(kefir_asmcmp_virtual_register_pair_at(&function->code.context, location_vreg, 0,
                                                                         &real_vreg));
                        REQUIRE_OK(kefir_asmcmp_virtual_register_pair_at(&function->code.context, location_vreg, 1,
                                                                         &imag_vreg));
                        REQUIRE_OK(kefir_asmcmp_amd64_movaps(
                            mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                            &KEFIR_ASMCMP_MAKE_VREG(entry->allocation_vreg), &KEFIR_ASMCMP_MAKE_VREG(real_vreg), NULL));
                        REQUIRE_OK(kefir_asmcmp_amd64_shufpd(
                            mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                            &KEFIR_ASMCMP_MAKE_VREG(entry->allocation_vreg), &KEFIR_ASMCMP_MAKE_VREG(imag_vreg),
            ...
            switch (entry->allocation_type) {
                ...
                case INLINE_ASSEMBLY_PARAMETER_ALLOCATION_SSE_REGISTER:
                    return KEFIR_SET_ERROR(KEFIR_NOT_SUPPORTED,
                                           "Indirect SSE parameters of inline assembly are not supported");
...
static kefir_result_t load_inputs(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(&context->ir_inline_assembly->parameter_list);
        ...
        if (vreg != KEFIR_ASMCMP_INDEX_NONE) {
            ...
            if (!has_read && !kefir_hashtreeset_has(&context->directly_read_vregs,
                                                    (kefir_hashtreeset_entry_t) entry->allocation_vreg)) {
                REQUIRE_OK(kefir_asmcmp_amd64_link_virtual_registers(
                    mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
...
static kefir_result_t pointer_operand(const struct kefir_ir_inline_assembly_parameter *asm_param,
    ...
    struct kefir_ir_typeentry *typeentry =
        kefir_ir_type_at(asm_param->location_type.type, asm_param->location_type.index);
...
static kefir_result_t format_normal_parameter(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
                                              struct inline_assembly_context *context,
                                              const struct kefir_ir_inline_assembly_parameter *asm_param,
                                              kefir_size_t override_size, kefir_bool_t high_half) {
        ...
        switch (asm_param->immediate_type) {
            ...
            case KEFIR_IR_INLINE_ASSEMBLY_IMMEDIATE_LITERAL_BASED: {
                ...
                REQUIRE_OK(kefir_asmcmp_format(mem, &function->code.context, &symbol, KEFIR_AMD64_STRING_LITERAL,
                                               function->codegen->symbol_prefix, asm_param->immediate_literal_base));
                REQUIRE_OK(kefir_asmcmp_inline_assembly_add_value(
                    mem, &function->code.context, context->inline_asm_idx,
    ...
    switch (entry->allocation_type) {
        case INLINE_ASSEMBLY_PARAMETER_ALLOCATION_GP_REGISTER: {
            struct kefir_asmcmp_value value;
            REQUIRE_OK(match_vreg_to_size(entry->allocation_vreg, operand_size, high_half, &value));
            REQUIRE_OK(
                kefir_asmcmp_inline_assembly_add_value(mem, &function->code.context, context->inline_asm_idx, &value));
        } break;

        case INLINE_ASSEMBLY_PARAMETER_ALLOCATION_SSE_REGISTER: {
            struct kefir_asmcmp_value value = KEFIR_ASMCMP_MAKE_VREG(entry->allocation_vreg);
            REQUIRE_OK(
                kefir_asmcmp_inline_assembly_add_value(mem, &function->code.context, context->inline_asm_idx, &value));
        } break;
        ...
        case INLINE_ASSEMBLY_PARAMETER_ALLOCATION_MEMORY: {
            ...
            if (operand_size <= 1) {
                value =
                    KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(entry->allocation_vreg, 0, KEFIR_ASMCMP_OPERAND_VARIANT_8BIT);
            } else if (operand_size <= 2) {
                value =
                    KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(entry->allocation_vreg, 0, KEFIR_ASMCMP_OPERAND_VARIANT_16BIT);
            } else if (operand_size <= 4) {
                value =
                    KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(entry->allocation_vreg, 0, KEFIR_ASMCMP_OPERAND_VARIANT_32BIT);
            } else if (operand_size <= 8) {
                value =
                    KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(entry->allocation_vreg, 0, KEFIR_ASMCMP_OPERAND_VARIANT_64BIT);
            } else {
                return KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Unable to match register variant wider than 8 bytes");
            }
            REQUIRE_OK(
...
static kefir_result_t default_param_modifier(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    *input_str = input;
    if (asm_param != NULL) {
        REQUIRE_OK(format_normal_parameter(mem, function, context, asm_param, 0, false));
...
static kefir_result_t byte_param_modifier(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    *input_str = input;
    if (asm_param != NULL) {
        REQUIRE_OK(format_normal_parameter(mem, function, context, asm_param, 1, high_half));
    } else if (jump_target != NULL) {
        REQUIRE_OK(format_label_parameter(mem, function, context, jump_target));
    } else {
        return KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Unexpected parameter matching result");
...
static kefir_result_t word_param_modifier(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    *input_str = input;
    if (asm_param != NULL) {
        REQUIRE_OK(format_normal_parameter(mem, function, context, asm_param, 2, false));
    } else if (jump_target != NULL) {
        REQUIRE_OK(format_label_parameter(mem, function, context, jump_target));
    } else {
        return KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Unexpected parameter matching result");
...
static kefir_result_t dword_param_modifier(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    *input_str = input;
    if (asm_param != NULL) {
        REQUIRE_OK(format_normal_parameter(mem, function, context, asm_param, 4, false));
    } else if (jump_target != NULL) {
        REQUIRE_OK(format_label_parameter(mem, function, context, jump_target));
    } else {
        return KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Unexpected parameter matching result");
...
static kefir_result_t qword_param_modifier(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    *input_str = input;
    if (asm_param != NULL) {
        REQUIRE_OK(format_normal_parameter(mem, function, context, asm_param, 8, false));
    } else if (jump_target != NULL) {
        REQUIRE_OK(format_label_parameter(mem, function, context, jump_target));
    } else {
        return KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Unexpected parameter matching result");
...
static kefir_result_t inline_assembly_build(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    while (template_iter != NULL) {
        ...
        if (format_specifier == NULL) {
            ...
            switch (format_specifier[1]) {
                ...
                case 'b': {
                    template_iter = format_specifier + 2;
                    kefir_result_t res = byte_param_modifier(mem, function, context, false, &template_iter);
                    if (res == KEFIR_NOT_FOUND) {
                        template_iter = format_specifier + 1;
                        res = default_param_modifier(mem, function, context, &template_iter);
                    }
                ...
                case 'h': {
                    template_iter = format_specifier + 2;
                    kefir_result_t res = byte_param_modifier(mem, function, context, true, &template_iter);
                    if (res == KEFIR_NOT_FOUND) {
                        template_iter = format_specifier + 1;
                        res = default_param_modifier(mem, function, context, &template_iter);
                    }
    ...
    for (struct kefir_hashtree_node *node = kefir_hashtree_iter(&context->cloberred_vregs, &iter); node != NULL;
        ...
        REQUIRE_OK(kefir_asmcmp_amd64_touch_virtual_register(
            mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context), vreg, NULL));
        node->value = (kefir_hashtree_value_t) vreg;
    }
...
static kefir_result_t store_register_aggregate_outputs(struct kefir_mem *mem,
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(&context->ir_inline_assembly->parameter_list);
        ...
        if (!entry->register_aggregate || ir_asm_param->klass == KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_VALUE ||
    ...
    switch (param_type->typecode) {
        case KEFIR_IR_TYPE_FLOAT32:
            REQUIRE_OK(kefir_asmcmp_amd64_fstp(
                mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                &KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(load_store_vreg, 0, KEFIR_ASMCMP_OPERAND_VARIANT_FP_SINGLE), NULL));
            break;
...
static kefir_result_t store_outputs(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(&context->ir_inline_assembly->parameter_list);
        ...
        if (ir_asm_param->klass == KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_VALUE ||
            ir_asm_param->klass == KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_READ_LOCATION ||
            ir_asm_param->klass == KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_IMMEDIATE) {
            continue;
        }
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(inline_assembly)(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_list_init(&context.available_gp_registers));
    REQUIRE_OK(kefir_list_init(&context.available_sse_registers));
    ...
    REQUIRE_OK(kefir_hashtreeset_init(&context.directly_read_vregs, &kefir_hashtree_uint_ops));
    REQUIRE_OK(kefir_hashtree_init(&context.cloberred_vregs, &kefir_hashtree_uint_ops));
    ...
    REQUIRE_ELSE(res == KEFIR_OK, {
        ...
        kefir_hashtree_free(mem, &context.cloberred_vregs);
        kefir_hashtree_free(mem, &context.explicitly_allocated);
        kefir_hashtreeset_free(mem, &context.directly_read_vregs);
        kefir_hashtree_free(mem, &context.jump_trampolines);
        kefir_list_free(mem, &context.available_gp_registers);
        kefir_list_free(mem, &context.available_sse_registers);
        return res;
    });
    ...
    memset(context.parameters, 0,
           sizeof(struct inline_assembly_parameter_allocation_entry) * context.inline_assembly->parameter_count);
    KEFIR_FREE(mem, context.parameters);
    res = kefir_hashtree_free(mem, &context.cloberred_vregs);
    REQUIRE_ELSE(res == KEFIR_OK, {
        kefir_hashtree_free(mem, &context.explicitly_allocated);
        kefir_hashtreeset_free(mem, &context.directly_read_vregs);
        kefir_hashtree_free(mem, &context.jump_trampolines);
    ...
    res = kefir_hashtreeset_free(mem, &context.directly_read_vregs);
    REQUIRE_ELSE(res == KEFIR_OK, {
        kefir_hashtree_free(mem, &context.jump_trampolines);
        kefir_list_free(mem, &context.available_gp_registers);
        kefir_list_free(mem, &context.available_sse_registers);
    ...
    res = kefir_list_free(mem, &context.available_sse_registers);
    REQUIRE_ELSE(res == KEFIR_OK, {
        kefir_list_free(mem, &context.available_sse_registers);
        return res;
    });
    REQUIRE_OK(kefir_list_free(mem, &context.available_gp_registers));
