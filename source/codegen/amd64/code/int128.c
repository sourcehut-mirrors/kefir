...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(int128_above)(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_cmp(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                      ...
                                      &KEFIR_ASMCMP_MAKE_VREG(arg1_lower_half_vreg), NULL));
    REQUIRE_OK(kefir_asmcmp_amd64_sbb(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                      &KEFIR_ASMCMP_MAKE_VREG(tmp_vreg), &KEFIR_ASMCMP_MAKE_VREG(arg1_upper_half_vreg),
                                      NULL));
    REQUIRE_OK(kefir_asmcmp_amd64_produce_virtual_register(
        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context), result_vreg, NULL));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(int128_from)(struct kefir_mem *mem,
    ...
    REQUIRE_OK(
        kefir_codegen_amd64_function_vreg_of(function, instruction->operation.parameters.refs[0], &lower_half_vreg));
    REQUIRE_OK(
        kefir_codegen_amd64_function_vreg_of(function, instruction->operation.parameters.refs[1], &upper_half_vreg));
    REQUIRE_OK(kefir_asmcmp_virtual_register_new(mem, &function->code.context,
