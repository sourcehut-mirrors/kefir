...
static kefir_result_t kefir_codegen_amd64_return_from_function_impl(struct kefir_mem *mem,
    ...
    if (function->generic.function->ir_func->declaration->no_return) {
        REQUIRE_OK(kefir_asmcmp_amd64_ud2(mem, &function->code,
    ...
    if (kefir_ir_type_length(function->generic.function->ir_func->declaration->result) > 0) {
        ...
        switch (function_return.location) {
            ...
            case KEFIR_ABI_AMD64_FUNCTION_PARAMETER_LOCATION_GENERAL_PURPOSE_REGISTER:
                ...
                REQUIRE_OK(kefir_list_insert_after(mem, touch_vregs, kefir_list_tail(touch_vregs),
                                                   (void *) (kefir_uptr_t) vreg));
                if (return_vreg != KEFIR_ID_NONE) {
                    ...
                    REQUIRE_OK(kefir_asmcmp_amd64_link_virtual_registers(
                        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context), vreg, tmp_vreg,
                        NULL));
                }
                break;
            ...
            case KEFIR_ABI_AMD64_FUNCTION_PARAMETER_LOCATION_MULTIPLE_REGISTERS: {
                ...
                REQUIRE_OK(
                    kefir_abi_amd64_function_decl_returns_layout(&function->abi_function_declaration, &return_layout));
                ...
                kefir_bool_t int128_return = false;
                const struct kefir_ir_typeentry *return_typeentry =
                    kefir_ir_type_at(function->generic.function->ir_func->declaration->result, 0);
                ...
                for (kefir_size_t i = 0; i < length; i++) {
                    ...
                    switch (subparam.location) {
                        ...
                        case KEFIR_ABI_AMD64_FUNCTION_PARAMETER_LOCATION_GENERAL_PURPOSE_REGISTER:
                            ...
                            if (return_vreg != KEFIR_ID_NONE) {
                                if (int128_return) {
                                    REQUIRE(i == 0 || i == 1, KEFIR_SET_ERROR(KEFIR_INVALID_STATE,
                                                                              "Unexpected length of multiple-register "
                                                                              "int128 return value"));
                                    kefir_asmcmp_virtual_register_index_t real_vreg;
                                    REQUIRE_OK(kefir_asmcmp_virtual_register_pair_at(&function->code.context,
                                                                                     return_vreg, i, &real_vreg));
                                    REQUIRE_OK(kefir_asmcmp_amd64_link_virtual_registers(
                                        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                        vreg, real_vreg, NULL));
                                } else if (i + 1 < length) {
                                    ...
                                    kefir_size_t load_size = layout->size % KEFIR_AMD64_ABI_QWORD;
                                    if (load_size == 0) {
                                    ...
                                    REQUIRE_OK(kefir_codegen_amd64_load_general_purpose_register(
                                        mem, function, tmp_vreg, return_vreg, load_size, i * KEFIR_AMD64_ABI_QWORD));
                                    REQUIRE_OK(kefir_asmcmp_amd64_link_virtual_registers(
                                        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                        vreg, tmp_vreg, NULL));
                                }
                        ...
                        case KEFIR_ABI_AMD64_FUNCTION_PARAMETER_LOCATION_SSE_REGISTER:
                            ...
                            if (return_vreg != KEFIR_ID_NONE) {
                                ...
                                if (i + 1 < length) {
                                    REQUIRE_OK(kefir_abi_amd64_function_parameter_multireg_at(&function_return, i + 1,
                                                                                              &next_subparam));
                                }

                                if (i + 1 < length && next_subparam.location ==
                                    ...
                                    kefir_asmcmp_virtual_register_index_t tmp_vreg;
                                    REQUIRE_OK(kefir_asmcmp_virtual_register_new(
                                        mem, &function->code.context, KEFIR_ASMCMP_VIRTUAL_REGISTER_FLOATING_POINT,
                                        &tmp_vreg));
                                    REQUIRE_OK(kefir_asmcmp_amd64_produce_virtual_register(
                                        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                        tmp_vreg, NULL));
            ...
            case KEFIR_ABI_AMD64_FUNCTION_PARAMETER_LOCATION_MEMORY:
                ...
                if (return_vreg != KEFIR_ID_NONE) {
                    ...
                    if (result_instr_ref != KEFIR_ID_NONE) {
                        ...
                        REQUIRE_OK(kefir_opt_code_container_instr(&function->generic.function->code, result_instr_ref,
                                                                  &returned_instr));
                        if (returned_instr->id == function->generic.variable_allocator.return_space_variable_ref) {
                            copy_return = false;
                            ...
                            REQUIRE_OK(kefir_opt_code_container_call(
                                &function->generic.function->code, returned_instr->operation.parameters.function_call.call_ref,
                                &call));
                            if (call->return_space != KEFIR_ID_NONE &&
                                call->return_space == function->generic.variable_allocator.return_space_variable_ref) {
                                copy_return = false;
                            }
                        }
                    }
            ...
            case KEFIR_ABI_AMD64_FUNCTION_PARAMETER_LOCATION_X87:
                if (return_vreg != KEFIR_ID_NONE) {
                    ...
                    for (res = kefir_codegen_amd64_x87_iter(&function->translator.x87, &iter, &stack_instr_ref); res == KEFIR_OK; res = kefir_codegen_amd64_x87_next(&iter, &stack_instr_ref), stack_index++) {
                        if (stack_instr_ref == result_instr_ref) {
                            ...
                            const kefir_size_t stack_size = kefir_codegen_amd64_x87_length(&function->translator.x87);
                            if (stack_size != 1) {
                                REQUIRE_OK(kefir_asmcmp_amd64_fxch(
                                    mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                    ...
                    if (!found_result) {
                        ...
                        REQUIRE_OK(kefir_codegen_amd64_x87_reset(mem, &function->translator.x87));
                    }
                } else {
                    REQUIRE_OK(KEFIR_CODEGEN_AMD64_FUNCTION_X87_CLEAR(mem, function, 0));
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(touch_vregs); iter != NULL; kefir_list_next(&iter)) {
...
kefir_result_t kefir_codegen_amd64_return_from_function(struct kefir_mem *mem,
    ...
    kefir_result_t res =
        kefir_codegen_amd64_return_from_function_impl(mem, function, result_instr_ref, return_vreg, &touch_vregs);
    REQUIRE_ELSE(res == KEFIR_OK, {
        kefir_list_free(mem, &touch_vregs);
        return res;
    });
    REQUIRE_OK(kefir_list_free(mem, &touch_vregs));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(return)(struct kefir_mem *mem,
    ...
    if (instruction->operation.parameters.refs[0] != KEFIR_ID_NONE &&
        !function->generic.function->ir_func->declaration->no_return) {
        REQUIRE_OK(
