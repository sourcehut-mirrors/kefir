...
static kefir_result_t is_local_var(struct kefir_mem *mem, struct kefir_codegen_amd64_function *function,
                                   kefir_opt_instruction_ref_t instr_ref, kefir_bool_t *res,
                                   kefir_asmcmp_virtual_register_index_t *local_var_vreg, kefir_int64_t *offset) {
    UNUSED(mem);
    const struct kefir_opt_instruction *location_instr;
    REQUIRE_OK(kefir_opt_code_container_instr(&function->generic.function->code, instr_ref, &location_instr));
    if (location_instr->operation.opcode == KEFIR_OPT_OPCODE_ALLOC_LOCAL &&
        location_instr->id != function->generic.variable_allocator.return_space_variable_ref) {
        kefir_id_t local_var_id;
        REQUIRE_OK(kefir_codegen_local_variable_allocator_mark_alive(mem, &function->generic.variable_allocator, instr_ref,
                                                                     &local_var_id));
        REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, instr_ref, local_var_vreg));
        *offset = 0;
        *res = true;
    } else if (location_instr->operation.opcode == KEFIR_OPT_OPCODE_REF_LOCAL &&
               location_instr->operation.parameters.refs[0] != function->generic.variable_allocator.return_space_variable_ref) {
        kefir_id_t local_var_id;
        REQUIRE_OK(kefir_codegen_local_variable_allocator_mark_alive(
            mem, &function->generic.variable_allocator, location_instr->operation.parameters.refs[0], &local_var_id));
        REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, location_instr->operation.parameters.refs[0],
                                                        local_var_vreg));
...
static kefir_result_t match_location_offset(struct kefir_codegen_amd64_function *function,
                                            const struct kefir_opt_instruction *instruction,
                                            kefir_opt_instruction_ref_t *location_ref_ptr, kefir_int64_t *offset_ptr) {
    *location_ref_ptr = instruction->operation.parameters.refs[KEFIR_OPT_MEMORY_ACCESS_LOCATION_REF];
    *offset_ptr = 0;

    const struct kefir_opt_instruction *location_instr;
    REQUIRE_OK(kefir_opt_code_container_instr(&function->generic.function->code, *location_ref_ptr, &location_instr));
    if (location_instr->operation.opcode == KEFIR_OPT_OPCODE_INT64_ADD) {
        ...
        REQUIRE_OK(kefir_opt_code_container_instr(&function->generic.function->code,
                                                  location_instr->operation.parameters.refs[0], &location_arg1_instr));
        REQUIRE_OK(kefir_opt_code_container_instr(&function->generic.function->code,
                                                  location_instr->operation.parameters.refs[1], &location_arg2_instr));

        if (location_arg2_instr->block_id == instruction->block_id &&
            ...
            *location_ref_ptr = location_arg2_instr->id;
            *offset_ptr = location_arg1_instr->operation.parameters.imm.integer;
        } else if (location_arg1_instr->block_id == instruction->block_id &&
            ...
            *location_ref_ptr = location_arg1_instr->id;
            *offset_ptr = location_arg2_instr->operation.parameters.imm.integer;
        }
    }
...
#define DEFINE_STORE(_width)                                                                                         \
    do {                                                                                                             \
        ...
        REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(                                                             \
            &function->translator, instruction->operation.parameters.refs[KEFIR_OPT_MEMORY_ACCESS_VALUE_REF], &value_vreg));      \
        ...
        kefir_asmcmp_virtual_register_index_t local_var_vreg;                                                        \
        ...
        REQUIRE_OK(is_local_var(mem, function,                                                                       \
                                ...
                                &local_var, &local_var_vreg, &load_offset));                                         \
        struct kefir_asmcmp_value target_value;                                                                      \
        if (local_var) {                                                                                             \
            target_value = KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(local_var_vreg, load_offset,                           \
            ...
            REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, location_ref, &target_vreg));                  \
            target_value =                                                                                           \
                KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(target_vreg, offset, KEFIR_ASMCMP_OPERAND_VARIANT_##_width##BIT); \
        }                                                                                                            \
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(int32_store)(struct kefir_mem *mem,
    ...
    struct kefir_asmcmp_value target_value;
    if (local_var) {
        target_value = KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(local_var_vreg, load_offset, target_variant);
    } else {
        kefir_opt_instruction_ref_t location_ref =
            instruction->operation.parameters.refs[KEFIR_OPT_MEMORY_ACCESS_LOCATION_REF];
        kefir_int64_t offset = 0;
        ...
        kefir_asmcmp_virtual_register_index_t target_vreg;
        REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, location_ref, &target_vreg));
        target_value = KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(target_vreg, offset, target_variant);
    }
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(long_double_store)(
    ...
    if (local_var) {
        target_value = KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(local_var_vreg, load_offset + KEFIR_AMD64_ABI_QWORD,
                                                          KEFIR_ASMCMP_OPERAND_VARIANT_64BIT);
        target_value2 =
            KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(local_var_vreg, load_offset, KEFIR_ASMCMP_OPERAND_VARIANT_80BIT);
    } else {
        kefir_opt_instruction_ref_t location_ref =
            instruction->operation.parameters.refs[KEFIR_OPT_MEMORY_ACCESS_LOCATION_REF];
        kefir_int64_t offset = 0;
        ...
        kefir_asmcmp_virtual_register_index_t target_vreg;
        REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, location_ref, &target_vreg));
        target_value = KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(target_vreg, offset + KEFIR_AMD64_ABI_QWORD,
                                                          KEFIR_ASMCMP_OPERAND_VARIANT_64BIT);
        target_value2 = KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(target_vreg, offset, KEFIR_ASMCMP_OPERAND_VARIANT_80BIT);
    }
    ...
    REQUIRE_OK(KEFIR_CODEGEN_AMD64_FUNCTION_X87_LOAD(
        mem, function, instruction->operation.parameters.refs[KEFIR_OPT_MEMORY_ACCESS_VALUE_REF]));
    REQUIRE_OK(KEFIR_CODEGEN_AMD64_FUNCTION_X87_CONSUME_BY(
        mem, function, instruction->operation.parameters.refs[KEFIR_OPT_MEMORY_ACCESS_VALUE_REF], instruction->id));
...
#define DEFINE_LOAD(_suffix, _width)                                                                                 \
    do {                                                                                                             \
        ...
        if (local_var) {                                                                                             \
            source_value = KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(local_var_vreg, load_offset,                           \
                                                              KEFIR_ASMCMP_OPERAND_VARIANT_##_width##BIT);           \
        } else {                                                                                                     \
            kefir_opt_instruction_ref_t location_ref =                                                               \
                instruction->operation.parameters.refs[KEFIR_OPT_MEMORY_ACCESS_LOCATION_REF];                        \
            ...
            REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, location_ref, &source_vreg));                  \
            source_value =                                                                                           \
                KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(source_vreg, offset, KEFIR_ASMCMP_OPERAND_VARIANT_##_width##BIT); \
        }                                                                                                            \
        kefir_asmcmp_virtual_register_index_t value_vreg;                                                            \
        ...
        REQUIRE_OK(kefir_codegen_amd64_function_translator_assign_vreg(mem, &function->translator, instruction->id, value_vreg));            \
    } while (false)

#define DEFINE_LOAD_NOEXTEND(_width)                                                                                 \
    do {                                                                                                             \
        kefir_bool_t local_var;                                                                                      \
        kefir_asmcmp_virtual_register_index_t local_var_vreg;                                                        \
        kefir_int64_t load_offset;                                                                                   \
        ...
        REQUIRE_OK(kefir_asmcmp_amd64_produce_virtual_register(                                                      \
            mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context), value_vreg, NULL));      \
        REQUIRE_OK(kefir_asmcmp_amd64_mov(mem, &function->code,                                                      \
                                          kefir_asmcmp_context_instr_tail(&function->code.context),                  \
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(int8_load)(struct kefir_mem *mem,
                                                               struct kefir_codegen_amd64_function *function,
                                                               const struct kefir_opt_instruction *instruction) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(function != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid codegen amd64 function"));
    ...
    switch (instruction->operation.parameters.memory_access.flags.load_extension) {
        case KEFIR_OPT_MEMORY_LOAD_NOEXTEND:
            DEFINE_LOAD_NOEXTEND(8);
            break;
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(int16_load)(struct kefir_mem *mem,
                                                                struct kefir_codegen_amd64_function *function,
                                                                const struct kefir_opt_instruction *instruction) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(function != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid codegen amd64 function"));
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(int32_load)(struct kefir_mem *mem,
                                                                struct kefir_codegen_amd64_function *function,
                                                                const struct kefir_opt_instruction *instruction) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(function != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid codegen amd64 function"));
    ...
    switch (instruction->operation.parameters.memory_access.flags.load_extension) {
        ...
        case KEFIR_OPT_MEMORY_LOAD_ZERO_EXTEND: {
            ...
            if (local_var) {
                source_value =
                    KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(local_var_vreg, load_offset, KEFIR_ASMCMP_OPERAND_VARIANT_32BIT);
            } else {
                kefir_opt_instruction_ref_t location_ref =
                    instruction->operation.parameters.refs[KEFIR_OPT_MEMORY_ACCESS_LOCATION_REF];
                kefir_int64_t offset = 0;
                ...
                REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, location_ref, &source_vreg));
                source_value =
                    KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(source_vreg, offset, KEFIR_ASMCMP_OPERAND_VARIANT_32BIT);
            }
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(float32_load)(struct kefir_mem *mem,
                                                                  struct kefir_codegen_amd64_function *function,
                                                                  const struct kefir_opt_instruction *instruction) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(function != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid codegen amd64 function"));
    REQUIRE(instruction != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer instruction"));
    REQUIRE(instruction->operation.parameters.memory_access.flags.load_extension == KEFIR_OPT_MEMORY_LOAD_NOEXTEND,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Unexpected optimizer floating-point load extension"));
    ...
    if (local_var) {
        source_value =
            KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(local_var_vreg, load_offset, KEFIR_ASMCMP_OPERAND_VARIANT_DEFAULT);
    } else {
        kefir_opt_instruction_ref_t location_ref =
            instruction->operation.parameters.refs[KEFIR_OPT_MEMORY_ACCESS_LOCATION_REF];
        kefir_int64_t offset = 0;
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(long_double_load)(struct kefir_mem *mem,
    ...
    if (local_var) {
        ...
        kefir_asmcmp_virtual_register_index_t source_vreg;
        REQUIRE_OK(kefir_codegen_amd64_function_translator_vreg_of(&function->translator, location_ref, &source_vreg));
        source_value = KEFIR_ASMCMP_MAKE_INDIRECT_VIRTUAL(source_vreg, offset, KEFIR_ASMCMP_OPERAND_VARIANT_80BIT);
    }
    ...
    REQUIRE_OK(kefir_asmcmp_amd64_produce_virtual_register(mem, &function->code, function->translator.prologue_tail, value_vreg,
                                                           &function->translator.prologue_tail));
