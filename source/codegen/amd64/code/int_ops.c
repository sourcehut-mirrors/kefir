...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(int_to_bool)(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_opt_code_container_instr(&function->generic.function->code, instruction->operation.parameters.refs[0],
                                              &arg_instr));
    if (IS_BOOL_OPCODE(arg_instr->operation.opcode)) {
        REQUIRE_OK(kefir_asmcmp_amd64_link_virtual_registers(mem, &function->code,
                                                             kefir_asmcmp_context_instr_tail(&function->code.context),
...
#define DEFINE_DIV_MOD(_impl, _res)                                                                                    \
    do {                                                                                                               \
        ...
        REQUIRE_OK(kefir_asmcmp_amd64_register_allocation_requirement(mem, &function->code, arg1_placement_lower_vreg, \
                                                                      KEFIR_AMD64_XASMGEN_REGISTER_RAX));              \
        if ((_res) == KEFIR_AMD64_XASMGEN_REGISTER_RDX) {                                                              \
            result_placement_vreg = arg1_placement_upper_vreg;                                                         \
        } else {                                                                                                       \
            result_placement_vreg = arg1_placement_lower_vreg;                                                         \
        }                                                                                                              \
                                                                                                                       \
        REQUIRE_OK(kefir_asmcmp_amd64_link_virtual_registers(mem, &function->code,                                     \
                                                             kefir_asmcmp_context_instr_tail(&function->code.context), \
...
#define DEFINE_DIV_MOD8(_impl, _res)                                                                                \
    do {                                                                                                            \
        ...
        REQUIRE_OK(kefir_asmcmp_amd64_register_allocation_requirement(mem, &function->code, arg1_placement_vreg,    \
                                                                      KEFIR_AMD64_XASMGEN_REGISTER_RAX));           \
        result_placement_vreg = arg1_placement_vreg;                                                                \
                                                                                                                    \
        _impl                                                                                                       \
                                                                                                                    \
            REQUIRE_OK(kefir_codegen_amd64_function_translator_assign_vreg(mem, &function->translator, instruction->id, result_vreg));      \
            ...
            DEFINE_DIV_MOD8(
                {
                    ...
                    REQUIRE_OK(kefir_asmcmp_amd64_mov(
                        mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                        &KEFIR_ASMCMP_MAKE_VREG8(result_vreg), &KEFIR_ASMCMP_MAKE_VREG8(result_placement_vreg), NULL));
                },
                KEFIR_AMD64_XASMGEN_REGISTER_RAX);
            break;
            ...
            DEFINE_DIV_MOD8(
                {
                    ...
                    REQUIRE_OK(kefir_asmcmp_amd64_mov(mem, &function->code,
                                                      ...
                                                      &KEFIR_ASMCMP_MAKE_VREG8_HIGH(result_placement_vreg), NULL));
                },
                KEFIR_AMD64_XASMGEN_REGISTER_RAX);
            break;
...
kefir_result_t KEFIR_CODEGEN_AMD64_INSTRUCTION_IMPL(int_bool_and)(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_opt_code_container_instr(&function->generic.function->code, instruction->operation.parameters.refs[0],
                                              &arg1_instr));
    REQUIRE_OK(kefir_opt_code_container_instr(&function->generic.function->code, instruction->operation.parameters.refs[1],
                                              &arg2_instr));
    if (use_instr_ref != KEFIR_ID_NONE) {
        REQUIRE_OK(kefir_opt_code_container_instr(&function->generic.function->code, use_instr_ref, &use_instr));
    }
#define IS_BOOL_OPCODE(_opcode)                                                                      \
    ((_opcode) == KEFIR_OPT_OPCODE_SCALAR_COMPARE || (_opcode) == KEFIR_OPT_OPCODE_INT8_BOOL_AND ||  \
     ...
     (_opcode) == KEFIR_OPT_OPCODE_INT16_TO_BOOL || (_opcode) == KEFIR_OPT_OPCODE_INT32_TO_BOOL ||   \
     (_opcode) == KEFIR_OPT_OPCODE_INT64_TO_BOOL)
    if (IS_BOOL_OPCODE(arg1_instr->operation.opcode)) {
        full_test1 = false;
    }
    if (IS_BOOL_OPCODE(arg2_instr->operation.opcode)) {
        full_test2 = false;
    }
#undef IS_BOOL_OPCODE
    ...
    if (use_instr == NULL || !KEFIR_AMD64_CODEGEN_INSTR_CONSUMES_8BIT_BOOL(use_instr, instruction->id)) {
        ...
        REQUIRE_OK(
            kefir_asmcmp_amd64_movzx(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                     &KEFIR_ASMCMP_MAKE_VREG64(result_vreg), &KEFIR_ASMCMP_MAKE_VREG8(tmp_vreg), NULL));
    } else {
        REQUIRE_OK(
            kefir_asmcmp_amd64_and(mem, &function->code, kefir_asmcmp_context_instr_tail(&function->code.context),
                                   &KEFIR_ASMCMP_MAKE_VREG8(result_vreg), &KEFIR_ASMCMP_MAKE_VREG8(tmp_vreg), NULL));
    }
#undef KEFIR_AMD64_CODEGEN_INSTR_CONSUMES_8BIT_BOOL
