...
static kefir_result_t free_value_liveness(struct kefir_mem *mem, struct kefir_hashtable *table,
                                          kefir_hashtable_key_t key, kefir_hashtable_value_t value, void *payload) {
    UNUSED(table);
    UNUSED(key);
    UNUSED(payload);
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ASSIGN_DECL_CAST(struct kefir_codegen_target_ir_value_liveness *, liveness, value);
    ...
    for (kefir_size_t i = 0; i < kefir_codegen_target_ir_code_block_count(liveness->code); i++) {
        ...
        if (liveness->blocks[block_ref].value_liveness_ranges_ready) {
            for (kefir_size_t j = 0; j < liveness->blocks[block_ref].value_liveness_ranges.length; j++) {
                REQUIRE_OK(kefir_hashset_free(
                    mem, &liveness->blocks[block_ref].value_liveness_ranges.indices[j].begin_liveness));
                REQUIRE_OK(kefir_hashset_free(
                    mem, &liveness->blocks[block_ref].value_liveness_ranges.indices[j].end_liveness));
            }
            REQUIRE_OK(
                kefir_hashset_free(mem, &liveness->blocks[block_ref].value_liveness_ranges.null_index.begin_liveness));
            REQUIRE_OK(
                kefir_hashset_free(mem, &liveness->blocks[block_ref].value_liveness_ranges.null_index.end_liveness));
...
kefir_result_t kefir_codegen_target_ir_liveness_free(struct kefir_mem *mem,
...
kefir_result_t kefir_codegen_target_ir_liveness_reset(struct kefir_mem *mem,
                                                      struct kefir_codegen_target_ir_liveness *liveness) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(liveness != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR liveness"));
...
static kefir_result_t add_to_entry(struct kefir_mem *mem, struct kefir_codegen_target_ir_liveness_entry *entry,
                                   kefir_codegen_target_ir_value_ref_t value_ref) {
    if (entry->length > 0 && entry->content[entry->length - 1].instr_ref == value_ref.instr_ref &&
    ...
    if (entry->length >= entry->capacity) {
        ...
        kefir_codegen_target_ir_value_ref_t *new_content =
            KEFIR_REALLOC(mem, entry->content, sizeof(kefir_codegen_target_ir_value_ref_t) * new_capacity);
        REQUIRE(new_content != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR liveness entry"));
...
kefir_result_t kefir_codegen_target_ir_liveness_value_at(const struct kefir_codegen_target_ir_liveness *liveness,
    ...
    kefir_result_t res = kefir_hashtable_at(
        &liveness->values, (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref), &table_value);
    if (res == KEFIR_NOT_FOUND) {
        res = KEFIR_SET_ERROR(KEFIR_NOT_FOUND, "Unable to find target IR value liveness");
    }
    REQUIRE_OK(res);
    ASSIGN_DECL_CAST(struct kefir_codegen_target_ir_value_liveness *, value_liveness, table_value);
...
static kefir_result_t add_value_liveness(struct kefir_mem *mem, struct kefir_codegen_target_ir_liveness *liveness,
                                         kefir_codegen_target_ir_value_ref_t value_ref,
                                         kefir_codegen_target_ir_block_ref_t block_ref,
                                         enum kefir_codegen_target_ir_liveness_class klass,
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        REQUIRE(value_liveness != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR value liveness"));
        res = kefir_hashtable_init(&value_liveness->per_block, &kefir_hashtable_uint_ops);
        REQUIRE_CHAIN(&res,
                      kefir_hashtable_insert(mem, &liveness->values, key, (kefir_hashtable_value_t) value_liveness));
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        *table_value_ptr = (((kefir_uint64_t) current_begin_ref) << 32) | (kefir_uint32_t) current_end_ref;
        ...
        REQUIRE_OK(kefir_hashtable_insert(mem, &value_liveness->per_block, (kefir_hashtable_key_t) per_block_key,
                                          table_value));
    }
    return KEFIR_OK;
}

static kefir_result_t has_upsilon_for(const struct kefir_codegen_target_ir_code *code,
                                      kefir_codegen_target_ir_instruction_ref_t instr_ref,
                                      kefir_codegen_target_ir_block_ref_t block_ref, kefir_bool_t *has_upsilon) {
    *has_upsilon = false;

    kefir_codegen_target_ir_instruction_ref_t iter_ref =
    ...
    for (; iter_ref != KEFIR_ID_NONE && !*has_upsilon;
         iter_ref = kefir_codegen_target_ir_code_control_prev(code, iter_ref)) {
...
static kefir_result_t propagate_instr_liveness(struct kefir_mem *mem,
    ...
    if (instr->operation.opcode == control_flow->code->klass->phi_opcode) {
        REQUIRE_OK(add_value_liveness(mem, liveness, value_ref, instr->block_ref,
                                      KEFIR_CODEGEN_TARGET_IR_LIVENESS_NORMAL, KEFIR_ID_NONE, instr->instr_ref));
    }
    ...
    for (res = kefir_codegen_target_ir_code_use_iter(control_flow->code, &use_iter, value_ref.instr_ref,
                                                     &user_instr_ref, &used_value);
         res == KEFIR_OK; res = kefir_codegen_target_ir_code_use_next(&use_iter, &user_instr_ref, &used_value)) {
        ...
        if (user_instr->operation.opcode == control_flow->code->klass->upsilon_opcode) {
            if (KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&user_instr->operation.parameters[0].upsilon_ref) ==
                KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref)) {
                REQUIRE(kefir_hashset_has(&control_flow->blocks[user_instr->block_ref].successors,
                                          (kefir_hashset_key_t) instr->block_ref),
                        KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Expected upsilon instruction to be placed in the "
                                                             "predecessor block of corresponding phi instruction"));
                REQUIRE_OK(add_value_liveness(mem, liveness, value_ref, user_instr->block_ref,
                                              KEFIR_CODEGEN_TARGET_IR_LIVENESS_UPSILON, user_instr->instr_ref,
                                              KEFIR_ID_NONE));
                REQUIRE_OK(add_to_entry(mem, &liveness->blocks[user_instr->block_ref].live_out, value_ref));
            }

            if (KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&user_instr->operation.parameters[1].direct.value_ref) ==
                KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref)) {
                if (instr->block_ref != user_instr->block_ref) {
                    REQUIRE_OK(kefir_list_insert_after(mem, queue, kefir_list_tail(queue),
                                                       (void *) (kefir_uptr_t) user_instr->block_ref));
                    REQUIRE_OK(add_to_entry(mem, &liveness->blocks[user_instr->block_ref].live_in, value_ref));
                    REQUIRE_OK(add_value_liveness(mem, liveness, value_ref, user_instr->block_ref,
                                                  KEFIR_CODEGEN_TARGET_IR_LIVENESS_NORMAL, KEFIR_ID_NONE,
                                                  user_instr->instr_ref));
                } else {
                    REQUIRE_OK(add_value_liveness(mem, liveness, value_ref, user_instr->block_ref,
                                                  KEFIR_CODEGEN_TARGET_IR_LIVENESS_NORMAL, instr->instr_ref,
                                                  user_instr->instr_ref));
                }
            }
            ...
            for (res = kefir_codegen_target_ir_code_phi_link_iter(control_flow->code, &iter, user_instr_ref,
                                                                  &link_block_ref, &link_value_ref);
                 res == KEFIR_OK;
                ...
                if (link_value_ref.instr_ref == value_ref.instr_ref && link_value_ref.aspect == value_ref.aspect) {
                    ...
                    if (!has_upsilon) {
                        REQUIRE_OK(kefir_list_insert_after(mem, queue, kefir_list_tail(queue),
                                                           (void *) (kefir_uptr_t) link_block_ref));
                        REQUIRE_OK(add_to_entry(mem, &liveness->blocks[link_block_ref].live_out, value_ref));
                        REQUIRE_OK(add_value_liveness(
                            mem, liveness, value_ref, link_block_ref, KEFIR_CODEGEN_TARGET_IR_LIVENESS_NORMAL,
                            kefir_codegen_target_ir_code_block_control_tail(liveness->code, link_block_ref),
                            KEFIR_ID_NONE));
                    }
                }
            }
    ...
    for (struct kefir_list_entry *head = kefir_list_head(queue); head != NULL; head = kefir_list_head(queue)) {
        ...
        if (block_ref == instr->block_ref) {
            REQUIRE_OK(add_value_liveness(mem, liveness, value_ref, block_ref, KEFIR_CODEGEN_TARGET_IR_LIVENESS_NORMAL,
                                          instr->instr_ref, KEFIR_ID_NONE));
        }
        ...
        REQUIRE_OK(add_value_liveness(mem, liveness, value_ref, block_ref, KEFIR_CODEGEN_TARGET_IR_LIVENESS_NORMAL,
                                      KEFIR_ID_NONE,
                                      kefir_codegen_target_ir_code_block_control_head(liveness->code, block_ref)));

        struct kefir_hashset_iterator predecessor_iter;
        kefir_result_t res;
        kefir_hashset_key_t key;
        for (res = kefir_hashset_iter(&control_flow->blocks[block_ref].predecessors, &predecessor_iter, &key);
             res == KEFIR_OK; res = kefir_hashset_next(&predecessor_iter, &key)) {
            ASSIGN_DECL_CAST(kefir_codegen_target_ir_block_ref_t, predecessor_block_ref, key);
            if (!kefir_codegen_target_ir_control_flow_is_reachable(control_flow, predecessor_block_ref)) {
                continue;
            }
            REQUIRE_OK(kefir_list_insert_after(mem, queue, kefir_list_tail(queue),
                                               (void *) (kefir_uptr_t) predecessor_block_ref));
            REQUIRE_OK(add_to_entry(mem, &liveness->blocks[predecessor_block_ref].live_out, value_ref));
            REQUIRE_OK(add_value_liveness(
                mem, liveness, value_ref, predecessor_block_ref, KEFIR_CODEGEN_TARGET_IR_LIVENESS_NORMAL,
                kefir_codegen_target_ir_code_block_control_tail(liveness->code, predecessor_block_ref), KEFIR_ID_NONE));
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
        }
...
static kefir_result_t get_liveness_index_for(
    ...
    struct kefir_codegen_target_ir_liveness_index **liveness_index_ptr) {

    if (instr_ref != KEFIR_ID_NONE) {
        kefir_size_t index;
        REQUIRE_OK(kefir_codegen_target_ir_numbering_instruction_seq_index(numbering, instr_ref, &index));
        REQUIRE(index < per_block_ranges->length,
                KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Unexpected target IR instruction sequential index"));
...
static kefir_result_t register_liveness_at(
    struct kefir_mem *mem, const struct kefir_codegen_target_ir_liveness *liveness,
    kefir_codegen_target_ir_value_ref_t value_ref, kefir_codegen_target_ir_block_ref_t block_ref,
    struct kefir_codegen_target_ir_liveness_value_block_ranges *per_block_ranges) {
    ...
    for (res = kefir_codegen_target_ir_value_liveness_at(liveness, &iter, value_ref, block_ref, &begin_ref, &end_ref);
         res == KEFIR_OK; res = kefir_codegen_target_ir_value_liveness_at_next(&iter, &begin_ref, &end_ref)) {
        ...
        REQUIRE_OK(kefir_hashset_add(mem, &begin_liveness->begin_liveness,
                                     (kefir_hashset_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref)));
        REQUIRE_OK(kefir_hashset_add(mem, &end_liveness->end_liveness,
                                     (kefir_hashset_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref)));
    }
    return KEFIR_OK;
}

static kefir_result_t build_per_block_liveness(
    struct kefir_mem *mem, const struct kefir_codegen_target_ir_control_flow *control_flow,
    const struct kefir_codegen_target_ir_liveness *liveness, kefir_codegen_target_ir_block_ref_t block_ref,
    struct kefir_codegen_target_ir_liveness_value_block_ranges *per_block_ranges) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(control_flow != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR control flow"));
    REQUIRE(liveness != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR liveness"));
    REQUIRE(per_block_ranges != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR per-block liveness ranges"));
    ...
    REQUIRE(per_block_ranges->indices != NULL,
            KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR liveness index"));
    per_block_ranges->length = indices_len;
    kefir_result_t res = KEFIR_OK;
    for (kefir_size_t i = 0; res == KEFIR_OK && i < indices_len; i++) {
        REQUIRE_CHAIN(&res,
                      kefir_hashset_init(&per_block_ranges->indices[i].begin_liveness, &kefir_hashtable_uint_ops));
    ...
    for (kefir_size_t i = 0; i < liveness->blocks[block_ref].live_in.length; i++) {
        REQUIRE_OK(register_liveness_at(mem, liveness, liveness->blocks[block_ref].live_in.content[i], block_ref,
                                        per_block_ranges));
    }
    for (kefir_size_t i = 0; i < liveness->blocks[block_ref].live_out.length; i++) {
        REQUIRE_OK(register_liveness_at(mem, liveness, liveness->blocks[block_ref].live_out.content[i], block_ref,
                                        per_block_ranges));
    }
...
kefir_result_t kefir_codegen_target_ir_liveness_build(struct kefir_mem *mem,
                                                      const struct kefir_codegen_target_ir_control_flow *control_flow,
                                                      struct kefir_codegen_target_ir_liveness *liveness) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(control_flow != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR control flow"));
    ...
    for (kefir_size_t i = 0; i < kefir_codegen_target_ir_code_block_count(liveness->code); i++) {
        ...
        for (kefir_codegen_target_ir_instruction_ref_t instr_ref =
            ...
            kefir_result_t res;
            for (res =
                     kefir_codegen_target_ir_code_value_iter(liveness->code, &value_iter, instr_ref, &value_ref, NULL);
                 res == KEFIR_OK; res = kefir_codegen_target_ir_code_value_next(&value_iter, &value_ref, NULL)) {
...
kefir_result_t kefir_codegen_target_ir_liveness_value_ranges(
    struct kefir_mem *mem, const struct kefir_codegen_target_ir_control_flow *control_flow,
    const struct kefir_codegen_target_ir_liveness *liveness, kefir_codegen_target_ir_block_ref_t block_ref,
    const struct kefir_codegen_target_ir_liveness_value_block_ranges **ranges_ptr) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(control_flow != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR control flow"));
    REQUIRE(liveness != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR liveness"));
    REQUIRE(liveness->blocks != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pre-built target IR liveness"));
    REQUIRE(block_ref != KEFIR_ID_NONE && block_ref < control_flow->code->blocks_length,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR block referenc"));

    if (!liveness->blocks[block_ref].value_liveness_ranges_ready &&
        kefir_codegen_target_ir_control_flow_is_reachable(control_flow, block_ref)) {
        REQUIRE_OK(build_per_block_liveness(mem, control_flow, liveness, block_ref,
                                            &liveness->blocks[block_ref].value_liveness_ranges));
...
kefir_result_t kefir_codegen_target_ir_liveness_build_update_alive_set(
    struct kefir_mem *mem, const struct kefir_codegen_target_ir_liveness *liveness,
    kefir_codegen_target_ir_instruction_ref_t instr_ref,
    const struct kefir_codegen_target_ir_liveness_value_block_ranges *per_block_ranges,
...
kefir_result_t kefir_codegen_target_ir_liveness_range_get(
    ...
    const struct kefir_codegen_target_ir_liveness_value_block_ranges *ranges,
    kefir_codegen_target_ir_instruction_ref_t instr_ref,
    const struct kefir_codegen_target_ir_liveness_index **entry_ptr) {
    REQUIRE(liveness != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR liveness"));
    REQUIRE(ranges != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR per-block liveness ranges"));
...
kefir_result_t kefir_codegen_target_ir_value_liveness_at(
    const struct kefir_codegen_target_ir_liveness *liveness,
    struct kefir_codegen_target_ir_value_liveness_block_iterator *iter, kefir_codegen_target_ir_value_ref_t value_ref,
    kefir_codegen_target_ir_block_ref_t block_ref, kefir_codegen_target_ir_instruction_ref_t *begin_ref_ptr,
    kefir_codegen_target_ir_instruction_ref_t *end_ref_ptr) {
    REQUIRE(liveness != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR liveness"));
    REQUIRE(iter != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
                                          "Expected valid pointer to target IR liveness per-block iterator"));
    ...
    kefir_hashtable_value_t table_value;
    kefir_result_t res = kefir_hashtable_at(
        &liveness->values, (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&iter->value_ref),
        &table_value);
    if (res == KEFIR_NOT_FOUND) {
...
kefir_result_t kefir_codegen_target_ir_value_liveness_at_next(
    ...
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR liveness per-block iterator"));

    kefir_hashtable_value_t table_value;
    kefir_result_t res = kefir_hashtable_at(
        &iter->liveness->values, (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&iter->value_ref),
        &table_value);
    if (res == KEFIR_NOT_FOUND) {
        res = KEFIR_SET_ERROR(KEFIR_NOT_FOUND, "Unable to find liveness for requested target IR value");
    }
...
kefir_result_t kefir_codegen_target_ir_value_liveness_upsilon_at(
...
kefir_result_t kefir_codegen_target_ir_value_liveness_iter(const struct kefir_codegen_target_ir_liveness *liveness,
                                                           struct kefir_codegen_target_ir_value_liveness_iterator *iter,
                                                           kefir_codegen_target_ir_value_ref_t value_ref,
    ...
    REQUIRE(liveness != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR liveness"));
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to target IR value liveness iterator"));
