...
static kefir_result_t free_attributes(struct kefir_mem *mem, struct kefir_hashtable *table, kefir_hashtable_key_t key,
                                      kefir_hashtable_value_t value, void *payload) {
    UNUSED(table);
    UNUSED(key);
    UNUSED(payload);
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ASSIGN_DECL_CAST(struct instruction_attributes *, attributes, value);
...
kefir_result_t kefir_codegen_target_ir_code_init(struct kefir_codegen_target_ir_code *code,
...
kefir_result_t kefir_codegen_target_ir_code_free(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code) {
    ...
    for (kefir_size_t i = 0; i < code->code_length; i++) {
        if (INSTR_AT_UNSAFE(code, i)->finalized && INSTR_AT_UNSAFE(code, i)->block_ref != KEFIR_ID_NONE) {
            if (INSTR_AT_UNSAFE(code, i)->operation.opcode == code->klass->phi_opcode) {
                ...
                KEFIR_FREE(mem, INSTR_AT_UNSAFE(code, i)->operation.phi_node);
                ...
                KEFIR_FREE(mem, INSTR_AT_UNSAFE(code, i)->operation.inline_asm_node);
            } else {
    ...
    for (kefir_size_t i = 0; i < code->allocated_chunks; i++) {
...
kefir_result_t kefir_codegen_target_ir_code_new_block(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                                      kefir_codegen_target_ir_block_ref_t *block_ref_ptr) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
    REQUIRE(block_ref_ptr != NULL,
    ...
    if (code->blocks_length + 1 >= code->blocks_capacity) {
        ...
        struct kefir_codegen_target_ir_block *new_blocks =
            KEFIR_REALLOC(mem, code->blocks, sizeof(struct kefir_codegen_target_ir_block) * new_capacity);
        REQUIRE(new_blocks != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR code blocks"));
...
kefir_result_t kefir_codegen_target_ir_code_indirect_jump_gate_block(
...
kefir_codegen_target_ir_block_ref_t kefir_codegen_target_ir_code_block_by_index(
    const struct kefir_codegen_target_ir_code *code, kefir_size_t index) {
...
const struct kefir_codegen_target_ir_block *kefir_codegen_target_ir_code_block_at(
    const struct kefir_codegen_target_ir_code *code, kefir_codegen_target_ir_block_ref_t block_ref) {
...
kefir_result_t kefir_codegen_target_ir_code_block_mark_externally_visible(
    struct kefir_codegen_target_ir_code *code, kefir_codegen_target_ir_block_ref_t block_ref) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
...
kefir_result_t kefir_codegen_target_ir_code_block_add_public_label(struct kefir_mem *mem,
    ...
    REQUIRE(public_label != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR block public label"));
...
kefir_bool_t kefir_codegen_target_ir_code_is_gate_block(const struct kefir_codegen_target_ir_code *code,
                                                        kefir_codegen_target_ir_block_ref_t block_ref) {
    REQUIRE(code != NULL, false);

    return code->indirect_jump_gate_block == block_ref ||
...
static kefir_result_t free_inline_asm_node(struct kefir_mem *mem, struct kefir_list *list,
    ...
    UNUSED(payload);
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(entry != NULL && entry->value,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR inline assembly fragment"));

    ASSIGN_DECL_CAST(struct kefir_codegen_target_ir_inline_assembly_fragment *, fragment, entry->value);
    KEFIR_FREE(mem, fragment);
    return KEFIR_OK;
}

static kefir_result_t track_use_instr(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    if (add_use) {
        if (code->use_entries_length >= code->use_entries_capacity) {
            ...
            struct kefir_codegen_target_ir_use_entry *new_use_entries =
                KEFIR_REALLOC(mem, code->use_entries, sizeof(struct kefir_codegen_target_ir_use_entry) * new_capacity);
            REQUIRE(new_use_entries != NULL,
                    KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR use entries"));
        ...
        for (kefir_size_t use_entry_idx = instr->use_entry_top; use_entry_idx != (kefir_size_t) ~0ull;) {
            ...
            if (use_entry->user_instr_ref != user_instr_ref ||
                (use_entry->used_aspect != used_aspect && used_aspect != ~0ull)) {
...
static kefir_result_t operand_record_uses(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                          kefir_codegen_target_ir_instruction_ref_t user_instr_ref,
                                          const struct kefir_codegen_target_ir_operand *operand, kefir_bool_t add_use) {
    switch (operand->type) {
        ...
        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF:
            REQUIRE_OK(track_use_instr(mem, code, user_instr_ref, operand->direct.value_ref.instr_ref,
                                       operand->direct.value_ref.aspect, add_use));
            break;
        ...
        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INDIRECT:
            switch (operand->indirect.type) {
                case KEFIR_CODEGEN_TARGET_IR_INDIRECT_VALUE_REF_BASIS:
                    REQUIRE_OK(track_use_instr(mem, code, user_instr_ref, operand->indirect.base.value_ref.instr_ref,
                                               operand->indirect.base.value_ref.aspect, add_use));
                    break;
            ...
            switch (operand->indirect.index_type) {
                case KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_VALUE_REF:
                    REQUIRE_OK(track_use_instr(mem, code, user_instr_ref, operand->indirect.index.value_ref.instr_ref,
                                               operand->indirect.index.value_ref.aspect, add_use));
                    break;
        ...
        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_UPSILON: {
            ...
            REQUIRE(phi_instr->operation.opcode == code->klass->phi_opcode,
                    KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Expected upsilon operand to point at phi instruction"));
            REQUIRE_OK(kefir_codegen_target_ir_code_value_props(code, operand->upsilon_ref, NULL));
            REQUIRE_OK(track_use_instr(mem, code, user_instr_ref, operand->upsilon_ref.instr_ref,
                                       operand->upsilon_ref.aspect, add_use));
        } break;
    }
    return KEFIR_OK;
}
...
static kefir_result_t record_uses(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    if (user_instr->operation.opcode == code->klass->phi_opcode) {
        ...
        for (res = kefir_codegen_target_ir_code_phi_link_iter(code, &iter, user_instr_ref, NULL, &link_value_ref);
             res == KEFIR_OK; res = kefir_codegen_target_ir_code_phi_link_next(&iter, NULL, &link_value_ref)) {
            REQUIRE_OK(
                track_use_instr(mem, code, user_instr_ref, link_value_ref.instr_ref, link_value_ref.aspect, add_use));
        }
        ...
        for (kefir_size_t i = 0; i < user_instr->operation.parameters_length; i++) {
...
static kefir_result_t store_strings_in_operand(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    switch (operand->type) {
        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_EXTERNAL_LABEL:
            operand->external_label.symbolic =
                kefir_string_pool_insert(mem, &code->strings, operand->external_label.symbolic, NULL);
            REQUIRE(operand->external_label.symbolic != NULL,
                    KEFIR_SET_ERROR(KEFIR_OBJALLOC_FAILURE, "Failed to insert external label into string pool"));
            break;

        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_RIP_INDIRECT_EXTERNAL:
            operand->rip_indirection.external =
                kefir_string_pool_insert(mem, &code->strings, operand->rip_indirection.external, NULL);
            REQUIRE(operand->rip_indirection.external != NULL,
                    KEFIR_SET_ERROR(KEFIR_OBJALLOC_FAILURE, "Failed to insert external label into string pool"));
            break;
        ...
        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INDIRECT:
            switch (operand->indirect.type) {
                case KEFIR_CODEGEN_TARGET_IR_INDIRECT_EXTERNAL_LABEL_BASIS:
                    operand->indirect.base.external_label =
                        kefir_string_pool_insert(mem, &code->strings, operand->indirect.base.external_label, NULL);
                    REQUIRE(
                        operand->indirect.base.external_label != NULL,
                        KEFIR_SET_ERROR(KEFIR_OBJALLOC_FAILURE, "Failed to insert external label into string pool"));
                    break;
...
kefir_result_t kefir_codegen_target_ir_code_new_instruction_inplace(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code, kefir_codegen_target_ir_block_ref_t block_ref,
...
kefir_result_t kefir_codegen_target_ir_code_finalize_instruction_inplace(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    if (instr->operation.opcode == code->klass->phi_opcode) {
        instr->operation.phi_node = KEFIR_MALLOC(mem, sizeof(struct kefir_codegen_target_ir_phi_node));
        REQUIRE(instr->operation.phi_node != NULL, KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR phi node"));
        ...
        REQUIRE(instr->operation.inline_asm_node != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Expected valid target IR inline assembly node"));
        kefir_codegen_target_ir_block_ref_t target_block_ref = instr->operation.inline_asm_node->target_block_ref;
        instr->operation.inline_asm_node = KEFIR_MALLOC(mem, sizeof(struct kefir_codegen_target_ir_inline_assembly_node));
        REQUIRE(instr->operation.inline_asm_node != NULL, KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR inline assembly node"));
        instr->operation.inline_asm_node->target_block_ref = target_block_ref;
        ...
        if (target_block_ref != KEFIR_ID_NONE) {
            REQUIRE_OK(
                kefir_codegen_target_ir_code_new_block(mem, code, &instr->operation.inline_asm_node->gate_block_ref));
            REQUIRE_OK(kefir_hashset_add(mem, &code->gate_blocks,
                                         (kefir_hashset_key_t) instr->operation.inline_asm_node->gate_block_ref));
            block = &code->blocks[instr->block_ref];
        } else {
        ...
        if (instr->operation.opcode == code->klass->upsilon_opcode) {
            REQUIRE(instr->operation.parameters_length > 0 && instr->operation.parameters[0].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_UPSILON,
                    KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Expected upsilon instruction to have upsilon type operand"));
        } else if (instr->operation.opcode == code->klass->assign_opcode) {
            REQUIRE(instr->operation.parameters_length > 0,
                    KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Unexpected shape of target IR assign instruction"));
        }
        struct kefir_codegen_target_ir_operand *operands = NULL;
        REQUIRE(instr->operation.parameters_length <= KEFIR_CODEGEN_TARGET_IR_OPERATION_MAX_OPERANDS, KEFIR_SET_ERROR(KEFIR_OUT_OF_BOUNDS, "Provided target IR instruction parameters exceed maximum operand count"));
        if (instr->operation.parameters_length > 0) {
            operands = KEFIR_MALLOC(mem, sizeof(struct kefir_codegen_target_ir_operand) * instr->operation.parameters_length);
            REQUIRE(operands != NULL, KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR instruction operands"));
            memcpy(operands, instr->operation.parameters, sizeof(struct kefir_codegen_target_ir_operand) * instr->operation.parameters_length);
        }
        instr->operation.parameters = operands;
        instr->finalized = true;
        ...
        for (kefir_size_t i = 0; i < instr->operation.parameters_length; i++) {
            REQUIRE(instr->operation.parameters[i].type != KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_UPSILON ||
                    ...
                    KEFIR_SET_ERROR(KEFIR_INVALID_STATE,
                                    "Upsilon type parameters may only be used in upsilon instructions"));
            REQUIRE_OK(store_strings_in_operand(mem, code, &operands[i]));
        }
    }
...
kefir_result_t kefir_codegen_target_ir_code_new_instruction(
    ...
    const struct kefir_codegen_target_ir_operation *operation,
    const struct kefir_codegen_target_ir_instruction_metadata *metadata,
    kefir_codegen_target_ir_instruction_ref_t *instr_ref_ptr) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
    ...
    kefir_codegen_target_ir_instruction_ref_t instr_ref;
    struct kefir_codegen_target_ir_operation *oper = NULL;
    REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction_inplace(mem, code, block_ref, &oper, metadata, &instr_ref));
    *oper = *operation;
    REQUIRE_OK(kefir_codegen_target_ir_code_finalize_instruction_inplace(mem, code, after_instr_ref, instr_ref));
...
static kefir_result_t drop_instruction(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    if (instr->operation.opcode == code->klass->phi_opcode) {
        ...
        KEFIR_FREE(mem, instr->operation.phi_node);
    } else if (instr->operation.opcode == code->klass->inline_asm_opcode) {
        REQUIRE_OK(kefir_list_free(mem, &instr->operation.inline_asm_node->fragments));
        KEFIR_FREE(mem, instr->operation.inline_asm_node);
    } else {
...
kefir_result_t kefir_codegen_target_ir_code_drop_block(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                                       kefir_codegen_target_ir_block_ref_t block_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
...
kefir_result_t kefir_codegen_target_ir_code_drop_instruction(struct kefir_mem *mem,
                                                             struct kefir_codegen_target_ir_code *code,
                                                             kefir_codegen_target_ir_instruction_ref_t instr_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
...
kefir_result_t kefir_codegen_target_ir_code_copy_instruction(struct kefir_mem *mem,
                                                             struct kefir_codegen_target_ir_code *code,
                                                             kefir_codegen_target_ir_block_ref_t block_ref,
                                                             kefir_codegen_target_ir_instruction_ref_t after_instr_ref,
    ...
    if (source_instr->operation.opcode == code->klass->phi_opcode) {
        REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(
            mem, code, block_ref, after_instr_ref,
            &(struct kefir_codegen_target_ir_operation) {.opcode = code->klass->phi_opcode}, &metadata, &instr_ref));

        for (kefir_size_t i = 0; i < source_instr->operation.phi_node->links_length; i++) {
            REQUIRE_OK(kefir_codegen_target_ir_code_phi_attach(
                mem, code, instr_ref, source_instr->operation.phi_node->links[i].link_block_ref,
        ...
        struct kefir_codegen_target_ir_inline_assembly_node inline_asm = {
            .target_block_ref = source_instr->operation.inline_asm_node->target_block_ref
        };
        REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(
            mem, code, block_ref, after_instr_ref,
        ...
        for (res = kefir_codegen_target_ir_code_inline_assembly_fragment_iter(code, &iter, source_instr_ref, &fragment);
            ...
            switch (fragment->type) {
                case KEFIR_CODEGEN_TARGET_IR_INLINE_ASSEMBLY_FRAGMENT_TEXT:
                    REQUIRE_OK(kefir_codegen_target_ir_code_inline_assembly_text_fragment(mem, code, instr_ref,
                                                                                          fragment->text));
                    break;
                ...
                case KEFIR_CODEGEN_TARGET_IR_INLINE_ASSEMBLY_FRAGMENT_OPERAND: {
                    ...
                    REQUIRE_OK(
                        kefir_codegen_target_ir_code_inline_assembly_operand_fragment(mem, code, instr_ref, &operand));
                } break;
            }
        }
        if (res != KEFIR_ITERATOR_END) {
        ...
        REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(mem, code, block_ref, after_instr_ref, &oper, &metadata,
                                                                &instr_ref));
    }
    ...
    for (res = kefir_codegen_target_ir_code_value_iter(code, &source_value_iter, source_instr_ref, &source_value_ref,
                                                       &source_value_type);
         res == KEFIR_OK;
        ...
        REQUIRE_OK(kefir_codegen_target_ir_code_add_aspect(
            mem, code,
            (struct kefir_codegen_target_ir_value_ref) {.instr_ref = instr_ref, .aspect = source_value_ref.aspect},
            &value_type_copy));
    }
    if (res != KEFIR_ITERATOR_END) {
        REQUIRE_OK(res);
...
kefir_result_t kefir_codegen_target_ir_code_phi_attach(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                                       kefir_codegen_target_ir_instruction_ref_t instr_ref,
                                                       kefir_codegen_target_ir_block_ref_t block_ref,
    ...
    for (kefir_size_t link_idx = 0; link_idx < instr->operation.phi_node->links_length; link_idx++) {
        if (instr->operation.phi_node->links[link_idx].link_block_ref == block_ref) {
            REQUIRE(
                instr->operation.phi_node->links[link_idx].link_value_ref.instr_ref == linked_value_ref.instr_ref &&
    ...
    if (instr->operation.phi_node->links_length >= instr->operation.phi_node->links_capacity) {
        ...
        struct kefir_codegen_target_ir_phi_link *new_links = KEFIR_REALLOC(
            mem, instr->operation.phi_node->links, sizeof(struct kefir_codegen_target_ir_phi_link) * new_capacity);
    ...
    for (kefir_size_t i = 0; i < instr->operation.phi_node->links_length; i++) {
        if (instr->operation.phi_node->links[i].link_block_ref == block_ref) {
            ...
            for (kefir_size_t j = 0; !has_other_uses && j < instr->operation.phi_node->links_length; j++) {
                ...
                if (instr->operation.phi_node->links[i].link_value_ref.instr_ref ==
                        instr->operation.phi_node->links[j].link_value_ref.instr_ref &&
                    instr->operation.phi_node->links[i].link_value_ref.aspect ==
                        instr->operation.phi_node->links[j].link_value_ref.aspect) {
                    has_other_uses = true;
                }
            }
            if (!has_other_uses) {
                REQUIRE_OK(track_use_instr(mem, code, instr_ref,
                                           instr->operation.phi_node->links[i].link_value_ref.instr_ref,
                                           instr->operation.phi_node->links[i].link_value_ref.aspect, false));
            }
            memmove(
                &instr->operation.phi_node->links[i], &instr->operation.phi_node->links[i + 1],
                (instr->operation.phi_node->links_length - (i + 1)) * sizeof(struct kefir_codegen_target_ir_phi_link));
            instr->operation.phi_node->links_length--;
            break;
        }
    }
...
kefir_result_t kefir_codegen_target_ir_code_phi_link_for(const struct kefir_codegen_target_ir_code *code,
...
kefir_result_t kefir_codegen_target_ir_code_phi_link_iter(const struct kefir_codegen_target_ir_code *code,
    ...
    iter->link_index = 0;
    REQUIRE(iter->link_index < iter->phi_node->links_length,
            KEFIR_SET_ERROR(KEFIR_ITERATOR_END, "End of target IR phi link iterator"));
...
kefir_result_t kefir_codegen_target_ir_code_phi_node_iter(const struct kefir_codegen_target_ir_code *code,
                                                          struct kefir_codegen_target_ir_value_phi_node_iterator *iter,
                                                          kefir_codegen_target_ir_block_ref_t block_ref,
                                                          kefir_codegen_target_ir_instruction_ref_t *instr_ref_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
    REQUIRE(iter != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
                                          "Expected valid pointer to target IR code block phi link iterator"));
    REQUIRE(block_ref != KEFIR_ID_NONE && block_ref < code->blocks_length,
            KEFIR_SET_ERROR(KEFIR_OUT_OF_BOUNDS, "Expected valid target IR code block reference"));
...
kefir_result_t kefir_codegen_target_ir_code_phi_node_next(struct kefir_codegen_target_ir_value_phi_node_iterator *iter,
                                                          kefir_codegen_target_ir_instruction_ref_t *instr_ref_ptr) {
    REQUIRE(iter != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
                                          "Expected valid pointer to target IR code block phi link iterator"));
...
kefir_size_t kefir_codegen_target_ir_code_num_of_uses(const struct kefir_codegen_target_ir_code *code,
...
kefir_result_t kefir_codegen_target_ir_code_use_iter(const struct kefir_codegen_target_ir_code *code,
                                                     ...
                                                     kefir_codegen_target_ir_instruction_ref_t instr_ref,
                                                     kefir_codegen_target_ir_instruction_ref_t *user_instr_ref_ptr,
    ...
    REQUIRE(iter->use_entry_index != (kefir_size_t) ~0ull,
            KEFIR_SET_ERROR(KEFIR_ITERATOR_END, "End of target IR code use instruction reference iterator"));
...
kefir_result_t kefir_codegen_target_ir_code_use_next(struct kefir_codegen_target_ir_use_iterator *iter,
                                                     kefir_codegen_target_ir_instruction_ref_t *user_instr_ref_ptr,
                                                     kefir_codegen_target_ir_value_ref_t *used_value_ref_ptr) {
    REQUIRE(iter != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
                                          "Expected valid target IR code use instruction reference iterator"));
...
kefir_result_t kefir_codegen_target_ir_code_add_aspect(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                                       kefir_codegen_target_ir_value_ref_t value_ref,
    ...
    if (code->value_types_length >= code->value_types_capacity) {
        ...
        struct kefir_codegen_target_ir_value_type *new_types =
            KEFIR_REALLOC(mem, code->value_types, sizeof(struct kefir_codegen_target_ir_value_type) * new_capacity);
...
kefir_result_t kefir_codegen_target_ir_code_replace_aspect(
...
kefir_result_t kefir_codegen_target_ir_code_add_instruction_attribute(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        REQUIRE(attrs != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR instruction attriubutes"));
        res = kefir_hashset_init(&attrs->attributes, &kefir_hashtable_uint_ops);
        REQUIRE_CHAIN(&res, kefir_hashtable_insert(mem, &code->attributes, (kefir_hashtable_key_t) instr_ref,
                                                   (kefir_hashtable_value_t) attrs));
        REQUIRE_ELSE(res == KEFIR_OK, {
...
kefir_result_t kefir_codegen_target_ir_code_instruction_output(
    const struct kefir_codegen_target_ir_code *code, kefir_codegen_target_ir_instruction_ref_t instr_ref,
...
kefir_result_t kefir_codegen_target_ir_code_value_props(
    const struct kefir_codegen_target_ir_code *code, kefir_codegen_target_ir_value_ref_t value_ref,
    const struct kefir_codegen_target_ir_value_type **value_type_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
...
static kefir_result_t value_ref_repace_use(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                           kefir_codegen_target_ir_instruction_ref_t user_instr_ref,
    ...
    if (value_ref->instr_ref == from_instr_ref) {
        kefir_result_t res = kefir_codegen_target_ir_code_value_props(
            code, (kefir_codegen_target_ir_value_ref_t) {.instr_ref = to_instr_ref, .aspect = value_ref->aspect}, NULL);
        if (res == KEFIR_NOT_FOUND) {
            res = KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST,
                                  "Unable to replace target IR instruction use with missing value aspect");
        }
        REQUIRE_OK(res);
...
static kefir_result_t operand_replace_uses(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                           kefir_codegen_target_ir_instruction_ref_t user_instr_ref,
                                           struct kefir_codegen_target_ir_operand *operand,
                                           kefir_codegen_target_ir_instruction_ref_t to_instr_ref,
                                           kefir_codegen_target_ir_instruction_ref_t from_instr_ref) {
    switch (operand->type) {
        ...
        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF:
            REQUIRE_OK(value_ref_repace_use(mem, code, user_instr_ref, &operand->direct.value_ref, to_instr_ref,
                                            from_instr_ref));
            break;

        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_UPSILON:
            REQUIRE_OK(
                value_ref_repace_use(mem, code, user_instr_ref, &operand->upsilon_ref, to_instr_ref, from_instr_ref));
            break;

        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INDIRECT:
            switch (operand->indirect.type) {
                case KEFIR_CODEGEN_TARGET_IR_INDIRECT_VALUE_REF_BASIS:
                    REQUIRE_OK(value_ref_repace_use(mem, code, user_instr_ref, &operand->indirect.base.value_ref,
                                                    to_instr_ref, from_instr_ref));
                    break;
            ...
            switch (operand->indirect.index_type) {
                case KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_VALUE_REF:
                    REQUIRE_OK(value_ref_repace_use(mem, code, user_instr_ref, &operand->indirect.index.value_ref,
                                                    to_instr_ref, from_instr_ref));
                    break;
...
static kefir_result_t replace_uses(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                   kefir_codegen_target_ir_instruction_ref_t user_instr_ref,
                                   kefir_codegen_target_ir_instruction_ref_t to_instr_ref,
                                   kefir_codegen_target_ir_instruction_ref_t from_instr_ref) {
    struct kefir_codegen_target_ir_instruction *user_instr = NULL;
    ...
    if (user_instr->operation.opcode == code->klass->phi_opcode) {
        for (kefir_size_t link_idx = 0; link_idx < user_instr->operation.phi_node->links_length; link_idx++) {
            struct kefir_codegen_target_ir_value_ref value_ref =
                user_instr->operation.phi_node->links[link_idx].link_value_ref;
            if (value_ref.instr_ref == from_instr_ref) {
                kefir_result_t res;
                res = kefir_codegen_target_ir_code_value_props(
                    code, (kefir_codegen_target_ir_value_ref_t) {.instr_ref = to_instr_ref, .aspect = value_ref.aspect},
                    NULL);
                if (res == KEFIR_NOT_FOUND) {
                    res = KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST,
                                          "Unable to replace target IR instruction use with missing value aspect");
        ...
        for (const struct kefir_list_entry *iter = kefir_list_head(&user_instr->operation.inline_asm_node->fragments);
             iter != NULL; kefir_list_next(&iter)) {
            ASSIGN_DECL_CAST(struct kefir_codegen_target_ir_inline_assembly_fragment *, fragment, iter->value);
            switch (fragment->type) {
                case KEFIR_CODEGEN_TARGET_IR_INLINE_ASSEMBLY_FRAGMENT_TEXT:
                    // Intetionally left blank
                    break;
                ...
                case KEFIR_CODEGEN_TARGET_IR_INLINE_ASSEMBLY_FRAGMENT_OPERAND:
                    REQUIRE_OK(operand_replace_uses(mem, code, user_instr_ref, &fragment->operand, to_instr_ref,
                                                    from_instr_ref));
            }
        }
    } else {
        for (kefir_size_t i = 0; i < user_instr->operation.parameters_length; i++) {
            REQUIRE_OK(operand_replace_uses(mem, code, user_instr_ref, (struct kefir_codegen_target_ir_operand *) &user_instr->operation.parameters[i],
                                            to_instr_ref, from_instr_ref));
        }
    }
...
kefir_result_t kefir_codegen_target_ir_code_replace_operation(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    kefir_codegen_target_ir_instruction_ref_t instr_ref, const struct kefir_codegen_target_ir_operation *operation,
    ...
    REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(mem, code, instr->block_ref, instr_ref, operation,
                                                            &metadata, &inserted_ref));
    ...
    for (res = kefir_codegen_target_ir_code_value_iter(code, &value_iter, instr_ref, &value_ref, &value_type);
        ...
        REQUIRE_OK(kefir_codegen_target_ir_code_add_aspect(
            mem, code, (kefir_codegen_target_ir_value_ref_t) {.instr_ref = inserted_ref, .aspect = value_ref.aspect},
            &value_type_copy));
    }
    if (res != KEFIR_ITERATOR_END) {
        REQUIRE_OK(res);
    ...
    ASSIGN_PTR(replaced_instr_ref, inserted_ref);
    return KEFIR_OK;
}

kefir_result_t kefir_codegen_target_ir_code_replace_instruction(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    for (kefir_size_t use_entry_index = from_instr->use_entry_top; use_entry_index != (kefir_size_t) ~0ull;
        ...
        REQUIRE_OK(
            replace_uses(mem, code, code->use_entries[use_entry_index].user_instr_ref, to_instr_ref, from_instr_ref));
    }
...
static kefir_result_t value_ref_replace(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                        kefir_codegen_target_ir_instruction_ref_t user_instr_ref,
                                        struct kefir_codegen_target_ir_value_ref *value_ref,
...
static kefir_result_t operand_replace_value_uses(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                                 kefir_codegen_target_ir_instruction_ref_t user_instr_ref,
                                                 struct kefir_codegen_target_ir_operand *operand,
                                                 kefir_codegen_target_ir_value_ref_t to_value_ref,
                                                 kefir_codegen_target_ir_value_ref_t from_value_ref) {
    switch (operand->type) {
        ...
        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF:
            REQUIRE_OK(
                value_ref_replace(mem, code, user_instr_ref, &operand->direct.value_ref, to_value_ref, from_value_ref));
            break;

        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_UPSILON:
            REQUIRE_OK(
                value_ref_replace(mem, code, user_instr_ref, &operand->upsilon_ref, to_value_ref, from_value_ref));
            break;

        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INDIRECT:
            switch (operand->indirect.type) {
                case KEFIR_CODEGEN_TARGET_IR_INDIRECT_VALUE_REF_BASIS:
                    REQUIRE_OK(value_ref_replace(mem, code, user_instr_ref, &operand->indirect.base.value_ref,
                                                 to_value_ref, from_value_ref));
                    break;
            ...
            switch (operand->indirect.index_type) {
                case KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_VALUE_REF:
                    REQUIRE_OK(value_ref_replace(mem, code, user_instr_ref, &operand->indirect.index.value_ref,
                                                 to_value_ref, from_value_ref));
                    break;
...
static kefir_result_t replace_value_uses(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                         kefir_codegen_target_ir_instruction_ref_t user_instr_ref,
                                         kefir_codegen_target_ir_value_ref_t to_value_ref,
                                         kefir_codegen_target_ir_value_ref_t from_value_ref) {
    struct kefir_codegen_target_ir_instruction *user_instr = NULL;
    ...
    if (user_instr->operation.opcode == code->klass->phi_opcode) {
        ...
        for (const struct kefir_list_entry *iter = kefir_list_head(&user_instr->operation.inline_asm_node->fragments);
            ...
            switch (fragment->type) {
                ...
                case KEFIR_CODEGEN_TARGET_IR_INLINE_ASSEMBLY_FRAGMENT_OPERAND:
                    REQUIRE_OK(operand_replace_value_uses(mem, code, user_instr_ref, &fragment->operand, to_value_ref,
                                                          from_value_ref));
            }
        }
    } else {
        for (kefir_size_t i = 0; i < user_instr->operation.parameters_length; i++) {
            REQUIRE_OK(operand_replace_value_uses(mem, code, user_instr_ref, (struct kefir_codegen_target_ir_operand *) &user_instr->operation.parameters[i],
                                                  to_value_ref, from_value_ref));
        }
    }
...
kefir_result_t kefir_codegen_target_ir_code_replace_value(struct kefir_mem *mem,
                                                          struct kefir_codegen_target_ir_code *code,
                                                          kefir_codegen_target_ir_value_ref_t to_value_ref,
                                                          kefir_codegen_target_ir_value_ref_t from_value_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
...
kefir_result_t kefir_codegen_target_ir_code_replace_value_in(struct kefir_mem *mem,
                                                             struct kefir_codegen_target_ir_code *code,
                                                             kefir_codegen_target_ir_instruction_ref_t user_instr_ref,
                                                             kefir_codegen_target_ir_value_ref_t to_value_ref,
                                                             kefir_codegen_target_ir_value_ref_t from_value_ref) {
...
kefir_result_t kefir_codegen_target_ir_code_move_after(struct kefir_codegen_target_ir_code *code,
                                                       kefir_codegen_target_ir_block_ref_t block_ref,
                                                       kefir_codegen_target_ir_instruction_ref_t move_after_ref,
                                                       kefir_codegen_target_ir_instruction_ref_t instr_ref) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
    REQUIRE(block_ref != KEFIR_ID_NONE || block_ref < code->blocks_length,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR block reference"));
...
kefir_result_t kefir_codegen_target_ir_code_inline_assembly_text_fragment(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    struct kefir_codegen_target_ir_inline_assembly_fragment *fragment =
        KEFIR_MALLOC(mem, sizeof(struct kefir_codegen_target_ir_inline_assembly_fragment));
    REQUIRE(fragment != NULL,
            KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR inline assembly fragment"));

    fragment->type = KEFIR_CODEGEN_TARGET_IR_INLINE_ASSEMBLY_FRAGMENT_TEXT;
    fragment->text = text;
    kefir_result_t res =
        kefir_list_insert_after(mem, &instr->operation.inline_asm_node->fragments,
                                kefir_list_tail(&instr->operation.inline_asm_node->fragments), fragment);
    REQUIRE_ELSE(res == KEFIR_OK, {
        KEFIR_FREE(mem, fragment);
        return res;
    });
...
kefir_result_t kefir_codegen_target_ir_code_value_next(
    struct kefir_codegen_target_ir_value_iterator *iter, struct kefir_codegen_target_ir_value_ref *value_ref_ptr,
    const struct kefir_codegen_target_ir_value_type **value_type_ptr) {
...
kefir_result_t kefir_codegen_target_ir_code_instruction_attribute_iter(
    const struct kefir_codegen_target_ir_code *code, struct kefir_codegen_target_ir_code_attribute_iterator *iter,
    kefir_codegen_target_ir_instruction_ref_t instr_ref, kefir_codegen_target_ir_native_id_t *attribute_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
    REQUIRE(iter != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
    ...
    REQUIRE_OK(res);
    ASSIGN_DECL_CAST(const struct instruction_attributes *, attributes, table_value);
...
kefir_result_t kefir_codegen_target_ir_code_instruction_attribute_next(
    struct kefir_codegen_target_ir_code_attribute_iterator *iter, kefir_codegen_target_ir_native_id_t *attribute_ptr) {
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR instruction attribute iterator"));
...
kefir_result_t kefir_codegen_target_ir_code_inline_assembly_fragment_iter(
    ...
    REQUIRE(iter->iter != NULL,
            KEFIR_SET_ERROR(KEFIR_ITERATOR_END, "End of target IR inline assembly fragment iterator"));
...
kefir_result_t kefir_codegen_target_ir_code_inline_assembly_fragment_next(
    ...
    const struct kefir_codegen_target_ir_inline_assembly_fragment **fragment_ptr) {
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR inline assembly fragment iterator"));
