...
static kefir_result_t do_replace(struct replace_state *state) {
    for (struct kefir_list_entry *head = kefir_list_head(&state->queue); head != NULL;
        ...
        kefir_codegen_target_ir_value_ref_t new_value_ref =
        ...
        for (res = kefir_codegen_target_ir_code_use_iter(state->code, &use_iter, state->old_value_ref.instr_ref,
                                                         &use_instr_ref, &use_value_ref);
             res == KEFIR_OK; res = kefir_codegen_target_ir_code_use_next(&use_iter, &use_instr_ref, &use_value_ref)) {
            if (use_value_ref.aspect != state->old_value_ref.aspect || use_instr_ref == new_value_ref.instr_ref) {
                continue;
            }
            ...
            if (use_instr->operation.opcode == state->code->klass->phi_opcode) {
                ...
                for (res = kefir_codegen_target_ir_code_phi_link_iter(state->code, &iter, use_instr_ref,
                                                                      &link_block_ref, &link_value_ref);
                     res == KEFIR_OK;) {
                    kefir_bool_t replace = false;
                    if (link_value_ref.instr_ref == state->old_value_ref.instr_ref &&
                        link_value_ref.aspect == state->old_value_ref.aspect) {
                        REQUIRE_OK(kefir_codegen_target_ir_control_flow_is_dominator(
                            state->control_flow, link_block_ref, new_instr->block_ref, &replace));
                    }

                    if (replace) {
                        REQUIRE_OK(kefir_codegen_target_ir_code_phi_drop(state->mem, state->code, use_instr_ref,
                                                                         link_block_ref));
                        REQUIRE_OK(kefir_codegen_target_ir_code_phi_attach(state->mem, state->code, use_instr_ref,
                                                                           link_block_ref, new_value_ref));
                        res = kefir_codegen_target_ir_code_phi_link_iter(state->code, &iter, use_instr_ref,
                                                                         &link_block_ref, &link_value_ref);
                    } else {
                ...
                if (use_instr->block_ref != new_instr->block_ref) {
                    REQUIRE_OK(kefir_codegen_target_ir_control_flow_is_dominator(
                        state->control_flow, use_instr->block_ref, new_instr->block_ref, &dominated_by_new_value));
                } else {
                ...
                if (dominated_by_new_value) {
                    REQUIRE_OK(kefir_codegen_target_ir_code_replace_value_in(state->mem, state->code, use_instr_ref,
                                                                             new_value_ref, state->old_value_ref));
                }
            }
        }
        if (res != KEFIR_ITERATOR_END) {
        ...
        for (res = kefir_hashset_iter(&state->control_flow->blocks[new_instr_block_ref].dominance_frontier, &iter,
                                      &entry);
            ...
            REQUIRE_OK(kefir_codegen_target_ir_control_flow_is_dominator(state->control_flow, frontier_block_ref,
                                                                         old_instr_block_ref, &dominated_by_old_value));
            ...
            kefir_codegen_target_ir_value_ref_t frontier_phi_value_ref = {.instr_ref = frontier_phi_instr_ref,
                                                                          .aspect = state->old_value_ref.aspect};
            REQUIRE_OK(kefir_codegen_target_ir_code_add_aspect(state->mem, state->code, frontier_phi_value_ref,
                                                               &old_value_type_copy));

            kefir_result_t res;
            struct kefir_hashset_iterator iter;
            kefir_hashset_key_t key;
            for (res = kefir_hashset_iter(&state->control_flow->blocks[frontier_block_ref].predecessors, &iter, &key);
                ...
                kefir_bool_t dominated_by_new_value = false;
                REQUIRE_OK(kefir_codegen_target_ir_control_flow_is_dominator(
                    state->control_flow, predecessor_block_ref, new_instr_block_ref, &dominated_by_new_value));
                if (dominated_by_new_value) {
                    REQUIRE_OK(kefir_codegen_target_ir_code_phi_attach(state->mem, state->code, frontier_phi_instr_ref,
                                                                       predecessor_block_ref, new_value_ref));
                } else {
                    REQUIRE_OK(kefir_codegen_target_ir_code_phi_attach(state->mem, state->code, frontier_phi_instr_ref,
                                                                       predecessor_block_ref, state->old_value_ref));
                }
            }
            if (res != KEFIR_ITERATOR_END) {
                REQUIRE_OK(res);
            ...
            REQUIRE_OK(kefir_list_insert_after(
                state->mem, &state->queue, NULL,
                (void *) (kefir_uptr_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&frontier_phi_value_ref)));
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
        }
...
kefir_result_t kefir_codegen_target_ir_partial_replace_value(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    const struct kefir_codegen_target_ir_control_flow *control_flow, kefir_codegen_target_ir_value_ref_t new_value_ref,
    kefir_codegen_target_ir_value_ref_t old_value_ref) {
    ...
    struct replace_state state = {
        .mem = mem, .code = code, .control_flow = control_flow, .old_value_ref = old_value_ref};
    ...
    kefir_result_t res = kefir_list_insert_after(
        mem, &state.queue, NULL, (void *) (kefir_uptr_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&new_value_ref));
