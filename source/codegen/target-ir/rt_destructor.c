...
static kefir_result_t resolve_operand(struct rt_destructor_state *state, const struct kefir_codegen_target_ir_operand *operand, struct kefir_asmcmp_value *value, kefir_bool_t immediate) {
    ...
    switch (operand->type) {
        ...
        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_UPSILON:
            return KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Unexpected upsilon target IR instruction operand");
    }
...
static kefir_result_t translate_instruction(struct rt_destructor_state *state, struct block_state *block_state, kefir_codegen_target_ir_instruction_ref_t instr_ref) {
    ...
    if (instr->operation.opcode == state->code->klass->phi_opcode) {
        ...
        const struct kefir_codegen_target_ir_value_type *value_type;
        REQUIRE_OK(kefir_codegen_target_ir_code_value_props(state->code, instr->operation.parameters[0].upsilon_ref, &value_type));
        if (instr->operation.parameters[0].upsilon_ref.aspect != KEFIR_CODEGEN_TARGET_IR_VALUE_FLAGS && value_type->kind != KEFIR_CODEGEN_TARGET_IR_VALUE_TYPE_LOCAL_VARIABLE) {
            kefir_asmcmp_virtual_register_index_t dst_vreg, src_vreg;
            REQUIRE_OK(resolve_value_virtual_register(state, instr->operation.parameters[0].upsilon_ref, &dst_vreg));
            REQUIRE_OK(resolve_value_virtual_register(state, instr->operation.parameters[1].direct.value_ref, &src_vreg));
    ...
    if (terminator_props.block_terminator && !terminator_props.function_terminator) {
        ...
        if (terminator_props.branch) {
            ...
            REQUIRE(!kefir_codegen_target_ir_control_flow_is_critical_edge(&state->control_flow, block_state->block_ref, terminator_props.target_block_refs[0]),
                KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Expected critical edges to be split before target IR destruction"));
            REQUIRE(!kefir_codegen_target_ir_control_flow_is_critical_edge(&state->control_flow, block_state->block_ref, terminator_props.target_block_refs[1]),
                KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Expected critical edges to be split before target IR destruction"));
