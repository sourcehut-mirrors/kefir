...
struct code_block_state {
    ...
    struct kefir_hashtable block_resource_inputs;
    struct kefir_hashtable virtual_register_refs;
    struct kefir_hashtable resource_refs;
...
struct constructor_state {
    ...
    struct kefir_hashset block_resource_defs;
};

static kefir_result_t mark_vreg_liveness(struct constructor_state *state, struct code_block_state *block_state,
                                         kefir_size_t linear_index, kefir_asmcmp_virtual_register_index_t vreg_idx) {
    ...
    kefir_result_t res = kefir_hashtable_at_mut(&vreg_state->block_lifetimes,
                                                (kefir_hashtable_key_t) block_state->block_ref, &value_ptr);
    if (res != KEFIR_NOT_FOUND) {
        ...
        lifetime = ((kefir_uint64_t) MIN(linear_index, current_linear_index_begin)) << 32 |
                   ((kefir_uint32_t) MAX(linear_index, current_linear_index_end));
        *value_ptr = lifetime;
    } else {
        REQUIRE_OK(kefir_hashtable_insert(state->mem, &vreg_state->block_lifetimes,
                                          (kefir_hashtable_key_t) block_state->block_ref,
                                          (kefir_hashtable_value_t) lifetime));
    }
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        *value_ptr = lifetime;
        ...
        REQUIRE_OK(kefir_hashtable_insert(state->mem, &block_state->alive_vregs, (kefir_hashtable_key_t) vreg_idx,
                                          (kefir_hashtable_value_t) lifetime));
    }
...
static kefir_result_t mark_value_liveness(struct constructor_state *state, struct code_block_state *block_state,
                                          kefir_size_t linear_index, const struct kefir_asmcmp_value *value) {
    switch (value->type) {
...
static kefir_result_t init_code_blocks(struct constructor_state *state) {
    ...
    for (kefir_asmcmp_instruction_index_t instr_idx = kefir_asmcmp_context_instr_head(state->asmcmp_ctx);
        ...
        if (block_state == NULL || label_idx != KEFIR_ASMCMP_INDEX_NONE) {
            ...
            REQUIRE_CHAIN(&res, kefir_hashtable_init(&block_state->block_resource_inputs, &kefir_hashtable_uint_ops));
            REQUIRE_CHAIN(&res, kefir_hashtable_init(&block_state->virtual_register_refs, &kefir_hashtable_uint_ops));
            REQUIRE_CHAIN(&res, kefir_hashtable_init(&block_state->resource_refs, &kefir_hashtable_uint_ops));
            REQUIRE_CHAIN(&res, kefir_hashtable_init(&block_state->alive_vregs, &kefir_hashtable_uint_ops));
            REQUIRE_CHAIN(&res, kefir_hashtree_insert(state->mem, &state->blocks, (kefir_hashtree_key_t) block_ref,
                                                      (kefir_hashtree_value_t) block_state));
            REQUIRE_ELSE(res == KEFIR_OK, {
                KEFIR_FREE(state->mem, block_state);
                return res;
            });
            REQUIRE_OK(kefir_hashtable_insert(state->mem, &state->block_head_instr, (kefir_hashtable_key_t) instr_idx,
                                              (kefir_hashtable_value_t) block_ref));
        }
        for (; label_idx != KEFIR_ASMCMP_INDEX_NONE;
             label_idx = kefir_asmcmp_context_instr_label_next(state->asmcmp_ctx, label_idx)) {
            ...
            if (label->external_dependencies) {
                REQUIRE_OK(
                    kefir_codegen_target_ir_code_block_mark_externally_visible(state->code, block_state->block_ref));
            }
            kefir_result_t res;
            ...
            for (res = kefir_hashtreeset_iter(&label->public_labels, &iter); res == KEFIR_OK;
                ...
                REQUIRE_OK(kefir_codegen_target_ir_code_block_add_public_label(state->mem, state->code,
                                                                               block_state->block_ref, public_label));
            }
            if (res != KEFIR_ITERATOR_END) {
                REQUIRE_OK(res);
            }
            REQUIRE_OK(kefir_hashtable_insert(state->mem, &state->label_blocks, (kefir_hashtable_key_t) label_idx,
                                              (kefir_hashtable_value_t) block_state->block_ref));
        }
        ...
        for (kefir_size_t i = 0; i < KEFIR_ASMCMP_INSTRUCTION_NUM_OF_OPERANDS; i++) {
            REQUIRE_OK(mark_value_liveness(state, block_state, state->asmcmp_instrs[instr_idx].linear_index,
                                           &asmcmp_instr->args[i]));
        }
...
static kefir_result_t init_relocation(kefir_asmcmp_external_label_relocation_t asmcmp_reloc,
                                      kefir_codegen_target_ir_external_label_relocation_t *relocation) {
    switch (asmcmp_reloc) {
...
static kefir_result_t init_variant(kefir_asmcmp_operand_variant_t asmcmp_variant, kefir_bool_t high_half,
    ...
    switch (asmcmp_variant) {
        ...
        case KEFIR_ASMCMP_OPERAND_VARIANT_8BIT:
            *variant = high_half ? KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_8BIT_HIGHER
...
static kefir_asmcmp_virtual_register_index_t store_virtual_register_aspect(
    struct constructor_state *state, kefir_asmcmp_virtual_register_index_t vreg,
    kefir_codegen_target_ir_operand_variant_t variant, kefir_codegen_target_ir_value_ref_t value_ref,
    ...
    switch (virtual_register->type) {
        case KEFIR_ASMCMP_VIRTUAL_REGISTER_UNSPECIFIED:
            REQUIRE_OK(kefir_codegen_target_ir_code_add_aspect(
                state->mem, state->code, value_ref,
                &(struct kefir_codegen_target_ir_value_type) {.kind = KEFIR_CODEGEN_TARGET_IR_VALUE_TYPE_UNSPECIFIED,
                                                              .variant = variant,
        ...
        case KEFIR_ASMCMP_VIRTUAL_REGISTER_GENERAL_PURPOSE:
            REQUIRE_OK(
                kefir_codegen_target_ir_code_add_aspect(state->mem, state->code, value_ref,
        ...
        case KEFIR_ASMCMP_VIRTUAL_REGISTER_FLOATING_POINT:
            REQUIRE_OK(kefir_codegen_target_ir_code_add_aspect(
                state->mem, state->code, value_ref,
                &(struct kefir_codegen_target_ir_value_type) {.kind = KEFIR_CODEGEN_TARGET_IR_VALUE_TYPE_FLOATING_POINT,
                                                              .variant = variant,
                                                              .constraint = constraint_copy,
                                                              .metadata = {.value_ref = vreg}}));
            break;
        ...
        case KEFIR_ASMCMP_VIRTUAL_REGISTER_EXTERNAL_MEMORY:
            REQUIRE_OK(kefir_codegen_target_ir_code_add_aspect(
                ...
                &(struct kefir_codegen_target_ir_value_type) {
                    ...
                    .variant = variant,
                    .parameters.memory = {.base_reg = virtual_register->parameters.memory.base_reg,
...
static kefir_result_t store_virtual_reigster_output(struct constructor_state *state,
                                                    struct code_block_state *block_state,
    ...
    kefir_result_t res =
        kefir_hashtable_at_mut(&block_state->virtual_register_refs, (kefir_hashtable_key_t) vreg, &value);
    if (res != KEFIR_NOT_FOUND) {
        *value = value_ref_encoded;
    } else {
        REQUIRE_OK(kefir_hashtable_insert(state->mem, &block_state->virtual_register_refs, (kefir_hashtable_key_t) vreg,
                                          value_ref_encoded));
    }
...
static kefir_result_t resolve_input_virtual_register(struct constructor_state *state,
                                                     struct code_block_state *block_state,
                                                     kefir_asmcmp_virtual_register_index_t vreg_idx,
                                                     struct kefir_codegen_target_ir_value_ref *value_ref) {
    kefir_hashtable_value_t value;
    kefir_result_t res =
        kefir_hashtable_at(&block_state->virtual_register_refs, (kefir_hashtable_key_t) vreg_idx, &value);
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
        *value_ref = KEFIR_CODEGEN_TARGET_IR_VALUE_REF_FROM(value);
    } else {
        if (block_state->block_ref == state->code->entry_block) {
            return KEFIR_SET_ERROR(KEFIR_INVALID_STATE,
                                   "Failed to resolve input virtual register in target IR construction");
        } else {
            struct kefir_codegen_target_ir_operation operation = {.opcode = state->code->klass->phi_opcode};
            REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(state->mem, state->code, block_state->block_ref,
                                                                    KEFIR_ID_NONE, &operation, NULL,
                                                                    &value_ref->instr_ref));
            value_ref->aspect = KEFIR_CODEGEN_TARGET_IR_VALUE_DIRECT_OUTPUT(0);
            REQUIRE_OK(store_virtual_reigster_output(state, block_state, vreg_idx,
                                                     KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT, *value_ref));
            REQUIRE_OK(kefir_hashtable_insert(state->mem, &block_state->block_inputs, (kefir_hashtable_key_t) vreg_idx,
                                              (kefir_hashtable_value_t) value_ref->instr_ref));
        }
    }
    return KEFIR_OK;
}

static kefir_result_t resolve_resource(struct constructor_state *state, struct code_block_state *block_state,
                                       kefir_codegen_target_ir_resource_id_t resource_id,
                                       struct kefir_codegen_target_ir_value_ref *value_ref) {
    kefir_hashtable_value_t value;
    kefir_result_t res = kefir_hashtable_at(&block_state->resource_refs, (kefir_hashtable_key_t) resource_id, &value);
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
        *value_ref = KEFIR_CODEGEN_TARGET_IR_VALUE_REF_FROM(value);
    } else if (block_state->block_ref == state->code->entry_block) {
        struct kefir_codegen_target_ir_operation operation = {.opcode = state->code->klass->placeholder_opcode};
        REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(
            state->mem, state->code, block_state->block_ref, KEFIR_ID_NONE, &operation, NULL, &value_ref->instr_ref));
        ...
        REQUIRE_OK(kefir_hashtable_insert(state->mem, &block_state->resource_refs, (kefir_hashtable_key_t) resource_id,
                                          (kefir_hashtable_value_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(value_ref)));
        REQUIRE_OK(
            kefir_hashset_add(state->mem, &state->block_resource_defs,
                              (kefir_hashtable_key_t) (((kefir_uint64_t) block_state->block_ref) << 32) | resource_id));
    } else {
        struct kefir_codegen_target_ir_operation operation = {.opcode = state->code->klass->phi_opcode};
        REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(
            state->mem, state->code, block_state->block_ref, KEFIR_ID_NONE, &operation, NULL, &value_ref->instr_ref));
        ...
        REQUIRE_OK(kefir_hashtable_insert(state->mem, &block_state->resource_refs, (kefir_hashtable_key_t) resource_id,
                                          (kefir_hashtable_value_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(value_ref)));
        REQUIRE_OK(kefir_hashtable_insert(state->mem, &block_state->block_resource_inputs,
                                          (kefir_hashtable_key_t) resource_id,
                                          (kefir_hashtable_value_t) value_ref->instr_ref));
        REQUIRE_OK(
            kefir_hashset_add(state->mem, &state->block_resource_defs,
                              (kefir_hashtable_key_t) (((kefir_uint64_t) block_state->block_ref) << 32) | resource_id));
...
static kefir_result_t init_operand(struct constructor_state *state, struct code_block_state *block_state,
                                   struct kefir_codegen_target_ir_operand *operand,
                                   const struct kefir_asmcmp_value *value,
                                   const struct kefir_codegen_target_ir_asmcmp_operand_classification *classification,
    ...
    switch (value->type) {
        ...
        case KEFIR_ASMCMP_VALUE_TYPE_INTEGER:
            REQUIRE(classification->class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ,
                    KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Expected integral constant to have read operand class"));
        ...
        case KEFIR_ASMCMP_VALUE_TYPE_VIRTUAL_REGISTER: {
            ...
            if (vreg->type == KEFIR_ASMCMP_VIRTUAL_REGISTER_IMMEDIATE_INTEGER) {
                ...
                if (classification->class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ ||
                    classification->class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ_WRITE) {
                    operand->type = KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF;
                    REQUIRE_OK(resolve_input_virtual_register(state, block_state, value->vreg.index,
                                                              &operand->direct.value_ref));
                    REQUIRE_OK(init_variant(value->vreg.variant, value->vreg.high_half, &operand->direct.variant));
                    operand->direct.tied = classification->class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ_WRITE;
                }
                if (classification->class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_WRITE ||
                    classification->class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ_WRITE) {
            ...
            switch (value->indirect.type) {
                ...
                case KEFIR_ASMCMP_INDIRECT_VIRTUAL_BASIS: {
                    ...
                    if (vreg->type == KEFIR_ASMCMP_VIRTUAL_REGISTER_IMMEDIATE_INTEGER) {
                        ...
                        REQUIRE_OK(resolve_input_virtual_register(state, block_state, value->indirect.base.vreg,
                                                                  &operand->indirect.base.value_ref));
                    }
                } break;
                ...
                case KEFIR_ASMCMP_INDIRECT_INTERNAL_LABEL_BASIS: {
                    ...
                    kefir_result_t res =
                        kefir_hashtable_at(&state->label_blocks,
                                           (kefir_hashtable_key_t) value->indirect.base.internal_label, &table_value);
                    if (res != KEFIR_NOT_FOUND) {
                        REQUIRE_OK(res);
                        ...
                        REQUIRE_OK(state->ops->get_native_id_by_label(value->indirect.base.internal_label,
                                                                      &operand->indirect.base.native_id,
                                                                      state->ops->payload));
                    }
                } break;
                ...
                case KEFIR_ASMCMP_INDIRECT_EXTERNAL_LABEL_BASIS:
                    ...
                    REQUIRE_OK(
                        init_relocation(value->indirect.base.external_type, &operand->indirect.base.external_type));
                    break;
        ...
        case KEFIR_ASMCMP_VALUE_TYPE_RIP_INDIRECT_INTERNAL: {
            kefir_hashtable_value_t table_value;
            kefir_result_t res = kefir_hashtable_at(
                &state->label_blocks, (kefir_hashtable_key_t) value->rip_indirection.internal, &table_value);
            if (res != KEFIR_NOT_FOUND) {
                REQUIRE_OK(res);
                ...
                REQUIRE_OK(state->ops->get_native_id_by_label(
                    value->rip_indirection.internal, &operand->rip_indirection.native_id, state->ops->payload));
            }
        ...
        case KEFIR_ASMCMP_VALUE_TYPE_INTERNAL_LABEL: {
            kefir_hashtable_value_t table_value;
            kefir_result_t res =
                kefir_hashtable_at(&state->label_blocks, (kefir_hashtable_key_t) value->internal_label, &table_value);
            if (res != KEFIR_NOT_FOUND) {
                REQUIRE_OK(res);
                ...
                REQUIRE_OK(state->ops->get_native_id_by_label(value->internal_label, &operand->native_id,
                                                              state->ops->payload));
            }
        } break;
...
static kefir_result_t terminate_current_block(struct constructor_state *state,
                                              struct code_block_state *current_block_state,
                                              struct code_block_state *next_block_state) {
    kefir_codegen_target_ir_instruction_ref_t current_block_tail_ref =
    ...
    if (current_block_tail_ref == KEFIR_ID_NONE) {
        ...
        REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(
            state->mem, state->code, current_block_state->block_ref,
            kefir_codegen_target_ir_code_block_control_tail(state->code, current_block_state->block_ref), &operation,
            NULL, NULL));
        return KEFIR_OK;
    }
    ...
    REQUIRE_OK(state->code->klass->is_block_terminator(state->code, current_block_tail, &terminator_props,
                                                       state->code->klass->payload));
    ...
    if (!terminator_props.block_terminator) {
        ...
        REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(
            ...
            &metadata, NULL));
               ...
               state->code->indirect_jump_gate_block == KEFIR_ID_NONE) {
        REQUIRE(
            !terminator_props.fallthrough,
            KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Unexpected fallthrough terminator instruction in target IR block"));
        REQUIRE_OK(kefir_codegen_target_ir_code_indirect_jump_gate_block(state->mem, state->code,
                                                                         &state->code->indirect_jump_gate_block));
        ...
        REQUIRE_CHAIN(&res, kefir_hashtable_init(&gate_block_state->resource_refs, &kefir_hashtable_uint_ops));
        REQUIRE_CHAIN(&res, kefir_hashtable_init(&gate_block_state->alive_vregs, &kefir_hashtable_uint_ops));
        REQUIRE_CHAIN(&res, kefir_hashtree_insert(state->mem, &state->blocks,
                                                  (kefir_hashtree_key_t) state->code->indirect_jump_gate_block,
...
static kefir_result_t match_implicit_parameter_to_vreg(struct constructor_state *state,
                                                       struct code_block_state *block_state,
    ...
    for (res = kefir_hashtable_iter(&block_state->alive_vregs, &iter, &table_key, &table_value); res == KEFIR_OK;
         res = kefir_hashtable_next(&iter, &table_key, &table_value)) {
        ASSIGN_DECL_CAST(kefir_asmcmp_virtual_register_index_t, candidate_vreg, table_key);
        ASSIGN_DECL_CAST(kefir_size_t, lifetime, table_value);
...
static kefir_result_t link_vregs_impl(struct constructor_state *state, struct code_block_state *current_block_state,
                                      kefir_asmcmp_virtual_register_index_t vreg1_idx,
                                      kefir_asmcmp_virtual_register_index_t vreg2_idx,
                                      const struct kefir_codegen_target_ir_instruction_metadata *metadata) {
    ...
    if (vreg1->type == KEFIR_ASMCMP_VIRTUAL_REGISTER_PAIR && vreg2->type == KEFIR_ASMCMP_VIRTUAL_REGISTER_PAIR) {
        ...
        REQUIRE_OK(link_vregs_impl(state, current_block_state, vreg1->parameters.pair.virtual_registers[0],
                                   vreg2->parameters.pair.virtual_registers[0], metadata));
        REQUIRE_OK(link_vregs_impl(state, current_block_state, vreg1->parameters.pair.virtual_registers[1],
                                   vreg2->parameters.pair.virtual_registers[1], metadata));
    } else {
        ...
        REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(
            ...
            kefir_codegen_target_ir_code_block_control_tail(state->code, current_block_state->block_ref), &operation,
            metadata, &instr_ref));
        REQUIRE_OK(store_virtual_reigster_output(
            state, current_block_state, vreg1_idx, KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT,
            (struct kefir_codegen_target_ir_value_ref) {.instr_ref = instr_ref,
                                                        .aspect = KEFIR_CODEGEN_TARGET_IR_VALUE_DIRECT_OUTPUT(0)}));
    }
    return KEFIR_OK;
}

static kefir_result_t touch_vreg_impl(struct constructor_state *state, struct code_block_state *current_block_state,
                                      kefir_asmcmp_virtual_register_index_t vreg1_idx,
                                      const struct kefir_codegen_target_ir_instruction_metadata *metadata) {
...
static kefir_result_t produce_vreg_impl(struct constructor_state *state, struct code_block_state *current_block_state,
                                        kefir_asmcmp_virtual_register_index_t vreg1_idx,
                                        const struct kefir_codegen_target_ir_instruction_metadata *metadata) {
    ...
    if (asmcmp_vreg->type == KEFIR_ASMCMP_VIRTUAL_REGISTER_PAIR) {
        REQUIRE_OK(
            produce_vreg_impl(state, current_block_state, asmcmp_vreg->parameters.pair.virtual_registers[0], metadata));
        REQUIRE_OK(
            produce_vreg_impl(state, current_block_state, asmcmp_vreg->parameters.pair.virtual_registers[1], metadata));
    } else {
        struct kefir_codegen_target_ir_operation operation = {.opcode = state->code->klass->placeholder_opcode};
        struct kefir_codegen_target_ir_value_ref value_ref;
        REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(
            state->mem, state->code, current_block_state->block_ref,
            kefir_codegen_target_ir_code_block_control_tail(state->code, current_block_state->block_ref), &operation,
            metadata, &value_ref.instr_ref));
        value_ref.aspect = KEFIR_CODEGEN_TARGET_IR_VALUE_DIRECT_OUTPUT(0);
        REQUIRE_OK(store_virtual_reigster_output(state, current_block_state, vreg1_idx,
                                                 KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT, value_ref));
    }
    return KEFIR_OK;
}
    ...
    for (kefir_asmcmp_instruction_index_t instr_idx = kefir_asmcmp_context_instr_head(state->asmcmp_ctx);
        ...
        if (res != KEFIR_NOT_FOUND) {
            ...
            if (current_block_state != NULL) {
                REQUIRE_OK(
                    terminate_current_block(state, current_block_state, (struct code_block_state *) node->value));
            }
            current_block_state = (struct code_block_state *) node->value;
        }

        REQUIRE(current_block_state != NULL,
                KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Expected non-NULL target IR code block state"));
        struct kefir_asmcmp_instruction *asmcmp_instr;
        REQUIRE_OK(kefir_asmcmp_context_instr_at(state->asmcmp_ctx, instr_idx, &asmcmp_instr));
        ...
        res = kefir_asmcmp_debug_info_source_map_at(&state->asmcmp_ctx->debug_info.source_map, instr_idx,
                                                    &source_location);
        if (res != KEFIR_NOT_FOUND) {
            REQUIRE_OK(res);
        ...
        if (classification.special == KEFIR_CODEGEN_TARGET_IR_ASMCMP_INSTRUCTION_SKIP) {
            ...
            REQUIRE_OK(link_vregs_impl(state, current_block_state, asmcmp_instr->args[0].vreg.index,
                                       asmcmp_instr->args[1].vreg.index, &metadata));
            continue;
        } else if (classification.special == KEFIR_CODEGEN_TARGET_IR_ASMCMP_INSTRUCTION_ATTRIBUTE) {
            REQUIRE_OK(
                kefir_hashset_add(state->mem, &state->auxiliarry_set, (kefir_hashset_key_t) classification.attribute));
            continue;
        ...
        if (classification.opcode == state->code->klass->inline_asm_opcode) {
            REQUIRE(asmcmp_instr->args[0].type == KEFIR_ASMCMP_VALUE_TYPE_INLINE_ASSEMBLY_INDEX,
                    KEFIR_SET_ERROR(KEFIR_INVALID_STATE,
                                    "Expected virtual assembly instruction with inline assembly parameter"));
            ...
            if (is_jump) {
                kefir_result_t res = kefir_hashtable_at(
                    &state->block_head_instr,
                    (kefir_hashtable_key_t) kefir_asmcmp_context_instr_next(state->asmcmp_ctx, instr_idx), &value);
                if (res != KEFIR_NOT_FOUND) {
                    struct kefir_hashtree_node *node;
                    REQUIRE_OK(kefir_hashtree_at(&state->blocks, (kefir_hashtree_key_t) value, &node));
            ...
            REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(
                state->mem, state->code, current_block_state->block_ref,
                kefir_codegen_target_ir_code_block_control_tail(state->code, current_block_state->block_ref),
                &(struct kefir_codegen_target_ir_operation) {.opcode = classification.opcode,
            ...
            for (res = kefir_asmcmp_inline_assembly_fragment_iter(state->asmcmp_ctx,
                                                                  asmcmp_instr->args[0].inline_asm_idx, &iter);
                ...
                switch (iter.fragment->type) {
                    case KEFIR_ASMCMP_INLINE_ASSEMBLY_FRAGMENT_TEXT:
                        REQUIRE_OK(kefir_codegen_target_ir_code_inline_assembly_text_fragment(
                            state->mem, state->code, inline_asm_ref, iter.fragment->text));
                        break;

                    case KEFIR_ASMCMP_INLINE_ASSEMBLY_FRAGMENT_VALUE: {
                        struct kefir_codegen_target_ir_operand operand;
                        REQUIRE_OK(init_operand(
                            state, current_block_state, &operand, &iter.fragment->value,
                            &(struct kefir_codegen_target_ir_asmcmp_operand_classification) {
                                .class = KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ, .implicit = false, .index = 0},
                            NULL, NULL));
                        REQUIRE_OK(kefir_codegen_target_ir_code_inline_assembly_operand_fragment(
                            state->mem, state->code, inline_asm_ref, &operand));
                    } break;
                }
        ...
        struct kefir_codegen_target_ir_operation operation = {.opcode = classification.opcode};
        ...
        for (kefir_size_t i = 0; i < KEFIR_ASMCMP_INSTRUCTION_NUM_OF_OPERANDS; i++) {
            ...
            if (classification.operands[i].implicit) {
                ...
                REQUIRE_OK(match_implicit_parameter_to_vreg(state, current_block_state, instr_idx,
                                                            classification.operands[i].implicit_parameter.phreg,
                                                            &vreg_idx));
                ...
                if (vreg->type == KEFIR_ASMCMP_VIRTUAL_REGISTER_IMMEDIATE_INTEGER) {
                    ...
                    if (classification.operands[i].class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ ||
                        ...
                        REQUIRE_OK(resolve_input_virtual_register(state, current_block_state, vreg_idx,
            ...
            if (output_vreg != KEFIR_ASMCMP_INDEX_NONE ||
                (classification.operands[i].class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_WRITE ||
                 classification.operands[i].class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ_WRITE)) {
        ...
        REQUIRE_OK(state->code->klass->instruction_resources(operation.opcode, &produced_resources, &consumed_resources,
                                                             state->code->klass->payload));
        ...
        for (kefir_size_t i = 0; i < output_index; i++) {
            ...
            if (output_vreg != KEFIR_ASMCMP_INDEX_NONE) {
                REQUIRE_OK(store_virtual_reigster_output(
                    state, current_block_state, output_vreg, output_vreg_variants[i],
                    (struct kefir_codegen_target_ir_value_ref) {
                        .instr_ref = instr_ref, .aspect = KEFIR_CODEGEN_TARGET_IR_VALUE_DIRECT_OUTPUT(output_regs++)}));
            } else {
                REQUIRE_OK(kefir_codegen_target_ir_code_add_aspect(
                    state->mem, state->code,
                    (struct kefir_codegen_target_ir_value_ref) {
                        .instr_ref = instr_ref, .aspect = KEFIR_CODEGEN_TARGET_IR_VALUE_INDIRECT_OUTPUT(output_regs++)},
                    &(struct kefir_codegen_target_ir_value_type) {
        ...
        if (produced_resources) {
            for (kefir_size_t i = 0; i < sizeof(kefir_uint64_t) * CHAR_BIT; i++) {
                if ((produced_resources >> i) & 1) {
                    struct kefir_codegen_target_ir_value_ref value_ref = {
                        .instr_ref = instr_ref, .aspect = KEFIR_CODEGEN_TARGET_IR_VALUE_RESOURCE(i)};
                    REQUIRE_OK(kefir_codegen_target_ir_code_add_aspect(
                        state->mem, state->code, value_ref,
                        &(struct kefir_codegen_target_ir_value_type) {
                            .kind = KEFIR_CODEGEN_TARGET_IR_VALUE_TYPE_RESOURCE,
                    ...
                    kefir_hashtable_value_t *table_value;
                    res = kefir_hashtable_at_mut(&current_block_state->resource_refs, (kefir_hashtable_key_t) i,
                                                 &table_value);
                    if (res != KEFIR_NOT_FOUND) {
                        REQUIRE_OK(res);
                        *table_value = KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref);
                    } else {
                        REQUIRE_OK(kefir_hashtable_insert(
                            state->mem, &current_block_state->resource_refs, (kefir_hashtable_key_t) i,
                            (kefir_hashtable_value_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref)));
                    }
                    REQUIRE_OK(kefir_hashset_add(
                        state->mem, &state->block_resource_defs,
                        (kefir_hashtable_key_t) (((kefir_uint64_t) current_block_state->block_ref) << 32) | i));
                }
            }
        }
        ...
        for (res = kefir_hashset_iter(&state->auxiliarry_set, &iter, &entry); res == KEFIR_OK;
            ...
            REQUIRE_OK(
                kefir_codegen_target_ir_code_add_instruction_attribute(state->mem, state->code, instr_ref, attribute));
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
        }
...
static kefir_result_t insert_vreg_phis(struct constructor_state *state,
                                       kefir_asmcmp_virtual_register_index_t vreg_idx) {
    kefir_result_t res;
    ...
    for (res = kefir_hashtable_iter(&vreg_state->block_lifetimes, &iter, &key, NULL); res == KEFIR_OK;
         res = kefir_hashtable_next(&iter, &key, NULL)) {
        ASSIGN_DECL_CAST(kefir_codegen_target_ir_block_ref_t, block_ref, key);
        if (!kefir_codegen_target_ir_control_flow_is_reachable(&state->control_flow, block_ref)) {
            continue;
        }
        REQUIRE_OK(kefir_codegen_target_ir_control_flow_find_closest_common_dominator(
            &state->control_flow, vreg_state->use_dominator_block_ref, block_ref,
            &vreg_state->use_dominator_block_ref));
    ...
    for (struct kefir_list_entry *head = kefir_list_head(&state->queue); head != NULL;
        ...
        for (res = kefir_hashset_iter(&state->control_flow.blocks[block_ref].dominance_frontier, &iter, &entry);
            ...
            ASSIGN_DECL_CAST(struct code_block_state *, frontier_block_state, node->value);

            kefir_bool_t inside_of_use_region;
            REQUIRE_OK(kefir_codegen_target_ir_control_flow_is_dominator(
                &state->control_flow, frontier_block_ref, vreg_state->use_dominator_block_ref, &inside_of_use_region));
            ...
            if (!kefir_hashtable_has(&frontier_block_state->block_inputs, (kefir_hashtree_key_t) vreg_idx) &&
                ...
                REQUIRE_OK(kefir_hashtable_insert(state->mem, &frontier_block_state->block_inputs,
                                                  (kefir_hashtable_key_t) vreg_idx,
                                                  (kefir_hashtable_value_t) frontier_phi_instr_ref));
                REQUIRE_OK(store_virtual_reigster_output(
                    state, frontier_block_state, vreg_idx, KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT,
                    (struct kefir_codegen_target_ir_value_ref) {
                        .instr_ref = frontier_phi_instr_ref,
                        .aspect = KEFIR_CODEGEN_TARGET_IR_VALUE_DIRECT_OUTPUT(0)}));
                REQUIRE_OK(
                    kefir_hashset_add(state->mem, &state->inserted_phis, (kefir_hashset_key_t) frontier_phi_instr_ref));
            }
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
...
static kefir_result_t insert_resource_phis(struct constructor_state *state,
                                           kefir_codegen_target_ir_resource_id_t resource_id) {
    for (kefir_size_t i = 0; i < kefir_codegen_target_ir_code_block_count(state->code); i++) {
        ...
        if (!kefir_codegen_target_ir_control_flow_is_reachable(&state->control_flow, block_ref) ||
            !kefir_hashset_has(&state->block_resource_defs, (((kefir_uint64_t) block_ref) << 32) | resource_id)) {
            continue;
        }
        ...
        REQUIRE_OK(kefir_hashtree_at(&state->blocks, (kefir_hashtree_key_t) block_ref, &node));
        ASSIGN_DECL_CAST(struct code_block_state *, block_state, node->value);
        if (kefir_hashtable_has(&block_state->resource_refs, (kefir_hashtable_key_t) resource_id)) {
            REQUIRE_OK(kefir_list_insert_after(state->mem, &state->queue, kefir_list_tail(&state->queue),
                                               (void *) (kefir_uptr_t) block_ref));
        }
    }
    ...
    for (struct kefir_list_entry *head = kefir_list_head(&state->queue); head != NULL;
        ...
        if (!kefir_hashtable_has(&block_state->resource_refs, (kefir_hashtree_key_t) resource_id)) {
            continue;
        }
        ...
        for (res = kefir_hashset_iter(&state->control_flow.blocks[block_ref].dominance_frontier, &iter, &entry);
            ...
            if (!kefir_hashtable_has(&frontier_block_state->block_resource_inputs,
                                     (kefir_hashtree_key_t) resource_id) &&
                !kefir_hashtable_has(&frontier_block_state->resource_refs, (kefir_hashtree_key_t) resource_id)) {
                kefir_codegen_target_ir_instruction_ref_t frontier_phi_instr_ref;
                struct kefir_codegen_target_ir_operation operation = {.opcode = state->code->klass->phi_opcode};
                REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(state->mem, state->code, frontier_block_ref,
                                                                        KEFIR_ID_NONE, &operation, NULL,
                ...
                REQUIRE_OK(kefir_list_insert_after(state->mem, &state->queue, kefir_list_tail(&state->queue),
                                                   (void *) (kefir_uptr_t) frontier_block_ref));
                REQUIRE_OK(kefir_hashtable_insert(state->mem, &frontier_block_state->block_resource_inputs,
                                                  (kefir_hashtable_key_t) resource_id,
                                                  (kefir_hashtable_value_t) frontier_phi_instr_ref));
                struct kefir_codegen_target_ir_value_ref value_ref = {
                    .instr_ref = frontier_phi_instr_ref, .aspect = KEFIR_CODEGEN_TARGET_IR_VALUE_RESOURCE(resource_id)};
                REQUIRE_OK(kefir_codegen_target_ir_code_add_aspect(
                    state->mem, state->code, value_ref,
                    &(struct kefir_codegen_target_ir_value_type) {
                        .kind = KEFIR_CODEGEN_TARGET_IR_VALUE_TYPE_RESOURCE,
                ...
                kefir_hashtable_value_t *value;
                kefir_result_t res =
                    kefir_hashtable_at_mut(&block_state->resource_refs, (kefir_hashtable_key_t) resource_id, &value);
                if (res != KEFIR_NOT_FOUND) {
                    *value = KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref);
                } else {
                    REQUIRE_OK(kefir_hashtable_insert(state->mem, &block_state->resource_refs,
                                                      (kefir_hashtable_key_t) res,
...
static kefir_result_t insert_phis(struct constructor_state *state) {
    ...
    for (kefir_size_t i = 0; i < sizeof(kefir_uint64_t) * CHAR_BIT; i++) {
        REQUIRE_OK(kefir_list_clear(state->mem, &state->queue));
        REQUIRE_OK(kefir_hashset_clear(state->mem, &state->auxiliarry_set));
        REQUIRE_OK(insert_resource_phis(state, i));
    }
    return KEFIR_OK;
}
...
static kefir_result_t find_link_for(struct constructor_state *state, struct phi_link_frame *frame,
                                    kefir_codegen_target_ir_instruction_ref_t phi_instr_ref,
                                    kefir_asmcmp_virtual_register_index_t vreg_idx,
    ...
    if (kefir_hashset_has(&state->inserted_phis, (kefir_hashset_key_t) phi_instr_ref)) {
        ...
        struct kefir_codegen_target_ir_operation operation = {.opcode = state->code->klass->placeholder_opcode};
        REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(
            state->mem, state->code, predecessor_block_ref, KEFIR_ID_NONE, &operation, NULL, &placeholder_instr_ref));
        *value_ref = (kefir_codegen_target_ir_value_ref_t) {.instr_ref = placeholder_instr_ref,
                                                            .aspect = KEFIR_CODEGEN_TARGET_IR_VALUE_DIRECT_OUTPUT(0)};
        REQUIRE_OK(store_virtual_register_aspect(state, vreg_idx, KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT,
                                                 *value_ref, NULL));
    } else {
        return KEFIR_SET_ERROR(KEFIR_INVALID_STATE,
                               "Failed to resolve input virtual register in target IR construction");
...
static kefir_result_t find_resource_link_for(struct constructor_state *state, struct phi_link_frame *frame,
                                             kefir_codegen_target_ir_resource_id_t resource_id,
                                             kefir_codegen_target_ir_block_ref_t predecessor_block_ref,
                                             struct kefir_codegen_target_ir_value_ref *value_ref) {
    for (; frame != NULL; frame = frame->parent) {
        kefir_hashtable_value_t table_value;
        kefir_result_t res = kefir_hashtable_at(&frame->resources, (kefir_hashtable_key_t) resource_id, &table_value);
        if (res != KEFIR_NOT_FOUND) {
            REQUIRE_OK(res);
            *value_ref = KEFIR_CODEGEN_TARGET_IR_VALUE_REF_FROM(table_value);
            return KEFIR_OK;
    ...
    REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(state->mem, state->code, predecessor_block_ref,
                                                            KEFIR_ID_NONE, &operation, NULL, &placeholder_instr_ref));
    *value_ref = (kefir_codegen_target_ir_value_ref_t) {.instr_ref = placeholder_instr_ref,
                                                        .aspect = KEFIR_CODEGEN_TARGET_IR_VALUE_RESOURCE(resource_id)};
    REQUIRE_OK(kefir_codegen_target_ir_code_add_aspect(
        state->mem, state->code, *value_ref,
        &(struct kefir_codegen_target_ir_value_type) {
            .kind = KEFIR_CODEGEN_TARGET_IR_VALUE_TYPE_RESOURCE,
...
static kefir_result_t link_successor_phis(struct constructor_state *state, struct phi_link_frame *frame,
                                          kefir_codegen_target_ir_block_ref_t block_ref) {
    kefir_result_t res;
    struct kefir_hashset_iterator iter;
    ...
    for (res = kefir_hashset_iter(&state->control_flow.blocks[block_ref].successors, &iter, &key); res == KEFIR_OK;
        ...
        ASSIGN_DECL_CAST(struct code_block_state *, successor_block_state, node->value);
        ...
        for (res = kefir_hashtable_iter(&successor_block_state->block_inputs, &table_iter, &table_key, &table_value);
            ...
            ASSIGN_DECL_CAST(kefir_asmcmp_virtual_register_index_t, vreg_idx, table_key);
            ASSIGN_DECL_CAST(kefir_codegen_target_ir_instruction_ref_t, phi_instr_ref, table_value);

            struct kefir_codegen_target_ir_value_ref value_ref;
            REQUIRE_OK(find_link_for(state, frame, phi_instr_ref, vreg_idx, block_ref, &value_ref));

            REQUIRE_OK(
                kefir_codegen_target_ir_code_phi_attach(state->mem, state->code, phi_instr_ref, block_ref, value_ref));
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
        }

        for (res = kefir_hashtable_iter(&successor_block_state->block_resource_inputs, &table_iter, &table_key,
                                        &table_value);
             res == KEFIR_OK; res = kefir_hashtable_next(&table_iter, &table_key, &table_value)) {
            ASSIGN_DECL_CAST(kefir_codegen_target_ir_resource_id_t, resource_id, table_key);
            ASSIGN_DECL_CAST(kefir_codegen_target_ir_instruction_ref_t, phi_instr_ref, table_value);

            struct kefir_codegen_target_ir_value_ref value_ref;
            REQUIRE_OK(find_resource_link_for(state, frame, resource_id, block_ref, &value_ref));
...
static kefir_result_t link_phis_impl(struct constructor_state *state) {
    ...
    for (struct kefir_list_entry *iter = kefir_list_head(&state->queue); iter != NULL;
        ...
        if (!frame->unfolded) {
            ...
            for (res = kefir_hashtable_iter(&block_state->resource_refs, &table_iter, &table_key, &table_value);
                 res == KEFIR_OK; res = kefir_hashtable_next(&table_iter, &table_key, &table_value)) {
                REQUIRE_OK(kefir_hashtable_insert(state->mem, &frame->resources, table_key, table_value));
            }
            if (res != KEFIR_ITERATOR_END) {
                REQUIRE_OK(res);
            }
            ...
            for (res = kefir_codegen_target_ir_control_flow_dominator_tree_iter(&state->control_flow, &iter,
                                                                                frame->block_ref, &dominated_block_ref);
                 res == KEFIR_OK;
...
static kefir_result_t free_code_block_state(struct kefir_mem *mem, struct kefir_hashtree *tree,
                                            kefir_hashtree_key_t key, kefir_hashtree_value_t value, void *payload) {
    UNUSED(tree);
    UNUSED(key);
    UNUSED(payload);
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ASSIGN_DECL_CAST(struct code_block_state *, block_state, value);
    ...
    REQUIRE_OK(kefir_hashtable_free(mem, &block_state->block_resource_inputs));
    REQUIRE_OK(kefir_hashtable_free(mem, &block_state->virtual_register_refs));
    REQUIRE_OK(kefir_hashtable_free(mem, &block_state->resource_refs));
    KEFIR_FREE(mem, block_state);
    return KEFIR_OK;
}

kefir_result_t kefir_codegen_target_ir_code_construct(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code, const struct kefir_asmcmp_context *asmcmp_ctx,
    const struct kefir_codegen_target_ir_code_constructor_metadata *constructor_metadata,
    const struct kefir_codegen_target_ir_code_constructor_ops *ops) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ...
    REQUIRE(ops != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code constructor parameters"));

    struct constructor_state state = {.mem = mem,
                                      .code = code,
    ...
    REQUIRE_OK(kefir_hashset_init(&state.block_resource_defs, &kefir_hashtable_uint_ops));

    kefir_result_t res = KEFIR_OK;
    state.asmcmp_instrs = KEFIR_MALLOC(mem, sizeof(struct asmcmp_instr_state) * asmcmp_ctx->code_length);
    REQUIRE_CHAIN_SET(&res, state.asmcmp_instrs != NULL,
                      KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate asmcmp instruction state"));
    ...
    res = kefir_hashset_free(mem, &state.block_resource_defs);
    REQUIRE_ELSE(res == KEFIR_OK, {
        kefir_hashset_free(mem, &state.inserted_phis);
        kefir_list_free(mem, &state.queue);
        kefir_hashset_free(mem, &state.auxiliarry_set);
...
static kefir_result_t free_metadata_value_refs(struct kefir_mem *mem, struct kefir_hashtable *table,
    ...
    UNUSED(payload);
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ASSIGN_DECL_CAST(struct metadata_value_refs *, value_refs, value);
    REQUIRE(value_refs != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR constructor metadata value references"));
...
kefir_result_t kefir_codegen_target_ir_code_constructor_metadata_init(
    ...
    REQUIRE(metadata != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to target IR code constructor metadata"));
...
kefir_result_t kefir_codegen_target_ir_code_constructor_metadata_free(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code_constructor_metadata *metadata) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(metadata != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code constructor metadata"));
...
kefir_result_t kefir_codegen_target_ir_code_constructor_metadata_add_code_ref(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code_constructor_metadata *metadata,
    kefir_asmcmp_instruction_index_t instr_idx, kefir_codegen_target_ir_metadata_code_ref_t code_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(metadata != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code constructor metadata"));
    REQUIRE(instr_idx != KEFIR_ASMCMP_INDEX_NONE,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code constructor metadata"));

    kefir_result_t res = kefir_hashtable_insert(mem, &metadata->code_refs, (kefir_hashtable_key_t) instr_idx,
                                                (kefir_hashtable_value_t) code_ref);
...
kefir_result_t kefir_codegen_target_ir_code_constructor_metadata_add_value_ref(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code_constructor_metadata *metadata,
    kefir_asmcmp_virtual_register_index_t vreg_idx, kefir_codegen_target_ir_metadata_value_ref_t value_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(metadata != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code constructor metadata"));
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        REQUIRE(value_refs != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE,
                                "Failed to allocate target IR constructor metadata value references"));
        res = kefir_hashset_init(&value_refs->refs, &kefir_hashtable_uint_ops);
        REQUIRE_CHAIN(&res, kefir_hashtable_insert(mem, &metadata->value_refs, (kefir_hashtable_key_t) vreg_idx,
                                                   (kefir_hashtable_value_t) value_refs));
        REQUIRE_ELSE(res == KEFIR_OK, {
            KEFIR_FREE(mem, value_refs);
            return res;
        });
...
kefir_result_t kefir_codegen_target_ir_code_constructor_metadata_get_code_ref(
    const struct kefir_codegen_target_ir_code_constructor_metadata *metadata,
    kefir_asmcmp_instruction_index_t instr_idx, kefir_codegen_target_ir_metadata_code_ref_t *code_ref_ptr) {
    REQUIRE(metadata != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code constructor metadata"));
...
kefir_result_t kefir_codegen_target_ir_code_constructor_metadata_value_ref_iter(
    ...
    kefir_asmcmp_instruction_index_t vreg_idx, kefir_codegen_target_ir_metadata_value_ref_t *value_ref_ptr) {
    REQUIRE(metadata != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code constructor metadata"));
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
                            "Expected valid pointer to target IR code constructor metadata value ref iterator"));
    ...
    ASSIGN_DECL_CAST(const struct metadata_value_refs *, value_refs, table_value);
    kefir_hashset_key_t key;
...
kefir_result_t kefir_codegen_target_ir_code_constructor_metadata_value_ref_next(
    ...
    REQUIRE(iter != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
                                          "Expected valid target IR code constructor metadata value ref iterator"));
