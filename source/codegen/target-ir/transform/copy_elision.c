...
static kefir_result_t do_copy_elision(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    for (kefir_codegen_target_ir_instruction_ref_t instr_ref =
        ...
        if (assign_instr->operation.opcode != code->klass->assign_opcode ||
            assign_instr->operation.parameters[0].direct.value_ref.aspect !=
        ...
        REQUIRE_OK(kefir_codegen_target_ir_code_value_props(
            code,
            (kefir_codegen_target_ir_value_ref_t) {.instr_ref = assign_instr->instr_ref,
        ...
        REQUIRE_OK(kefir_codegen_target_ir_code_value_props(
            code, assign_instr->operation.parameters[0].direct.value_ref, &assigned_value_type));
        ...
        if (elide_copy) {
            REQUIRE_OK(kefir_codegen_target_ir_code_replace_instruction(
                mem, code, assign_instr->operation.parameters[0].direct.value_ref.instr_ref, assign_instr->instr_ref));
...
kefir_result_t kefir_codegen_target_ir_transform_copy_elision(struct kefir_mem *mem,
