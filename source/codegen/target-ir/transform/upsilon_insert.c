...
static kefir_result_t get_phi_output(const struct kefir_codegen_target_ir_code *code,
                                     kefir_codegen_target_ir_instruction_ref_t instr_ref,
                                     kefir_codegen_target_ir_value_ref_t *value_ref_ptr,
...
static kefir_result_t do_patch_value(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    if (copy_value_ref->instr_ref == KEFIR_ID_NONE) {
        ...
        REQUIRE(phi_value_type->constraint.type != KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT,
                KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Unexpectd constrained phi value reference"));
    ...
    REQUIRE_OK(kefir_codegen_target_ir_code_replace_value_in(
        mem, code, kefir_codegen_target_ir_code_block_control_tail(code, block_ref), *copy_value_ref, patch_value_ref));
    return KEFIR_OK;
}

static kefir_result_t patch_operand_value(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    if (operand->type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF &&
        KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&phi_value_ref) ==
            KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&operand->direct.value_ref)) {
        ...
        if (operand->indirect.type == KEFIR_CODEGEN_TARGET_IR_INDIRECT_VALUE_REF_BASIS &&
            KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&phi_value_ref) ==
                KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&operand->indirect.base.value_ref)) {
        ...
        if (operand->indirect.index_type == KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_VALUE_REF &&
            KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&phi_value_ref) ==
                KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&operand->indirect.index.value_ref)) {
...
static kefir_result_t patch_terminator_instruction(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                                   kefir_codegen_target_ir_value_ref_t phi_value_ref,
                                                   kefir_codegen_target_ir_value_ref_t copy_value_ref,
                                                   kefir_codegen_target_ir_block_ref_t block_ref) {
        ...
        for (res = kefir_codegen_target_ir_code_inline_assembly_fragment_iter(code, &iter, tail_instr_ref, &fragment);
            ...
            switch (fragment->type) {
                ...
                case KEFIR_CODEGEN_TARGET_IR_INLINE_ASSEMBLY_FRAGMENT_OPERAND: {
                    REQUIRE_OK(
                        patch_operand_value(mem, code, phi_value_ref, &copy_value_ref, block_ref, &fragment->operand));
                } break;
            }
        }
        if (res != KEFIR_ITERATOR_END) {
...
static kefir_result_t insert_upsilons_step(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                           struct kefir_hashtable *phis, struct kefir_list *queue,
                                           kefir_codegen_target_ir_block_ref_t block_ref) {
    kefir_result_t res;
    kefir_hashtable_key_t phis_key;
    kefir_hashtable_value_t phis_value;
    struct kefir_hashtable_iterator phis_iter;
    for (res = kefir_hashtable_iter(phis, &phis_iter, &phis_key, NULL); res == KEFIR_OK;
        ...
        for (res =
                 kefir_codegen_target_ir_code_use_iter(code, &use_iter, phi_value_ref.instr_ref, &use_instr_ref, NULL);
            ...
            struct kefir_codegen_target_ir_value_ref user_value_ref;
            for (res = kefir_codegen_target_ir_code_value_iter(code, &user_value_iter, use_instr_ref, &user_value_ref,
                                                               NULL);
                 res == KEFIR_OK && !used_elsewhere;
                 res = kefir_codegen_target_ir_code_value_next(&user_value_iter, &user_value_ref, NULL)) {
                kefir_hashtable_value_t table_value;
                res = kefir_hashtable_at(
                    phis, (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&user_value_ref),
                    &table_value);
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(queue); iter != NULL; kefir_list_next(&iter)) {
        ASSIGN_DECL_CAST(kefir_uint64_t, phis_key, (kefir_uptr_t) iter->value);
    ...
    for (res = kefir_hashtable_iter(phis, &phis2_iter, &phis2_key, &phis2_value); res == KEFIR_OK;
...
static kefir_result_t insert_upsilons_into(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
...
static kefir_result_t insert_upsilons(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                      struct kefir_hashtable *phis,
    ...
    for (kefir_size_t i = 0; i < kefir_codegen_target_ir_code_block_count(code); i++) {
        ...
        if (!kefir_codegen_target_ir_control_flow_is_reachable(control_flow, block_ref) ||
            ...
            kefir_hashset_size(&control_flow->blocks[block_ref].successors) !=
                1 /* Assuming critical edges are split */) {
            continue;
        }
        ...
        ASSIGN_DECL_CAST(kefir_codegen_target_ir_block_ref_t, successor_block_ref, succ_key);
        ...
        for (res = kefir_codegen_target_ir_code_phi_node_iter(code, &phi_iter, successor_block_ref, &phi_ref);
            ...
            REQUIRE_OK(
                kefir_codegen_target_ir_code_phi_link_for(code, value_ref.instr_ref, block_ref, &link_value_ref));
            ...
            REQUIRE_OK(kefir_hashtable_insert(mem, phis,
                                              (kefir_hashset_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref),
                                              KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&link_value_ref)));
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
        }
...
kefir_result_t kefir_codegen_target_ir_transform_insert_upsilons(struct kefir_mem *mem,
...
kefir_result_t kefir_codegen_target_ir_transform_remove_upsilons(struct kefir_mem *mem,
                                                                 struct kefir_codegen_target_ir_code *code) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
