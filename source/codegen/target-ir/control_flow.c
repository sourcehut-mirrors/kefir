...
kefir_result_t kefir_codegen_target_ir_control_flow_init(struct kefir_codegen_target_ir_control_flow *control_flow,
                                                         const struct kefir_codegen_target_ir_code *code) {
    REQUIRE(control_flow != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to target IR control flow"));
...
static kefir_result_t store_terminator_target(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_hashset_add(mem, &control_flow->blocks[source_block_ref].successors,
                                 (kefir_hashset_key_t) target_block_ref));
    REQUIRE_OK(kefir_hashset_add(mem, &control_flow->blocks[target_block_ref].predecessors,
                                 (kefir_hashset_key_t) source_block_ref));
    return KEFIR_OK;
}

static kefir_result_t scan_operand(struct kefir_mem *mem, struct kefir_codegen_target_ir_control_flow *control_flow,
                                   const struct kefir_codegen_target_ir_operand *operand,
                                   kefir_codegen_target_ir_block_ref_t gate_block_ref,
                                   const struct kefir_codegen_target_ir_block_terminator_props *terminator_props) {
    switch (operand->type) {
        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_BLOCK_REF:
            if (terminator_props->inline_assembly && terminator_props->undefined_target) {
                ...
                REQUIRE_OK(kefir_hashtreeset_add(mem, &control_flow->indirect_jump_targets,
                                                 (kefir_hashtreeset_entry_t) operand->block_ref));
            }
            break;
        ...
        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INDIRECT:
            switch (operand->indirect.type) {
                case KEFIR_CODEGEN_TARGET_IR_INDIRECT_BLOCK_REF_BASIS:
                    if (terminator_props->inline_assembly && terminator_props->undefined_target) {
                        REQUIRE_OK(store_terminator_target(mem, control_flow, gate_block_ref,
                                                           operand->indirect.base.block_ref));
                    } else {
                        REQUIRE_OK(kefir_hashtreeset_add(mem, &control_flow->indirect_jump_targets,
                                                         (kefir_hashtreeset_entry_t) operand->indirect.base.block_ref));
                    }
                    break;
        ...
        case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_RIP_INDIRECT_BLOCK_REF:
            if (terminator_props->inline_assembly && terminator_props->undefined_target) {
                REQUIRE_OK(
                    store_terminator_target(mem, control_flow, gate_block_ref, operand->rip_indirection.block_ref));
            } else {
                REQUIRE_OK(kefir_hashtreeset_add(mem, &control_flow->indirect_jump_targets,
                                                 (kefir_hashtreeset_entry_t) operand->rip_indirection.block_ref));
            }
            break;
...
static kefir_result_t find_dominance_frontier(struct kefir_mem *mem,
                                              struct kefir_codegen_target_ir_control_flow *control_flow) {
...
static kefir_result_t build_dominance_tree(struct kefir_mem *mem,
                                           struct kefir_codegen_target_ir_control_flow *control_flow) {
    for (kefir_size_t i = 0; i < control_flow->blocks_length; i++) {
        kefir_codegen_target_ir_block_ref_t block_ref =
            kefir_codegen_target_ir_code_block_by_index(control_flow->code, i);
        kefir_codegen_target_ir_block_ref_t immediate_dominator_ref =
            control_flow->blocks[block_ref].immediate_dominator;
...
static kefir_result_t build_postdominance_tree(struct kefir_mem *mem,
                                               struct kefir_codegen_target_ir_control_flow *control_flow) {
    for (kefir_size_t i = 0; i < control_flow->blocks_length; i++) {
        kefir_codegen_target_ir_block_ref_t block_ref =
            kefir_codegen_target_ir_code_block_by_index(control_flow->code, i);
        kefir_codegen_target_ir_block_ref_t immediate_postdominator_ref =
            control_flow->blocks[block_ref].immediate_postdominator;

        struct dominator_tree_node *node = NULL;
        kefir_hashtable_value_t table_value;
        kefir_result_t res = kefir_hashtable_at(&control_flow->postdominator_tree,
                                                (kefir_hashtable_key_t) immediate_postdominator_ref, &table_value);
        if (res == KEFIR_NOT_FOUND) {
            node = KEFIR_MALLOC(mem, sizeof(struct dominator_tree_node));
            REQUIRE(node != NULL, KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate dominator tree node"));
            res = kefir_hashset_init(&node->children, &kefir_hashtable_uint_ops);
            REQUIRE_CHAIN(&res, kefir_hashtable_insert(mem, &control_flow->postdominator_tree,
                                                       (kefir_hashtable_key_t) immediate_postdominator_ref,
                                                       (kefir_hashtable_value_t) node));
            REQUIRE_ELSE(res == KEFIR_OK, {
                KEFIR_FREE(mem, node);
                return res;
...
static kefir_result_t walk_dominance_tree_impl(struct kefir_mem *mem,
                                               struct kefir_codegen_target_ir_control_flow *control_flow,
                                               struct kefir_list *queue) {
    ...
    if (control_flow->code->entry_block != KEFIR_ID_NONE) {
        REQUIRE_OK(kefir_list_insert_after(mem, queue, kefir_list_tail(queue),
                                           (void *) (kefir_uptr_t) control_flow->code->entry_block));
    }
    ...
    for (struct kefir_list_entry *iter = kefir_list_head(queue); iter != NULL; iter = kefir_list_head(queue)) {
        ...
        if (entry & (1ull << 63)) {
            ...
            for (res = kefir_codegen_target_ir_control_flow_dominator_tree_iter(control_flow, &dom_iter, block_ref,
                                                                                &dominated_block_ref);
                 res == KEFIR_OK;
...
static kefir_result_t walk_dominance_tree(struct kefir_mem *mem,
                                          struct kefir_codegen_target_ir_control_flow *control_flow) {
    struct kefir_list queue;
    REQUIRE_OK(kefir_list_init(&queue));
...
static kefir_result_t walk_postdominance_tree_impl(struct kefir_mem *mem,
                                                   struct kefir_codegen_target_ir_control_flow *control_flow,
                                                   struct kefir_list *queue) {
    kefir_size_t linear_index = 0;
    REQUIRE_OK(kefir_list_insert_after(
        mem, queue, kefir_list_tail(queue),
        (void *) (kefir_uptr_t) (kefir_codegen_target_ir_code_block_count(control_flow->code) + 1)));
    ...
    for (struct kefir_list_entry *iter = kefir_list_head(queue); iter != NULL; iter = kefir_list_head(queue)) {
        ...
        if (entry & (1ull << 63)) {
            ...
            for (res = kefir_codegen_target_ir_control_flow_postdominator_tree_iter(control_flow, &dom_iter, block_ref,
                                                                                    &postdominated_block_ref);
                 res == KEFIR_OK; res = kefir_codegen_target_ir_control_flow_postdominator_tree_next(
                                      &dom_iter, &postdominated_block_ref)) {
...
static kefir_result_t walk_postdominance_tree(struct kefir_mem *mem,
                                              struct kefir_codegen_target_ir_control_flow *control_flow) {
    struct kefir_list queue;
    REQUIRE_OK(kefir_list_init(&queue));
...
kefir_result_t kefir_codegen_target_ir_control_flow_build(struct kefir_mem *mem,
                                                          struct kefir_codegen_target_ir_control_flow *control_flow) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ...
    REQUIRE(control_flow->blocks == NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Target IR control flow has already been built"));

    control_flow->blocks_length = control_flow->code->blocks_length;
    control_flow->blocks =
        KEFIR_MALLOC(mem, sizeof(struct kefir_codegen_target_ir_block_control_flow) * control_flow->blocks_length);
    REQUIRE(control_flow->blocks != NULL,
            KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR control flow blocks"));
    for (kefir_size_t i = 0; i < control_flow->blocks_length; i++) {
    ...
    for (kefir_size_t i = 0; i < control_flow->blocks_length; i++) {
        ...
        const struct kefir_codegen_target_ir_block *block =
            kefir_codegen_target_ir_code_block_at(control_flow->code, block_ref);
        ...
        if (!kefir_hashtreeset_empty(&block->public_labels)) {
            REQUIRE_OK(kefir_hashtreeset_add(mem, &control_flow->indirect_jump_targets,
                                             (kefir_hashtreeset_entry_t) block_ref));
        }
        ...
        REQUIRE_OK(
            kefir_codegen_target_ir_code_instruction(control_flow->code, current_block_tail_ref, &current_block_tail));

        struct kefir_codegen_target_ir_block_terminator_props terminator_props;
        REQUIRE_OK(control_flow->code->klass->is_block_terminator(
            control_flow->code, current_block_tail, &terminator_props, control_flow->code->klass->payload));

        if (!terminator_props.block_terminator || terminator_props.function_terminator) {
            continue;
        }

        REQUIRE(!terminator_props.fallthrough,
                KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Target IR code contains blocks with fallthrough terminators"));
        if (terminator_props.undefined_target) {
            REQUIRE_OK(kefir_hashtreeset_add(mem, &control_flow->indirect_jump_sources,
                                             (kefir_hashtreeset_entry_t) block_ref));
        }
        ...
        for (kefir_codegen_target_ir_instruction_ref_t instr_ref =
            ...
            REQUIRE_OK(control_flow->code->klass->is_block_terminator(
                control_flow->code, instr, &instr_terminator_props, control_flow->code->klass->payload));
            if (instr->operation.opcode == control_flow->code->klass->phi_opcode) {
                ...
                if (instr->operation.inline_asm_node.target_block_ref != KEFIR_ID_NONE) {
                    ...
                    kefir_result_t res;
                    for (res = kefir_codegen_target_ir_code_inline_assembly_fragment_iter(control_flow->code, &iter,
                                                                                          instr_ref, &fragment);
                         res == KEFIR_OK;
    ...
    kefir_result_t res;
    for (res = kefir_hashtreeset_iter(&control_flow->indirect_jump_sources, &source_iter); res == KEFIR_OK;
         res = kefir_hashtreeset_next(&source_iter)) {
        for (res = kefir_hashtreeset_iter(&control_flow->indirect_jump_targets, &target_iter); res == KEFIR_OK;
             res = kefir_hashtreeset_next(&target_iter)) {
            REQUIRE(control_flow->code->indirect_jump_gate_block != KEFIR_ID_NONE,
                    KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Expected target IR indirect jump gate to exist"));
            REQUIRE_OK(store_terminator_target(mem, control_flow, source_iter.entry,
                                               control_flow->code->indirect_jump_gate_block));
            REQUIRE_OK(store_terminator_target(mem, control_flow, control_flow->code->indirect_jump_gate_block,
                                               target_iter.entry));
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
...
kefir_result_t kefir_codegen_target_ir_control_flow_free(struct kefir_mem *mem,
                                                         struct kefir_codegen_target_ir_control_flow *control_flow) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(control_flow != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR control flow"));
...
kefir_result_t kefir_codegen_target_ir_control_flow_reset(struct kefir_mem *mem,
                                                          struct kefir_codegen_target_ir_control_flow *control_flow) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(control_flow != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR control flow"));
...
kefir_bool_t kefir_codegen_target_ir_control_flow_is_reachable(
    const struct kefir_codegen_target_ir_control_flow *control_flow, kefir_codegen_target_ir_block_ref_t block_ref) {
    REQUIRE(control_flow != NULL, false);
    REQUIRE(block_ref != KEFIR_ID_NONE && block_ref < kefir_codegen_target_ir_code_block_count(control_flow->code),
            false);
...
kefir_bool_t kefir_codegen_target_ir_control_flow_is_critical_edge(
    const struct kefir_codegen_target_ir_control_flow *control_flow,
    kefir_codegen_target_ir_block_ref_t source_block_ref, kefir_codegen_target_ir_block_ref_t target_block_ref) {
    REQUIRE(control_flow != NULL, false);
    REQUIRE(source_block_ref != KEFIR_ID_NONE &&
                source_block_ref < kefir_codegen_target_ir_code_block_count(control_flow->code),
            false);
    REQUIRE(target_block_ref != KEFIR_ID_NONE &&
                target_block_ref < kefir_codegen_target_ir_code_block_count(control_flow->code),
            false);

    return kefir_hashset_has(&control_flow->blocks[source_block_ref].successors,
                             (kefir_hashset_key_t) target_block_ref) &&
...
kefir_result_t kefir_codegen_target_ir_control_flow_is_dominator(
    const struct kefir_codegen_target_ir_control_flow *structure, kefir_codegen_target_ir_block_ref_t dominated_block,
    ...
    if (dominated_block == dominator_block) {
        *result_ptr = true;
               ...
               structure->blocks[dominated_block].immediate_dominator != KEFIR_ID_NONE) {
...
kefir_result_t kefir_codegen_target_ir_control_flow_is_postdominator(
    ...
    if (dominated_block == dominator_block) {
        *result_ptr = true;
               ...
               structure->blocks[dominated_block].immediate_postdominator != KEFIR_ID_NONE) {
        ...
        *result_ptr = linear_index != (kefir_size_t) -1ll && postdominated_block_max_linear != (kefir_size_t) -1ll &&
                      linear_index < postdominated_linear && postdominated_linear < postdominated_block_max_linear;
    } else {
        *result_ptr = false;
    }
    return KEFIR_OK;
...
kefir_result_t kefir_codegen_target_ir_control_flow_find_closest_common_dominator(
    struct kefir_codegen_target_ir_control_flow *control_flow, kefir_codegen_target_ir_block_ref_t block_ref,
    ...
    do {
        ...
        REQUIRE_OK(kefir_codegen_target_ir_control_flow_is_dominator(control_flow, block_ref, dominator_block_ref,
                                                                     &is_dominator));
        if (is_dominator) {
...
kefir_result_t kefir_codegen_target_ir_control_flow_dominator_tree_iter(
    ...
    kefir_codegen_target_ir_block_ref_t block_ref, kefir_codegen_target_ir_block_ref_t *dominated_block_ref_ptr) {
    REQUIRE(control_flow != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR control flow"));
    REQUIRE(iter != NULL,
...
kefir_result_t kefir_codegen_target_ir_control_flow_postdominator_tree_iter(
    ...
    kefir_hashtable_value_t table_value;
    kefir_result_t res =
        kefir_hashtable_at(&control_flow->postdominator_tree, (kefir_hashtable_key_t) block_ref, &table_value);
    if (res == KEFIR_NOT_FOUND) {
