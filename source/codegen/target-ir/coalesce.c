...
kefir_result_t kefir_codegen_target_ir_coalesce_init(struct kefir_codegen_target_ir_coalesce *coalesce) {
    REQUIRE(coalesce != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to target IR coalescing"));

    REQUIRE_OK(kefir_graph_init(&coalesce->coalesce_graph));
    return KEFIR_OK;
}

kefir_result_t kefir_codegen_target_ir_coalesce_free(struct kefir_mem *mem,
...
kefir_result_t kefir_codegen_target_ir_coalesce_reset(struct kefir_mem *mem,
                                                      struct kefir_codegen_target_ir_coalesce *coalesce) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(coalesce != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR coalescing"));
...
static kefir_result_t record_coalesce(struct kefir_mem *mem,
    ...
    REQUIRE(
        KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref) != KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&other_value_ref),
        KEFIR_OK);
    ...
    if (!interferes) {
        REQUIRE_OK(
            kefir_graph_add_edge(mem, &coalesce->coalesce_graph,
...
static kefir_result_t coalesce_callback(struct kefir_mem *mem, kefir_codegen_target_ir_value_ref_t value_ref,
                                        kefir_codegen_target_ir_value_ref_t other_value_ref, void *payload) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ASSIGN_DECL_CAST(struct try_coalesce_payload *, coalesce_payload, payload);
    REQUIRE(coalesce_payload != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR coalesce payload"));

    REQUIRE_OK(
        record_coalesce(mem, coalesce_payload->interference, coalesce_payload->coalesce, value_ref, other_value_ref));
    return KEFIR_OK;
}
    ...
    for (struct kefir_list_entry *head = kefir_list_head(queue); head != NULL; head = kefir_list_head(queue)) {
        ...
        for (kefir_codegen_target_ir_instruction_ref_t instr_ref =
            ...
            if (instr->operation.opcode == control_flow->code->klass->phi_opcode) {
                kefir_codegen_target_ir_value_ref_t value_ref;
                kefir_result_t res =
                    kefir_codegen_target_ir_code_instruction_output(control_flow->code, instr_ref, 0, &value_ref, NULL);
                ...
                for (res = kefir_codegen_target_ir_code_phi_link_iter(control_flow->code, &iter, instr_ref,
                ...
                REQUIRE_OK(record_coalesce(mem, interference, coalesce, instr->operation.parameters[0].direct.value_ref,
                                           value_ref));
            } else if (instr->operation.opcode == control_flow->code->klass->upsilon_opcode &&
                    instr->operation.parameters_length > 1 &&
                       instr->operation.parameters[1].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF) {
                REQUIRE_OK(record_coalesce(mem, interference, coalesce, instr->operation.parameters[0].upsilon_ref,
                                           instr->operation.parameters[1].direct.value_ref));
            } else {
                struct try_coalesce_payload payload = {.coalesce = coalesce, .interference = interference};
                REQUIRE_OK(klass->extract_coalescing(mem, control_flow->code, instr,
                                                     &(struct kefir_codegen_target_ir_coalesce_callback) {
                                                         .coalesce = coalesce_callback, .payload = &payload},
                                                     klass->payload));
            }
        }
        ...
        for (res = kefir_codegen_target_ir_control_flow_dominator_tree_iter(control_flow, &iter, block_ref,
                                                                            &dominated_block_ref);
             res == KEFIR_OK;
...
kefir_result_t kefir_codegen_target_ir_coalesce_build(struct kefir_mem *mem,
                                                      struct kefir_codegen_target_ir_coalesce *coalesce,
                                                      const struct kefir_codegen_target_ir_control_flow *control_flow,
                                                      const struct kefir_codegen_target_ir_interference *interference,
                                                      const struct kefir_codegen_target_ir_coalesce_class *klass) {
...
kefir_result_t kefir_codegen_target_ir_coalesce_iter(const struct kefir_codegen_target_ir_coalesce *coalesce,
    ...
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to target IR coalesce iterator"));

    kefir_graph_vertex_id_t vertex;
    kefir_result_t res =
        kefir_graph_edge_iter(&coalesce->coalesce_graph, &iter->iter,
                              (kefir_graph_vertex_id_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref), &vertex);
    if (res == KEFIR_ITERATOR_END) {
