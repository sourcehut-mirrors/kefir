...
kefir_result_t kefir_codegen_target_ir_regalloc_init(struct kefir_codegen_target_ir_regalloc *regalloc,
    ...
    REQUIRE(regalloc != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to target IR register allocator"));
    REQUIRE(klass != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR register allocator class"));
...
kefir_result_t kefir_codegen_target_ir_regalloc_free(struct kefir_mem *mem,
                                                     struct kefir_codegen_target_ir_regalloc *regalloc) {
...
static kefir_result_t build_conflicts(struct kefir_mem *mem, struct regalloc_state *state,
                                      kefir_codegen_target_ir_value_ref_t value_ref, kefir_bool_t *has_conflicts) {
    kefir_result_t res;
    ...
    for (res = kefir_codegen_target_ir_interference_iter(state->interference, &iter, value_ref, &conflict_value_ref);
         res == KEFIR_OK; res = kefir_codegen_target_ir_interference_next(&iter, &conflict_value_ref)) {
        ...
        res = kefir_codegen_target_ir_code_value_props(state->control_flow->code, conflict_value_ref,
                                                       &conflict_value_type);
        ...
        if (conflict_value_type->constraint.type == KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT) {
            ...
            REQUIRE_OK(state->regalloc->klass->register_allocation(conflict_value_type->constraint.physical_register,
                                                                   &allocation, state->regalloc->klass->payload));
            REQUIRE_OK(state->regalloc_state.add_conflict(mem, allocation, state->regalloc_state.payload));
            ASSIGN_PTR(has_conflicts, true);
            continue;
        }

        kefir_hashtable_value_t table_value;
        res = kefir_hashtable_at(&state->regalloc->allocation,
                                 (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&conflict_value_ref),
                                 &table_value);
        if (res == KEFIR_NOT_FOUND) {
            continue;
        }
        REQUIRE_OK(res);
        REQUIRE_OK(state->regalloc_state.add_conflict(mem, (kefir_codegen_target_ir_regalloc_allocation_t) table_value,
                                                      state->regalloc_state.payload));
        ASSIGN_PTR(has_conflicts, true);
    }
...
static kefir_result_t build_constraints_and_hints(struct kefir_mem *mem, struct regalloc_state *state,
                                                  kefir_codegen_target_ir_value_ref_t value_ref,
                                                  kefir_bool_t try_constrain) {
    REQUIRE_OK(state->regalloc_state.reset(mem, state->regalloc_state.payload));

    REQUIRE_OK(build_conflicts(mem, state, value_ref, NULL));
    ...
    if (state->coalesce != NULL) {
        ...
        kefir_bool_t has_coalesced_conflicts = false;
        for (res = kefir_codegen_target_ir_coalesce_iter(state->coalesce, &iter, value_ref, &coalesce_value_ref);
             res == KEFIR_OK; res = kefir_codegen_target_ir_coalesce_next(&iter, &coalesce_value_ref)) {
            const struct kefir_codegen_target_ir_value_type *coalesce_value_type = NULL;
            res = kefir_codegen_target_ir_code_value_props(state->control_flow->code, coalesce_value_ref,
                                                           &coalesce_value_type);
            if (res == KEFIR_NOT_FOUND) {
                continue;
            }
        ...
        for (res = kefir_codegen_target_ir_coalesce_iter(state->coalesce, &iter, value_ref, &coalesce_value_ref);
            ...
            if (try_constrain && !has_coalesced_conflicts) {
                REQUIRE_OK(build_conflicts(mem, state, coalesce_value_ref, &has_coalesced_conflicts));
            }

            if (coalesce_value_type->constraint.type == KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT) {
                kefir_codegen_target_ir_regalloc_allocation_t allocation;
                REQUIRE_OK(state->regalloc->klass->register_allocation(
                    coalesce_value_type->constraint.physical_register, &allocation, state->regalloc->klass->payload));
                REQUIRE_OK(
                    state->regalloc_state.add_allocation_hint(mem, allocation, 32, state->regalloc_state.payload));
                continue;
            }

            kefir_hashtable_value_t table_value;
            res = kefir_hashtable_at(
                &state->regalloc->allocation,
                (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&coalesce_value_ref), &table_value);
            if (res == KEFIR_NOT_FOUND) {
                continue;
            }
            REQUIRE_OK(res);
            ASSIGN_DECL_CAST(kefir_codegen_target_ir_regalloc_allocation_t, hint, table_value);

            kefir_uint32_t priority = 0;
            res = kefir_hashtable_at(
                &state->coalescing_priorities,
                (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&coalesce_value_ref), &table_value);
            if (res != KEFIR_NOT_FOUND) {
                REQUIRE_OK(res);
                priority = table_value;
            }

            REQUIRE_OK(state->regalloc_state.add_allocation_hint(mem, hint, priority, state->regalloc_state.payload));
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
        }
...
static kefir_result_t get_value_hotness(struct regalloc_state *state, kefir_codegen_target_ir_value_ref_t value_ref,
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        REQUIRE_OK(state->regalloc->klass->is_rematerializable(state->control_flow->code, state->liveness, value_ref,
                                                               &is_rematerializable, state->regalloc->klass->payload));
...
static kefir_result_t try_evict_neighbor(struct kefir_mem *mem, struct regalloc_state *state,
                                         kefir_codegen_target_ir_value_ref_t value_ref) {
    struct kefir_codegen_target_ir_value_hotness_fragment hotness_fragment;
    ...
    if (KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&evict_value) != KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref)) {
        REQUIRE_OK(
            kefir_hashtable_delete(mem, &state->regalloc->allocation,
                                   (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&evict_value)));
        res = kefir_hashtable_delete(mem, &state->coalescing_priorities,
                                     (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&evict_value));
        if (res != KEFIR_NOT_FOUND) {
            REQUIRE_OK(res);
        }
...
static kefir_result_t schedule_coalescing(struct kefir_mem *mem, struct regalloc_state *state,
                                          kefir_codegen_target_ir_value_ref_t value_ref) {
    kefir_result_t res;
    if (state->coalesce != NULL) {
        struct kefir_codegen_target_ir_coalesce_iterator iter;
        ...
        for (res = kefir_codegen_target_ir_coalesce_iter(state->coalesce, &iter, value_ref, &coalesce_value_ref);
            ...
            if (res == KEFIR_NOT_FOUND) {
                ...
                REQUIRE_OK(kefir_list_insert_after(
                    mem, &state->value_queue, NULL,
                    (void *) (kefir_uptr_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&coalesce_value_ref)));
            } else {
                REQUIRE_OK(res);
            }
        }
...
static kefir_result_t do_regalloc(struct kefir_mem *mem, struct regalloc_state *state) {
    kefir_result_t res;
    for (struct kefir_list_entry *head = kefir_list_head(&state->value_queue); head != NULL;
        ...
        if (kefir_hashtable_has(&state->regalloc->allocation,
            ...
            REQUIRE_OK(schedule_coalescing(mem, state, value_ref));
            continue;
        }
        ...
        for (res = kefir_codegen_target_ir_code_use_iter(state->control_flow->code, &use_iter, value_ref.instr_ref,
                                                         &use_instr_ref, &use_value_ref);
        ...
        kefir_bool_t try_constrain = value_type->constraint.type != KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT;
        REQUIRE_OK(build_constraints_and_hints(mem, state, value_ref, try_constrain));

        kefir_codegen_target_ir_regalloc_allocation_t allocation;
        kefir_uint32_t coalescing_priority = 0;
        res = state->regalloc->klass->do_allocate(mem, value_type, state->stack_frame, state->regalloc_state.payload,
                                                  true, coalesce_spill, &allocation, &coalescing_priority,
                                                  state->regalloc->klass->payload);
        if (res != KEFIR_OUT_OF_SPACE) {
            REQUIRE_OK(res);
        } else {
            if (try_constrain) {
                REQUIRE_OK(build_constraints_and_hints(mem, state, value_ref, false));
                res = state->regalloc->klass->do_allocate(
                    mem, value_type, state->stack_frame, state->regalloc_state.payload, true, coalesce_spill,
                    &allocation, &coalescing_priority, state->regalloc->klass->payload);
            }
            if (res == KEFIR_OUT_OF_SPACE) {
                REQUIRE_OK(try_evict_neighbor(mem, state, value_ref));
                REQUIRE_OK(state->regalloc->klass->do_allocate(
                    mem, value_type, state->stack_frame, state->regalloc_state.payload, false, coalesce_spill,
                    &allocation, &coalescing_priority, state->regalloc->klass->payload));
            } else {
                REQUIRE_OK(res);
            }
        }
        REQUIRE_OK(kefir_hashtable_insert(mem, &state->regalloc->allocation,
                                          (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref),
                                          (kefir_hashtable_value_t) allocation));
        if (coalescing_priority > 1) {
            REQUIRE_OK(
                kefir_hashtable_insert(mem, &state->coalescing_priorities,
                                       (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref),
                                       (kefir_hashtable_value_t) (coalescing_priority - 1)));
        }
...
static kefir_result_t regalloc_run_impl(struct kefir_mem *mem, struct regalloc_state *state,
                                        kefir_codegen_target_ir_block_ref_t entry_block_ref,
                                        kefir_bool_t preallocated_only) {
    REQUIRE_OK(kefir_list_insert_after(mem, &state->block_queue, kefir_list_tail(&state->block_queue),
                                       (void *) (kefir_uptr_t) entry_block_ref));
    for (struct kefir_list_entry *head = kefir_list_head(&state->block_queue); head != NULL;
        ...
        for (res = kefir_codegen_target_ir_control_flow_dominator_tree_iter(state->control_flow, &iter, block_ref,
                                                                            &dominated_block_ref);
             res == KEFIR_OK;
        ...
        for (kefir_codegen_target_ir_instruction_ref_t instr_ref =
                 kefir_codegen_target_ir_code_block_control_head(state->control_flow->code, block_ref);
            ...
            kefir_result_t res;
            for (res = kefir_codegen_target_ir_code_value_iter(state->control_flow->code, &value_iter, instr_ref,
                                                               &value_ref, NULL);
                ...
                if (preallocated_only) {
                    const struct kefir_codegen_target_ir_value_type *value_type;
                    REQUIRE_OK(
                        kefir_codegen_target_ir_code_value_props(state->control_flow->code, value_ref, &value_type));
                    if (value_type->constraint.type != KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT) {
                        continue;
                    }
                }
                REQUIRE_OK(kefir_list_insert_after(
                    mem, &state->value_queue, kefir_list_tail(&state->value_queue),
                    (void *) (kefir_uptr_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref)));
            }
            if (res != KEFIR_ITERATOR_END) {
                REQUIRE_OK(res);
            }
...
kefir_size_t kefir_codegen_target_ir_regalloc_num_of_allocations(
...
kefir_result_t kefir_codegen_target_ir_regalloc_forget(struct kefir_mem *mem,
                                                       struct kefir_codegen_target_ir_regalloc *regalloc,
                                                       kefir_codegen_target_ir_value_ref_t value_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(regalloc != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR register allocator"));

    kefir_result_t res = kefir_hashtable_delete(
        mem, &regalloc->allocation, (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref));
    if (res == KEFIR_NOT_FOUND) {
        res = KEFIR_OK;
    }
    REQUIRE_OK(res);

    res = kefir_hashtable_delete(mem, &regalloc->allocation,
                                 (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref));
    if (res == KEFIR_NOT_FOUND) {
        res = KEFIR_OK;
    }
...
kefir_result_t kefir_codegen_target_ir_regalloc_reset(struct kefir_mem *mem,
                                                      struct kefir_codegen_target_ir_regalloc *regalloc) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(regalloc != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR register allocator"));
...
kefir_result_t kefir_codegen_target_ir_regalloc_run(struct kefir_mem *mem,
                                                    struct kefir_codegen_target_ir_regalloc *regalloc,
                                                    const struct kefir_codegen_target_ir_control_flow *control_flow,
                                                    const struct kefir_codegen_target_ir_liveness *liveness,
                                                    const struct kefir_codegen_target_ir_interference *interference,
    ...
    struct regalloc_state state = {.regalloc = regalloc,
    ...
    kefir_result_t res = kefir_codegen_target_ir_hotness_build(mem, &state.hotness, state.control_flow, state.liveness);
    REQUIRE_CHAIN(&res, regalloc_run_impl(mem, &state, control_flow->code->entry_block, true));
    REQUIRE_CHAIN(&res, regalloc_run_impl(mem, &state, control_flow->code->entry_block, false));
...
kefir_result_t kefir_codegen_target_ir_regalloc_get(const struct kefir_codegen_target_ir_regalloc *regalloc,
    ...
    kefir_hashtable_value_t table_value;
    kefir_result_t res =
        kefir_hashtable_at(&regalloc->allocation,
