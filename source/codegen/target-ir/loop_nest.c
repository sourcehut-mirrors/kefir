...
struct kefir_codegen_target_ir_loop *kefir_codegen_target_ir_loop_nest_top(
    const struct kefir_codegen_target_ir_loop_nest *nest) {
    REQUIRE(nest != NULL, NULL);
    return nest->nest.value;
}

struct kefir_codegen_target_ir_loop *kefir_codegen_target_ir_loop_parent(
    const struct kefir_codegen_target_ir_loop *loop) {
...
struct kefir_codegen_target_ir_loop *kefir_codegen_target_ir_loop_first_child(
    const struct kefir_codegen_target_ir_loop *loop) {
    REQUIRE(loop != NULL, NULL);
...
struct kefir_codegen_target_ir_loop *kefir_codegen_target_ir_loop_next_sibling(
    const struct kefir_codegen_target_ir_loop *loop) {
    REQUIRE(loop != NULL, NULL);
...
static kefir_result_t free_loop(struct kefir_mem *mem, struct kefir_hashtable *table, kefir_hashtable_key_t key,
    ...
    ASSIGN_DECL_CAST(struct kefir_codegen_target_ir_loop *, loop, value);
    ...
    memset(loop, 0, sizeof(struct kefir_codegen_target_ir_loop));
    KEFIR_FREE(mem, loop);
    return KEFIR_OK;
}
...
static kefir_result_t free_loop_nest(struct kefir_mem *mem, struct kefir_list *list, struct kefir_list_entry *entry,
    ...
    ASSIGN_DECL_CAST(struct kefir_codegen_target_ir_loop_nest *, nest, entry->value);
...
kefir_result_t kefir_codegen_target_ir_loop_collection_init(struct kefir_codegen_target_ir_loop_collection *loops) {
    REQUIRE(loops != NULL,
...
kefir_result_t kefir_codegen_target_ir_loop_collection_free(struct kefir_mem *mem,
...
static kefir_result_t build_loop_impl(struct kefir_mem *mem,
    ...
    for (struct kefir_list_entry *iter = kefir_list_head(traversal_queue); iter != NULL;
        ...
        ASSIGN_DECL_CAST(kefir_codegen_target_ir_block_ref_t, block_id, (kefir_uptr_t) iter->value);
        ...
        if (block_id != loop->header_ref) {
            ...
            for (res = kefir_hashset_iter(&control_flow->blocks[block_id].predecessors, &pred_iter, &entry);
                ...
                REQUIRE_OK(kefir_codegen_target_ir_control_flow_is_dominator(control_flow, entry, loop->header_ref,
                                                                             &is_dominator));
                if (is_dominator) {
    ...
    for (res = kefir_hashset_iter(&loop->blocks, &iter, &key); res == KEFIR_OK; res = kefir_hashset_next(&iter, &key)) {
        ASSIGN_DECL_CAST(kefir_codegen_target_ir_block_ref_t, block_id, key);
...
static kefir_result_t build_loop(struct kefir_mem *mem, struct kefir_codegen_target_ir_loop_collection *loops,
                                 const struct kefir_codegen_target_ir_control_flow *control_flow,
                                 kefir_codegen_target_ir_block_ref_t loop_entry_block_id,
                                 kefir_codegen_target_ir_block_ref_t loop_exit_block_id) {
    struct kefir_codegen_target_ir_loop *loop = NULL;

    kefir_hashtable_value_t table_value;
    kefir_result_t res = kefir_hashtable_at(&loops->loops, (kefir_hashtable_key_t) loop_entry_block_id, &table_value);
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
        loop = (struct kefir_codegen_target_ir_loop *) table_value;
    } else {
        loop = KEFIR_MALLOC(mem, sizeof(struct kefir_codegen_target_ir_loop));
        ...
        for (res = kefir_hashset_iter(&control_flow->blocks[loop_entry_block_id].predecessors, &iter, &entry);
            ...
            kefir_bool_t is_dominator;
            REQUIRE_OK(kefir_codegen_target_ir_control_flow_is_dominator(control_flow, loop_entry_block_id, entry,
                                                                         &is_dominator));
...
static kefir_bool_t loop_contained_within(const struct kefir_codegen_target_ir_loop *loop,
                                          const struct kefir_codegen_target_ir_loop *contained_loop) {
    return kefir_hashset_subset(&contained_loop->blocks, &loop->blocks);
}

static kefir_result_t insert_into_nest(struct kefir_mem *mem, struct kefir_codegen_target_ir_loop *loop,
    ...
    for (struct kefir_tree_node *child = kefir_tree_first_child(nest); child != NULL;
        ...
        if (loop_contained_within(loop, (const struct kefir_codegen_target_ir_loop *) child->value)) {
...
static kefir_result_t update_loop_nest(struct kefir_mem *mem, struct kefir_codegen_target_ir_loop_collection *loops,
                                       struct kefir_codegen_target_ir_loop *loop) {
    for (const struct kefir_list_entry *iter = kefir_list_head(&loops->nests); iter != NULL; kefir_list_next(&iter)) {
        ASSIGN_DECL_CAST(struct kefir_codegen_target_ir_loop_nest *, nest, iter->value);

        if (loop_contained_within(loop, (const struct kefir_codegen_target_ir_loop *) nest->nest.value)) {
            REQUIRE_OK(insert_into_nest(mem, loop, &nest->nest));
            return KEFIR_OK;
        } else if (loop_contained_within((const struct kefir_codegen_target_ir_loop *) nest->nest.value, loop)) {
            struct kefir_tree_node *nest_node;
            REQUIRE_OK(kefir_tree_insert_parent(mem, &nest->nest, (void *) loop, &nest_node));
            loop->nest_node = nest_node->parent;
            ((struct kefir_codegen_target_ir_loop *) nest_node->value)->nest_node = nest_node;
            return KEFIR_OK;
        }
    }

    struct kefir_codegen_target_ir_loop_nest *nest =
        KEFIR_MALLOC(mem, sizeof(struct kefir_codegen_target_ir_loop_nest));
...
static kefir_result_t finalize_loop(struct kefir_mem *mem, struct kefir_codegen_target_ir_loop_collection *loops,
                                    struct kefir_codegen_target_ir_loop *loop) {
    struct kefir_codegen_target_ir_loop *parent = kefir_codegen_target_ir_loop_parent(loop);
    if (parent != NULL) {
    ...
    for (struct kefir_codegen_target_ir_loop *nested = kefir_codegen_target_ir_loop_first_child(loop); nested != NULL;
         nested = kefir_codegen_target_ir_loop_next_sibling(nested)) {
    ...
    for (res = kefir_hashset_iter(&loop->blocks, &iter, &iter_key); res == KEFIR_OK;
        ...
        ASSIGN_DECL_CAST(kefir_codegen_target_ir_block_ref_t, block_ref, iter_key);
        ...
        if (res != KEFIR_NOT_FOUND) {
            REQUIRE_OK(res);
            ASSIGN_DECL_CAST(struct kefir_codegen_target_ir_loop *, current, *table_value_ptr);
...
static kefir_result_t build_loop_nests(struct kefir_mem *mem, struct kefir_codegen_target_ir_loop_collection *loops) {
    kefir_result_t res;
    struct kefir_codegen_target_ir_loop *loop;
    struct kefir_codegen_target_ir_loop_collection_iterator iter;
    for (res = kefir_codegen_target_ir_loop_collection_iter(loops, &loop, &iter); res == KEFIR_OK && loop != NULL;
         res = kefir_codegen_target_ir_loop_collection_next(&loop, &iter)) {
    ...
    const struct kefir_codegen_target_ir_loop_nest *nest;
    struct kefir_codegen_target_ir_loop_nest_collection_iterator nest_iter;
    for (res = kefir_codegen_target_ir_loop_nest_collection_iter(loops, &nest, &nest_iter);
         res == KEFIR_OK && nest != NULL; res = kefir_codegen_target_ir_loop_nest_collection_next(&nest, &nest_iter)) {
        REQUIRE_OK(finalize_loop(mem, loops, kefir_codegen_target_ir_loop_nest_top(nest)));
    }
    if (res != KEFIR_ITERATOR_END) {
        REQUIRE_OK(res);
    }
...
kefir_result_t kefir_codegen_target_ir_loop_collection_build(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_loop_collection *loops,
    const struct kefir_codegen_target_ir_control_flow *control_flow) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(loops != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer loop collection"));
    ...
    kefir_size_t block_count = kefir_codegen_target_ir_code_block_count(control_flow->code);
    for (kefir_size_t i = 0; i < block_count; i++) {
        kefir_codegen_target_ir_block_ref_t block_ref =
            kefir_codegen_target_ir_code_block_by_index(control_flow->code, i);
        if (!IS_BLOCK_REACHABLE(control_flow, block_ref)) {
            continue;
        }
        ...
        struct kefir_hashset_iterator succ_iter;
        kefir_hashset_key_t entry;
        for (res = kefir_hashset_iter(&control_flow->blocks[block_ref].successors, &succ_iter, &entry); res == KEFIR_OK;
             res = kefir_hashset_next(&succ_iter, &entry)) {
            ASSIGN_DECL_CAST(kefir_codegen_target_ir_block_ref_t, successor_block_ref, entry);
            if (!IS_BLOCK_REACHABLE(control_flow, successor_block_ref)) {
                continue;
            }

            kefir_bool_t is_dominator;
            REQUIRE_OK(kefir_codegen_target_ir_control_flow_is_dominator(control_flow, block_ref, successor_block_ref,
                                                                         &is_dominator));
            if (is_dominator) {
                REQUIRE_OK(build_loop(mem, loops, control_flow, successor_block_ref, block_ref));
            }
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
...
kefir_result_t kefir_codegen_target_ir_loop_collection_find_loop(
...
kefir_result_t kefir_codegen_target_ir_loop_collection_iter(
    const struct kefir_codegen_target_ir_loop_collection *loops, struct kefir_codegen_target_ir_loop **loop_ptr,
    struct kefir_codegen_target_ir_loop_collection_iterator *iter) {
    REQUIRE(loops != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer loop collection"));
    REQUIRE(iter != NULL,
    ...
    ASSIGN_PTR(loop_ptr, (struct kefir_codegen_target_ir_loop *) value);
    return KEFIR_OK;
}

kefir_result_t kefir_codegen_target_ir_loop_collection_next(
    struct kefir_codegen_target_ir_loop **loop_ptr, struct kefir_codegen_target_ir_loop_collection_iterator *iter) {
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer optimizer loop collection iterator"));
...
kefir_result_t kefir_codegen_target_ir_loop_nest_collection_iter(
    ...
    const struct kefir_codegen_target_ir_loop_nest **nest_ptr,
    struct kefir_codegen_target_ir_loop_nest_collection_iterator *iter) {
    REQUIRE(loops != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer loop collection"));
    REQUIRE(iter != NULL,
    ...
    ASSIGN_PTR(nest_ptr, (const struct kefir_codegen_target_ir_loop_nest *) iter->iter->value);
    return KEFIR_OK;
}
