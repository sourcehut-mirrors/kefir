...
static kefir_result_t classify_instruction(const struct kefir_codegen_target_ir_code *code, kefir_codegen_target_ir_instruction_ref_t instr_ref,
    struct kefir_codegen_target_ir_instruction_destructor_classification *classification, void *payload) {
    UNUSED(payload);
    ...
    memset(classification, 0, sizeof(struct kefir_codegen_target_ir_instruction_destructor_classification));
