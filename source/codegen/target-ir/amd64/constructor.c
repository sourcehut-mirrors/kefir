...
static kefir_result_t is_jump(const struct kefir_asmcmp_context *context, kefir_asmcmp_instruction_index_t instr_idx,
                              kefir_bool_t *is_jump_ptr, void *payload) {
    UNUSED(payload);
    ...
    *is_jump_ptr = false;
    ...
    if (instr->opcode == KEFIR_ASMCMP_AMD64_OPCODE(inline_assembly)) {
        ...
        kefir_result_t res;
        for (res = kefir_asmcmp_inline_assembly_fragment_iter(context, instr->args[0].inline_asm_idx, &iter);
            ...
            switch (iter.fragment->type) {
                ...
                case KEFIR_ASMCMP_INLINE_ASSEMBLY_FRAGMENT_VALUE:
                    if (iter.fragment->value.type == KEFIR_ASMCMP_VALUE_TYPE_INTERNAL_LABEL ||
                        ...
                        (iter.fragment->value.type == KEFIR_ASMCMP_VALUE_TYPE_INDIRECT &&
...
#define DEF_OPCODE0(_opcode, _mnemonic, _variant, _flags)                                                            \
    case KEFIR_ASMCMP_AMD64_OPCODE(_opcode):                                                                         \
        *is_jump_ptr =                                                                                               \
            ((_flags) & (KEFIR_AMD64_INSTRDB_CONTROL_FLOW_JUMP | KEFIR_AMD64_INSTRDB_CONTROL_FLOW_JUMP_FALLTHROUGH | \
                         KEFIR_AMD64_INSTRDB_CONTROL_FLOW_TERMINATE_CONTROL_FLOW)) != 0;                             \
        break;
...
static kefir_result_t classify_instruction(
    const struct kefir_asmcmp_instruction *instruction,
...
#undef INSTR3
    ...
    for (kefir_size_t i = 0; i < KEFIR_ASMCMP_INSTRUCTION_NUM_OF_OPERANDS; i++) {
        if (classification->operands[i].class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_WRITE &&
            ...
            (instruction->args[i].vreg.variant == KEFIR_ASMCMP_OPERAND_VARIANT_8BIT ||
             instruction->args[i].vreg.variant == KEFIR_ASMCMP_OPERAND_VARIANT_16BIT)) {
...
#define MATCH_VARIANT(_operand, _arg)                                                                       \
    do {                                                                                                    \
        ...
        } else if ((_arg)->type == KEFIR_ASMCMP_VALUE_TYPE_RIP_INDIRECT_INTERNAL ||                         \
                   (_arg)->type == KEFIR_ASMCMP_VALUE_TYPE_RIP_INDIRECT_EXTERNAL) {                         \
        ...
        case KEFIR_ASMCMP_AMD64_OPCODE(imul1):
            if (implicit_params) {
                ...
                if ((instruction->args[0].type == KEFIR_ASMCMP_VALUE_TYPE_VIRTUAL_REGISTER &&
                    ...
                    ((instruction->args[0].type == KEFIR_ASMCMP_VALUE_TYPE_RIP_INDIRECT_INTERNAL ||
                      instruction->args[0].type == KEFIR_ASMCMP_VALUE_TYPE_RIP_INDIRECT_EXTERNAL) &&
...
static kefir_result_t compute_vreg_pair_part_spill_offset(kefir_asmcmp_virtual_register_pair_type_t pair_type,
                                                          kefir_size_t index, kefir_size_t *offset_ptr, void *payload) {
    UNUSED(payload);
    REQUIRE(offset_ptr != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to offset"));
    REQUIRE(index <= 1,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid index of virtual register pair components"));

    switch (pair_type) {
        case KEFIR_ASMCMP_VIRTUAL_REGISTER_PAIR_GENERIC:
            return KEFIR_SET_ERROR(KEFIR_INVALID_STATE,
                                   "Unable to calculate spil area offset for component of generic pair");
