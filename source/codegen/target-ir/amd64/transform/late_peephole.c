...
#define NO_MATCH_ERROR KEFIR_SET_ERROR(KEFIR_NO_MATCH, "Unable to match late amd64 peephole transformation")
    ...
    if (instr->operation.opcode == KEFIR_TARGET_IR_AMD64_OPCODE(add) ||
        ...
        if (classification.classification.operands[0].class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ_WRITE &&
            ...
            union kefir_codegen_target_ir_amd64_regalloc_entry lhs_regalloc_entry, rhs_regalloc_entry, dst_regalloc_entry;
            kefir_result_t res = kefir_codegen_target_ir_regalloc_get(regalloc, classification.operands[0].output, &dst_regalloc_entry.allocation);
            REQUIRE(res != KEFIR_NOT_FOUND, NO_MATCH_ERROR);
            REQUIRE_OK(res);
            res = kefir_codegen_target_ir_regalloc_get(regalloc, instr->operation.parameters[classification.operands[0].read_index].direct.value_ref, &lhs_regalloc_entry.allocation);
            REQUIRE(res != KEFIR_NOT_FOUND, NO_MATCH_ERROR);
            REQUIRE_OK(res);
            REQUIRE(lhs_regalloc_entry.type == KEFIR_CODEGEN_TARGET_IR_AMD64_REGALLOC_TYPE_GP, NO_MATCH_ERROR);
            res = kefir_codegen_target_ir_regalloc_get(regalloc, instr->operation.parameters[classification.operands[1].read_index].direct.value_ref, &rhs_regalloc_entry.allocation);
            REQUIRE(res != KEFIR_NOT_FOUND, NO_MATCH_ERROR);
            REQUIRE_OK(res);
            ...
            if (rhs_regalloc_entry.type == KEFIR_CODEGEN_TARGET_IR_AMD64_REGALLOC_TYPE_GP) {
                kefir_asm_amd64_xasmgen_register_t lhs_reg = lhs_regalloc_entry.reg.value, rhs_reg = rhs_regalloc_entry.reg.value, dst_reg = dst_regalloc_entry.reg.value;
                if (instr->operation.parameters[classification.operands[0].read_index].direct.variant == KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_32BIT) {
