...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_const_operand(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    const struct kefir_codegen_target_ir_instruction *instr, kefir_bool_t enable_lhs, kefir_bool_t *replaced) {
    ...
    if (classification.classification.operands[0].class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ_WRITE &&
        ...
        if (has_rhs_value) {
            ...
            operands[classification.operands[1].read_index] = (struct kefir_codegen_target_ir_operand) {
                .type = KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INTEGER,
                .immediate = {.uint_immediate = rhs_value,
            ...
            *replaced = true;
            ...
            oper.parameters = operands;
            operands[classification.operands[0].read_index] =
...
static kefir_result_t peephole_indirect(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    for (kefir_size_t i = 0; i < instr->operation.parameters_length; i++) {
        if (instr->operation.parameters[i].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INDIRECT &&
            ...
            if (res != KEFIR_NO_MATCH) {
                ...
                operands[i].indirect.index_type = KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_NONE;
                operands[i].indirect.offset +=
        ...
        if (instr->operation.parameters[i].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INDIRECT &&
            ...
            REQUIRE_OK(kefir_codegen_target_ir_code_value_props(
                code, instr->operation.parameters[i].indirect.base.value_ref, &value_type));
            ...
            if (value_type->constraint.type != KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT &&
                ...
                REQUIRE_OK(kefir_codegen_target_ir_code_instruction(
                    code, instr->operation.parameters[i].indirect.base.value_ref.instr_ref, &base_instr));

                if (base_instr->operation.opcode == code->klass->assign_opcode &&
                    base_instr->operation.parameters_length > 0 &&
                    base_instr->operation.parameters[0].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF) {
                    REQUIRE_OK(kefir_codegen_target_ir_code_value_props(
                        code, base_instr->operation.parameters[0].direct.value_ref, &value_type));
                    if (value_type->constraint.type != KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT &&
                        (value_type->variant == KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT ||
                         value_type->variant == KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_64BIT)) {
                        if (!replace) {
                        ...
                        operands[i].indirect.base.value_ref =
                            base_instr->operation.parameters[0].direct.value_ref;
                        replace = true;
                        continue;
                    }
                }
        ...
        if (instr->operation.parameters[i].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INDIRECT &&
            ...
            const struct kefir_codegen_target_ir_value_type *value_type;
            REQUIRE_OK(kefir_codegen_target_ir_code_value_props(
                code, instr->operation.parameters[i].indirect.index.value_ref, &value_type));

            if (value_type->constraint.type != KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT &&
                (value_type->variant == KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT ||
                 value_type->variant == KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_64BIT)) {
                const struct kefir_codegen_target_ir_instruction *index_instr;
                REQUIRE_OK(kefir_codegen_target_ir_code_instruction(
                    code, instr->operation.parameters[i].indirect.index.value_ref.instr_ref, &index_instr));

                if (index_instr->operation.opcode == code->klass->assign_opcode &&
                    index_instr->operation.parameters[0].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF) {
                    REQUIRE_OK(kefir_codegen_target_ir_code_value_props(
                        code, index_instr->operation.parameters[0].direct.value_ref, &value_type));
                    if (value_type->constraint.type != KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT &&
                        (value_type->variant == KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT ||
                         value_type->variant == KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_64BIT)) {
                        if (!replace) {
                        ...
                        operands[i].indirect.index.value_ref =
                            index_instr->operation.parameters[0].direct.value_ref;
                        replace = true;
                        continue;
                    }
                }
        ...
        REQUIRE_OK(kefir_codegen_target_ir_code_instruction(
            code, instr->operation.parameters[i].indirect.base.value_ref.instr_ref, &base_instr));
        if (instr->operation.parameters[i].indirect.index_type == KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_VALUE_REF) {
            REQUIRE_OK(kefir_codegen_target_ir_code_instruction(
                code, instr->operation.parameters[i].indirect.index.value_ref.instr_ref, &index_instr));
        }
        ...
        if (base_instr->operation.opcode == KEFIR_TARGET_IR_AMD64_OPCODE(lea) &&
            ...
            (base_instr->operation.parameters[0].indirect.index_type == KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_NONE ||
             instr->operation.parameters[i].indirect.index_type == KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_NONE)) {
            const struct kefir_codegen_target_ir_value_type *value_type;
            REQUIRE_OK(kefir_codegen_target_ir_code_value_props(
                code, base_instr->operation.parameters[0].indirect.base.value_ref, &value_type));
            if (value_type->constraint.type == KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT) {
                continue;
            }

            if (base_instr->operation.parameters[0].indirect.index_type ==
                KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_VALUE_REF) {
                REQUIRE_OK(kefir_codegen_target_ir_code_value_props(
                    code, base_instr->operation.parameters[0].indirect.index.value_ref, &value_type));
                if (value_type->constraint.type == KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT) {
                    continue;
                }
            }
            ...
            kefir_codegen_target_ir_indirect_index_type_t index_type = operation.parameters[i].indirect.index_type;
            struct kefir_codegen_target_ir_operand_indirect_index index = operation.parameters[i].indirect.index;
            kefir_int64_t offset = operation.parameters[i].indirect.offset;
            kefir_codegen_target_ir_operand_variant_t variant = operation.parameters[i].indirect.variant;

            operands[i] = base_instr->operation.parameters[0];
            if (index_type != KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_NONE) {
                operands[i].indirect.index_type = index_type;
                operands[i].indirect.index = index;
            }
            operands[i].indirect.offset += offset;
            operands[i].indirect.variant = variant;
            replace = true;
                   ...
                   instr->operation.parameters[i].indirect.index_type == KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_NONE &&
                   instr->operation.parameters[i].indirect.offset == 0) {
            if (!replace) {
                operation = instr->operation;
                if (operation.parameters_length > 0) {
                    memcpy(operands, operation.parameters, sizeof(struct kefir_codegen_target_ir_operand) * operation.parameters_length);
            ...
            kefir_codegen_target_ir_operand_variant_t variant = operation.parameters[i].indirect.variant;
            operands[i].type = KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_RIP_INDIRECT_EXTERNAL;
            operands[i].rip_indirection = base_instr->operation.parameters[0].rip_indirection;
            operands[i].rip_indirection = base_instr->operation.parameters[0].rip_indirection;
            operands[i].rip_indirection.variant = variant;
            replace = true;
        } else if (instr->operation.parameters[i].indirect.index_type == KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_NONE &&
                   base_instr->operation.opcode == KEFIR_TARGET_IR_AMD64_OPCODE(add) &&
                   base_instr->operation.parameters[0].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF &&
                   !base_instr->operation.parameters[0].segment.present &&
                   (base_instr->operation.parameters[0].direct.variant ==
                   ...
                   (base_instr->operation.parameters[1].direct.variant ==
                        KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT ||
                    base_instr->operation.parameters[1].direct.variant ==
                        KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_64BIT)) {
            const struct kefir_codegen_target_ir_value_type *value_type;
            ...
            REQUIRE_OK(kefir_codegen_target_ir_code_value_props(
                code, base_instr->operation.parameters[1].direct.value_ref, &value_type));
            if (value_type->constraint.type == KEFIR_CODEGEN_TARGET_IR_ALLOCATION_REQUIREMENT) {
                continue;
            }
            ...
            operands[i] = (struct kefir_codegen_target_ir_operand) {
                ...
                .indirect = {.type = KEFIR_CODEGEN_TARGET_IR_INDIRECT_VALUE_REF_BASIS,
                             ...
                             .index.scale = 1,
                             .offset = operation.parameters[i].indirect.offset,
                             .variant = operation.parameters[i].indirect.variant}};
            replace = true;
        } else if (index_instr != NULL && index_instr->operation.opcode == KEFIR_TARGET_IR_AMD64_OPCODE(shl) &&
                   index_instr->operation.parameters[0].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF &&
                   (index_instr->operation.parameters[0].direct.variant ==
                        KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT ||
                    index_instr->operation.parameters[0].direct.variant ==
                        KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_64BIT ||
                    index_instr->operation.parameters[0].direct.variant ==
                   ...
                   (1 << index_instr->operation.parameters[1].immediate.int_immediate) *
                           instr->operation.parameters[i].indirect.index.scale <=
                       8) {
            if (!replace) {
                operation = instr->operation;
                if (operation.parameters_length > 0) {
            ...
            operands[i].indirect.index.value_ref = index_instr->operation.parameters[0].direct.value_ref;
            operands[i].indirect.index.scale =
                (1 << index_instr->operation.parameters[1].immediate.int_immediate) *
                operation.parameters[i].indirect.index.scale;
            replace = true;
        } else if ((base_instr->operation.opcode == KEFIR_TARGET_IR_AMD64_OPCODE(add) ||
                    base_instr->operation.opcode == KEFIR_TARGET_IR_AMD64_OPCODE(sub)) &&
                   base_instr->operation.parameters[0].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF &&
                   !base_instr->operation.parameters[0].segment.present &&
                   (base_instr->operation.parameters[0].direct.variant ==
                        KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT ||
            ...
            kefir_int64_t value =
                kefir_codegen_target_ir_sign_extend(base_instr->operation.parameters[1].immediate.int_immediate,
                                                    base_instr->operation.parameters[1].immediate.variant);
            ...
            operands[i].indirect.base.value_ref = base_instr->operation.parameters[0].direct.value_ref;
            operands[i].indirect.offset += value;
            replace = true;
        } else if (index_instr != NULL &&
                   (index_instr->operation.opcode == KEFIR_TARGET_IR_AMD64_OPCODE(add) ||
                    index_instr->operation.opcode == KEFIR_TARGET_IR_AMD64_OPCODE(sub)) &&
                   index_instr->operation.parameters[0].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF &&
            ...
            kefir_int64_t value =
                kefir_codegen_target_ir_sign_extend(index_instr->operation.parameters[1].immediate.int_immediate,
                                                    index_instr->operation.parameters[1].immediate.variant);
            ...
            operands[i].indirect.index.value_ref = index_instr->operation.parameters[0].direct.value_ref;
            operands[i].indirect.offset += value * operation.parameters[i].indirect.index.scale;
            replace = true;
        }
    }

    if (replace) {
        REQUIRE_OK(kefir_codegen_target_ir_code_replace_operation(mem, code, instr->instr_ref, &operation, NULL));
        *replaced = true;
    }
    return KEFIR_OK;
}

static kefir_result_t peephole_untie(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    for (kefir_size_t i = 0; i < KEFIR_ASMCMP_INSTRUCTION_NUM_OF_OPERANDS; i++) {
        if (classification.classification.operands[i].class != KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ_WRITE ||
            ...
            instr->operation.parameters[classification.operands[i].read_index].type !=
                KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF ||
            !instr->operation.parameters[classification.operands[i].read_index].direct.tied) {
            continue;
        }
        ...
        if (output_value_type->variant != KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_64BIT &&
            ...
            (output_value_type->variant != KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_DEFAULT ||
             output_value_type->kind != KEFIR_CODEGEN_TARGET_IR_VALUE_TYPE_GENERAL_PURPOSE)) {
            kefir_result_t res;
            ...
            for (res = kefir_codegen_target_ir_code_use_iter(code, &use_iter, output_value_ref.instr_ref,
                ...
                for (kefir_size_t i = 0; i < user_instr->operation.parameters_length && !ext_uses; i++) {
                    if (user_instr->operation.parameters[i].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF &&
                        ...
                        (user_instr->operation.parameters[i].direct.tied ||
                         !(user_instr->operation.parameters[i].direct.variant ==
                           ...
                           (user_instr->operation.parameters[i].direct.variant ==
                            ...
                            output_value_type->variant == KEFIR_CODEGEN_TARGET_IR_OPERAND_VARIANT_64BIT)))) {
                        ext_uses = true;
                    } else if (user_instr->operation.parameters[i].type ==
                                   KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INDIRECT &&
                               user_instr->operation.parameters[i].indirect.type ==
                                   KEFIR_CODEGEN_TARGET_IR_INDIRECT_VALUE_REF_BASIS &&
                               user_instr->operation.parameters[i].indirect.index_type ==
                                   KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_NONE &&
                               user_instr->operation.parameters[i].indirect.base.value_ref.instr_ref ==
                                   output_value_ref.instr_ref &&
                               user_instr->operation.parameters[i].indirect.base.value_ref.aspect ==
                                   output_value_ref.aspect) {
                        ext_uses = true;
                    }
                }
            }
        ...
        if (!ext_uses) {
            ...
            operands[classification.operands[i].read_index].direct.tied = false;
            replace = true;
        }
    }

    if (replace) {
        REQUIRE_OK(kefir_codegen_target_ir_code_replace_operation(mem, code, instr_ref, &operation, NULL));
        *replaced = true;
    }
...
static kefir_result_t do_peephole(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    for (; !reached_fixpoint;) {
        ...
        for (kefir_size_t i = 0; i < kefir_codegen_target_ir_code_length(code); i++) {
            const struct kefir_codegen_target_ir_instruction *instr;
            REQUIRE_OK(kefir_codegen_target_ir_code_at_index(code, i, &instr));
            if (instr->block_ref == KEFIR_ID_NONE) {
                continue;
            }
...
kefir_result_t kefir_codegen_target_ir_amd64_transform_peephole(struct kefir_mem *mem,
