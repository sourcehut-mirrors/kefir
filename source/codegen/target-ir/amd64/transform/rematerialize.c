...
static kefir_result_t do_rematerialize(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    REQUIRE_OK(kefir_codegen_target_ir_code_new_instruction(
        mem, code, block_ref, kefir_codegen_target_ir_code_control_prev(code, insert_before_ref), &operation, &metadata,
        &rematerialized_instr_ref));
    ...
    for (res = kefir_codegen_target_ir_code_value_iter(code, &instr_value_iter, value_ref.instr_ref, &instr_value_ref,
                                                       &value_type);
         res == KEFIR_OK;
        ...
        REQUIRE_OK(kefir_codegen_target_ir_code_add_aspect(
            mem, code,
            (kefir_codegen_target_ir_value_ref_t) {.instr_ref = rematerialized_instr_ref,
    ...
    for (res =
             kefir_codegen_target_ir_code_instruction_attribute_iter(code, &attr_iter, value_ref.instr_ref, &attribute);
         res == KEFIR_OK; res = kefir_codegen_target_ir_code_instruction_attribute_next(&attr_iter, &attribute)) {
        REQUIRE_OK(
            kefir_codegen_target_ir_code_add_instruction_attribute(mem, code, rematerialized_instr_ref, attribute));
    }
    if (res != KEFIR_ITERATOR_END) {
        REQUIRE_OK(res);
    }

    *rematerialized_value_ref =
        (kefir_codegen_target_ir_value_ref_t) {.instr_ref = rematerialized_instr_ref, .aspect = value_ref.aspect};
    return KEFIR_OK;
}

static kefir_result_t is_rematerializable_value(const struct kefir_codegen_target_ir_code *code,
                                                kefir_codegen_target_ir_value_ref_t value_ref,
...
static kefir_result_t liveness_interval_includes(const struct kefir_codegen_target_ir_liveness *liveness,
                                                 kefir_codegen_target_ir_value_ref_t value_ref,
                                                 kefir_codegen_target_ir_block_ref_t block_ref, kefir_size_t seq_idx,
                                                 kefir_bool_t *includes) {
    kefir_codegen_target_ir_instruction_ref_t begin_ref, end_ref;
    kefir_result_t res = kefir_codegen_target_ir_liveness_value_at(
        liveness, value_ref, block_ref, KEFIR_CODEGEN_TARGET_IR_LIVENESS_NORMAL, &begin_ref, &end_ref);
    ...
    if (begin_ref != KEFIR_ID_NONE) {
        REQUIRE_OK(
            kefir_codegen_target_ir_numbering_instruction_seq_index(&liveness->numbering, begin_ref, &begin_seq_idx));
    }
    if (end_ref != KEFIR_ID_NONE) {
        REQUIRE_OK(
            kefir_codegen_target_ir_numbering_instruction_seq_index(&liveness->numbering, end_ref, &end_seq_idx));
    }
...
kefir_result_t kefir_codegen_target_ir_amd64_is_rematerializable(
    const struct kefir_codegen_target_ir_code *code, const struct kefir_codegen_target_ir_liveness *liveness,
...
static kefir_result_t distance_between(const struct kefir_codegen_target_ir_code *code,
                                       kefir_codegen_target_ir_instruction_ref_t instr1_ref,
                                       kefir_codegen_target_ir_instruction_ref_t instr2_ref, kefir_size_t max_distance,
                                       kefir_size_t *distance) {
    *distance = 0;
    ...
    REQUIRE(*distance == max_distance,
            KEFIR_SET_ERROR(KEFIR_NOT_FOUND, "Unable to find the distance between instructions"));
    return KEFIR_OK;
}

static kefir_result_t try_rematerialize(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    const struct kefir_codegen_target_ir_regalloc *regalloc, kefir_codegen_target_ir_block_ref_t block_ref,
    kefir_codegen_target_ir_instruction_ref_t insert_before_ref, kefir_codegen_target_ir_instruction_ref_t use_ref,
    kefir_codegen_target_ir_value_ref_t value_ref, kefir_codegen_target_ir_value_ref_t *rematerialized_value_ref,
    const struct kefir_hashset *alive_values, struct kefir_hashtable *local_rematerializations,
        ...
        switch (original_instr->operation.parameters[i].type) {
            case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF:
                REQUIRE(kefir_hashset_has(alive_values, KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(
                                                            &original_instr->operation.parameters[i].direct.value_ref)),
                        KEFIR_OK);
                break;

            case KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INDIRECT:
                REQUIRE(original_instr->operation.opcode == KEFIR_TARGET_IR_AMD64_OPCODE(lea), KEFIR_OK);
                if (original_instr->operation.parameters[i].indirect.type ==
                    KEFIR_CODEGEN_TARGET_IR_INDIRECT_VALUE_REF_BASIS) {
                    REQUIRE(kefir_hashset_has(alive_values,
                                              KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(
                                                  &original_instr->operation.parameters[i].indirect.base.value_ref)),
                            KEFIR_OK);
                }
                if (original_instr->operation.parameters[i].indirect.index_type ==
                    KEFIR_CODEGEN_TARGET_IR_INDIRECT_INDEX_VALUE_REF) {
                    REQUIRE(kefir_hashset_has(alive_values,
                                              KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(
                                                  &original_instr->operation.parameters[i].indirect.index.value_ref)),
                            KEFIR_OK);
                }
                break;
    ...
    res = kefir_hashtable_at_mut(local_rematerializations,
                                 (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref),
                                 &table_value);
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
        ASSIGN_DECL_CAST(kefir_codegen_target_ir_instruction_ref_t, rematerialized_instr_ref, (*table_value));
        kefir_size_t distance = 0;
        REQUIRE_OK(distance_between(code, rematerialized_instr_ref, use_ref,
                                    regalloc->klass->transforms->rematerialization_locality, &distance));
    ...
    if (original_instr->block_ref == insert_before_instr->block_ref) {
        kefir_size_t distance = 0;
        REQUIRE_OK(distance_between(code, value_ref.instr_ref, use_ref,
                                    regalloc->klass->transforms->rematerialization_locality, &distance));
    ...
    if (table_value != NULL) {
        *table_value = rematerialized_value_ref->instr_ref;
        ...
        REQUIRE_OK(kefir_hashtable_insert(mem, local_rematerializations,
                                          (kefir_hashtable_key_t) KEFIR_CODEGEN_TARGET_IR_VALUE_REF_INTO(&value_ref),
...
static kefir_result_t rematerialize_block(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    ...
    for (kefir_codegen_target_ir_instruction_ref_t instr_ref =
        ...
        for (res = kefir_hashset_iter(alive_values, &iter, &entry); res == KEFIR_OK && !has_alive_flags;
        ...
        if (instr->operation.opcode == code->klass->phi_opcode ||
            ...
            if (do_replace) {
                ...
                REQUIRE_OK(
                    kefir_codegen_target_ir_code_replace_operation(mem, code, instr_ref, &oper, &replaced_instr_ref));
    ...
    for (res = kefir_hashset_iter(&control_flow->blocks[block_ref].successors, &iter, &key); res == KEFIR_OK;
        ...
        for (res = kefir_codegen_target_ir_code_phi_node_iter(code, &phi_iter, successor_block_ref, &phi_ref);
            ...
            REQUIRE_OK(try_rematerialize(mem, code, regalloc, block_ref, no_alive_flags_ref,
                                         kefir_codegen_target_ir_code_block_control_tail(code, block_ref),
                                         link_value_ref, &rematerialized_value_ref, alive_values,
                                         local_rematerializations, &did_rematerialize));
            if (did_rematerialize) {
                REQUIRE_OK(kefir_codegen_target_ir_code_phi_drop(mem, code, phi_ref, block_ref));
                REQUIRE_OK(
                    kefir_codegen_target_ir_code_phi_attach(mem, code, phi_ref, block_ref, rematerialized_value_ref));
            }
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
...
kefir_result_t kefir_codegen_target_ir_amd64_transform_rematerialize(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
    const struct kefir_codegen_target_ir_control_flow *control_flow,
    const struct kefir_codegen_target_ir_liveness *liveness, const struct kefir_codegen_target_ir_regalloc *regalloc) {
    ...
    for (kefir_size_t i = 0; i < kefir_codegen_target_ir_code_block_count(code); i++) {
        ...
        if (kefir_codegen_target_ir_control_flow_is_reachable(control_flow, block_ref) &&
            ...
            res = rematerialize_block(mem, code, control_flow, liveness, regalloc, block_ref, &alive_values,
                                      &local_rematerializations);
