...
#define CONST_EVAL_BASE(_op, _cond, _ext)                                                                              \
    do {                                                                                                               \
        ...
        if (classification.classification.operands[0].class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ_WRITE &&    \
            ...
            kefir_result_t res = kefir_codegen_target_ir_amd64_match_immediate_operand(                                \
                code, &instr->operation.parameters[classification.operands[0].read_index], (_ext), &lhs);              \
                                                                                                                       \
            if (res != KEFIR_NO_MATCH) {                                                                               \
                REQUIRE_OK(res);                                                                                       \
                const struct kefir_codegen_target_ir_value_type *value_type;                                           \
                REQUIRE_OK(                                                                                            \
                    kefir_codegen_target_ir_code_value_props(code, classification.operands[0].output, &value_type));   \
...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_xor(struct kefir_mem *mem,
    ...
    kefir_codegen_target_ir_value_ref_t zero_ref = {.aspect = KEFIR_CODEGEN_TARGET_IR_VALUE_DIRECT_OUTPUT(0)};
    ...
    REQUIRE_OK(kefir_codegen_target_ir_code_value_props(code, classification.operands[0].output, &output_value_type));
    ...
    REQUIRE_OK(kefir_codegen_target_ir_code_replace_value(mem, code, zero_ref, classification.operands[0].output));
...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_and(struct kefir_mem *mem,
                                                          struct kefir_codegen_target_ir_code *code,
                                                          const struct kefir_codegen_target_ir_instruction *instr,
                                                          kefir_bool_t *replaced) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ...
    if (instr->operation.parameters[classification.operands[1].read_index].immediate.uint_immediate == 0) {
        struct kefir_codegen_target_ir_operand operands[] = {
            {.type = KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INTEGER, .immediate.int_immediate = 0}
        };
        REQUIRE_OK(kefir_codegen_target_ir_code_replace_operation(
            mem, code, instr_ref,
            &(struct kefir_codegen_target_ir_operation) {
        ...
        for (res = kefir_codegen_target_ir_code_use_iter(code, &use_iter, instr_ref, &use_instr_ref, &used_value_ref);
            ...
            if (user_instr->operation.opcode == KEFIR_TARGET_IR_AMD64_OPCODE(movsx) ||
                ...
                if (user_classification.operands[1].read_index != KEFIR_CODEGEN_TARGET_IR_TIED_READ_INDEX_NONE &&
                    user_instr->operation.parameters[user_classification.operands[1].read_index].type ==
                        KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF &&
                    user_instr->operation.parameters[user_classification.operands[1].read_index]
                            .direct.value_ref.instr_ref == instr_ref &&
                    user_instr->operation.parameters[user_classification.operands[1].read_index]
                            .direct.value_ref.aspect == KEFIR_CODEGEN_TARGET_IR_VALUE_DIRECT_OUTPUT(0)) {
...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_or(struct kefir_mem *mem,
                                                         struct kefir_codegen_target_ir_code *code,
                                                         const struct kefir_codegen_target_ir_instruction *instr,
                                                         kefir_bool_t *replaced) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
...
kefir_result_t peephole_const_operand_nonneg(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                             const struct kefir_codegen_target_ir_instruction *instr,
                                             kefir_bool_t *replaced) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_shl(struct kefir_mem *mem,
                                                          struct kefir_codegen_target_ir_code *code,
                                                          const struct kefir_codegen_target_ir_instruction *instr,
                                                          kefir_bool_t *replaced) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_shr(struct kefir_mem *mem,
                                                          struct kefir_codegen_target_ir_code *code,
                                                          const struct kefir_codegen_target_ir_instruction *instr,
                                                          kefir_bool_t *replaced) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_sar(struct kefir_mem *mem,
                                                          struct kefir_codegen_target_ir_code *code,
                                                          const struct kefir_codegen_target_ir_instruction *instr,
                                                          kefir_bool_t *replaced) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_rol(struct kefir_mem *mem,
                                                          struct kefir_codegen_target_ir_code *code,
                                                          const struct kefir_codegen_target_ir_instruction *instr,
                                                          kefir_bool_t *replaced) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_btc(struct kefir_mem *mem,
                                                          struct kefir_codegen_target_ir_code *code,
                                                          const struct kefir_codegen_target_ir_instruction *instr,
                                                          kefir_bool_t *replaced) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_shxd(struct kefir_mem *mem,
                                                           struct kefir_codegen_target_ir_code *code,
                                                           const struct kefir_codegen_target_ir_instruction *instr,
                                                           kefir_bool_t *replaced) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ...
    if (classification.operands[2].read_index != KEFIR_CODEGEN_TARGET_IR_TIED_READ_INDEX_NONE &&
        ...
        kefir_int64_t rhs_value = 0;
        kefir_result_t res = kefir_codegen_target_ir_amd64_match_immediate(
            code, instr->operation.parameters[classification.operands[2].read_index].direct.value_ref, true,
            &rhs_value);
        if (res != KEFIR_NO_MATCH) {
            REQUIRE_OK(res);
            if (rhs_value >= 0) {
                ...
                oper.parameters = operands;
                operands[classification.operands[2].read_index] = (struct kefir_codegen_target_ir_operand) {
                    .type = KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_INTEGER,
                    .immediate = {.uint_immediate = rhs_value,
