...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_setcc(struct kefir_mem *mem,
    ...
    REQUIRE(classification.classification.operands[0].class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ_WRITE &&
                ...
                !oper.parameters[classification.operands[0].read_index].direct.tied,
            KEFIR_OK);

    kefir_codegen_target_ir_value_ref_t input_value_ref =
        oper.parameters[classification.operands[0].read_index].direct.value_ref;
    ...
    kefir_codegen_target_ir_value_ref_t placeholder_value_ref = {.aspect =
                                                                     KEFIR_CODEGEN_TARGET_IR_VALUE_DIRECT_OUTPUT(0)};
    struct kefir_codegen_target_ir_value_type placeholder_input_value_type = {.kind = input_value_type->kind,
...
static kefir_result_t peephole_setcc_preamble(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code,
                                              const struct kefir_codegen_target_ir_instruction *instr,
    ...
    *replace_instr_ref = KEFIR_ID_NONE;
    ...
    REQUIRE(
        ...
        KEFIR_OK);
    REQUIRE(user_instr->operation.parameters[0].direct.value_ref.aspect == classification.operands[0].output.aspect,
            KEFIR_OK);
    REQUIRE(user_instr->operation.parameters[1].type == KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF, KEFIR_OK);
    REQUIRE(
        user_instr->operation.parameters[1].direct.value_ref.instr_ref == classification.operands[0].output.instr_ref,
        KEFIR_OK);
    REQUIRE(user_instr->operation.parameters[1].direct.value_ref.aspect == classification.operands[0].output.aspect,
            KEFIR_OK);
    ...
    for (kefir_codegen_target_ir_instruction_ref_t iter_ref = instr->instr_ref; iter_ref != KEFIR_ID_NONE;
        ...
        if (iter_ref == test_user_instr_ref) {
            found_user = true;
            break;
        } else if (iter_ref == user_instr_ref || iter_ref == instr->instr_ref) {
            continue;
        }
        ...
        for (kefir_size_t i = 0; i < resources_len; i++) {
            ...
            kefir_result_t res = kefir_codegen_target_ir_code_value_props(
                code,
                (kefir_codegen_target_ir_value_ref_t) {.instr_ref = iter_ref,
...
#define SETCC_IMPL(_setcc, _setncc, _jz, _jnz, ...)                                                                    \
    kefir_result_t kefir_codegen_target_ir_amd64_peephole_##_setcc(                                                    \
...
SETCC_IMPL(setge, setl, jl, jge, KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_SF,
           KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_OF)
SETCC_IMPL(setl, setge, jge, jl, KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_SF,
           KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_OF)
SETCC_IMPL(setg, setle, jle, jg, KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_SF,
           KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_OF, KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_ZF)
SETCC_IMPL(setle, setg, jg, jle, KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_SF,
           KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_OF, KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_ZF)
SETCC_IMPL(setbe, seta, ja, jbe, KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_CF,
           KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_ZF)
SETCC_IMPL(seta, setbe, jbe, ja, KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_CF,
           KEFIR_CODEGEN_TARGET_IR_AMD64_RESOURCE_FLAG_ZF)
