...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_div(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code, const struct kefir_codegen_target_ir_instruction *instr, kefir_bool_t *replaced) {
    ...
    for (kefir_uint32_t i = 0; i < sizeof(kefir_uint64_t) * CHAR_BIT; i++) {
        if (oper.parameters[classification.operands[0].read_index].immediate.uint_immediate == (1ull << i)) {
            ...
            REQUIRE_OK(kefir_codegen_target_ir_code_get_single_user(code, oper.parameters[classification.operands[1].read_index].direct.value_ref, &single_user_ref));
            if (single_user_ref == instr_ref) {
                arg1_value_type_copy.constraint.type = KEFIR_CODEGEN_TARGET_IR_ALLOCATION_NO_CONSTRAINT;
            }
            REQUIRE_OK(kefir_codegen_target_ir_code_get_single_user(code, oper.parameters[classification.operands[2].read_index].direct.value_ref, &single_user_ref));
            if (single_user_ref == instr_ref) {
...
kefir_result_t kefir_codegen_target_ir_amd64_peephole_idiv(struct kefir_mem *mem, struct kefir_codegen_target_ir_code *code, const struct kefir_codegen_target_ir_instruction *instr, kefir_bool_t *replaced) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));
    REQUIRE(instr != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR instruction"));
    REQUIRE(replaced != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to boolean flag"));
