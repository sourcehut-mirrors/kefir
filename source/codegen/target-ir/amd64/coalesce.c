...
static kefir_result_t extract_coalesce(struct kefir_mem *mem, const struct kefir_codegen_target_ir_code *code,
                                       const struct kefir_codegen_target_ir_instruction *instruction,
                                       const struct kefir_codegen_target_ir_coalesce_callback *callback,
                                       void *payload) {
    UNUSED(mem);
    ...
    REQUIRE(instruction->operation.opcode != code->klass->phi_opcode &&
                ...
                instruction->operation.opcode != code->klass->placeholder_opcode,
            KEFIR_OK);
    ...
    for (kefir_size_t i = 0; i < KEFIR_ASMCMP_INSTRUCTION_NUM_OF_OPERANDS; i++) {
        if (classification.classification.operands[i].class == KEFIR_CODEGEN_TARGET_IR_ASMCMP_OPERAND_READ_WRITE &&
            ...
            instruction->operation.parameters[classification.operands[i].read_index].type ==
                KEFIR_CODEGEN_TARGET_IR_OPERAND_TYPE_VALUE_REF) {
            REQUIRE_OK(callback->coalesce(
                mem, instruction->operation.parameters[classification.operands[i].read_index].direct.value_ref,
                classification.operands[i].output, callback->payload));
        }
    }
    return KEFIR_OK;
}
...
kefir_result_t kefir_codegen_target_ir_amd64_coalesce_init(struct kefir_codegen_target_ir_amd64_coalesce_class *klass,
    ...
    REQUIRE(klass != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to target IR amd64 coalesce class"));
