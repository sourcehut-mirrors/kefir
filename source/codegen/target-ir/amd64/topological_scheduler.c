...
static kefir_result_t do_schedule(struct kefir_mem *mem,
    const struct kefir_codegen_target_ir_code_schedule *schedule,
    struct kefir_codegen_target_ir_code_schedule_builder *schedule_builder,
    kefir_codegen_target_ir_block_ref_t entry_point_ref,
    void *payload) {
    UNUSED(payload);
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(schedule != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR schedule"));
    REQUIRE(schedule_builder != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR schedule builder"));
    REQUIRE(entry_point_ref != KEFIR_ID_NONE && entry_point_ref < kefir_codegen_target_ir_code_block_count(schedule->code), KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR schedule entry point"));
    ASSIGN_DECL_CAST(const struct kefir_codegen_target_ir_control_flow *, control_flow,
        payload);

    struct kefir_list queue;
    REQUIRE_OK(kefir_list_init(&queue));
    kefir_result_t res = kefir_list_insert_after(mem, &queue, kefir_list_tail(&queue), (void *) (kefir_uptr_t) entry_point_ref);
...
kefir_result_t kefir_codegen_target_ir_amd64_topological_scheduler_init(const struct kefir_codegen_target_ir_control_flow *control_flow,
