...
kefir_result_t kefir_codegen_target_ir_numbering_init(struct kefir_codegen_target_ir_numbering *numbering) {
    REQUIRE(numbering != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to target IR numbering"));
...
kefir_result_t kefir_codegen_target_ir_numbering_free(struct kefir_mem *mem,
...
kefir_result_t kefir_codegen_target_ir_numbering_reset(struct kefir_mem *mem,
                                                       struct kefir_codegen_target_ir_numbering *numbering) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(numbering != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR numbering"));
...
kefir_result_t kefir_codegen_target_ir_numbering_build(struct kefir_mem *mem,
                                                       struct kefir_codegen_target_ir_numbering *numbering,
                                                       const struct kefir_codegen_target_ir_code *code) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(numbering != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR numbering"));
    REQUIRE(numbering->instruction_seq_nums == NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid clean target IR numbering"));
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR code"));

    numbering->instruction_seq_nums = KEFIR_MALLOC(mem, sizeof(kefir_uint32_t) * code->code_length);
    REQUIRE(numbering->instruction_seq_nums != NULL,
            KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR numbering"));
    numbering->length = code->code_length;

    numbering->block_lengths =
        KEFIR_MALLOC(mem, sizeof(kefir_uint32_t) * kefir_codegen_target_ir_code_block_count(code));
    REQUIRE(numbering->block_lengths != NULL,
            KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR numbering"));
...
kefir_result_t kefir_codegen_target_ir_numbering_block_length(const struct kefir_codegen_target_ir_numbering *numbering,
    ...
    REQUIRE(numbering != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR numbering"));
    REQUIRE(block_ref != KEFIR_ID_NONE && block_ref < numbering->block_count,
            KEFIR_SET_ERROR(KEFIR_OUT_OF_BOUNDS, "Block reference is out of numbering bounds"));
...
kefir_result_t kefir_codegen_target_ir_numbering_instruction_seq_index(
