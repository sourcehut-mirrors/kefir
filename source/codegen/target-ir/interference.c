...
static kefir_result_t free_interfence_entry(struct kefir_mem *mem, struct kefir_hashtable *table,
    ...
    ASSIGN_DECL_CAST(struct interference_entry *, entry, value);
...
kefir_result_t kefir_codegen_target_ir_interference_init(struct kefir_codegen_target_ir_interference *interference) {
    REQUIRE(interference != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to target IR interference"));
...
kefir_result_t kefir_codegen_target_ir_interference_free(struct kefir_mem *mem,
...
kefir_result_t kefir_codegen_target_ir_interference_reset(struct kefir_mem *mem,
                                                          struct kefir_codegen_target_ir_interference *interference) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(interference != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR interference"));
...
static kefir_result_t add_interference(struct kefir_mem *mem, struct kefir_codegen_target_ir_interference *interference,
                                       kefir_codegen_target_ir_value_ref_t value_ref,
                                       kefir_codegen_target_ir_value_ref_t interfere_value_ref) {
    REQUIRE(value_ref.instr_ref != KEFIR_ID_NONE &&
                KEFIR_TARGET_IR_INSTR_REF_INDEX_OF(value_ref.instr_ref) < interference->length,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Interfering value reference is out of bounds"));

    kefir_hashtable_value_t table_value;
    struct interference_entry *entry = NULL;
    kefir_result_t res = kefir_hashtable_at(
        &interference->interference[KEFIR_TARGET_IR_INSTR_REF_INDEX_OF(value_ref.instr_ref)].value_entries,
        (kefir_hashtable_key_t) value_ref.aspect, &table_value);
    if (res == KEFIR_NOT_FOUND) {
        entry = KEFIR_MALLOC(mem, sizeof(struct interference_entry));
        REQUIRE(entry != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR interference entry"));
        entry->interference = NULL;
        entry->length = 0;
        entry->capacity = 0;
        res = kefir_hashtable_insert(
            mem, &interference->interference[KEFIR_TARGET_IR_INSTR_REF_INDEX_OF(value_ref.instr_ref)].value_entries,
            (kefir_hashtable_key_t) value_ref.aspect, (kefir_hashtable_value_t) entry);
    ...
    if (entry->length >= entry->capacity) {
        ...
        kefir_codegen_target_ir_value_ref_t *new_interference =
            KEFIR_REALLOC(mem, entry->interference, sizeof(kefir_codegen_target_ir_value_ref_t) * new_capacity);
        REQUIRE(new_interference != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR interference entry"));
...
static kefir_result_t record_interference(struct kefir_mem *mem,
                                          struct kefir_codegen_target_ir_interference *interference,
                                          kefir_codegen_target_ir_value_ref_t value_ref,
...
static kefir_result_t build_block_interference_impl(struct kefir_mem *mem,
    ...
    for (kefir_codegen_target_ir_instruction_ref_t instr_ref =
        ...
        kefir_result_t res;
        for (res =
                 kefir_codegen_target_ir_code_value_iter(control_flow->code, &value_iter, instr_ref, &value_ref, NULL);
        ...
        if (instr->operation.opcode == control_flow->code->klass->upsilon_opcode) {
            REQUIRE_OK(
                record_interference(mem, interference, instr->operation.parameters[0].upsilon_ref, alive_values));
        }
    }
    return KEFIR_OK;
}

static kefir_result_t build_block_interference(struct kefir_mem *mem,
                                               struct kefir_codegen_target_ir_interference *interference,
                                               const struct kefir_codegen_target_ir_control_flow *control_flow,
                                               const struct kefir_codegen_target_ir_liveness *liveness,
                                               kefir_codegen_target_ir_block_ref_t block_ref) {
    ...
    kefir_result_t res =
        build_block_interference_impl(mem, liveness, interference, control_flow, block_ref, &alive_values);
    REQUIRE_ELSE(res == KEFIR_OK, {
...
kefir_result_t kefir_codegen_target_ir_interference_build(
    struct kefir_mem *mem, struct kefir_codegen_target_ir_interference *interference,
    const struct kefir_codegen_target_ir_control_flow *control_flow,
    const struct kefir_codegen_target_ir_liveness *liveness) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(interference != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR interference"));
    ...
    interference->interference = KEFIR_MALLOC(
        mem, sizeof(struct kefir_codegen_target_ir_instruction_interference) * control_flow->code->code_length);
    REQUIRE(interference->interference != NULL,
            KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate target IR interference"));
    ...
    for (kefir_size_t i = 0; i < interference->length; i++) {
        ...
        REQUIRE_OK(
            kefir_hashtable_on_removal(&interference->interference[i].value_entries, free_interfence_entry, NULL));
    }
...
kefir_result_t kefir_codegen_target_ir_interference_has(const struct kefir_codegen_target_ir_interference *interference,
                                                        kefir_codegen_target_ir_value_ref_t value_ref,
                                                        kefir_codegen_target_ir_value_ref_t other_value_ref,
                                                        kefir_bool_t *has_interference) {
    REQUIRE(interference != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid target IR interference"));
    REQUIRE(value_ref.instr_ref != KEFIR_ID_NONE &&
                KEFIR_TARGET_IR_INSTR_REF_INDEX_OF(value_ref.instr_ref) < interference->length,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Interfering value reference is out of bounds"));
    REQUIRE(has_interference != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to target IR interference flag"));
    ...
    *has_interference = false;
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        for (kefir_size_t i = 0; i < entry->length && !*has_interference; i++) {
            if (entry->interference[i].instr_ref == other_value_ref.instr_ref &&
                entry->interference[i].aspect == other_value_ref.aspect) {
...
kefir_result_t kefir_codegen_target_ir_interference_iter(
    const struct kefir_codegen_target_ir_interference *interference,
    struct kefir_codegen_target_ir_interference_iterator *iter, kefir_codegen_target_ir_value_ref_t value_ref,
    ...
    REQUIRE(value_ref.instr_ref != KEFIR_ID_NONE &&
            ...
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Interfering value reference is out of bounds"));
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to target IR interference iterator"));
