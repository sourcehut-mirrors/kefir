...
static kefir_result_t free_source_map_entry(struct kefir_mem *mem, struct kefir_hashtree *tree,
    ...
    ASSIGN_DECL_CAST(struct source_map_entry *, entry, value);
...
kefir_result_t kefir_asmcmp_debug_info_source_map_init(struct kefir_asmcmp_debug_info_source_map *source_map) {
    REQUIRE(source_map != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to asmcmp source map"));
...
kefir_result_t kefir_asmcmp_debug_info_source_map_free(struct kefir_mem *mem,
...
static kefir_result_t free_definition_entry(struct kefir_mem *mem, struct kefir_hashtable *table,
    ...
    REQUIRE(entry != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info definition entry"));
...
kefir_result_t kefir_asmcmp_debug_info_code_map_init(struct kefir_asmcmp_debug_info_code_map *code_map) {
    REQUIRE(code_map != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to asmcmp debug info code map"));
...
kefir_result_t kefir_asmcmp_debug_info_code_map_free(struct kefir_mem *mem,
...
static kefir_result_t free_value_entry(struct kefir_mem *mem, struct kefir_hashtable *table, kefir_hashtable_key_t key,
    ...
    REQUIRE(entry != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info value map entry"));
...
kefir_result_t kefir_asmcmp_debug_info_value_map_init(struct kefir_asmcmp_debug_info_value_map *value_map) {
    REQUIRE(value_map != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to asmcmp debug info value map"));
...
kefir_result_t kefir_asmcmp_debug_info_value_map_free(struct kefir_mem *mem,
...
kefir_result_t kefir_asmcmp_debug_info_source_map_add_location(struct kefir_mem *mem,
                                                               struct kefir_asmcmp_debug_info_source_map *source_map,
    ...
    REQUIRE(begin < end,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Asmcmp source map begin index shall preceede end index"));
    ...
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
        ASSIGN_DECL_CAST(struct source_map_entry *, entry, node->value);
        REQUIRE(begin >= entry->end,
                KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Overlapping asmcmp index ranges in location assignment"));
    }
...
kefir_result_t kefir_asmcmp_debug_info_source_map_at(const struct kefir_asmcmp_debug_info_source_map *source_map,
                                                     kefir_asmcmp_instruction_index_t index,
                                                     const struct kefir_source_location **location_ptr) {
    REQUIRE(source_map != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp source map"));
    ...
    ASSIGN_DECL_CAST(const struct source_map_entry *, entry, node->value);
...
kefir_result_t kefir_asmcmp_code_map_add_fragment(struct kefir_mem *mem,
                                                  struct kefir_asmcmp_debug_info_code_map *code_map,
    ...
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(code_map != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info code map"));
    REQUIRE(begin_label != KEFIR_ASMCMP_INDEX_NONE,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp label index"));
    REQUIRE(end_label != KEFIR_ASMCMP_INDEX_NONE,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp label index"));
    ...
    if (res == KEFIR_NOT_FOUND) {
        ...
        REQUIRE(entry != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate asmcmp debug info definition entry"));
        entry->fragments = NULL;
        entry->fragments_capacity = 0;
        entry->fragments_length = 0;
        res = kefir_hashtable_insert(mem, &code_map->fragments, (kefir_hashtable_key_t) code_reference,
    ...
    if (entry->fragments_length >= entry->fragments_capacity) {
        ...
        struct kefir_asmcmp_debug_info_code_fragment *new_fragments =
            KEFIR_REALLOC(mem, entry->fragments, sizeof(struct kefir_asmcmp_debug_info_code_fragment) * new_capacity);
        REQUIRE(new_fragments != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate asmcmp debug info definition fragment"));
...
kefir_result_t kefir_asmcmp_value_map_add_fragment(struct kefir_mem *mem,
                                                   struct kefir_asmcmp_debug_info_value_map *value_map,
                                                   kefir_asmcmp_debug_info_value_reference_t value_reference,
    ...
    kefir_hashtable_value_t table_value;
    kefir_result_t res =
        kefir_hashtable_at(&value_map->fragments, (kefir_hashtable_key_t) value_reference, &table_value);
    ...
    if (res == KEFIR_NOT_FOUND) {
        ...
        REQUIRE(entry != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate asmcmp debug info value map entry"));
        entry->fragments = NULL;
        entry->fragments_capacity = 0;
        entry->fragments_length = 0;
        res = kefir_hashtable_insert(mem, &value_map->fragments, (kefir_hashtable_key_t) value_reference,
                                     (kefir_hashtable_value_t) entry);
        REQUIRE_ELSE(res == KEFIR_OK, {
            KEFIR_FREE(mem, entry);
            return res;
    ...
    if (entry->fragments_length >= entry->fragments_capacity) {
        kefir_size_t new_capacity = MAX(entry->fragments_capacity * 2, 4);
        struct kefir_asmcmp_debug_info_value_fragment *new_fragments =
            KEFIR_REALLOC(mem, entry->fragments, sizeof(struct kefir_asmcmp_debug_info_value_fragment) * new_capacity);
        REQUIRE(new_fragments != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate asmcmp debug info value map fragment"));
        entry->fragments = new_fragments;
        entry->fragments_capacity = new_capacity;
    }
...
static kefir_result_t coalesce_code_map_entry(struct kefir_mem *mem,
                                              struct kefir_asmcmp_debug_info_code_map_entry *entry,
...
kefir_result_t kefir_asmcmp_code_map_coalesce(struct kefir_mem *mem,
                                              struct kefir_asmcmp_debug_info_code_map *code_map) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(code_map != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info code map"));
    ...
    for (res = kefir_hashtable_iter(&code_map->fragments, &iter, &key, &value); res == KEFIR_OK;
...
kefir_result_t kefir_asmcmp_code_map_iter(const struct kefir_asmcmp_debug_info_code_map *code_map,
                                          struct kefir_asmcmp_code_map_iterator *iter,
                                          kefir_asmcmp_debug_info_code_reference_t *code_ref_ptr) {
    REQUIRE(code_map != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info code map"));
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to asmcmp debug info code map iterator"));
...
kefir_result_t kefir_asmcmp_code_map_next(struct kefir_asmcmp_code_map_iterator *iter,
                                          kefir_asmcmp_debug_info_code_reference_t *code_ref_ptr) {
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info code map iterator"));
...
kefir_result_t kefir_asmcmp_code_map_fragment_iter(const struct kefir_asmcmp_debug_info_code_map *code_map,
                                                   kefir_asmcmp_debug_info_code_reference_t code_ref,
                                                   struct kefir_asmcmp_code_map_fragment_iterator *iter,
                                                   const struct kefir_asmcmp_debug_info_code_fragment **fragment_ptr) {
    REQUIRE(code_map != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info code map"));
    REQUIRE(iter != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
                                          "Expected valid pointer to asmcmp debug info code fragment iterator"));
    ...
    iter->entry = entry;
    iter->index = 0;
    REQUIRE(iter->index < entry->fragments_length,
            KEFIR_SET_ERROR(KEFIR_ITERATOR_END, "End of asmcmp debug information code fragments"));
    ASSIGN_PTR(fragment_ptr, &iter->entry->fragments[iter->index]);
    return KEFIR_OK;
}

kefir_result_t kefir_asmcmp_code_map_fragment_next(struct kefir_asmcmp_code_map_fragment_iterator *iter,
                                                   const struct kefir_asmcmp_debug_info_code_fragment **fragment_ptr) {
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info code fragment iterator"));

    iter->index++;
    REQUIRE(iter->index < iter->entry->fragments_length,
            KEFIR_SET_ERROR(KEFIR_ITERATOR_END, "End of asmcmp debug information code fragments"));
    ASSIGN_PTR(fragment_ptr, &iter->entry->fragments[iter->index]);
    return KEFIR_OK;
}

kefir_result_t kefir_asmcmp_value_map_iter(const struct kefir_asmcmp_debug_info_value_map *value_map,
                                           struct kefir_asmcmp_value_map_iterator *iter,
                                           kefir_asmcmp_debug_info_value_reference_t *code_ref_ptr) {
    REQUIRE(value_map != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info value map"));
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to asmcmp debug info value map iterator"));
...
kefir_result_t kefir_asmcmp_value_map_next(struct kefir_asmcmp_value_map_iterator *iter,
                                           kefir_asmcmp_debug_info_value_reference_t *code_ref_ptr) {
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info code map iterator"));
...
kefir_result_t kefir_asmcmp_value_map_fragment_iter(
    const struct kefir_asmcmp_debug_info_value_map *value_map, kefir_asmcmp_debug_info_value_reference_t value_ref,
    struct kefir_asmcmp_value_map_fragment_iterator *iter,
    const struct kefir_asmcmp_debug_info_value_fragment **fragment_ptr) {
    REQUIRE(value_map != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info value map"));
    REQUIRE(iter != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
                                          "Expected valid pointer to asmcmp debug info value fragment iterator"));
    ...
    iter->index = 0;
    REQUIRE(iter->index < entry->fragments_length,
            KEFIR_SET_ERROR(KEFIR_ITERATOR_END, "End of asmcmp debug information value fragments"));
    ASSIGN_PTR(fragment_ptr, &iter->entry->fragments[iter->index]);
    return KEFIR_OK;
}

kefir_result_t kefir_asmcmp_value_map_fragment_next(
    struct kefir_asmcmp_value_map_fragment_iterator *iter,
    const struct kefir_asmcmp_debug_info_value_fragment **fragment_ptr) {
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid asmcmp debug info value fragment iterator"));
