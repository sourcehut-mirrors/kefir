...
kefir_result_t kefir_amd64_sysv_thread_local_reference(struct kefir_codegen_naive_amd64 *codegen,
                                                       const char *identifier, kefir_bool_t local) {
        ...
        if (local) {
            REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_LEA(
                ...
                kefir_asm_amd64_xasmgen_operand_indirect(
                    ...
                    kefir_asm_amd64_xasmgen_operand_label(
                        &codegen->xasmgen_helpers.operands[1],
                        kefir_asm_amd64_xasmgen_helpers_format(&codegen->xasmgen_helpers, KEFIR_AMD64_EMUTLS_V,
                                                               identifier)),
                    0)));
        } else {
            REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_MOV(
