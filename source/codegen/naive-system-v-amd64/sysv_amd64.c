...
#include "kefir/codegen/naive-system-v-amd64/symbolic_labels.h"
#include "kefir/codegen/naive-system-v-amd64/instr.h"
#include "kefir/codegen/naive-system-v-amd64/inline_assembly.h"
#include "kefir/codegen/naive-system-v-amd64/tls.h"
...
static kefir_result_t cg_module_globals(struct kefir_codegen_naive_amd64_sysv_module *module,
...
static kefir_result_t cg_module_externals(struct kefir_codegen_naive_amd64_sysv_module *module,
                                          struct kefir_codegen_naive_amd64 *codegen) {
...
static kefir_result_t cg_module_prologue(struct kefir_codegen_naive_amd64_sysv_module *module,
                                         struct kefir_codegen_naive_amd64 *codegen) {
...
static kefir_result_t cg_function_prologue(struct kefir_codegen_naive_amd64 *codegen,
...
static kefir_result_t cg_function_epilogue(struct kefir_codegen_naive_amd64 *codegen,
                                           const struct kefir_amd64_sysv_function *func) {
    REQUIRE_OK(
...
static kefir_result_t cg_function_body(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
    ...
    REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_LEA(
        &codegen->xasmgen, kefir_asm_amd64_xasmgen_operand_reg(KEFIR_AMD64_SYSV_ABI_PROGRAM_REG),
        kefir_asm_amd64_xasmgen_operand_indirect(
            &codegen->xasmgen_helpers.operands[0],
            kefir_asm_amd64_xasmgen_operand_label(
                &codegen->xasmgen_helpers.operands[1],
                kefir_asm_amd64_xasmgen_helpers_format(&codegen->xasmgen_helpers, KEFIR_AMD64_SYSV_PROCEDURE_BODY_LABEL,
                                                       sysv_func->func->name)),
            0)));
...
static kefir_result_t cg_translate_function_gates(struct kefir_codegen_naive_amd64 *codegen,
    ...
    for (const struct kefir_hashtree_node *node = kefir_hashtree_iter(tree, &iter); node != NULL;
        ...
        ASSIGN_DECL_CAST(struct kefir_abi_amd64_function_decl *, sysv_decl, node->value);
        const struct kefir_ir_function_decl *ir_func_decl;
        REQUIRE_OK(kefir_abi_amd64_function_decl_ir(sysv_decl, &ir_func_decl));
        if (virtualDecl) {
            if (ir_func_decl->name == NULL) {
                ...
                REQUIRE_OK(KEFIR_AMD64_XASMGEN_LABEL(
                    &codegen->xasmgen, KEFIR_AMD64_SYSV_FUNCTION_VIRTUAL_GATE_NAMED_LABEL, ir_func_decl->name));
            }
        } else {
            if (ir_func_decl->name == NULL || ir_func_decl->vararg) {
                ...
                REQUIRE_OK(KEFIR_AMD64_XASMGEN_LABEL(&codegen->xasmgen, KEFIR_AMD64_SYSV_FUNCTION_GATE_NAMED_LABEL,
                                                     ir_func_decl->name, ir_func_decl->id));
            }
        }
...
static kefir_result_t cg_translate_tls_entries(struct kefir_codegen_naive_amd64 *codegen,
...
static kefir_result_t cg_translate_strings(struct kefir_codegen_naive_amd64 *codegen,
                                           struct kefir_codegen_naive_amd64_sysv_module *module) {
...
static kefir_result_t cg_translate_data_storage(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                                struct kefir_codegen_naive_amd64_sysv_module *module,
...
static kefir_result_t cg_translate_emulated_tls(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                                struct kefir_codegen_naive_amd64_sysv_module *module) {
    bool first = true;
    struct kefir_hashtree_node_iterator iter;
    const char *identifier = NULL;
...
static kefir_result_t cg_translate_data(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                        struct kefir_codegen_naive_amd64_sysv_module *module) {
...
static kefir_result_t cg_translate_global_inline_assembly(struct kefir_codegen_naive_amd64 *codegen,
                                                          struct kefir_codegen_naive_amd64_sysv_module *module) {
    struct kefir_hashtree_node_iterator iter;
...
static kefir_result_t cg_translate_inline_assembly_fragments(struct kefir_mem *mem,
                                                             struct kefir_codegen_naive_amd64 *codegen,
                                                             struct kefir_codegen_naive_amd64_sysv_module *module) {
    struct kefir_hashtree_node_iterator iter;
    for (const struct kefir_hashtree_node *node = kefir_hashtree_iter(&module->inline_assembly, &iter); node != NULL;
        ...
        REQUIRE_OK(KEFIR_AMD64_XASMGEN_LABEL(&codegen->xasmgen, KEFIR_AMD64_SYSTEM_V_RUNTIME_INLINE_ASSEMBLY_FRAGMENT,
                                             inline_asm->id));
        REQUIRE_OK(kefir_codegen_naive_amd64_sysv_inline_assembly_embed(mem, module, codegen, inline_asm));
        REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_ADD(
            &codegen->xasmgen, kefir_asm_amd64_xasmgen_operand_reg(KEFIR_AMD64_XASMGEN_REGISTER_RBX),
            kefir_asm_amd64_xasmgen_operand_imm(&codegen->xasmgen_helpers.operands[0], 2 * KEFIR_AMD64_ABI_QWORD)));
        REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_JMP(
...
static kefir_result_t cg_translate(struct kefir_mem *mem, struct kefir_codegen *cg_iface,
    ...
    struct kefir_codegen_naive_amd64 *codegen = (struct kefir_codegen_naive_amd64 *) cg_iface->data;
    struct kefir_codegen_naive_amd64_sysv_module sysv_module;
    REQUIRE_OK(kefir_codegen_naive_amd64_sysv_module_alloc(mem, &sysv_module, opt_module->ir_module));
    ...
    REQUIRE_OK(kefir_codegen_naive_amd64_sysv_module_free(mem, &sysv_module));
    return KEFIR_OK;
}
...
static kefir_result_t cg_close(struct kefir_mem *mem, struct kefir_codegen *cg) {
    ...
    struct kefir_codegen_naive_amd64 *codegen = (struct kefir_codegen_naive_amd64 *) cg->data;
    REQUIRE_OK(KEFIR_AMD64_XASMGEN_CLOSE(mem, &codegen->xasmgen));
    return KEFIR_OK;
}

kefir_result_t kefir_codegen_naive_sysv_amd64_init(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                                   FILE *out, const struct kefir_codegen_configuration *config) {
