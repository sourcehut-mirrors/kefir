#include "kefir/codegen/naive-system-v-amd64/inline_assembly.h"
...
kefir_result_t kefir_codegen_naive_amd64_sysv_inline_assembly_invoke(
    struct kefir_mem *mem, struct kefir_codegen_naive_amd64_sysv_module *sysv_module,
    struct kefir_codegen_naive_amd64 *codegen, const struct kefir_ir_inline_assembly *inline_asm) {
        ...
        if (!parameter_immediate) {
            ...
            switch (asm_param->constraint) {
                ...
                case KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_CONSTRAINT_REGISTER:
                    REQUIRE(param_type != INLINE_ASSEMBLY_PARAM_AGGREGATE || param_size <= KEFIR_AMD64_ABI_QWORD,
                            KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Unable to satisfy IR inline assembly constraints"));
                ...
                case KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_CONSTRAINT_REGISTER_MEMORY:
                    if ((param_type == INLINE_ASSEMBLY_PARAM_NORMAL_SCALAR || param_size <= KEFIR_AMD64_ABI_QWORD) &&
...
static kefir_result_t preserve_dirty_regs(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                          struct inline_assembly_params *params) {
    struct kefir_hashtree_node_iterator iter;
    for (const struct kefir_hashtree_node *node = kefir_hashtree_iter(&params->dirty_regs, &iter); node != NULL;
        ...
        if (inline_assembly_is_register_preserved(reg)) {
            ...
            if (kefir_asm_amd64_xasmgen_register_is_floating_point(reg)) {
                ...
                params->parameter_base_offset += 2 * KEFIR_AMD64_ABI_QWORD;
            } else {
                REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_PUSH(&codegen->xasmgen, kefir_asm_amd64_xasmgen_operand_reg(reg)));
                params->parameter_base_offset += KEFIR_AMD64_ABI_QWORD;
            }
        }
    }
...
static kefir_result_t restore_dirty_regs(struct kefir_codegen_naive_amd64 *codegen,
                                         struct inline_assembly_params *params) {
    if (params->dirty_cc) {
...
static kefir_result_t drop_inputs(struct kefir_codegen_naive_amd64 *codegen, struct inline_assembly_params *params) {
    if (params->num_of_inputs > 0) {
        REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_ADD(
            &codegen->xasmgen, kefir_asm_amd64_xasmgen_operand_reg(KEFIR_AMD64_XASMGEN_REGISTER_RSP),
            kefir_asm_amd64_xasmgen_operand_immu(&codegen->xasmgen_helpers.operands[0],
                                                 KEFIR_AMD64_ABI_QWORD * params->num_of_inputs)));
    }
    return KEFIR_OK;
}
...
static kefir_result_t load_inputs(struct kefir_codegen_naive_amd64 *codegen,
                    ...
                    case INLINE_ASSEMBLY_PARAM_REGISTER_DIRECT:
                        REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_MOV(
                            ...
                            kefir_asm_amd64_xasmgen_operand_indirect(
                                ...
                                params->parameter_base_offset + KEFIR_AMD64_ABI_QWORD * param_map->input_index)));
                ...
                switch (param_map->type) {
                    case INLINE_ASSEMBLY_PARAM_REGISTER_DIRECT: {
                        ...
                        REQUIRE_OK(match_register_to_size(
                            param_map->reg,
                            param_map->direct_read ? MIN(param_map->parameter_read_props.size, KEFIR_AMD64_ABI_QWORD)
...
static kefir_result_t store_register_aggregate_outputs(struct kefir_codegen_naive_amd64 *codegen,
                                                       struct inline_assembly_params *params,
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(&params->register_aggregate_params); iter != NULL;
        ...
        REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_MOV(
            ...
            kefir_asm_amd64_xasmgen_operand_indirect(
                &codegen->xasmgen_helpers.operands[0],
                kefir_asm_amd64_xasmgen_operand_reg(KEFIR_AMD64_XASMGEN_REGISTER_RSP),
                params->parameter_base_offset + KEFIR_AMD64_ABI_QWORD * asm_param->load_store_index +
...
static kefir_result_t store_outputs(struct kefir_codegen_naive_amd64 *codegen,
                                    const struct kefir_ir_inline_assembly *inline_asm,
                                    struct inline_assembly_params *params) {
...
static kefir_result_t format_label_parameter(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                             const struct kefir_ir_inline_assembly *inline_asm,
                                             struct inline_assembly_params *params,
...
static kefir_result_t format_normal_parameter(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                              struct inline_assembly_params *params,
                                              const struct kefir_ir_inline_assembly_parameter *asm_param,
                                              kefir_size_t override_size) {
    if (asm_param->klass == KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_IMMEDIATE) {
    ...
    switch (param_map->type) {
        ...
        case INLINE_ASSEMBLY_PARAM_STACK: {
            kefir_int64_t offset = params->parameter_base_offset + KEFIR_AMD64_ABI_QWORD * param_map->input_index;
            char register_symbol[128];
            const struct kefir_asm_amd64_xasmgen_operand *operand = NULL;
            REQUIRE_OK(
...
static kefir_result_t default_param_modifier(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                             const struct kefir_ir_inline_assembly *inline_asm,
...
static kefir_result_t byte_param_modifier(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                          const struct kefir_ir_inline_assembly *inline_asm,
                                          struct inline_assembly_params *params, const char **input_str) {
    const char *input = *input_str;
    const struct kefir_ir_inline_assembly_parameter *asm_param;
...
static kefir_result_t word_param_modifier(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                          const struct kefir_ir_inline_assembly *inline_asm,
                                          struct inline_assembly_params *params, const char **input_str) {
    const char *input = *input_str;
    const struct kefir_ir_inline_assembly_parameter *asm_param;
...
static kefir_result_t dword_param_modifier(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                           const struct kefir_ir_inline_assembly *inline_asm,
                                           struct inline_assembly_params *params, const char **input_str) {
    const char *input = *input_str;
    const struct kefir_ir_inline_assembly_parameter *asm_param;
...
static kefir_result_t qword_param_modifier(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                           const struct kefir_ir_inline_assembly *inline_asm,
                                           struct inline_assembly_params *params, const char **input_str) {
    const char *input = *input_str;
    const struct kefir_ir_inline_assembly_parameter *asm_param;
...
static kefir_result_t label_param_modifier(struct kefir_mem *mem, struct kefir_codegen_naive_amd64 *codegen,
                                           const struct kefir_ir_inline_assembly *inline_asm,
                                           struct inline_assembly_params *params, const char **input_str) {
    const char *input = *input_str;
    const struct kefir_ir_inline_assembly_parameter *asm_param;
...
static kefir_result_t format_template(struct kefir_mem *mem, struct kefir_codegen_naive_amd64_sysv_module *sysv_module,
...
static kefir_result_t generate_jump_trampolines(struct kefir_codegen_naive_amd64 *codegen,
                                                const struct kefir_ir_inline_assembly *inline_asm,
                                                struct inline_assembly_params *params) {
        ...
        REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_LEA(
            ...
            kefir_asm_amd64_xasmgen_operand_indirect(
                ...
                kefir_asm_amd64_xasmgen_operand_offset(
                    ...
                    2 * KEFIR_AMD64_ABI_QWORD * jump_target->target),
                0)));
        REQUIRE_OK(KEFIR_AMD64_XASMGEN_INSTR_JMP(
...
kefir_result_t kefir_codegen_naive_amd64_sysv_inline_assembly_embed(
    struct kefir_mem *mem, struct kefir_codegen_naive_amd64_sysv_module *sysv_module,
    struct kefir_codegen_naive_amd64 *codegen, const struct kefir_ir_inline_assembly *inline_asm) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(sysv_module != NULL,
