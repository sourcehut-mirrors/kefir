...
kefir_result_t function_gate_removal(struct kefir_mem *mem, struct kefir_hashtree *tree, kefir_hashtree_key_t key,
    ...
    UNUSED(key);
    UNUSED(payload);
    ASSIGN_DECL_CAST(struct kefir_abi_amd64_function_decl *, sysv_decl, value);
    REQUIRE_OK(kefir_abi_amd64_function_decl_free(mem, sysv_decl));
...
kefir_result_t type_layout_removal(struct kefir_mem *mem, struct kefir_hashtree *tree, kefir_hashtree_key_t key,
    ...
    UNUSED(key);
    UNUSED(payload);
    ASSIGN_DECL_CAST(struct kefir_abi_amd64_type_layout *, layout, value);
    REQUIRE_OK(kefir_abi_amd64_type_layout_free(mem, layout));
    KEFIR_FREE(mem, layout);
    return KEFIR_OK;
}

kefir_result_t kefir_codegen_naive_amd64_sysv_module_alloc(struct kefir_mem *mem,
...
kefir_result_t kefir_codegen_naive_amd64_sysv_module_free(struct kefir_mem *mem,
                                                          struct kefir_codegen_naive_amd64_sysv_module *sysv_module) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(sysv_module != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid AMD64 System-V module"));
...
struct kefir_abi_amd64_function_decl *kefir_codegen_naive_amd64_sysv_module_function_decl(
    struct kefir_mem *mem, struct kefir_codegen_naive_amd64_sysv_module *sysv_module, kefir_id_t func_id,
    bool virtualDecl) {
    ...
    if (res == KEFIR_NOT_FOUND) {
        ...
        struct kefir_abi_amd64_function_decl *sysv_decl =
            KEFIR_MALLOC(mem, sizeof(struct kefir_abi_amd64_function_decl));
        REQUIRE(sysv_decl != NULL, NULL);
        res = kefir_abi_amd64_function_decl_alloc(mem, KEFIR_ABI_AMD64_VARIANT_SYSTEM_V, decl, sysv_decl);
        ...
        REQUIRE_ELSE(res == KEFIR_OK, {
            kefir_abi_amd64_function_decl_free(mem, sysv_decl);
            KEFIR_FREE(mem, sysv_decl);
            return NULL;
        });
        return sysv_decl;
    } else if (res == KEFIR_OK) {
        return (struct kefir_abi_amd64_function_decl *) node->value;
    } else {
        return NULL;
    }
}

struct kefir_abi_amd64_type_layout *kefir_codegen_naive_amd64_sysv_module_type_layout(
    struct kefir_mem *mem, struct kefir_codegen_naive_amd64_sysv_module *sysv_module, kefir_id_t id) {
    REQUIRE(mem != NULL, NULL);
    REQUIRE(sysv_module != NULL, NULL);
    ...
    if (res == KEFIR_OK) {
        return (struct kefir_abi_amd64_type_layout *) node->value;
    } else {
        REQUIRE(res == KEFIR_NOT_FOUND, NULL);
    }
    struct kefir_ir_type *type = kefir_ir_module_get_named_type(sysv_module->module, id);
    REQUIRE(type != NULL, NULL);
    struct kefir_abi_amd64_type_layout *layout = KEFIR_MALLOC(mem, sizeof(struct kefir_abi_amd64_type_layout));
    REQUIRE(layout != NULL, NULL);
    res = kefir_abi_amd64_type_layout(mem, KEFIR_ABI_AMD64_VARIANT_SYSTEM_V, type, layout);
    REQUIRE_ELSE(res == KEFIR_OK, {
        KEFIR_FREE(mem, layout);
        return NULL;
    });
...
kefir_result_t kefir_codegen_naive_amd64_sysv_module_declare_tls(
    struct kefir_mem *mem, struct kefir_codegen_naive_amd64_sysv_module *sysv_module, const char *identifier) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
