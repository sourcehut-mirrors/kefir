...
static kefir_result_t free_allocation(struct kefir_mem *mem, struct kefir_hashtable *table, kefir_hashtable_key_t key,
...
static kefir_result_t on_new_instruction(struct kefir_mem *mem, const struct kefir_opt_code_container *code,
    ...
    if (debug_info->next_instruction_code_ref != KEFIR_OPT_CODE_DEBUG_INSTRUCTION_CODE_REF_NONE) {
        ...
        kefir_result_t res =
            kefir_hashtable_at(&debug_info->code_ref_instructions,
                               (kefir_hashtable_key_t) debug_info->next_instruction_code_ref, &table_value);
        ...
        if (res != KEFIR_NOT_FOUND) {
            ...
            REQUIRE(code_ref != NULL,
                    KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate debug information code reference"));
            ...
            REQUIRE_CHAIN(&res, kefir_hashtable_insert(mem, &debug_info->code_ref_instructions,
                                                       (kefir_hashtable_key_t) debug_info->next_instruction_code_ref,
...
static kefir_result_t kefir_opt_code_debug_info_replace_reference(struct kefir_mem *,
...
static kefir_result_t on_replace_instruction(struct kefir_mem *mem, const struct kefir_opt_code_container *code,
                                             kefir_opt_instruction_ref_t to_instr_ref,
                                             kefir_opt_instruction_ref_t from_instr_ref, void *payload) {
    UNUSED(code);
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
...
static kefir_result_t on_drop_instruction(struct kefir_mem *mem, const struct kefir_opt_code_container *code,
    ...
    kefir_hashtable_value_t table_value;
    kefir_result_t res =
        kefir_hashtable_at(&debug_info->instruction_code_refs, (kefir_hashtable_key_t) instr_ref, &table_value);
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
...
kefir_result_t kefir_opt_code_debug_info_next_instruction_code_reference(
    struct kefir_opt_code_debug_info *debug_info, kefir_opt_code_debug_info_code_ref_t code_ref) {
    REQUIRE(debug_info != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer debug information"));
...
kefir_result_t kefir_opt_code_debug_info_instruction_code_reference(
    const struct kefir_opt_code_debug_info *debug_info, kefir_opt_instruction_ref_t instr_ref,
...
kefir_result_t kefir_opt_code_debug_info_code_reference(
    const struct kefir_opt_code_debug_info *debug_info, kefir_opt_code_debug_info_code_ref_t code_ref,
    const struct kefir_opt_code_debug_info_code_reference **code_reference_ptr) {
    REQUIRE(debug_info != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer debug information"));
    ...
    kefir_hashtable_value_t table_value;
    kefir_result_t res =
        kefir_hashtable_at(&debug_info->code_ref_instructions, (kefir_hashtable_key_t) code_ref, &table_value);
    if (res == KEFIR_NOT_FOUND) {
        res =
            KEFIR_SET_ERROR(KEFIR_NOT_FOUND, "Unable to find optimizer debug information code reference instructions");
    }
    REQUIRE_OK(res);
...
kefir_bool_t kefir_opt_code_debug_info_is_active_ref(const struct kefir_opt_code_debug_info *debug_info,
...
kefir_result_t kefir_opt_code_debug_info_add_local_variable_allocation(
    ...
    kefir_hashtable_value_t table_value;
    kefir_result_t res =
        kefir_hashtable_at(&debug_info->local_variables, (kefir_hashtable_key_t) variable_ref, &table_value);
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
        ...
        REQUIRE(local_variable != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate local variable debug information"));
        local_variable->variable_ref = variable_ref;
        res = kefir_hashset_init(&local_variable->allocations, &kefir_hashtable_uint_ops);
        REQUIRE_CHAIN(&res,
                      kefir_hashtable_insert(mem, &debug_info->local_variables, (kefir_hashtable_key_t) variable_ref,
                                             (kefir_hashtable_value_t) local_variable));
        REQUIRE_ELSE(res == KEFIR_OK, {
            KEFIR_FREE(mem, local_variable);
            return res;
        });
...
kefir_result_t kefir_opt_code_debug_info_add_allocation_placement(struct kefir_mem *mem,
                                                                  struct kefir_opt_code_debug_info *debug_info,
                                                                  kefir_opt_instruction_ref_t allocation_ref,
                                                                  kefir_opt_instruction_ref_t placement_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(debug_info != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer debug information"));
    REQUIRE(debug_info->record_debug_info, KEFIR_OK);
    ...
    kefir_hashtable_value_t table_value;
    kefir_result_t res =
        kefir_hashtable_at(&debug_info->allocations, (kefir_hashtable_key_t) allocation_ref, &table_value);
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
        ...
        REQUIRE(placement != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate allocation placement debug information"));
        placement->allocation_ref = allocation_ref;
        res = kefir_hashset_init(&placement->placement, &kefir_hashtable_uint_ops);
        REQUIRE_CHAIN(&res,
                      kefir_hashtable_insert(mem, &debug_info->allocations, (kefir_hashtable_key_t) allocation_ref,
                                             (kefir_hashtable_value_t) placement));
        REQUIRE_ELSE(res == KEFIR_OK, {
            KEFIR_FREE(mem, placement);
            return res;
        });
...
kefir_result_t kefir_opt_code_debug_info_local_variable(
    const struct kefir_opt_code_debug_info *debug_info, kefir_opt_code_debug_info_local_variable_ref_t variable_ref,
    const struct kefir_opt_code_debug_info_local_variable **local_variable_ptr) {
    REQUIRE(debug_info != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer debug information"));

    kefir_hashtable_value_t table_value;
    kefir_result_t res =
        kefir_hashtable_at(&debug_info->local_variables, (kefir_hashtree_key_t) variable_ref, &table_value);
    if (res == KEFIR_NOT_FOUND) {
...
static kefir_result_t kefir_opt_code_debug_info_replace_reference(struct kefir_mem *mem,
    ...
    if (replacement_instr_ref != KEFIR_ID_NONE) {
        for (res = kefir_hashtable_iter(&debug_info->local_variables, &iter, NULL, &table_value); res == KEFIR_OK;
    ...
    for (res = kefir_hashtable_iter(&debug_info->allocations, &iter, NULL, &table_value); res == KEFIR_OK;
         res = kefir_hashtable_next(&iter, NULL, &table_value)) {
        ASSIGN_DECL_CAST(struct kefir_opt_code_debug_info_allocation_placement *, placement, table_value);
    ...
    if (res != KEFIR_NOT_FOUND) {
        if (replacement_instr_ref != KEFIR_ID_NONE) {
            ...
            *table_value_ptr = (kefir_hashtable_value_t) NULL;
            ...
            REQUIRE_CHAIN(&res, kefir_hashtable_insert(mem, &debug_info->allocations,
                                                       (kefir_hashtable_key_t) replacement_instr_ref,
                                                       (kefir_hashtable_value_t) placement));
            REQUIRE_ELSE(res == KEFIR_OK, {
...
kefir_result_t kefir_opt_code_debug_info_allocation_placement_iter(
    ...
    struct kefir_opt_code_debug_info_allocation_placement_iterator *iter,
    const struct kefir_opt_code_debug_info_allocation_placement **placement_ptr) {
    REQUIRE(debug_info != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer debug information"));
    REQUIRE(iter != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
