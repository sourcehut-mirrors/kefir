...
kefir_result_t kefir_opt_code_memssa_init(struct kefir_opt_code_memssa *memssa) {
    ...
    memssa->instruction_bindings = NULL;
    memssa->instruction_bindings_length = 0;
    return KEFIR_OK;
}
...
kefir_result_t kefir_opt_code_memssa_free(struct kefir_mem *mem, struct kefir_opt_code_memssa *memssa) {
    ...
    KEFIR_FREE(mem, memssa->instruction_bindings);
    for (kefir_size_t i = 0; i < memssa->node_length; i++) {
        ...
        KEFIR_FREE(mem, memssa->node_uses[i].uses);
    }
    KEFIR_FREE(mem, memssa->nodes);
    KEFIR_FREE(mem, memssa->node_uses);
    KEFIR_FREE(mem, memssa->blocks);
    return KEFIR_OK;
}

kefir_result_t kefir_opt_code_memssa_reset(struct kefir_mem *mem, struct kefir_opt_code_memssa *memssa) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(memssa != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer memory ssa"));
...
kefir_result_t kefir_opt_code_memssa_provision(struct kefir_mem *mem, struct kefir_opt_code_memssa *memssa,
                                               kefir_size_t length) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(memssa != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer memory ssa"));
    REQUIRE(length > memssa->instruction_bindings_length, KEFIR_OK);

    kefir_opt_code_memssa_node_ref_t *new_refs =
        KEFIR_REALLOC(mem, memssa->instruction_bindings, sizeof(kefir_opt_code_memssa_node_ref_t) * length);
    REQUIRE(new_refs != NULL,
            KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate memory SSA instruction bindings"));
    memset(&new_refs[memssa->instruction_bindings_length], -1,
           sizeof(kefir_opt_code_memssa_node_ref_t) * (length - memssa->instruction_bindings_length));

    memssa->instruction_bindings = new_refs;
    memssa->instruction_bindings_length = length;
    return KEFIR_OK;
}
...
static kefir_result_t new_node(struct kefir_mem *mem, struct kefir_opt_code_memssa *memssa,
    ...
    if (memssa->node_length + 1 >= memssa->node_capacity) {
        ...
        struct kefir_opt_code_memssa_node_uses *new_uses =
            KEFIR_REALLOC(mem, memssa->node_uses, sizeof(struct kefir_opt_code_memssa_node_uses) * new_capacity);
        REQUIRE(new_uses != NULL, KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate memory ssa node uses"));
        memssa->node_uses = new_uses;
...
kefir_result_t kefir_opt_code_memssa_new_root_node(struct kefir_mem *mem, struct kefir_opt_code_memssa *memssa,
    ...
    memssa->node_uses[node_ref].uses = NULL;
    memssa->node_uses[node_ref].uses_length = 0;
    memssa->node_length++;
...
static kefir_result_t add_use(struct kefir_mem *mem, struct kefir_opt_code_memssa_node_uses *uses,
                              kefir_opt_code_memssa_node_ref_t node_ref) {
    kefir_size_t new_length = uses->uses_length + 1;
    kefir_opt_code_memssa_node_ref_t *new_uses =
        KEFIR_REALLOC(mem, uses->uses, sizeof(kefir_opt_code_memssa_node_ref_t) * new_length);
    REQUIRE(new_uses != NULL, KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate memssa node uses"));

    uses->uses = new_uses;
    uses->uses_length = new_length;
    uses->uses[new_length - 1] = node_ref;
    return KEFIR_OK;
}
...
kefir_result_t kefir_opt_code_memssa_new_consume_node(struct kefir_mem *mem, struct kefir_opt_code_memssa *memssa,
    ...
    REQUIRE(predecessor_ref == KEFIR_ID_NONE || predecessor_ref < memssa->node_length,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer memory ssa node reference"));
    REQUIRE(instr_ref == KEFIR_ID_NONE || KEFIR_OPT_INSTR_REF_INDEX_OF(instr_ref) < memssa->instruction_bindings_length,
            KEFIR_SET_ERROR(KEFIR_OUT_OF_BOUNDS, "Optimizer instruction out of memory SSA bounds"));
    REQUIRE(instr_ref == KEFIR_ID_NONE ||
                memssa->instruction_bindings[KEFIR_OPT_INSTR_REF_INDEX_OF(instr_ref)] == KEFIR_ID_NONE,
    ...
    if (predecessor_ref != KEFIR_ID_NONE) {
        REQUIRE_OK(add_use(mem, &memssa->node_uses[predecessor_ref], node_ref));
    }
    if (instr_ref != KEFIR_ID_NONE) {
        memssa->instruction_bindings[KEFIR_OPT_INSTR_REF_INDEX_OF(instr_ref)] = node_ref;
    }
    memssa->node_length++;
    ASSIGN_PTR(node_ref_ptr, node_ref);
    return KEFIR_OK;
...
kefir_result_t kefir_opt_code_memssa_new_produce_consume_node(struct kefir_mem *mem,
    ...
    memssa->nodes[node_ref].type = KEFIR_OPT_CODE_MEMSSA_PRODUCE_CONSUME_NODE;
    memssa->nodes[node_ref].predecessor_ref = predecessor_ref;
    memssa->nodes[node_ref].instr_ref = instr_ref;
    memssa->node_uses[node_ref].uses = NULL;
    memssa->node_uses[node_ref].uses_length = 0;
...
kefir_result_t kefir_opt_code_memssa_phi_attach(struct kefir_mem *mem, struct kefir_opt_code_memssa *memssa,
    ...
    if (node_ref != KEFIR_ID_NONE) {
        REQUIRE_OK(add_use(mem, &memssa->node_uses[node_ref], phi_node_ref));
    }
    return KEFIR_OK;
}
...
kefir_result_t kefir_opt_code_memssa_instruction_binding(const struct kefir_opt_code_memssa *memssa,
                                                         ...
                                                         kefir_opt_code_memssa_node_ref_t *node_ref) {
    REQUIRE(memssa != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer memory ssa"));
    REQUIRE(KEFIR_OPT_INSTR_REF_INDEX_OF(instr_ref) < memssa->instruction_bindings_length,
            KEFIR_SET_ERROR(KEFIR_NOT_FOUND, "Unable to find memssa instruction binding"));

    kefir_opt_code_memssa_node_ref_t ref = memssa->instruction_bindings[KEFIR_OPT_INSTR_REF_INDEX_OF(instr_ref)];
    REQUIRE(ref != KEFIR_ID_NONE, KEFIR_SET_ERROR(KEFIR_NOT_FOUND, "Unable to find memssa instruction binding"));
    ASSIGN_PTR(node_ref, ref);
    return KEFIR_OK;
}
...
static kefir_result_t update_node_ref(struct kefir_mem *mem, struct kefir_opt_code_memssa *memssa,
                                      kefir_opt_code_memssa_node_ref_t to_node_ref,
                                      kefir_opt_code_memssa_node_ref_t from_node_ref,
                                      kefir_opt_instruction_ref_t user_ref,
                                      kefir_opt_code_memssa_node_ref_t *at_node_ref) {
    if (*at_node_ref == from_node_ref) {
        *at_node_ref = to_node_ref;

        REQUIRE_OK(add_use(mem, &memssa->node_uses[to_node_ref], user_ref));
    }
    return KEFIR_OK;
}

kefir_result_t kefir_opt_code_memssa_replace(struct kefir_mem *mem, struct kefir_opt_code_memssa *memssa,
                                             kefir_opt_code_memssa_node_ref_t to_node_ref,
                                             kefir_opt_code_memssa_node_ref_t from_node_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(memssa != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer memory ssa"));
    REQUIRE(from_node_ref != KEFIR_ID_NONE && from_node_ref < memssa->node_length,
            KEFIR_SET_ERROR(KEFIR_OUT_OF_BOUNDS, "Expected valid memory ssa node reference"));
    REQUIRE(to_node_ref != KEFIR_ID_NONE && to_node_ref < memssa->node_length,
            KEFIR_SET_ERROR(KEFIR_OUT_OF_BOUNDS, "Expected valid memory ssa node reference"));
    REQUIRE(from_node_ref != to_node_ref, KEFIR_OK);

    struct kefir_opt_code_memssa_node_uses *from_node = &memssa->node_uses[from_node_ref];

    for (kefir_size_t i = 0; i < from_node->uses_length; i++) {
        kefir_opt_code_memssa_node_ref_t use_ref = from_node->uses[i];

        struct kefir_opt_code_memssa_node *use_node = &memssa->nodes[use_ref];
        switch (use_node->type) {
            case KEFIR_OPT_CODE_MEMSSA_ROOT_NODE:
            ...
            case KEFIR_OPT_CODE_MEMSSA_CONSUME_NODE:
                REQUIRE_OK(
                    update_node_ref(mem, memssa, to_node_ref, from_node_ref, use_ref, &use_node->predecessor_ref));
                break;

            case KEFIR_OPT_CODE_MEMSSA_PHI_NODE:
                for (kefir_size_t i = 0; i < use_node->phi.link_count; i++) {
                    REQUIRE_OK(update_node_ref(mem, memssa, to_node_ref, from_node_ref, use_ref,
                                               &use_node->phi.links[i].node_ref));
                }
                break;
        }
    }
    KEFIR_FREE(mem, from_node->uses);
    from_node->uses = NULL;
    from_node->uses_length = 0;
    return KEFIR_OK;
}

static kefir_result_t delete_use(struct kefir_opt_code_memssa_node_uses *uses,
                                 kefir_opt_code_memssa_node_ref_t node_ref) {
    for (kefir_size_t i = 0; i < uses->uses_length; i++) {
        if (uses->uses[i] == node_ref) {
            if (i + 1 < uses->uses_length) {
                memmove(&uses->uses[i], &uses->uses[i + 1],
                        sizeof(kefir_opt_code_memssa_node_ref_t) * (uses->uses_length - i - 1));
            }
            uses->uses_length--;
            break;
        }
...
kefir_result_t kefir_opt_code_memssa_unbind(struct kefir_mem *mem, struct kefir_opt_code_memssa *memssa,
                                            kefir_opt_code_memssa_node_ref_t node_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(memssa != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer memory ssa"));
    REQUIRE(node_ref != KEFIR_ID_NONE && node_ref < memssa->node_length,
    ...
    struct kefir_opt_code_memssa_node *node = &memssa->nodes[node_ref];
    struct kefir_opt_code_memssa_node_uses *node_uses = &memssa->node_uses[node_ref];
    REQUIRE(node_uses->uses_length == 0,
            KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Expected memssa node to have zero uses before unbinding"));
    if (node->instr_ref != KEFIR_ID_NONE) {
        switch (node->type) {
            case KEFIR_OPT_CODE_MEMSSA_ROOT_NODE:
                // Intentionally left blank
                break;
            ...
            case KEFIR_OPT_CODE_MEMSSA_PRODUCE_CONSUME_NODE:
            case KEFIR_OPT_CODE_MEMSSA_CONSUME_NODE:
                REQUIRE_OK(delete_use(&memssa->node_uses[node->predecessor_ref], node_ref));
                node->predecessor_ref = KEFIR_ID_NONE;
                if (node->instr_ref != KEFIR_ID_NONE) {
                    memssa->instruction_bindings[KEFIR_OPT_INSTR_REF_INDEX_OF(node->instr_ref)] = KEFIR_ID_NONE;
                }
            ...
            case KEFIR_OPT_CODE_MEMSSA_PHI_NODE:
                for (kefir_size_t i = 0; i < node->phi.link_count; i++) {
                    REQUIRE_OK(delete_use(&memssa->node_uses[node->phi.links[i].node_ref], node_ref));
                }
                KEFIR_FREE(mem, node->phi.links);
                node->phi.links = NULL;
...
kefir_result_t kefir_opt_code_memssa_use_iter(const struct kefir_opt_code_memssa *memssa,
    ...
    iter->uses = &memssa->node_uses[node_ref];
    iter->index = 0;

    REQUIRE(iter->index < iter->uses->uses_length,
            KEFIR_SET_ERROR(KEFIR_ITERATOR_END, "End of memory ssa node use iterator"));
    kefir_opt_code_memssa_node_ref_t use_ref = iter->uses->uses[iter->index++];
    ASSIGN_PTR(node_ref_ptr, use_ref);
    return KEFIR_OK;
}
