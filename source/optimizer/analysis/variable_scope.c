...
static kefir_result_t free_block_scopes(struct kefir_mem *mem, struct kefir_hashtree *tree, kefir_hashtree_key_t key,
    ...
    ASSIGN_DECL_CAST(struct block_variable_scopes *, scopes, value);
...
static kefir_result_t free_scope_variables(struct kefir_mem *mem, struct kefir_hashtree *tree, kefir_hashtree_key_t key,
    ...
    ASSIGN_DECL_CAST(struct kefir_opt_code_scope_variables *, variables, value);
...
static kefir_result_t process_scope(struct kefir_mem *mem, const struct kefir_opt_code_container *code,
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        REQUIRE_CHAIN(&res, kefir_hashtree_insert(mem, &state->block_scopes, (kefir_hashtree_key_t) block_ref,
...
static kefir_result_t build_scope_interferences(struct kefir_mem *mem, struct kefir_opt_code_variable_scopes *scopes,
    ...
    for (struct kefir_hashtree_node *node = kefir_hashtree_iter(&state->block_scopes, &block_iter); node != NULL;
         node = kefir_hashtree_next(&block_iter)) {
        ASSIGN_DECL_CAST(const struct block_variable_scopes *, block_scopes, node->value);
        ...
        for (res = kefir_hashset_iter(&block_scopes->scopes, &iter, &entry); res == KEFIR_OK;
            ...
            for (res = kefir_hashset_iter(&state->global_scopes, &iter2, &entry2); res == KEFIR_OK;
                 res = kefir_hashset_next(&iter2, &entry2)) {
                ASSIGN_DECL_CAST(kefir_opt_instruction_ref_t, scope_ref2, entry2);
    ...
    kefir_hashset_key_t entry;
    struct kefir_hashset_iterator iter;
    for (res = kefir_hashset_iter(&state->global_scopes, &iter, &entry); res == KEFIR_OK;
         res = kefir_hashset_next(&iter, &entry)) {
        ASSIGN_DECL_CAST(kefir_opt_instruction_ref_t, scope_ref, entry);
...
static kefir_result_t scopes_build_impl(struct kefir_mem *mem, struct kefir_opt_code_variable_scopes *scopes,
                                        const struct kefir_opt_code_liveness *liveness, struct state *state) {
    kefir_result_t res;
    ...
    for (kefir_opt_block_id_t block_id = 0; block_id < block_count; block_id++) {
        ...
        for (res = kefir_hashset_iter(&liveness->blocks[block_id].alive_instr, &iter, &entry); res == KEFIR_OK;
            ...
            if (instr->operation.opcode == KEFIR_OPT_OPCODE_LOCAL_SCOPE) {
                ...
                struct kefir_hashtree_node *node;
                res = kefir_hashtree_at(&scopes->scope_variables,
                                        (kefir_hashtree_key_t) instr->operation.parameters.refs[0], &node);
                if (res != KEFIR_NOT_FOUND) {
                    REQUIRE_OK(res);
                    ...
                    REQUIRE_CHAIN(&res,
                                  kefir_hashtree_insert(mem, &scopes->scope_variables,
                                                        (kefir_hashtree_key_t) instr->operation.parameters.refs[0],
                                                        (kefir_hashtree_value_t) variables));
                    REQUIRE_ELSE(res == KEFIR_OK, {
                        KEFIR_FREE(mem, variables);
                        return res;
                    });
