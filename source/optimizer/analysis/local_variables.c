...
static kefir_result_t free_variable_liveness(struct kefir_mem *mem, struct kefir_hashtree *tree, kefir_hashtree_key_t key, kefir_hashtree_value_t value, void *payload) {
    ...
    REQUIRE_OK(kefir_bucketset_free(mem, &liveness->local_conflicts));
...
kefir_result_t kefir_opt_code_variable_conflicts_init(struct kefir_opt_code_variable_conflicts *vars) {
    ...
    REQUIRE_OK(kefir_bucketset_init(&vars->globally_alive, &kefir_bucketset_uint_ops));
    return KEFIR_OK;
}
...
kefir_result_t kefir_opt_code_variable_conflicts_free(struct kefir_mem *mem, struct kefir_opt_code_variable_conflicts *vars) {
    ...
    REQUIRE_OK(kefir_bucketset_free(mem, &vars->globally_alive));
    return KEFIR_OK;
}
...
static kefir_result_t process_instr(struct kefir_mem *mem, struct kefir_opt_code_variable_conflicts *vars, const struct kefir_opt_code_liveness *liveness, kefir_opt_block_id_t block_id, kefir_opt_instruction_ref_t instr_ref) {
    REQUIRE(!kefir_bucketset_has(&vars->globally_alive, (kefir_bucketset_entry_t) instr_ref), KEFIR_OK);
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        if (has_local_markings) {
            ...
            res = kefir_bucketset_init(&local_conflicts->local_conflicts, &kefir_bucketset_uint_ops);
            ...
            REQUIRE_OK(kefir_bucketset_add(mem, &vars->globally_alive, (kefir_bucketset_entry_t) instr_ref));
        }
    }
    ...
    if (local_conflicts != NULL) {
        ...
        for (res = kefir_bucketset_iter(&liveness->blocks[block_id].alive_instr,  &iter, &entry);
            res == KEFIR_OK;
            ...
            if (has_local_markings) {
                REQUIRE_OK(kefir_bucketset_add(mem, &local_conflicts->local_conflicts, entry));
            }
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
