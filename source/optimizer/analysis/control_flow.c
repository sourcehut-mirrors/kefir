...
static kefir_result_t redirect_edge(struct kefir_mem *mem, struct kefir_opt_code_control_flow *control_flow,
    ...
    REQUIRE_OK(kefir_hashset_delete(&control_flow->blocks[old_source_block_id].successors,
                                    (kefir_hashset_key_t) target_block_id));
    REQUIRE_OK(kefir_hashset_add(mem, &control_flow->blocks[new_source_block_id].successors,
                                 (kefir_hashset_key_t) target_block_id));

    if (kefir_hashset_has(&control_flow->blocks[target_block_id].predecessors,
                          (kefir_hashset_key_t) old_source_block_id)) {
        REQUIRE_OK(kefir_hashset_delete(&control_flow->blocks[target_block_id].predecessors,
                                        (kefir_hashset_key_t) old_source_block_id));
        REQUIRE_OK(kefir_hashset_add(mem, &control_flow->blocks[target_block_id].predecessors,
                                     (kefir_hashset_key_t) new_source_block_id));
    }
    ...
    if (control_flow->blocks[target_block_id].immediate_dominator == old_source_block_id) {
        ...
        kefir_hashtable_value_t table_value;
        kefir_result_t res = kefir_hashtable_at(&control_flow->dominator_tree,
                                                (kefir_hashtable_key_t) old_source_block_id, &table_value);
        if (res != KEFIR_NOT_FOUND) {
            REQUIRE_OK(res);
            ASSIGN_DECL_CAST(struct dominator_tree_node *, node, table_value);
            REQUIRE_OK(kefir_hashset_delete(&node->children, (kefir_hashset_key_t) target_block_id));
        }
        REQUIRE_OK(update_dominator_tree(mem, control_flow, new_source_block_id, target_block_id));
...
static kefir_result_t drop_edge(struct kefir_opt_code_control_flow *control_flow, kefir_opt_block_id_t src_block_id,
                                kefir_opt_block_id_t dst_block_id) {
    REQUIRE_OK(
        kefir_hashset_delete(&control_flow->blocks[src_block_id].successors, (kefir_hashset_key_t) dst_block_id));
    REQUIRE_OK(
        kefir_hashset_delete(&control_flow->blocks[dst_block_id].predecessors, (kefir_hashset_key_t) src_block_id));
    return KEFIR_OK;
}
...
kefir_result_t kefir_opt_code_control_flow_modify_bypass_block(struct kefir_mem *mem,
    ...
    REQUIRE(
        kefir_hashset_has(&control_flow->blocks[block_id].successors, (kefir_hashset_key_t) bypassed_block_id),
        KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Expected bypassed optimizer block to be a successor of the other one"));

    REQUIRE_OK(drop_edge(control_flow, block_id, bypassed_block_id));

    kefir_result_t res;
    struct kefir_hashset_iterator iter;
    kefir_hashset_key_t entry;
    for (res = kefir_hashset_iter(&control_flow->blocks[bypassed_block_id].successors, &iter, &entry); res == KEFIR_OK;
