...
kefir_result_t kefir_opt_constructor_free(struct kefir_mem *mem, struct kefir_opt_constructor_state *state) {
    ...
    REQUIRE_OK(kefir_hashtable_free(mem, &state->local_lifetime_marks_per_block));
...
static kefir_result_t stack_ensure_depth(struct kefir_mem *mem, struct kefir_opt_constructor_state *state,
    ...
    while (kefir_list_length(&state->current_block->stack) < depth) {
        ...
        REQUIRE_OK(
            kefir_opt_code_container_new_phi(mem, &state->function->code, state->current_block->block_id, &instr_ref));
        REQUIRE_OK(kefir_list_insert_after(mem, &state->current_block->stack, NULL, (void *) (kefir_uptr_t) instr_ref));
        REQUIRE_OK(
            kefir_list_insert_after(mem, &state->current_block->phi_stack, NULL, (void *) (kefir_uptr_t) instr_ref));
    }
    return KEFIR_OK;
}
...
kefir_result_t kefir_opt_constructor_get_local_allocation(
    struct kefir_mem *mem, struct kefir_opt_constructor_state *state, kefir_opt_instruction_ref_t scope_ref,
    kefir_uint64_t variable_id, kefir_id_t type_id, kefir_size_t type_index, kefir_opt_instruction_ref_t *instr_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(state != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer constructor state"));
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        REQUIRE(
            instr->operation.opcode == KEFIR_OPT_OPCODE_ALLOC_LOCAL && instr->operation.parameters.refs[0] == scope_ref,
            KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Scope mismatch in local variable allocation construction"));
...
kefir_result_t kefir_opt_constructor_get_local_scope(struct kefir_mem *mem, struct kefir_opt_constructor_state *state,
                                                     kefir_uint64_t scope_id, kefir_opt_instruction_ref_t *instr_ref) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(state != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer constructor state"));
    REQUIRE(state->entry_block != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Expected valid current optimizer code block state"));
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        *instr_ref = (kefir_opt_instruction_ref_t) node->value;
        ...
        REQUIRE_OK(
            kefir_opt_code_builder_local_scope(mem, &state->function->code, state->entry_block->block_id, &scope_ref));
        REQUIRE_OK(kefir_hashtree_insert(mem, &state->local_scopes, (kefir_hashtree_key_t) scope_id,
                                         (kefir_hashtree_value_t) scope_ref));
        *instr_ref = scope_ref;
    }
