...
static kefir_result_t map_block(struct do_inline_param *param, kefir_opt_block_id_t block_id, kefir_opt_block_id_t *mapped_block_id_ptr) {
    ...
    REQUIRE_OK(kefir_hashtree_insert(param->mem, &param->block_mapping, (kefir_hashtree_key_t) block_id, (kefir_hashtree_value_t) mapped_block_id));
...
static kefir_result_t get_instr_ref_mapping(struct do_inline_param *param, kefir_opt_instruction_ref_t instr_ref, kefir_opt_instruction_ref_t *mapped_instr_ref) {
...
static kefir_result_t inline_operation_ref1(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    ...
    REQUIRE_OK(kefir_opt_code_container_new_instruction(param->mem, param->dst_code, mapped_block_id, &(struct kefir_opt_operation) {
...
static kefir_result_t inline_operation_ref2(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_branch(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
...
static kefir_result_t inline_operation_branch_compare(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id, mapped_target_block_id, mapped_alternative_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
    REQUIRE_OK(map_block(param, instr->operation.parameters.branch.target_block, &mapped_target_block_id));
    REQUIRE_OK(map_block(param, instr->operation.parameters.branch.alternative_block, &mapped_alternative_block_id));
    ...
    REQUIRE_OK(kefir_opt_code_container_new_instruction(param->mem, param->dst_code, mapped_block_id, &(struct kefir_opt_operation) {
        ...
        .parameters = {
            ...
            .branch = {
                .comparison.operation = instr->operation.parameters.branch.comparison.operation,
                .target_block = mapped_target_block_id,
                .alternative_block = mapped_alternative_block_id
            }
        }
...
static kefir_result_t inline_operation_call_ref(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));

    const struct kefir_opt_call_node *src_call_node;
    REQUIRE_OK(kefir_opt_code_container_call(&param->src_function->code, instr->operation.parameters.function_call.call_ref, &src_call_node));
    ...
    REQUIRE_OK(kefir_opt_code_container_new_call(param->mem, param->dst_code, mapped_block_id, src_call_node->function_declaration_id,
        src_call_node->argument_count, mapped_ref1, &dst_call_ref, mapped_instr_ref_ptr));
    ...
    for (kefir_size_t i = 0; i < src_call_node->argument_count; i++) {
        ...
        REQUIRE_OK(kefir_opt_code_container_call_set_argument(param->mem, param->dst_code, dst_call_ref, i, mapped_ref1));
    }
    return KEFIR_OK;
}

static kefir_result_t inline_operation_immediate(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
    if (instr->operation.opcode == KEFIR_OPT_OPCODE_BLOCK_LABEL) {
        ...
        REQUIRE_OK(kefir_opt_code_container_new_instruction(param->mem, param->dst_code, mapped_block_id, &(struct kefir_opt_operation) {
            .opcode = instr->operation.opcode,
            .parameters = {
                .imm.block_ref = mapped_block_label_id
            }
        }, mapped_instr_ref_ptr));
    } else {
        REQUIRE_OK(kefir_opt_code_container_new_instruction(param->mem, param->dst_code, mapped_block_id, &(struct kefir_opt_operation) {
...
static kefir_result_t inline_operation_none(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_variable(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_type(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_ref_offset(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_load_mem(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_store_mem(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_typed_ref1(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_typed_ref2(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_compare_ref2(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_bitfield(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_stack_alloc(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_atomic_op(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_overflow_arith(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_inline_asm(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));

    const struct kefir_opt_inline_assembly_node *src_inline_asm_node;
    REQUIRE_OK(kefir_opt_code_container_inline_assembly(&param->src_function->code, instr->operation.parameters.inline_asm_ref, &src_inline_asm_node));

    kefir_opt_instruction_ref_t inline_asm_instr_ref;
    kefir_opt_inline_assembly_id_t inline_asm_ref;
    REQUIRE_OK(kefir_opt_code_container_new_inline_assembly(param->mem, param->dst_code, mapped_block_id, src_inline_asm_node->inline_asm_id, src_inline_asm_node->parameter_count,
        &inline_asm_ref, &inline_asm_instr_ref));
    ...
    for (kefir_size_t i = 0; i < src_inline_asm_node->parameter_count; i++) {
        ...
        REQUIRE_OK(kefir_opt_code_container_inline_assembly_set_parameter(param->mem, param->dst_code, inline_asm_ref, i, &(struct kefir_opt_inline_assembly_parameter) {
            .load_store_ref = mapped_ref1,
            .read_ref = mapped_ref2
        }));
    }
    ...
    if (src_inline_asm_node->default_jump_target != KEFIR_ID_NONE) {
        ...
        REQUIRE_OK(kefir_opt_code_container_inline_assembly_set_default_jump_target(param->dst_code, inline_asm_ref, mapped_target_block_id));
    }
    ...
    for (struct kefir_hashtree_node *node = kefir_hashtree_iter(&src_inline_asm_node->jump_targets, &iter);
        ...
        ASSIGN_DECL_CAST(kefir_id_t, target_id,
            node->key);
        ASSIGN_DECL_CAST(kefir_opt_block_id_t, target_block_id,
            node->value);
        kefir_opt_block_id_t mapped_target_block_id;
        REQUIRE_OK(map_block(param, target_block_id, &mapped_target_block_id));
        REQUIRE_OK(kefir_opt_code_container_inline_assembly_add_jump_target(param->mem, param->dst_code, inline_asm_ref, target_id, mapped_target_block_id));
    }
    return KEFIR_OK;
}

static kefir_result_t inline_operation_phi_ref(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));

    kefir_opt_phi_id_t phi_ref;
    REQUIRE_OK(kefir_opt_code_container_new_phi(param->mem, param->dst_code, mapped_block_id, &phi_ref, mapped_instr_ref_ptr));
    return KEFIR_OK;
}

static kefir_result_t inline_operation_index(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t generate_placeholder(struct kefir_mem *mem, const struct kefir_opt_module *module, struct kefir_opt_code_container *code, kefir_opt_block_id_t block_id, kefir_id_t type_id, kefir_size_t type_index, kefir_opt_instruction_ref_t insert_after, kefir_opt_instruction_ref_t *instr_ref_ptr) {
    ...
    switch (ir_typeentry->typecode) {
        ...
        case KEFIR_IR_TYPE_WORD:
            REQUIRE_OK(kefir_opt_code_builder_int_placeholder(mem, code, block_id, instr_ref_ptr));
            break;

        case KEFIR_IR_TYPE_FLOAT32:
            REQUIRE_OK(kefir_opt_code_builder_float32_placeholder(mem, code, block_id, instr_ref_ptr));
            break;

        case KEFIR_IR_TYPE_FLOAT64:
            REQUIRE_OK(kefir_opt_code_builder_float64_placeholder(mem, code, block_id, instr_ref_ptr));
            break;

        case KEFIR_IR_TYPE_COMPLEX_FLOAT32:
            REQUIRE_OK(kefir_opt_code_builder_float32_placeholder(mem, code, block_id, instr_ref_ptr));
            REQUIRE_OK(kefir_opt_code_builder_complex_float32_from(mem, code, block_id, *instr_ref_ptr, *instr_ref_ptr, instr_ref_ptr));
            break;

        case KEFIR_IR_TYPE_COMPLEX_FLOAT64:
            REQUIRE_OK(kefir_opt_code_builder_float64_placeholder(mem, code, block_id, instr_ref_ptr));
            REQUIRE_OK(kefir_opt_code_builder_complex_float64_from(mem, code, block_id, *instr_ref_ptr, *instr_ref_ptr, instr_ref_ptr));
            break;

        default:
            REQUIRE_OK(kefir_opt_code_builder_alloc_local(mem, code, block_id,
                type_id, 0, instr_ref_ptr));
            REQUIRE_OK(kefir_opt_code_builder_zero_memory(mem, code, block_id, *instr_ref_ptr,
                type_id, 0,
                &zero_instr_ref));
...
static kefir_result_t inline_return(struct do_inline_param *param, const struct kefir_opt_instruction *instr, kefir_opt_instruction_ref_t *mapped_instr_ref) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));

    if (kefir_ir_type_length(param->src_function->ir_func->declaration->result) > 0) {
        const struct kefir_ir_typeentry *ir_typeentry = kefir_ir_type_at(param->src_function->ir_func->declaration->result, 0);
        REQUIRE(ir_typeentry != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Unable to retrieve IR type entry"));

        if (param->result_phi_ref == KEFIR_ID_NONE) {
            REQUIRE_OK(kefir_opt_code_container_new_phi(param->mem, param->dst_code, param->inline_successor_block_id, &param->result_phi_ref, &param->result_phi_instr));
        }
        ...
        if (instr->operation.parameters.refs[0] != KEFIR_ID_NONE) {
            ...
            REQUIRE_OK(generate_placeholder(param->mem, param->module, param->dst_code, mapped_block_id, param->src_function->ir_func->declaration->result_type_id, 0, mapped_block->control_flow.tail, mapped_instr_ref));
        }
        REQUIRE_OK(kefir_opt_code_container_phi_attach(param->mem, param->dst_code, param->result_phi_ref, mapped_block_id, *mapped_instr_ref));
    }
    REQUIRE_OK(kefir_opt_code_builder_finalize_jump(param->mem, param->dst_code, mapped_block_id, param->inline_successor_block_id, mapped_instr_ref));

    return KEFIR_OK;
}

static kefir_result_t do_inline_instr(kefir_opt_instruction_ref_t instr_ref, void *payload) {
    ASSIGN_DECL_CAST(struct do_inline_param *, param,
        payload);
    ...
    if (instr->operation.opcode == KEFIR_OPT_OPCODE_RETURN) {
        ...
        REQUIRE(instr->operation.parameters.index < call_node->argument_count,
            KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Mismatch between inlined function argument count and call site arguments"));
    ...
    #undef OPCODE_DEF
        ...
        if (is_control_flow) {
            ...
            REQUIRE_OK(kefir_opt_code_container_insert_control(param->dst_code, mapped_block_id, mapped_control_prev, mapped_instr_ref));
        }
    }
    REQUIRE_OK(kefir_hashtree_insert(param->mem, &param->instr_mapping, (kefir_hashtree_key_t) instr_ref, (kefir_hashtree_value_t) mapped_instr_ref));

    return KEFIR_OK;
}

static kefir_result_t inline_blocks(struct do_inline_param *param) {
    struct kefir_opt_code_container_tracer tracer = {
        .trace_instruction = do_inline_instr,
        .payload = param
    };
...
static kefir_result_t map_inlined_phis(struct do_inline_param *param) {
    ...
    for (kefir_opt_block_id_t src_block_id = 0; src_block_id < num_of_src_blocks; src_block_id++) {
        ...
        for (res = kefir_opt_code_block_phi_head(&param->src_function->code, src_block, &src_phi_ref);
            ...
            struct kefir_hashtree_node_iterator iter;
            for (struct kefir_hashtree_node *node = kefir_hashtree_iter(&src_phi_node->links, &iter);
                node != NULL;
                node = kefir_hashtree_next(&iter)) {
                ASSIGN_DECL_CAST(kefir_opt_block_id_t, link_block_id,
                    node->key);
                ASSIGN_DECL_CAST(kefir_opt_block_id_t, link_instr_ref,
                    node->value);

                struct kefir_hashtree_node *node;
                kefir_result_t res = kefir_hashtree_at(&param->block_mapping, (kefir_hashtree_key_t) link_block_id, &node);
                if (res != KEFIR_NOT_FOUND) {
                    REQUIRE_OK(res);
                    ASSIGN_DECL_CAST(kefir_opt_block_id_t, mapped_block_id,
                        node->value);
                    kefir_opt_instruction_ref_t mapped_instr_ref;
                    REQUIRE_OK(get_instr_ref_mapping(param, link_instr_ref, &mapped_instr_ref));
                    REQUIRE_OK(kefir_opt_code_container_phi_attach(param->mem, param->dst_code, dst_phi_ref, mapped_block_id, mapped_instr_ref));
                }
            }
        }
        REQUIRE_OK(res);
...
static kefir_result_t link_inlined_entry_block(struct do_inline_param *param) {
    ...
    if (param->result_phi_instr != KEFIR_ID_NONE) {
        REQUIRE_OK(kefir_opt_code_container_replace_references(param->mem, param->dst_code, param->result_phi_instr, tail_instr_ref));
    }
    REQUIRE_OK(kefir_opt_code_container_drop_control(param->dst_code, tail_instr_ref));
    REQUIRE_OK(kefir_opt_code_container_drop_instr(param->mem, param->dst_code, tail_instr_ref));

    REQUIRE_OK(kefir_opt_code_builder_finalize_jump(param->mem, param->dst_code, pred_block->id, inlined_entry_block_id, NULL));
...
static kefir_result_t do_inline(struct kefir_mem *mem, const struct kefir_opt_module *module, struct kefir_opt_code_container *dst_code, const struct kefir_opt_function *src_function, kefir_opt_block_id_t inline_predecessor_block_id, kefir_opt_block_id_t inline_successor_block_id, kefir_opt_call_id_t original_call_ref) {
    struct do_inline_param param = {
        .mem = mem,
        .module = module,
...
static kefir_result_t can_inline_function(const struct kefir_opt_function *callee_function, const struct kefir_opt_function *called_function, kefir_size_t argument_count, kefir_bool_t *can_inline_ptr) {
    kefir_bool_t can_inline = true;

    if (!called_function->ir_func->flags.inline_function ||
    ...
    for (kefir_opt_block_id_t block_id = 0; can_inline && block_id < called_func_blocks; block_id++) {
        ...
        for (res = kefir_opt_code_block_instr_head(&called_function->code, block, &instr_ref);
            ...
            if (instruction->operation.opcode == KEFIR_OPT_OPCODE_GET_ARGUMENT && instruction->operation.parameters.index >= argument_count) {
                can_inline = false;
            }
        }
        REQUIRE_OK(res);
...
kefir_result_t kefir_opt_try_inline_function_call(struct kefir_mem *mem, const struct kefir_opt_module *module, struct kefir_opt_function *func, struct kefir_opt_code_structure *structure, kefir_opt_instruction_ref_t instr_ref, kefir_bool_t *did_inline_ptr) {
    ...
    REQUIRE(instr->operation.opcode == KEFIR_OPT_OPCODE_INVOKE, KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Unable to inline non-function call instruction"));
    ...
    REQUIRE_OK(can_inline_function(func, called_func, call_node->argument_count, &can_inline));

    if (can_inline) {
        kefir_opt_block_id_t block_id = instr->block_id;
        kefir_opt_block_id_t split_block_id;
        REQUIRE_OK(
            kefir_opt_code_split_block_after(mem, &func->code, structure, instr_ref, &split_block_id));
        REQUIRE_OK(do_inline(mem, module, &func->code, called_func, block_id, split_block_id, call_node->node_id));
