...
static kefir_result_t inline_operation_bitint_ref1(struct do_inline_param *param,
...
static kefir_result_t inline_operation_bitint_ref2(struct do_inline_param *param,
                                                   const struct kefir_opt_instruction *instr,
                                                   kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_bitint2_ref1(struct do_inline_param *param,
                                                    const struct kefir_opt_instruction *instr,
                                                    kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
    ...
    REQUIRE_OK(kefir_opt_code_container_new_instruction(
        ...
        &(struct kefir_opt_operation) {.opcode = instr->operation.opcode,
                                       .parameters = {.refs = {mapped_ref1, KEFIR_ID_NONE, KEFIR_ID_NONE},
                                                      .bitwidth = instr->operation.parameters.bitwidth,
                                                      .src_bitwidth = instr->operation.parameters.src_bitwidth}},
        mapped_instr_ref_ptr));
    return KEFIR_OK;
}
...
static kefir_result_t inline_operation_call_ref(struct do_inline_param *param,
    ...
    REQUIRE_OK(kefir_opt_code_container_call(param->src_code, instr->operation.parameters.function_call.call_ref,
                                             &src_call_node));
...
static kefir_result_t inline_operation_bitint_load(struct do_inline_param *param,
                                                   const struct kefir_opt_instruction *instr,
                                                   kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_bitint_bitfield(struct do_inline_param *param,
                                                       const struct kefir_opt_instruction *instr,
                                                       kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_bitint_store(struct do_inline_param *param,
                                                    const struct kefir_opt_instruction *instr,
                                                    kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
...
static kefir_result_t inline_operation_tmpobj(struct do_inline_param *param, const struct kefir_opt_instruction *instr,
                                              kefir_opt_instruction_ref_t *mapped_instr_ref_ptr) {
    kefir_opt_block_id_t mapped_block_id;
    REQUIRE_OK(map_block(param, instr->block_id, &mapped_block_id));
    ...
    REQUIRE_OK(kefir_opt_code_container_new_instruction(
        param->mem, param->dst_code, mapped_block_id,
        &(struct kefir_opt_operation) {.opcode = instr->operation.opcode,
                                       .parameters = {.refs = {KEFIR_ID_NONE, KEFIR_ID_NONE, KEFIR_ID_NONE},
                                                      .tmp_object = instr->operation.parameters.tmp_object}},
        mapped_instr_ref_ptr));
    return KEFIR_OK;
}
...
static kefir_result_t inline_operation_inline_asm(struct do_inline_param *param,
    ...
    REQUIRE_OK(kefir_opt_code_container_inline_assembly(param->src_code, instr->operation.parameters.inline_asm_ref,
                                                        &src_inline_asm_node));
...
static kefir_result_t generate_placeholder(struct do_inline_param *param, kefir_opt_block_id_t block_id,
    ...
    switch (ir_typeentry->typecode) {
        ...
        case KEFIR_IR_TYPE_INT128:
            REQUIRE_OK(kefir_opt_code_builder_int_constant(param->mem, param->dst_code, block_id, 0, instr_ref_ptr));
            REQUIRE_OK(kefir_opt_code_builder_int128_zero_extend_64bits(param->mem, param->dst_code, block_id,
                                                                        *instr_ref_ptr, instr_ref_ptr));
            break;
...
static kefir_result_t do_inline_instr(kefir_opt_instruction_ref_t instr_ref, void *payload) {
    ...
    const struct kefir_opt_instruction *instr;
    REQUIRE_OK(kefir_opt_code_container_instr(param->src_code, instr_ref, &instr));
...
#undef OPCODE_DEF
        ...
        REQUIRE_OK(kefir_opt_code_instruction_is_control_flow(param->src_code, instr_ref, &is_control_flow));
        if (is_control_flow) {
            kefir_opt_instruction_ref_t control_prev;
            REQUIRE_OK(kefir_opt_instruction_prev_control(param->src_code, instr_ref, &control_prev));
...
static kefir_result_t inline_blocks(struct do_inline_param *param) {
    ...
    REQUIRE_OK(kefir_opt_code_container_trace(param->mem, param->src_code, &tracer));

    return KEFIR_OK;
}

static kefir_result_t map_inlined_phis(struct do_inline_param *param) {
    kefir_size_t num_of_src_blocks = kefir_opt_code_container_block_count(param->src_code);
    ...
    for (kefir_opt_block_id_t src_block_id = 0; src_block_id < num_of_src_blocks; src_block_id++) {
        ...
        REQUIRE_OK(kefir_opt_code_container_block(param->src_code, src_block_id, &src_block));
        kefir_opt_instruction_ref_t src_phi_instr_ref;
        kefir_result_t res;
        for (res = kefir_opt_code_block_phi_head(param->src_code, src_block_id, &src_phi_instr_ref);
             res == KEFIR_OK && src_phi_instr_ref != KEFIR_ID_NONE;
             res = kefir_opt_phi_next_sibling(param->src_code, src_phi_instr_ref, &src_phi_instr_ref)) {

            const struct kefir_opt_instruction *src_phi_instr;
            REQUIRE_OK(kefir_opt_code_container_instr(param->src_code, src_phi_instr_ref, &src_phi_instr));

            const struct kefir_opt_phi_node *src_phi_node;
            REQUIRE_OK(kefir_opt_code_container_phi(param->src_code, src_phi_instr->operation.parameters.phi_ref,
                                                    &src_phi_node));
...
static kefir_result_t link_inlined_entry_block(struct do_inline_param *param) {
    ...
    REQUIRE_OK(map_block(param, param->src_code->entry_point, &inlined_entry_block_id));
...
static kefir_result_t inline_debug_allocation_info(struct do_inline_param *param) {
    ...
    for (res = kefir_opt_code_debug_info_local_variable_iter(&param->src_function->debug_info, &alloc_iter,
                                                             &local_variable);
         res == KEFIR_OK; res = kefir_opt_code_debug_info_local_variable_next(&alloc_iter, &local_variable)) {
        struct kefir_hashset_iterator iter;
        kefir_hashset_key_t key;
        for (res = kefir_hashset_iter(&local_variable->allocations, &iter, &key); res == KEFIR_OK;
             res = kefir_hashset_next(&iter, &key)) {
            ASSIGN_DECL_CAST(kefir_opt_instruction_ref_t, alloc_instr_ref, key);
            ...
            if (res != KEFIR_NOT_FOUND) {
                ...
                kefir_opt_block_id_t mapped_block_id;
                REQUIRE_OK(map_block(param, param->src_code->entry_point, &mapped_block_id));
            ...
            REQUIRE_OK(kefir_opt_code_debug_info_add_local_variable_allocation(
                param->mem, &param->dst_function->debug_info, local_variable->variable_ref, mapped_alloc_instr_ref));
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
        }
    ...
    for (res = kefir_opt_code_debug_info_allocation_placement_iter(&param->src_function->debug_info, &placement_iter,
                                                                   &allocation_placement);
         res == KEFIR_OK;
         res = kefir_opt_code_debug_info_allocation_placement_next(&placement_iter, &allocation_placement)) {
        struct kefir_hashtree_node *node;
        kefir_opt_instruction_ref_t mapped_alloc_instr_ref;
        res = kefir_hashtree_at(&param->instr_mapping, (kefir_hashtree_key_t) allocation_placement->allocation_ref,
                                &node);
        if (res != KEFIR_NOT_FOUND) {
            REQUIRE_OK(res);
            ...
            REQUIRE_OK(kefir_opt_code_builder_int_placeholder(param->mem, param->dst_code, mapped_block_id,
                                                              &mapped_alloc_instr_ref));
            REQUIRE_OK(kefir_hashtree_insert(param->mem, &param->instr_mapping,
                                             (kefir_hashtree_key_t) allocation_placement->allocation_ref,
                                             mapped_alloc_instr_ref));
        }

        struct kefir_hashset_iterator iter;
        kefir_hashset_key_t key;
        for (res = kefir_hashset_iter(&allocation_placement->placement, &iter, &key); res == KEFIR_OK;
             res = kefir_hashset_next(&iter, &key)) {
            ASSIGN_DECL_CAST(kefir_opt_instruction_ref_t, placement_ref, key);
            res = kefir_hashtree_at(&param->instr_mapping, (kefir_hashtree_key_t) placement_ref, &node);
            if (res == KEFIR_NOT_FOUND) {
                continue;
            }
            REQUIRE_OK(res);
            ASSIGN_DECL_CAST(kefir_opt_instruction_ref_t, mapped_ref, node->value);
            REQUIRE_OK(kefir_opt_code_debug_info_add_allocation_placement(param->mem, &param->dst_function->debug_info,
...
static kefir_result_t do_inline_impl(struct do_inline_param *param) {
    if (param->src_function == param->dst_function) {
...
static kefir_result_t do_inline(struct kefir_mem *mem, const struct kefir_opt_module *module,
    ...
    res = kefir_opt_code_container_free(mem, &param.src_code_clone);
    REQUIRE_ELSE(res == KEFIR_OK, {
        kefir_hashtree_free(mem, &param.block_mapping);
        kefir_hashtree_free(mem, &param.instr_mapping);
        return res;
