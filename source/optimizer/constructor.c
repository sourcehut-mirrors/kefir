...
static kefir_result_t construct_inline_asm(struct kefir_mem *mem, const struct kefir_opt_module *module,
    ...
    if (kefir_list_length(&ir_inline_asm->jump_target_list) > 0) {
        ...
        REQUIRE_OK(kefir_opt_code_container_inline_assembly_set_default_jump_target(code, instr_ref,
        ...
        for (const struct kefir_list_entry *iter = kefir_list_head(&ir_inline_asm->jump_target_list); iter != NULL;
            ...
            REQUIRE_OK(kefir_opt_code_container_inline_assembly_add_jump_target(
                mem, &state->function->code, instr_ref, ir_jump_target->uid, target_block_state->block_id));
        }
    }
    return KEFIR_OK;
}
...
static kefir_result_t translate_instruction(struct kefir_mem *mem, const struct kefir_opt_module *module,
    ...
    switch (instr->opcode) {
        ...
        case KEFIR_IR_OPCODE_TEMPORARY_OBJECT:
...
#define UNARY_OP(_id, _opcode)                                                                         \
    case _opcode:                                                                                      \
        ...
        break;
            ...
            UNARY_OP(int128_neg, KEFIR_IR_OPCODE_INT128_NEG)
            UNARY_OP(int128_not, KEFIR_IR_OPCODE_INT128_NOT)
            UNARY_OP(int128_bool_not, KEFIR_IR_OPCODE_INT128_BOOL_NOT)

            UNARY_OP(int128_signed_to_float32, KEFIR_IR_OPCODE_INT128_SIGNED_TO_FLOAT32)
            UNARY_OP(int128_signed_to_float64, KEFIR_IR_OPCODE_INT128_SIGNED_TO_FLOAT64)
            UNARY_OP(int128_signed_to_long_double, KEFIR_IR_OPCODE_INT128_SIGNED_TO_LONG_DOUBLE)
            UNARY_OP(int128_signed_to_decimal32, KEFIR_IR_OPCODE_INT128_SIGNED_TO_DECIMAL32)
            UNARY_OP(int128_signed_to_decimal64, KEFIR_IR_OPCODE_INT128_SIGNED_TO_DECIMAL64)
            UNARY_OP(int128_signed_to_decimal128, KEFIR_IR_OPCODE_INT128_SIGNED_TO_DECIMAL128)
            UNARY_OP(int128_unsigned_to_float32, KEFIR_IR_OPCODE_INT128_UNSIGNED_TO_FLOAT32)
            UNARY_OP(int128_unsigned_to_float64, KEFIR_IR_OPCODE_INT128_UNSIGNED_TO_FLOAT64)
            UNARY_OP(int128_unsigned_to_long_double, KEFIR_IR_OPCODE_INT128_UNSIGNED_TO_LONG_DOUBLE)
            ...
            UNARY_OP(decimal32_neg, KEFIR_IR_OPCODE_DECIMAL32_NEG)
            UNARY_OP(decimal64_neg, KEFIR_IR_OPCODE_DECIMAL64_NEG)
            UNARY_OP(decimal128_neg, KEFIR_IR_OPCODE_DECIMAL128_NEG)
...
#define BITINT2_UNARY_OP(_id, _opcode)                                                                             \
    case _opcode:                                                                                                  \
        ...
        break;
            ...
            BITINT_UNARY_OP(bitint_signed_to_float, KEFIR_IR_OPCODE_BITINT_SIGNED_TO_FLOAT)
            BITINT_UNARY_OP(bitint_unsigned_to_float, KEFIR_IR_OPCODE_BITINT_UNSIGNED_TO_FLOAT)
            BITINT_UNARY_OP(bitint_signed_to_double, KEFIR_IR_OPCODE_BITINT_SIGNED_TO_DOUBLE)
            ...
            BITINT_UNARY_OP(bitint_to_bool, KEFIR_IR_OPCODE_BITINT_TO_BOOL)
...
#define BINARY_OP(_id, _opcode)                                                                                    \
    case _opcode:                                                                                                  \
        ...
        break;
            ...
            BINARY_OP(int128_equal, KEFIR_IR_OPCODE_INT128_EQUAL)
            BINARY_OP(int128_greater, KEFIR_IR_OPCODE_INT128_GREATER)
            ...
            BINARY_OP(decimal32_add, KEFIR_IR_OPCODE_DECIMAL32_ADD)
            BINARY_OP(decimal64_add, KEFIR_IR_OPCODE_DECIMAL64_ADD)
            BINARY_OP(decimal128_add, KEFIR_IR_OPCODE_DECIMAL128_ADD)
            BINARY_OP(decimal32_sub, KEFIR_IR_OPCODE_DECIMAL32_SUB)
            BINARY_OP(decimal64_sub, KEFIR_IR_OPCODE_DECIMAL64_SUB)
            BINARY_OP(decimal128_sub, KEFIR_IR_OPCODE_DECIMAL128_SUB)
            BINARY_OP(decimal32_mul, KEFIR_IR_OPCODE_DECIMAL32_MUL)
            BINARY_OP(decimal64_mul, KEFIR_IR_OPCODE_DECIMAL64_MUL)
            BINARY_OP(decimal128_mul, KEFIR_IR_OPCODE_DECIMAL128_MUL)
            BINARY_OP(decimal32_div, KEFIR_IR_OPCODE_DECIMAL32_DIV)
            BINARY_OP(decimal64_div, KEFIR_IR_OPCODE_DECIMAL64_DIV)
            BINARY_OP(decimal128_div, KEFIR_IR_OPCODE_DECIMAL128_DIV)
            BINARY_OP(decimal32_equal, KEFIR_IR_OPCODE_DECIMAL32_EQUAL)
            BINARY_OP(decimal64_equal, KEFIR_IR_OPCODE_DECIMAL64_EQUAL)
            BINARY_OP(decimal128_equal, KEFIR_IR_OPCODE_DECIMAL128_EQUAL)
            BINARY_OP(decimal32_greater, KEFIR_IR_OPCODE_DECIMAL32_GREATER)
            BINARY_OP(decimal64_greater, KEFIR_IR_OPCODE_DECIMAL64_GREATER)
            BINARY_OP(decimal128_greater, KEFIR_IR_OPCODE_DECIMAL128_GREATER)
            BINARY_OP(decimal32_less, KEFIR_IR_OPCODE_DECIMAL32_LESS)
            BINARY_OP(decimal64_less, KEFIR_IR_OPCODE_DECIMAL64_LESS)
            BINARY_OP(decimal128_less, KEFIR_IR_OPCODE_DECIMAL128_LESS)
...
#define LOAD_OP(_id, _extension, _opcode)                                                                         \
    case _opcode: {                                                                                               \
        ...
        REQUIRE_OK(kefir_opt_constructor_stack_push(mem, state, instr_ref));                                      \
            ...
            LOAD_OP(float32_load, KEFIR_OPT_MEMORY_LOAD_NOEXTEND, KEFIR_IR_OPCODE_FLOAT32_LOAD)
            LOAD_OP(float64_load, KEFIR_OPT_MEMORY_LOAD_NOEXTEND, KEFIR_IR_OPCODE_FLOAT64_LOAD)
...
#define BITINT_ATOMIC_LOAD_OP(_id, _opcode)                                                                          \
    case _opcode: {                                                                                                  \
        ...
        REQUIRE_OK(kefir_opt_constructor_stack_push(mem, state, instr_ref));                                         \
            ...
            BITINT_ATOMIC_LOAD_OP(bitint_atomic_load, KEFIR_IR_OPCODE_BITINT_ATOMIC_LOAD)
...
#define BITINT_ATOMIC_STORE_OP(_id, _opcode)                                                                         \
    case _opcode: {                                                                                                  \
        ...
        REQUIRE_OK(kefir_opt_constructor_stack_push(mem, state, instr_ref));                                         \
            ...
            BITINT_ATOMIC_STORE_OP(bitint_atomic_store, KEFIR_IR_OPCODE_BITINT_ATOMIC_STORE)
...
#undef OVERFLOW_ARITH
        ...
        case KEFIR_IR_OPCODE_FLOAT_SLOT:
        case KEFIR_IR_OPCODE_SLOT_READ:
        case KEFIR_IR_OPCODE_SLOT_WRITE:
...
static kefir_result_t translate_code(struct kefir_mem *mem, const struct kefir_opt_module *module,
    ...
    for (; state->ir_location < kefir_irblock_length(ir_block); state->ir_location++) {
        ...
        REQUIRE(instr != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Expected valid IR instruction to be returned"));
        REQUIRE_OK(kefir_opt_code_debug_info_next_instruction_code_reference(&state->function->debug_info,
                                                                             state->ir_location));
        ...
        REQUIRE_OK(kefir_opt_code_debug_info_next_instruction_code_reference(
            &state->function->debug_info, KEFIR_OPT_CODE_DEBUG_INSTRUCTION_CODE_REF_NONE));
    }
...
static kefir_result_t link_blocks_impl(struct kefir_mem *mem, struct kefir_opt_constructor_state *state,
    ...
    for (; source_iter != NULL && target_iter != NULL;
        ...
        ASSIGN_DECL_CAST(kefir_opt_instruction_ref_t, phi_instr_ref, (kefir_uptr_t) target_iter->value);

        kefir_opt_instruction_ref_t existing_ref;
        kefir_result_t res = kefir_opt_code_container_phi_link_for(&state->function->code, phi_instr_ref,
                                                                   source_block_id, &existing_ref);
        if (res != KEFIR_NOT_FOUND) {
            REQUIRE_OK(res);
            REQUIRE(existing_ref == instr_ref,
                    KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Mismatch with existing phi link for a source block"));
        } else {
            REQUIRE_OK(kefir_opt_code_container_phi_attach(mem, &state->function->code, phi_instr_ref, source_block_id,
                                                           instr_ref));
        }
    }
...
static kefir_result_t link_blocks_equalize_stack(struct kefir_mem *mem, struct kefir_opt_constructor_state *state,
    ...
    while (kefir_list_length(&source_state->stack) > kefir_list_length(&target_state->phi_stack)) {
        kefir_opt_instruction_ref_t instr_ref;
        REQUIRE_OK(kefir_opt_code_container_new_phi(mem, &state->function->code, target_block_id, &instr_ref));
        REQUIRE_OK(kefir_list_insert_after(mem, &target_state->stack, NULL, (void *) (kefir_uptr_t) instr_ref));
        REQUIRE_OK(kefir_list_insert_after(mem, &target_state->phi_stack, NULL, (void *) (kefir_uptr_t) instr_ref));
    }
    return KEFIR_OK;
}
