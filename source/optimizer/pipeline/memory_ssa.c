...
static kefir_result_t check_clobber(struct kefir_mem *mem, const struct kefir_opt_code_container *code,
    ...
    *do_alias = true;
    ...
    REQUIRE_OK(kefir_opt_code_may_alias(code, escapes, ir_module, location1_ref, size1, offset1, location2_ref, size2,
                                        offset2, do_alias));
    return KEFIR_OK;
}

struct node_ref_queue {
    kefir_opt_code_memssa_node_ref_t *queue;
    kefir_size_t top;
    kefir_size_t capacity;
};

static kefir_result_t queue_push(struct kefir_mem *mem, struct node_ref_queue *queue,
                                 kefir_opt_code_memssa_node_ref_t node_ref) {
    if (queue->top >= queue->capacity) {
        kefir_size_t new_capacity = MAX(8, 2 * queue->capacity);
        kefir_opt_code_memssa_node_ref_t *new_queue =
            KEFIR_REALLOC(mem, queue->queue, sizeof(kefir_opt_code_memssa_node_ref_t) * new_capacity);
        REQUIRE(new_queue != NULL, KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate memssa node queue"));

        queue->queue = new_queue;
        queue->capacity = new_capacity;
    }

    queue->queue[queue->top++] = node_ref;
    return KEFIR_OK;
}
...
static kefir_result_t find_upstream_clobber(struct kefir_mem *mem, const struct kefir_opt_code_container *code,
                                            ...
                                            struct node_ref_queue *queue, struct kefir_hashset *visited) {
        ...
        switch (iter_node->type) {
            ...
            case KEFIR_OPT_CODE_MEMSSA_PHI_NODE:
                for (kefir_size_t i = 0; i < iter_node->phi.link_count; i++) {
                    REQUIRE_OK(queue_push(mem, queue, iter_node->phi.links[i].node_ref));
                }
                break;

            case KEFIR_OPT_CODE_MEMSSA_CONSUME_NODE:
                REQUIRE_OK(queue_push(mem, queue, iter_node->predecessor_ref));
                break;
...
static kefir_result_t do_optimize_nonvolatile_load(struct kefir_mem *mem, struct kefir_opt_function *func,
    ...
    *did_replace = false;
    ...
    res = find_upstream_clobber(mem, &func->code, escapes, memssa, ir_module, node_ref, &clobber_ref, queue, visited);
    REQUIRE(res != KEFIR_NOT_FOUND, KEFIR_OK);
    REQUIRE_OK(res);
    ...
    res = kefir_opt_code_util_classify_memory_access(instr, &location1_ref, &size1, &offset1);
    if (res == KEFIR_NO_MATCH) {
    ...
    res = kefir_opt_code_util_classify_memory_access(clobber_instr, &location2_ref, &size2, &offset2);
    if (res == KEFIR_NO_MATCH) {
    ...
    REQUIRE_OK(kefir_opt_code_must_alias(&func->code, ir_module, location2_ref, size2, offset2, location1_ref, size1,
                                         offset1, &must_alias));
...
static kefir_result_t do_deduplicate_nonvolatile_load(
    ...
    const struct kefir_ir_module *ir_module, const struct kefir_opt_instruction *instr) {
    UNUSED(control_flow);
    UNUSED(module);
    kefir_opt_instruction_ref_t instr_ref = instr->id;
    ...
    for (kefir_opt_code_memssa_node_ref_t iter_node_ref = node->predecessor_ref; iter_node_ref != KEFIR_ID_NONE;) {
        ...
        switch (iter_node->type) {
            ...
            case KEFIR_OPT_CODE_MEMSSA_PRODUCE_CONSUME_NODE: {
                ...
                REQUIRE_OK(kefir_opt_code_may_alias(&func->code, escapes, ir_module, location1_ref, size1, offset1,
                                                    location2_ref, size2, offset2, &terminate));
            } break;
        }
        ...
        for (res = kefir_opt_code_memssa_use_iter(memssa, &use_iter, iter_node_ref, &use_node_ref); res == KEFIR_OK;
            ...
            res = kefir_opt_code_util_classify_memory_access(use_instr, &location2_ref, &size2, &offset2);
            if (res == KEFIR_NO_MATCH) {
                continue;
            }
            REQUIRE_OK(res);
            ...
            kefir_bool_t must_alias;
            REQUIRE_OK(kefir_opt_code_must_alias(&func->code, ir_module, location1_ref, size1, offset1, location2_ref,
    ...
    *has_clobbers = true;
    ...
    for (; queue->top > 0; first_iter = false, scanned_nodes++) {
            ...
            switch (iter_node->type) {
                ...
                case KEFIR_OPT_CODE_MEMSSA_CONSUME_NODE: {
                    ...
                    if (res != KEFIR_NOT_FOUND) {
                        ...
                        kefir_bool_t must_alias;
                        REQUIRE_OK(kefir_opt_code_must_alias(code, ir_module, location1_ref, size1, offset1,
        ...
        for (res = kefir_opt_code_memssa_use_iter(memssa, &use_iter, iter_ref, &use_node_ref);
            ...
            REQUIRE_OK(queue_push(mem, queue, use_node_ref));
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
        }
...
static kefir_result_t do_optimize_nonvolatile_store(
    ...
    const struct kefir_opt_code_control_flow *control_flow, struct kefir_opt_code_sequencing *sequencing,
    struct kefir_opt_code_memssa *memssa, const struct kefir_opt_code_escape_analysis *escapes,
    const struct kefir_ir_module *ir_module, const struct kefir_opt_instruction *instr, struct node_ref_queue *queue,
    ...
    res = has_downstream_clobbers(mem, &func->code, escapes, memssa, ir_module, node_ref, &downstream_clobbers, queue,
                                  visited, &consumers);
    ...
    kefir_int64_t offset1, offset2;
    res = kefir_opt_code_util_classify_memory_access(instr, &location1_ref, &size1, &offset1);
    REQUIRE_CHAIN(&res, kefir_opt_code_util_classify_memory_access(clobber_instr, &location2_ref, &size2, &offset2));
    REQUIRE(res != KEFIR_NO_MATCH, KEFIR_OK);
    REQUIRE_OK(res);
...
static kefir_result_t do_optimize(struct kefir_mem *mem, struct kefir_opt_module *module,
                                  ...
                                  const struct kefir_opt_code_control_flow *control_flow,
                                  struct kefir_opt_code_sequencing *sequencing, struct kefir_opt_code_memssa *memssa,
                                  const struct kefir_opt_code_escape_analysis *escapes, struct node_ref_queue *queue,
                                  struct kefir_hashset *visited) {
                ...
                case KEFIR_OPT_OPCODE_DECIMAL128_LOAD:
                    if (!instr->operation.parameters.memory_access.flags.volatile_access) {
                        ...
                        REQUIRE_OK(do_optimize_nonvolatile_load(mem, func, control_flow, sequencing, memssa, escapes,
                                                                module->ir_module, instr, &did_replace, queue,
                                                                visited));
                        if (!did_replace) {
                ...
                case KEFIR_OPT_OPCODE_DECIMAL128_STORE:
                    if (!instr->operation.parameters.memory_access.flags.volatile_access) {
                        REQUIRE_OK(do_optimize_nonvolatile_store(mem, module, func, control_flow, sequencing, memssa,
                                                                 escapes, module->ir_module, instr, queue, visited));
                    }
                    break;
...
static kefir_result_t memory_ssa_apply(struct kefir_mem *mem, struct kefir_opt_module *module,
    ...
    struct node_ref_queue queue = {.queue = NULL, .top = 0, .capacity = 0};
    struct kefir_hashset visited;
    ...
    REQUIRE_CHAIN(
        &res, do_optimize(mem, module, func, config, &control_flow, &sequencing, &memssa, &escapes, &queue, &visited));
    KEFIR_FREE(mem, queue.queue);
    REQUIRE_ELSE(res == KEFIR_OK, {
