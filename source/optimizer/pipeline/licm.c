...
static kefir_result_t try_rotate(struct licm_state *state) {
    REQUIRE(state->loop->header_ref != state->control_flow.code->entry_point &&
                state->loop->header_ref != state->control_flow.code->gate_block &&
                !kefir_hashset_has(&state->control_flow.indirect_jump_target_blocks,
                                   (kefir_hashset_key_t) state->loop->header_ref),
    ...
    REQUIRE(state->loop->latches.occupied == 1, KEFIR_OK);
    kefir_result_t res;
    struct kefir_hashset_iterator iter;
    kefir_hashset_key_t key;
    REQUIRE_OK(kefir_hashset_iter(&state->loop->latches, &iter, &key));
    ASSIGN_DECL_CAST(kefir_opt_block_id_t, latch_block_ref, key);

    kefir_opt_instruction_ref_t latch_tail_ref;
    REQUIRE_OK(kefir_opt_code_block_instr_control_tail(&state->func->code, latch_block_ref, &latch_tail_ref));
    const struct kefir_opt_instruction *latch_tail;
    REQUIRE_OK(kefir_opt_code_container_instr(&state->func->code, latch_tail_ref, &latch_tail));
    REQUIRE(latch_tail->operation.opcode == KEFIR_OPT_OPCODE_JUMP &&
                latch_tail->operation.parameters.branch.target_block == state->loop->header_ref,
            KEFIR_OK);

    kefir_opt_instruction_ref_t instr_ref;
    for (res = kefir_opt_code_block_instr_head(&state->func->code, state->loop->header_ref, &instr_ref);
         res == KEFIR_OK && instr_ref != KEFIR_ID_NONE;
         res = kefir_opt_instruction_next_sibling(&state->func->code, instr_ref, &instr_ref)) {
        const struct kefir_opt_instruction *instr;
        REQUIRE_OK(kefir_opt_code_container_instr(&state->func->code, instr_ref, &instr));
        ...
        for (res = kefir_opt_code_container_instruction_use_instr_iter(&state->func->code, instr_ref, &use_iter);
            ...
            REQUIRE(kefir_hashset_has(&state->loop->blocks, (kefir_hashset_key_t) use_instr->block_id), KEFIR_OK);
        }
        if (res != KEFIR_ITERATOR_END) {
            REQUIRE_OK(res);
        }
    ...
    REQUIRE_OK(kefir_opt_code_block_instr_control_tail(&state->func->code, state->loop->header_ref, &header_tail_ref));
    const struct kefir_opt_instruction *header_tail;
    REQUIRE_OK(kefir_opt_code_container_instr(&state->func->code, header_tail_ref, &header_tail));
    switch (header_tail->operation.opcode) {
        case KEFIR_OPT_OPCODE_BRANCH_COMPARE: {
            const struct kefir_opt_instruction *lhs_instr, *rhs_instr;
            REQUIRE_OK(kefir_opt_code_container_instr(&state->func->code, header_tail->operation.parameters.refs[0],
                                                      &lhs_instr));
            REQUIRE_OK(kefir_opt_code_container_instr(&state->func->code, header_tail->operation.parameters.refs[1],
                                                      &rhs_instr));

            REQUIRE(
                lhs_instr->operation.opcode == KEFIR_OPT_OPCODE_PHI || lhs_instr->block_id != state->loop->header_ref,
                KEFIR_OK);
            REQUIRE(
                rhs_instr->operation.opcode == KEFIR_OPT_OPCODE_PHI || rhs_instr->block_id != state->loop->header_ref,
                KEFIR_OK);

            kefir_opt_instruction_ref_t preheader_branch_lhs_ref = lhs_instr->id, latch_branch_lhs_ref = lhs_instr->id;
            kefir_opt_instruction_ref_t preheader_branch_rhs_ref = rhs_instr->id, latch_branch_rhs_ref = rhs_instr->id;
            if (lhs_instr->operation.opcode == KEFIR_OPT_OPCODE_PHI && lhs_instr->block_id == state->loop->header_ref) {
                REQUIRE_OK(kefir_opt_code_container_phi_link_for(&state->func->code, lhs_instr->id, latch_block_ref,
                                                                 &latch_branch_lhs_ref));
                REQUIRE_OK(kefir_opt_code_container_phi_link_for(
                    &state->func->code, lhs_instr->id, state->loop->preheader_ref, &preheader_branch_lhs_ref));
            }
            if (rhs_instr->operation.opcode == KEFIR_OPT_OPCODE_PHI && rhs_instr->block_id == state->loop->header_ref) {
                REQUIRE_OK(kefir_opt_code_container_phi_link_for(&state->func->code, rhs_instr->id, latch_block_ref,
                                                                 &latch_branch_rhs_ref));
                REQUIRE_OK(kefir_opt_code_container_phi_link_for(
                    &state->func->code, rhs_instr->id, state->loop->preheader_ref, &preheader_branch_rhs_ref));
            }
            ...
            if (kefir_hashset_has(&state->loop->blocks,
                ...
                target_block_ref = state->loop->header_ref;
                ...
                REQUIRE(
                    kefir_hashset_has(
                        &state->loop->blocks,
                        (kefir_hashset_key_t) header_tail->operation.parameters.branch.alternative_block) &&
                        !kefir_hashset_has(&state->loop->blocks,
                                           (kefir_hashset_key_t) header_tail->operation.parameters.branch.target_block),
                    KEFIR_OK);
                ...
                alternative_block_ref = state->loop->header_ref;
            }

            kefir_opt_instruction_ref_t preheader_tail_ref;
            REQUIRE_OK(kefir_opt_code_block_instr_control_tail(&state->func->code, state->loop->preheader_ref,
                                                               &preheader_tail_ref));
            REQUIRE_OK(kefir_opt_code_container_drop_control(&state->func->code, preheader_tail_ref));
            REQUIRE_OK(kefir_opt_code_container_drop_instr(state->mem, &state->func->code, preheader_tail_ref));
            REQUIRE_OK(kefir_opt_code_builder_finalize_branch_compare(
                state->mem, &state->func->code, state->loop->preheader_ref,
                header_tail->operation.parameters.branch.comparison.operation, preheader_branch_lhs_ref,
                preheader_branch_rhs_ref, target_block_ref, alternative_block_ref, NULL));

            REQUIRE_OK(kefir_opt_code_container_drop_control(&state->func->code, latch_tail_ref));
            REQUIRE_OK(kefir_opt_code_container_drop_instr(state->mem, &state->func->code, latch_tail_ref));
            REQUIRE_OK(kefir_opt_code_builder_finalize_branch_compare(
                state->mem, &state->func->code, latch_block_ref,
                header_tail->operation.parameters.branch.comparison.operation, latch_branch_lhs_ref,
                latch_branch_rhs_ref, target_block_ref, alternative_block_ref, NULL));

            REQUIRE_OK(kefir_opt_code_container_drop_control(&state->func->code, header_tail_ref));
            REQUIRE_OK(kefir_opt_code_container_drop_instr(state->mem, &state->func->code, header_tail_ref));
            REQUIRE_OK(kefir_opt_code_builder_finalize_jump(state->mem, &state->func->code, state->loop->header_ref,
                                                            in_loop_block_ref, NULL));

            kefir_opt_instruction_ref_t phi_instr_ref;
            for (res = kefir_opt_code_block_phi_head(&state->func->code, exit_loop_block_ref, &phi_instr_ref);
                 res == KEFIR_OK && phi_instr_ref != KEFIR_ID_NONE;
                 kefir_opt_phi_next_sibling(&state->func->code, phi_instr_ref, &phi_instr_ref)) {
                kefir_opt_instruction_ref_t link_ref;
                REQUIRE_OK(kefir_opt_code_container_phi_link_for(&state->func->code, phi_instr_ref,
                                                                 state->loop->header_ref, &link_ref));
                REQUIRE_OK(kefir_opt_code_container_phi_attach(state->mem, &state->func->code, phi_instr_ref,
                                                               state->loop->preheader_ref, link_ref));
                REQUIRE_OK(kefir_opt_code_container_phi_attach(state->mem, &state->func->code, phi_instr_ref,
                                                               latch_block_ref, link_ref));
            }
            REQUIRE_OK(res);
        } break;
