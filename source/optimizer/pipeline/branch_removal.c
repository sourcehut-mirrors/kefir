                    ...
                    if (arg1->operation.opcode == KEFIR_OPT_OPCODE_INT_CONST ||
                        ...
                        if (arg1->operation.parameters.imm.integer != 0) {
                            REQUIRE_OK(kefir_opt_code_builder_finalize_jump(
                                mem, &func->code, block->id, instr->operation.parameters.branch.target_block,
                                &replacement_ref));
                        } else {
                            REQUIRE_OK(kefir_opt_code_builder_finalize_jump(
                                mem, &func->code, block->id, instr->operation.parameters.branch.alternative_block,
                                &replacement_ref));
                        }

                        REQUIRE_OK(
                            kefir_opt_code_container_replace_references(&func->code, replacement_ref, instr->id));
                    }
                } break;
