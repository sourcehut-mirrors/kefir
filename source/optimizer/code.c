...
kefir_result_t kefir_opt_code_container_instr(const struct kefir_opt_code_container *code, kefir_opt_id_t instr_id,
    ...
    REQUIRE(instr->block_id != KEFIR_OPT_ID_NONE,
...
kefir_result_t kefir_opt_code_container_add_control(const struct kefir_opt_code_container *code,
                                                    struct kefir_opt_code_block *block, kefir_opt_id_t instr_id) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(block != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code block"));
...
kefir_result_t kefir_opt_code_container_insert_control(const struct kefir_opt_code_container *code,
                                                       struct kefir_opt_code_block *block,
                                                       kefir_opt_id_t after_instr_id, kefir_opt_id_t instr_id) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(
        after_instr_id != KEFIR_OPT_ID_NONE || block != NULL,
        KEFIR_SET_ERROR(
    ...
    if (after_instr_id != KEFIR_OPT_ID_NONE) {
    ...
    if (after_instr == NULL) {
        ...
        if (after_instr->control_flow.next != KEFIR_OPT_ID_NONE) {
...
kefir_result_t kefir_opt_code_block_instr_control_tail(const struct kefir_opt_code_container *code,
    ...
    struct kefir_opt_instruction *ptr = NULL;
    if (block->control_flow.tail != KEFIR_OPT_ID_NONE) {
