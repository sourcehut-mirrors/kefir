...
kefir_result_t kefir_opt_code_container_new_block(struct kefir_mem *mem, struct kefir_opt_code_container *code,
    ...
    block->content.head = KEFIR_OPT_ID_NONE;
    block->content.tail = KEFIR_OPT_ID_NONE;
    block->control_flow.head = KEFIR_OPT_ID_NONE;
    block->control_flow.tail = KEFIR_OPT_ID_NONE;
...
kefir_result_t kefir_opt_code_container_block(const struct kefir_opt_code_container *code, kefir_opt_id_t block_id,
...
kefir_result_t kefir_opt_code_container_instr(const struct kefir_opt_code_container *code, kefir_opt_id_t instr_id,
                                              struct kefir_opt_instruction **instr_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(instr_id != KEFIR_OPT_ID_NONE && instr_id < code->length,
    ...
    *instr_ptr = &code->code[instr_id];
    return KEFIR_OK;
}

kefir_result_t kefir_opt_code_container_append(struct kefir_mem *mem, struct kefir_opt_code_container *code,
                                               kefir_opt_id_t block_id, const struct kefir_opt_operation *operation,
    ...
    instr->control_flow.prev = KEFIR_OPT_ID_NONE;
    instr->control_flow.next = KEFIR_OPT_ID_NONE;
    instr->siblings.prev = block->content.tail;
    instr->siblings.next = KEFIR_OPT_ID_NONE;

    if (block->content.tail != KEFIR_OPT_ID_NONE) {
        struct kefir_opt_instruction *prev_instr = &code->code[block->content.tail];
        REQUIRE(prev_instr->siblings.next == KEFIR_OPT_ID_NONE,
...
kefir_result_t kefir_opt_code_container_add_control(const struct kefir_opt_code_container *code,
                                                    kefir_opt_id_t block_id, kefir_opt_id_t instr_id) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));

    struct kefir_opt_instruction *instr = NULL;
    REQUIRE_OK(kefir_opt_code_container_instr(code, instr_id, &instr));
    REQUIRE(instr->block_id == block_id,
            KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Provided instruction is not a part of needed block"));
    REQUIRE(instr->control_flow.prev == KEFIR_OPT_ID_NONE && instr->control_flow.next == KEFIR_OPT_ID_NONE,
            KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Provided instruction is already a part of control flow"));

    struct kefir_opt_code_block *block = NULL;
    REQUIRE_OK(kefir_opt_code_container_block(code, block_id, &block));

    kefir_opt_id_t tail = block->control_flow.tail;

    if (block->control_flow.head != KEFIR_OPT_ID_NONE) {
        struct kefir_opt_instruction *prev_instr = &code->code[block->control_flow.tail];
        REQUIRE(prev_instr->control_flow.next == KEFIR_OPT_ID_NONE,
                KEFIR_SET_ERROR(KEFIR_INVALID_STATE,
                                "Expected previous instruction in block control flow to have no successors"));
        prev_instr->control_flow.next = instr_id;
    } else {
        block->control_flow.head = instr_id;
    }
    block->control_flow.tail = instr_id;
    instr->control_flow.prev = tail;
    return KEFIR_OK;
}

kefir_result_t kefir_opt_code_block_instr_iter(const struct kefir_opt_code_container *code,
    ...
    if (block->content.head != KEFIR_OPT_ID_NONE) {
...
kefir_result_t kefir_opt_code_block_instr_control_iter(const struct kefir_opt_code_container *code,
                                                       const struct kefir_opt_code_block *block,
                                                       const struct kefir_opt_instruction **instr_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(block != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code block"));
...
kefir_result_t kefir_opt_instruction_prev_sibling(const struct kefir_opt_code_container *code,
    ...
    struct kefir_opt_instruction *ptr = NULL;
    if (instr->siblings.prev != KEFIR_OPT_ID_NONE) {
...
kefir_result_t kefir_opt_instruction_next_sibling(const struct kefir_opt_code_container *code,
    ...
    struct kefir_opt_instruction *ptr = NULL;
    if (instr->siblings.next != KEFIR_OPT_ID_NONE) {
...
kefir_result_t kefir_opt_instruction_prev_control(const struct kefir_opt_code_container *code,
    ...
    struct kefir_opt_instruction *ptr = NULL;
    if (instr->control_flow.prev != KEFIR_OPT_ID_NONE) {
...
kefir_result_t kefir_opt_instruction_next_control(const struct kefir_opt_code_container *code,
    ...
    struct kefir_opt_instruction *ptr = NULL;
    if (instr->control_flow.next != KEFIR_OPT_ID_NONE) {
...
const struct kefir_opt_code_block *kefir_opt_code_block_iter(const struct kefir_opt_code_container *code,
                                                             struct kefir_opt_code_block_iterator *iter) {
...
const struct kefir_opt_code_block *kefir_opt_code_block_next(struct kefir_opt_code_block_iterator *iter) {
    REQUIRE(iter != NULL, NULL);
