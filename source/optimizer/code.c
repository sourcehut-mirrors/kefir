...
kefir_result_t kefir_opt_code_container_new_block(struct kefir_mem *mem, struct kefir_opt_code_container *code,
    ...
    if (code->blocks_length == code->blocks_capacity) {
        ...
        struct kefir_opt_code_block *new_blocks =
            KEFIR_REALLOC(mem, code->blocks, new_capacity * sizeof(struct kefir_opt_code_block));
        REQUIRE(new_blocks != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to reallocate optimizer code blocks"));
...
static kefir_result_t code_container_block_mutable(const struct kefir_opt_code_container *code,
    ...
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(block_id < code->blocks_length,
            KEFIR_SET_ERROR(KEFIR_NOT_FOUND, "Cannot find optimizer code block with specified identifier"));
...
kefir_size_t kefir_opt_code_container_block_count(const struct kefir_opt_code_container *code) {
    REQUIRE(code != NULL, 0);
...
static kefir_result_t drop_block_impl(struct kefir_mem *mem, struct kefir_opt_code_container *code,
    ...
    for (res = kefir_opt_code_block_instr_head(code, block_id, &instr_ref);
        ...
        if (verify_uses) {
            ...
            for (res = kefir_hashset_iter(&instr->uses.instruction, &iter, &key); res == KEFIR_OK;
                 res = kefir_hashset_next(&iter, &key)) {
                ASSIGN_DECL_CAST(kefir_opt_instruction_ref_t, use_instr_ref, key);
...
static kefir_result_t update_uses_callback(kefir_opt_instruction_ref_t used_instr_ref, void *payload) {
    ...
    REQUIRE_OK(
        kefir_hashset_add(param->mem, &used_instr->uses.instruction, (kefir_hashset_key_t) param->user_ref));
    return KEFIR_OK;
}
...
static kefir_result_t drop_uses_callback(kefir_opt_instruction_ref_t used_instr_ref, void *payload) {
    ...
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
        REQUIRE_OK(kefir_hashset_delete(&used_instr->uses.instruction,
                                            (kefir_hashset_key_t) param->user_ref));
    }
    return KEFIR_OK;
}
...
kefir_result_t drop_instr_impl(struct kefir_mem *mem, struct kefir_opt_code_container *code,
    ...
    if (verify_uses) {
        ...
        REQUIRE(kefir_hashset_size(&used_instr->uses.instruction) == 0,
    ...
    if (GENERATION_OF(instr->id) < MAX_GENERATION) {
...
kefir_result_t kefir_opt_code_container_copy_instruction(struct kefir_mem *mem, struct kefir_opt_code_container *code,
    ...
    switch (src_operation.opcode) {
        case KEFIR_OPT_OPCODE_PHI: {
            ...
            REQUIRE_OK(kefir_opt_code_container_new_phi(mem, code, block_id, &phi_instr_ref));
            ASSIGN_PTR(instr_ref_ptr, phi_instr_ref);
            ...
            for (struct kefir_hashtree_node *node = kefir_hashtree_iter(&src_phi_node->links, &iter); node != NULL;
                ...
                REQUIRE_OK(
                    kefir_opt_code_container_phi_attach(mem, code, phi_instr_ref, link_block_id, link_instr_ref));
            }
        } break;
        ...
        case KEFIR_OPT_OPCODE_INVOKE_VIRTUAL: {
            ...
            REQUIRE_OK(kefir_opt_code_container_new_call(
                ...
                src_operation.parameters.function_call.indirect_ref, &call_instr_ref));
            ASSIGN_PTR(instr_ref_ptr, call_instr_ref);
            REQUIRE_OK(
                kefir_opt_code_container_call(code, src_operation.parameters.function_call.call_ref, &src_call_node));

            for (kefir_size_t i = 0; i < src_call_node->argument_count; i++) {
                REQUIRE_OK(kefir_opt_code_container_call_set_argument(mem, code, call_instr_ref, i,
                                                                      src_call_node->arguments[i]));
            }
            if (src_call_node->return_space != KEFIR_ID_NONE) {
                REQUIRE_OK(kefir_opt_code_container_call_set_return_space(mem, code, call_instr_ref,
                                                                          src_call_node->return_space));
            }
        } break;
        ...
        case KEFIR_OPT_OPCODE_INLINE_ASSEMBLY: {
            ...
            REQUIRE_OK(kefir_opt_code_container_new_inline_assembly(
                ...
                &inline_asm_instr_ref));
            ASSIGN_PTR(instr_ref_ptr, inline_asm_instr_ref);
            REQUIRE_OK(kefir_opt_code_container_inline_assembly(code, src_operation.parameters.inline_asm_ref,
                                                                &src_inline_asm_node));

            for (kefir_size_t i = 0; i < src_inline_asm_node->parameter_count; i++) {
                REQUIRE_OK(kefir_opt_code_container_inline_assembly_set_parameter(mem, code, inline_asm_instr_ref, i,
            ...
            if (src_inline_asm_node->default_jump_target != KEFIR_ID_NONE) {
                REQUIRE_OK(kefir_opt_code_container_inline_assembly_set_default_jump_target(
                    code, inline_asm_instr_ref, src_inline_asm_node->default_jump_target));
            }
            ...
            for (struct kefir_hashtree_node *node = kefir_hashtree_iter(&src_inline_asm_node->jump_targets, &iter);
                ...
                REQUIRE_OK(kefir_opt_code_container_inline_assembly_add_jump_target(mem, code, inline_asm_instr_ref,
...
static kefir_result_t kefir_opt_code_container_drop_call(struct kefir_mem *mem,
    ...
    for (kefir_size_t i = 0; i < call_node->argument_count; i++) {
        ...
        if (res != KEFIR_NOT_FOUND) {
            REQUIRE_OK(res);
            REQUIRE_OK(kefir_hashset_delete(&used_instr->uses.instruction,
                                                (kefir_hashset_key_t) call_node->output_ref));
        }
    }
    ...
    for (kefir_size_t i = 0; i < inline_asm_node->parameter_count; i++) {
        if (inline_asm_node->parameters[i].location_ref != KEFIR_ID_NONE) {
            ...
            if (res != KEFIR_NOT_FOUND) {
                REQUIRE_OK(res);
                REQUIRE_OK(kefir_hashset_delete(&used_instr->uses.instruction,
                                                    (kefir_hashset_key_t) inline_asm_node->output_ref));
            }
        }
...
static kefir_result_t add_used_instructions(struct kefir_mem *mem, const struct kefir_opt_code_container *code,
    ...
    REQUIRE_OK(kefir_hashset_add(mem, &used_instr->uses.instruction, (kefir_hashset_key_t) use_instr_ref));
    return KEFIR_OK;
}
...
static kefir_result_t remove_used_instructions(struct kefir_mem *mem, const struct kefir_opt_code_container *code,
    ...
    struct kefir_opt_instruction *used_instr = NULL;
    REQUIRE_OK(code_container_instr_mutable(code, instr_ref, &used_instr));
    REQUIRE_OK(kefir_hashset_delete(&used_instr->uses.instruction, (kefir_hashset_key_t) use_instr_ref));
    return KEFIR_OK;
}
...
kefir_result_t kefir_opt_code_container_phi_replace(struct kefir_mem *mem, struct kefir_opt_code_container *code,
    ...
    if (drop_use) {
        ...
        REQUIRE_OK(
            kefir_hashset_delete(&used_instr->uses.instruction, (kefir_hashset_key_t) phi_instr_ref));
    }
...
kefir_result_t kefir_opt_code_container_new_tail_call(struct kefir_mem *mem, struct kefir_opt_code_container *code,
    ...
    REQUIRE_OK(new_call_impl(
        ...
        function_ref != KEFIR_ID_NONE ? KEFIR_OPT_OPCODE_TAIL_INVOKE_VIRTUAL : KEFIR_OPT_OPCODE_TAIL_INVOKE,
        instr_ref_ptr));
    return KEFIR_OK;
}
...
kefir_result_t kefir_opt_code_container_call_set_argument(struct kefir_mem *mem, struct kefir_opt_code_container *code,
    ...
    struct kefir_opt_instruction *call_instr = NULL;
    REQUIRE_OK(code_container_instr_mutable(code, call_instr_ref, &call_instr));
    REQUIRE(call_instr->operation.opcode == KEFIR_OPT_OPCODE_INVOKE ||
                call_instr->operation.opcode == KEFIR_OPT_OPCODE_INVOKE_VIRTUAL ||
                call_instr->operation.opcode == KEFIR_OPT_OPCODE_TAIL_INVOKE ||
                call_instr->operation.opcode == KEFIR_OPT_OPCODE_TAIL_INVOKE_VIRTUAL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected optimizer call instruction reference"));

    struct kefir_opt_call_node *call_node = NULL;
    REQUIRE_OK(code_container_call_mutable(code, call_instr->operation.parameters.function_call.call_ref, &call_node));
...
kefir_result_t kefir_opt_code_container_inline_assembly_get_parameter(
    const struct kefir_opt_code_container *code, kefir_opt_instruction_ref_t inline_asm_instr_ref,
    ...
    struct kefir_opt_instruction *inline_asm_instr = NULL;
    REQUIRE_OK(code_container_instr_mutable(code, inline_asm_instr_ref, &inline_asm_instr));
    REQUIRE(inline_asm_instr->operation.opcode == KEFIR_OPT_OPCODE_INLINE_ASSEMBLY,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected optimizer inline assembly instruction"));

    struct kefir_opt_inline_assembly_node *inline_asm_node = NULL;
    REQUIRE_OK(code_container_inline_assembly_mutable(code, inline_asm_instr->operation.parameters.inline_asm_ref,
                                                      &inline_asm_node));
...
kefir_result_t kefir_opt_code_container_inline_assembly_set_parameter(
    struct kefir_mem *mem, const struct kefir_opt_code_container *code,
    kefir_opt_instruction_ref_t inline_asm_instr_ref, kefir_size_t param_index,
    const struct kefir_opt_inline_assembly_parameter *param_ptr) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(param_ptr != NULL,
...
kefir_result_t kefir_opt_code_container_inline_assembly_add_jump_target(
    struct kefir_mem *mem, const struct kefir_opt_code_container *code,
    kefir_opt_instruction_ref_t inline_asm_instr_ref, kefir_id_t target_id, kefir_opt_block_id_t target_block) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
...
kefir_result_t kefir_opt_code_block_phi_head(const struct kefir_opt_code_container *code, kefir_opt_block_id_t block_id,
                                             kefir_opt_instruction_ref_t *phi_instr_ref_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(phi_instr_ref_ptr != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to optimizer phi instruction reference"));
    ...
    if (block->phi_nodes.head != KEFIR_ID_NONE) {
        struct kefir_opt_phi_node *phi_node = NULL;
        REQUIRE_OK(code_container_phi_mutable(code, block->phi_nodes.head, &phi_node));

        *phi_instr_ref_ptr = phi_node->output_ref;
    } else {
        *phi_instr_ref_ptr = KEFIR_ID_NONE;
    }
    return KEFIR_OK;
}

kefir_result_t kefir_opt_code_block_phi_tail(const struct kefir_opt_code_container *code, kefir_opt_block_id_t block_id,
                                             kefir_opt_instruction_ref_t *phi_instr_ref_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(phi_instr_ref_ptr != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to optimizer phi instruction reference"));
    ...
    if (block->phi_nodes.head != KEFIR_ID_NONE) {
        struct kefir_opt_phi_node *phi_node = NULL;
        REQUIRE_OK(code_container_phi_mutable(code, block->phi_nodes.tail, &phi_node));
...
kefir_result_t kefir_opt_code_block_call_head(const struct kefir_opt_code_container *code,
                                              kefir_opt_block_id_t block_id,
                                              kefir_opt_instruction_ref_t *call_instr_ref_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(call_instr_ref_ptr != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to optimizer call instruction reference"));
    ...
    if (block->call_nodes.head != KEFIR_ID_NONE) {
        ...
        REQUIRE_OK(code_container_call_mutable(code, block->call_nodes.head, &call_node));
        *call_instr_ref_ptr = call_node->output_ref;
    } else {
        *call_instr_ref_ptr = KEFIR_ID_NONE;
    }
    return KEFIR_OK;
}
...
kefir_result_t kefir_opt_code_block_call_tail(const struct kefir_opt_code_container *code,
    ...
    if (block->call_nodes.tail != KEFIR_ID_NONE) {
        struct kefir_opt_call_node *call_node;
        REQUIRE_OK(code_container_call_mutable(code, block->call_nodes.tail, &call_node));
        *call_instr_ref_ptr = call_node->output_ref;
    } else {
        *call_instr_ref_ptr = KEFIR_ID_NONE;
    }
...
kefir_result_t kefir_opt_code_block_inline_assembly_head(const struct kefir_opt_code_container *code,
                                                         kefir_opt_block_id_t block_id,
                                                         kefir_opt_instruction_ref_t *inline_asm_instr_ref_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(inline_asm_instr_ref_ptr != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
                            "Expected valid pointer to optimizer inline assembly instruction reference"));

    struct kefir_opt_code_block *block = NULL;
    REQUIRE_OK(code_container_block_mutable(code, block_id, &block));

    if (block->inline_assembly_nodes.head != KEFIR_ID_NONE) {
        struct kefir_opt_inline_assembly_node *node;
        REQUIRE_OK(code_container_inline_assembly_mutable(code, block->inline_assembly_nodes.head, &node));
        *inline_asm_instr_ref_ptr = node->output_ref;
    } else {
        *inline_asm_instr_ref_ptr = KEFIR_ID_NONE;
    }
    return KEFIR_OK;
}
...
kefir_result_t kefir_opt_code_block_inline_assembly_tail(const struct kefir_opt_code_container *code,
    ...
    if (block->inline_assembly_nodes.tail != KEFIR_ID_NONE) {
        struct kefir_opt_inline_assembly_node *node;
        REQUIRE_OK(code_container_inline_assembly_mutable(code, block->inline_assembly_nodes.tail, &node));
        *inline_asm_instr_ref_ptr = node->output_ref;
    } else {
        *inline_asm_instr_ref_ptr = KEFIR_ID_NONE;
    }
...
kefir_result_t kefir_opt_phi_prev_sibling(const struct kefir_opt_code_container *code,
                                          ...
                                          kefir_opt_instruction_ref_t *phi_instr_ref_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(phi_instr_ref != KEFIR_ID_NONE,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer phi instruction reference"));
    REQUIRE(phi_instr_ref_ptr != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to optimizer phi instruction reference"));
    ...
    struct kefir_opt_phi_node *phi = NULL;
    REQUIRE_OK(code_container_phi_mutable(code, phi_instr->operation.parameters.phi_ref, &phi));

    if (phi->siblings.prev != KEFIR_ID_NONE) {
        REQUIRE_OK(code_container_phi_mutable(code, phi->siblings.prev, &phi));
        *phi_instr_ref_ptr = phi->output_ref;
    } else {
        *phi_instr_ref_ptr = KEFIR_ID_NONE;
    }
    return KEFIR_OK;
...
kefir_result_t kefir_opt_phi_next_sibling(const struct kefir_opt_code_container *code,
    ...
    if (phi->siblings.next != KEFIR_ID_NONE) {
        REQUIRE_OK(code_container_phi_mutable(code, phi->siblings.next, &phi));
        *phi_instr_ref_ptr = phi->output_ref;
    } else {
        *phi_instr_ref_ptr = KEFIR_ID_NONE;
    }
...
kefir_result_t kefir_opt_call_prev_sibling(const struct kefir_opt_code_container *code, kefir_opt_call_id_t call_ref,
                                           kefir_opt_instruction_ref_t *call_instr_ref_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(call_ref != KEFIR_ID_NONE,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer call node identifier"));
    REQUIRE(call_instr_ref_ptr != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid pointer to optimizer call instruction reference"));

    struct kefir_opt_instruction *call_instr = NULL;
    REQUIRE_OK(code_container_instr_mutable(code, call_ref, &call_instr));
    REQUIRE(call_instr->operation.opcode == KEFIR_OPT_OPCODE_INVOKE ||
                call_instr->operation.opcode == KEFIR_OPT_OPCODE_INVOKE_VIRTUAL ||
                call_instr->operation.opcode == KEFIR_OPT_OPCODE_TAIL_INVOKE ||
                call_instr->operation.opcode == KEFIR_OPT_OPCODE_TAIL_INVOKE_VIRTUAL,
    ...
    if (call_node->siblings.prev != KEFIR_ID_NONE) {
        REQUIRE_OK(code_container_call_mutable(code, call_node->siblings.prev, &call_node));
        *call_instr_ref_ptr = call_node->output_ref;
    } else {
        *call_instr_ref_ptr = KEFIR_ID_NONE;
    }
...
kefir_result_t kefir_opt_call_next_sibling(const struct kefir_opt_code_container *code, kefir_opt_call_id_t call_ref,
                                           kefir_opt_instruction_ref_t *call_instr_ref_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(call_ref != KEFIR_ID_NONE,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer call node identifier"));
    ...
    if (call_node->siblings.next != KEFIR_ID_NONE) {
        REQUIRE_OK(code_container_call_mutable(code, call_node->siblings.next, &call_node));
        *call_instr_ref_ptr = call_node->output_ref;
    } else {
        *call_instr_ref_ptr = KEFIR_ID_NONE;
    }
...
kefir_result_t kefir_opt_inline_assembly_prev_sibling(const struct kefir_opt_code_container *code,
                                                      ...
                                                      kefir_opt_instruction_ref_t *inline_asm_instr_ref_ptr) {
    REQUIRE(code != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code container"));
    REQUIRE(inline_asm_instr_ref != KEFIR_ID_NONE,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer inline assembly instruction reference"));
    REQUIRE(inline_asm_instr_ref_ptr != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER,
                            "Expected valid pointer to optimizer inline assembly instruction reference"));
    ...
    if (inline_asm_node->siblings.prev != KEFIR_ID_NONE) {
        REQUIRE_OK(code_container_inline_assembly_mutable(code, inline_asm_node->siblings.prev, &inline_asm_node));
        *inline_asm_instr_ref_ptr = inline_asm_node->output_ref;
    } else {
        *inline_asm_instr_ref_ptr = KEFIR_ID_NONE;
    }
    return KEFIR_OK;
...
kefir_result_t kefir_opt_inline_assembly_next_sibling(const struct kefir_opt_code_container *code,
    ...
    if (inline_asm_node->siblings.next != KEFIR_ID_NONE) {
        REQUIRE_OK(code_container_inline_assembly_mutable(code, inline_asm_node->siblings.next, &inline_asm_node));
        *inline_asm_instr_ref_ptr = inline_asm_node->output_ref;
    } else {
        *inline_asm_instr_ref_ptr = KEFIR_ID_NONE;
    }
...
static kefir_result_t verify_dead_uses(struct kefir_opt_code_container *code,
    ...
    kefir_hashset_key_t key;
    kefir_result_t res;
    for (res = kefir_hashset_iter(&used_instr->uses.instruction, &iter, &key); res == KEFIR_OK;
         res = kefir_hashset_next(&iter, &key)) {
        ASSIGN_DECL_CAST(kefir_opt_instruction_ref_t, use_instr_ref, key);
...
kefir_result_t kefir_opt_code_container_drop_dead_code(struct kefir_mem *mem, struct kefir_opt_code_container *code,
    ...
    for (kefir_opt_instruction_ref_t i = 0; i < code->length; i++) {
        ...
        kefir_bool_t is_alive;
        REQUIRE_OK(index->is_instruction_alive(instr->id, &is_alive, index->payload));
        if (!is_alive) {
            REQUIRE_OK(verify_dead_uses(code, index, instr->id));
            kefir_bool_t is_control_flow;
            REQUIRE_OK(kefir_opt_code_instruction_is_control_flow(code, instr->id, &is_control_flow));
            if (is_control_flow) {
                REQUIRE_OK(kefir_opt_code_container_drop_control(code, instr->id));
            }
            REQUIRE_OK(drop_instr_impl(mem, code, instr->id, false));
            if (code->event_listener != NULL) {
                REQUIRE_OK(
                    code->event_listener->on_drop_instruction(mem, code, instr->id, code->event_listener->payload));
            }
        }
    }
...
kefir_result_t kefir_opt_phi_node_link_iter(const struct kefir_opt_code_container *code,
                                            kefir_opt_instruction_ref_t phi_instr_ref,
...
kefir_result_t kefir_opt_code_container_instruction_use_instr_iter(const struct kefir_opt_code_container *code,
    ...
    kefir_result_t res = kefir_hashset_iter(&used_instr->uses.instruction, &iter->iter, &key);
    REQUIRE_OK(res);
    iter->use_instr_ref = (kefir_opt_instruction_ref_t) key;
    return KEFIR_OK;
}
...
static kefir_result_t replace_references_ref_index2(struct kefir_opt_instruction *instr,
                                                    kefir_opt_instruction_ref_t to_ref,
                                                    kefir_opt_instruction_ref_t from_ref) {
    REPLACE_REF(&instr->operation.parameters.refs[0], to_ref, from_ref);
    return KEFIR_OK;
...
kefir_result_t kefir_opt_code_container_replace_references_in(struct kefir_mem *mem,
                                                              const struct kefir_opt_code_container *code,
                                                              kefir_opt_instruction_ref_t user_ref,
                                                              kefir_opt_instruction_ref_t to_ref,
                                                              kefir_opt_instruction_ref_t from_ref) {
...
kefir_result_t kefir_opt_code_container_replace_references(struct kefir_mem *mem,
    ...
    kefir_result_t res;
    struct kefir_hashset_iterator user_iter;
    kefir_hashset_key_t key;
    for (res = kefir_hashset_iter(&from_instr->uses.instruction, &user_iter, &key); res == KEFIR_OK;) {
        ASSIGN_DECL_CAST(kefir_opt_instruction_ref_t, user_ref, key);
        res = kefir_hashset_next(&user_iter, &key);
        REQUIRE_OK(kefir_opt_code_container_replace_references_in(mem, code, user_ref, to_ref, from_ref));
    }
    if (res != KEFIR_ITERATOR_END) {
        REQUIRE_OK(res);
    }

    if (code->event_listener != NULL && code->event_listener->on_replace_instruction != NULL) {
        REQUIRE_OK(
