...
kefir_result_t kefir_opt_code_schedule_of(const struct kefir_opt_code_schedule *schedule,
    ...
    kefir_hashtable_value_t table_value;
    kefir_result_t res =
        kefir_hashtable_at(&schedule->instructions_by_ref, (kefir_hashtable_key_t) instr_ref, &table_value);
    if (res == KEFIR_NOT_FOUND) {
...
kefir_result_t kefir_opt_code_schedule_at(const struct kefir_opt_code_schedule *schedule,
                                          kefir_uint32_t block_linear_position, kefir_uint32_t linear_position,
                                          kefir_opt_instruction_ref_t *instr_ref_ptr) {
    REQUIRE(schedule != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code schedule"));
    REQUIRE(block_linear_position < schedule->blocks_length,
            KEFIR_SET_ERROR(KEFIR_NOT_FOUND, "Unable to find requested optimizer instruction schedule"));
    REQUIRE(linear_position < schedule->blocks[block_linear_position].instructions_length,
            KEFIR_SET_ERROR(KEFIR_NOT_FOUND, "Unable to find requested optimizer instruction schedule"));
...
kefir_result_t kefir_opt_code_schedule_block_by_index(const struct kefir_opt_code_schedule *schedule,
    ...
    REQUIRE(schedule != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer code schedule"));
    REQUIRE(index < schedule->blocks_length,
            KEFIR_SET_ERROR(KEFIR_NOT_FOUND, "Unable to find requested optimizer schedule block"));
...
static kefir_result_t builder_schedule_block(struct kefir_mem *mem, kefir_opt_block_id_t block_id,
                                             kefir_uint32_t *linear_index_ptr, void *payload) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ASSIGN_DECL_CAST(struct builder_payload *, builder_payload, payload);
    REQUIRE(builder_payload != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid optimizer schedule builder payload"));

    REQUIRE(!kefir_hashtable_has(&builder_payload->schedule->blocks_by_ref, (kefir_hashtable_key_t) block_id),
            KEFIR_SET_ERROR(KEFIR_ALREADY_EXISTS, "Optimizer block has altrady been scheduled"));
    ...
    if (builder_payload->schedule->blocks_length >= builder_payload->schedule->blocks_capacity) {
        ...
        struct kefir_opt_code_block_schedule *new_blocks = KEFIR_REALLOC(
            mem, builder_payload->schedule->blocks, sizeof(struct kefir_opt_code_block_schedule) * new_capacity);
        REQUIRE(new_blocks != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate optimizer block schedule"));
    ...
    struct kefir_opt_code_block_schedule *block_schedule =
        &builder_payload->schedule->blocks[builder_payload->schedule->blocks_length];
    ...
    REQUIRE_OK(kefir_hashtable_insert(mem, &builder_payload->schedule->blocks_by_ref, (kefir_hashtable_key_t) block_id,
                                      (kefir_hashtable_value_t) block_schedule->linear_position));
    builder_payload->schedule->blocks_length++;
    ASSIGN_PTR(linear_index_ptr, block_schedule->linear_position);
    return KEFIR_OK;
}

static kefir_result_t builder_schedule_instruction(struct kefir_mem *mem, kefir_opt_block_id_t block_id,
                                                   kefir_opt_instruction_ref_t instr_ref,
                                                   kefir_uint32_t *linear_index_ptr, void *payload) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    ASSIGN_DECL_CAST(struct builder_payload *, builder_payload, payload);
    ...
    REQUIRE(!kefir_hashtable_has(&builder_payload->schedule->instructions_by_ref, (kefir_hashtable_key_t) instr_ref),
            KEFIR_SET_ERROR(KEFIR_ALREADY_EXISTS, "Optimizer instruction has altrady been scheduled"));
    kefir_hashtable_value_t table_value;
    kefir_result_t res =
        kefir_hashtable_at(&builder_payload->schedule->blocks_by_ref, (kefir_hashtable_key_t) block_id, &table_value);
    if (res == KEFIR_NOT_FOUND) {
    ...
    if (block_schedule->instructions_length >= block_schedule->instructions_capacity) {
        ...
        struct kefir_opt_code_instruction_schedule *new_instructions = KEFIR_REALLOC(
            mem, block_schedule->instructions, sizeof(struct kefir_opt_code_instruction_schedule) * new_capacity);
        REQUIRE(new_instructions != NULL,
                KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate optimizer instruction schedule"));
    ...
    struct kefir_opt_code_instruction_schedule *instruction_schedule =
        &block_schedule->instructions[block_schedule->instructions_length];
    ...
    table_value = (((kefir_uint64_t) block_schedule->linear_position) << 32) |
                  (kefir_uint32_t) instruction_schedule->linear_position;
    REQUIRE_OK(kefir_hashtable_insert(mem, &builder_payload->schedule->instructions_by_ref,
                                      (kefir_hashtable_key_t) instr_ref, table_value));
