...
static kefir_result_t perform_jump(struct kefir_mem *mem, struct kefir_ast_translator_context *context,
    ...
    REQUIRE_OK(
        kefir_ast_translator_mark_associated_scope_objects_lifetime(mem, context, builder, original_position->self));
    REQUIRE_OK(
        kefir_ast_translator_mark_associated_scope_objects_lifetime(mem, context, builder, target_position->self));
