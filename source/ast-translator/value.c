...
static kefir_result_t load_bitfield(struct kefir_irbuilder_block *builder, struct kefir_ast_type_layout *layout,
    ...
    if (KEFIR_AST_TYPE_IS_BIT_PRECISE_INTEGRAL_TYPE(layout->type)) {
        REQUIRE_OK(
            KEFIR_IRBUILDER_BLOCK_APPENDU32_4(builder, KEFIR_IR_OPCODE_BITINT_LOAD_PRECISE, bits, false, mem_flags, 0));
    ...
    } else if (KEFIR_AST_TYPE_IS_EXTENDED_INTEGER(layout->type)) {
        switch (layout->type->tag) {
            case KEFIR_AST_TYPE_SCALAR_SIGNED_INT128:
            case KEFIR_AST_TYPE_SCALAR_UNSIGNED_INT128:
                if (bits <= 64) {
                    REQUIRE_OK(load_native_bitfield(builder, config, bits, mem_flags));
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_INT128_ZERO_EXTEND_64BITS, 0));
                } else {
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_VSTACK_PICK, 0));
                    REQUIRE_OK(load_native_bitfield(builder, config, 64, mem_flags));
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_VSTACK_EXCHANGE, 1));
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_INT_CONST, 8));
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_INT64_ADD, 0));
                    REQUIRE_OK(load_native_bitfield(builder, config, bits - 64, mem_flags));
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_INT128_FROM, 0));
                }
                break;
        ...
        switch (member_layout->type->tag) {
            ...
            case KEFIR_AST_TYPE_SCALAR_UNSIGNED_INT128:
                ...
                if (signedness) {
                    ...
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_INT128_RSHIFT, 0));
                }
                break;
...
kefir_result_t kefir_ast_translator_load_value(const struct kefir_ast_type *type,
    ...
    switch (normalized_data_model) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_FLOAT:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_FLOAT32_LOAD, mem_flags));
            break;
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_DOUBLE:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_FLOAT64_LOAD, mem_flags));
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_INT128:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_INT128_LOAD, mem_flags));
            break;
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL32:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_DECIMAL32_LOAD, mem_flags));
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL64:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_DECIMAL64_LOAD, mem_flags));
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL128:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_DECIMAL128_LOAD, mem_flags));
            break;
...
static kefir_result_t atomic_store_value(struct kefir_mem *mem, const struct kefir_ast_type *type,
    ...
    switch (normalized_data_model) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_BITINT: {
            const kefir_uint64_t mem_flags = retrieve_memflags(type);
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU32_4(builder, KEFIR_IR_OPCODE_BITINT_ATOMIC_STORE,
...
kefir_result_t kefir_ast_translator_store_value(struct kefir_mem *mem, const struct kefir_ast_type *type,
    ...
    switch (normalized_data_model) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_FLOAT:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_FLOAT32_STORE, mem_flags));
            break;
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_INT128:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_INT128_STORE, mem_flags));
            break;
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL32:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_DECIMAL32_STORE, mem_flags));
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL64:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_DECIMAL64_STORE, mem_flags));
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL128:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_DECIMAL128_STORE, mem_flags));
            break;
...
kefir_result_t kefir_ast_translator_atomic_compare_exchange_value(struct kefir_mem *mem,
    ...
    switch (normalized_data_model) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL32:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_DECIMAL32_ATOMIC_CMPXCHG,
                                                       atomic_memory_order));
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL64:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_DECIMAL64_ATOMIC_CMPXCHG,
                                                       atomic_memory_order));
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL128:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_DECIMAL128_ATOMIC_CMPXCHG,
                                                       atomic_memory_order));
            break;
