...
static kefir_result_t translate_vla_declaration(struct kefir_mem *mem, const struct kefir_ast_node_base *node,
    ...
    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_STACK_ALLOC, 0));
    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_INT64_STORE, KEFIR_IR_MEMORY_FLAG_NONE));
    return KEFIR_OK;
}
...
static kefir_result_t translate_variably_modified(const struct kefir_ast_node_base *node, void *payload) {
    ...
    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(param->builder, KEFIR_IR_OPCODE_VSTACK_POP, 0));
    return KEFIR_OK;
}
...
static kefir_result_t translate_init_declarator(struct kefir_mem *mem, const struct kefir_ast_node_base *node,
    ...
    if (node->properties.declaration_props.static_assertion ||
        ...
        if (storage == KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_CONSTEXPR) {
            ...
            if (KEFIR_AST_TYPE_IS_SCALAR_TYPE(unqualified_type)) {
                if (init_decl == NULL) {
                    trivial_initializer = true;
                } else if (init_decl->initializer->type == KEFIR_AST_INITIALIZER_EXPRESSION) {
                    REQUIRE_OK(kefir_ast_try_translate_constant(
                        NULL, unqualified_type,
                        KEFIR_AST_NODE_CONSTANT_EXPRESSION_VALUE(init_decl->initializer->expression), NULL, NULL,
                        &trivial_initializer));
                }
            }
...
kefir_result_t kefir_ast_translate_declaration(struct kefir_mem *mem, const struct kefir_ast_node_base *node,
    ...
    if (context->function_debug_info != NULL && KEFIR_SOURCE_LOCATION_IS_VALID(&node->source_location)) {
        ...
        REQUIRE_OK(kefir_ir_debug_function_source_map_insert(mem, &context->function_debug_info->source_map,
                                                             &context->module->symbols, &node->source_location,
