...
static kefir_result_t translate_static_identifier(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    REQUIRE_CHAIN(&res, kefir_ast_translate_object_type(
                            mem, context, object_type, scoped_identifier->object.alignment->value, env, &global_builder,
                            &scoped_identifier_layout->layout, source_location));
...
static kefir_result_t translate_static_thread_local_identifier(
    ...
    kefir_result_t res = kefir_ast_translate_object_type(
        mem, context, scoped_identifier->object.type, scoped_identifier->object.alignment->value, env, &global_builder,
        &scoped_identifier_layout->layout, source_location);
    REQUIRE_ELSE(res == KEFIR_OK, {
        KEFIR_IRBUILDER_TYPE_FREE(&global_builder);
        return res;
...
static kefir_result_t translate_local_scoped_identifier_object(
    ...
    switch (scoped_identifier->object.storage) {
        ...
        case KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_STATIC_THREAD_LOCAL:
            REQUIRE_OK(translate_static_thread_local_identifier(mem, context, env, local_layout, identifier,
    ...
    switch (scoped_identifier->klass) {
        case KEFIR_AST_SCOPE_IDENTIFIER_OBJECT:
            REQUIRE_OK(translate_local_scoped_identifier_object(mem, context, builder, identifier, scoped_identifier,
                                                                env, local_layout, scope_type_layout, wrapper_structure,
                                                                &scoped_identifier->source_location));
            break;
...
static kefir_result_t traverse_local_scope(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    const kefir_size_t begin = kefir_ir_type_length(builder->type);
    if (!empty_scope) {
        *scope_type_layout = kefir_ast_new_type_layout(mem, NULL, 0, begin);
        REQUIRE_OK(KEFIR_IRBUILDER_TYPE_APPEND(builder, KEFIR_IR_TYPE_STRUCT, 0, 0));
    }
    struct kefir_ast_identifier_flat_scope_iterator iter;
    kefir_result_t res;
    for (res = kefir_ast_identifier_flat_scope_iter(scope, &iter); res == KEFIR_OK;
    ...
    if (!empty_children) {
        const kefir_size_t children_begin = kefir_ir_type_length(builder->type);
        ...
        typeentry->param++;
        REQUIRE_OK(KEFIR_IRBUILDER_TYPE_APPEND(builder, KEFIR_IR_TYPE_UNION, 0, 0));
        for (struct kefir_tree_node *child = kefir_tree_first_child(root); child != NULL;
             child = kefir_tree_next_sibling(child)) {
    ...
    if (!empty_children) {
        for (struct kefir_tree_node *child = kefir_tree_first_child(root); child != NULL;
            ...
            REQUIRE_OK(traverse_local_scope_flat(mem, context, child, builder, env, type_bundle, type_traits, module,
                                                 local_layout, scope_type_layout));
        }
    } else {
        for (struct kefir_tree_node *child = kefir_tree_first_child(root); child != NULL;
             child = kefir_tree_next_sibling(child)) {
...
kefir_result_t kefir_ast_translator_build_local_scope_layout(struct kefir_mem *mem,
    ...
    if (!kefir_ast_identifier_block_scope_empty(&context->ordinary_scope)) {
        ...
        if (env->configuration->flat_local_scope_layout) {
            REQUIRE_OK(traverse_local_scope_flat(mem, &context->context, &context->ordinary_scope.root, &builder, env,
                                                 ...
                                                 layout, &layout->local_type_layout));
        } else {
