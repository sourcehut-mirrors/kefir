...
static kefir_result_t kefir_ast_translate_initializer_impl(struct kefir_mem *, struct kefir_ast_translator_context *,
...
static kefir_result_t traverse_scalar(const struct kefir_ast_designator *designator,
    ...
    if (!KEFIR_AST_TYPE_IS_SCALAR_TYPE(type_layout->type) && KEFIR_AST_TYPE_IS_SCALAR_TYPE(expr_type)) {
        REQUIRE_OK(initialize_aggregate_with_scalar(param->mem, param->context, param->builder, type_layout, expression,
               ...
               !expression->properties.expression_props->preserve_after_eval) {
        ...
        REQUIRE_OK(kefir_ast_translator_store_layout_value(param->mem, param->context, param->builder,
                                                           param->translator_type->object.ir_type, type_layout, false,
...
static kefir_result_t traverse_string_literal(const struct kefir_ast_designator *designator,
    ...
    if (type_layout->type->array_type.boundary == KEFIR_AST_ARRAY_BOUNDED ||
        type_layout->type->array_type.boundary == KEFIR_AST_ARRAY_BOUNDED_STATIC) {
        length = MIN(length, kefir_ast_type_array_const_length(&type_layout->type->array_type));
    }

    REQUIRE_OK(translate_address(param->translator_type, designator, param->builder));
    const struct kefir_ast_type *array_type = kefir_ast_type_array(param->mem, param->context->ast_context->type_bundle,
                                                                   type_layout->type->array_type.element_type, length,
                                                                   &type_layout->type->array_type.qualifications);
    REQUIRE_OK(kefir_ast_translate_expression(param->mem, expression, param->builder, param->context));
...
static kefir_result_t kefir_ast_translate_initializer_impl(
    ...
    REQUIRE(unqualified_type != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_STATE, "Unable to obtain unqualified AST type"));
...
kefir_result_t kefir_ast_translate_initializer(struct kefir_mem *mem, struct kefir_ast_translator_context *context,
    ...
    REQUIRE_OK(kefir_hashtreeset_free(mem, &repeated_expressions));
    return KEFIR_OK;
}
...
kefir_result_t kefir_ast_translate_default_initializer(struct kefir_mem *mem,
    ...
    if (KEFIR_AST_TYPE_IS_INTEGRAL_TYPE(unqualified_type) &&
        ...
        !KEFIR_AST_TYPE_IS_128BIT_INTEGER_TYPE(unqualified_type)) {
        REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_VSTACK_PICK, 0));
        ...
        REQUIRE_OK(kefir_ast_translator_store_value(mem, type, context, builder, source_location));
               ...
               unqualified_type->tag == KEFIR_AST_TYPE_SCALAR_INTERCHANGE_FLOAT32) {
        REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_VSTACK_PICK, 0));
        ...
        REQUIRE_OK(kefir_ast_translator_store_value(mem, type, context, builder, source_location));
               ...
               unqualified_type->tag == KEFIR_AST_TYPE_SCALAR_INTERCHANGE_FLOAT64 ||
               unqualified_type->tag == KEFIR_AST_TYPE_SCALAR_EXTENDED_FLOAT32) {
        REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_VSTACK_PICK, 0));
