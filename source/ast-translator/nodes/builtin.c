        ...
        REQUIRE(*(_ir_decl) != NULL,                                                                              \
            ...
            switch (classification) {
                ...
                case KEFIR_AST_TYPE_DATA_MODEL_INT128: {
                    ...
                    REQUIRE_OK(
                        kefir_ast_type_is_signed(context->ast_context->type_traits, unqualified_type, &is_signed));
                    if (is_signed) {
                        REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(
                            builder, KEFIR_IR_OPCODE_INT128_SIGNED_TO_BITINT,
                            context->ast_context->type_traits->data_model->scalar_width.int128_bits));
                    } else {
                        REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(
                            builder, KEFIR_IR_OPCODE_INT128_UNSIGNED_TO_BITINT,
                            context->ast_context->type_traits->data_model->scalar_width.int128_bits));
                    }
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(
                        builder, KEFIR_IR_OPCODE_BITINT_BUILTIN_FFS,
                        context->ast_context->type_traits->data_model->scalar_width.int128_bits));
                } break;
...
#define CLZ_IMPL(_width)                                                                                               \
    do {                                                                                                               \
        ...
        if (default_value_node != NULL) {                                                                              \
            ...
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_BITINT_EQUAL, (_width)));              \
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_INT8_BOOL_NOT, 0));                    \
            kefir_size_t branchIndex = KEFIR_IRBUILDER_BLOCK_CURRENT_INDEX(builder);                                   \
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64_2(builder, KEFIR_IR_OPCODE_BRANCH, 0,                           \
                                                         KEFIR_IR_BRANCH_CONDITION_8BIT));                             \
            ...
            REQUIRE_OK(kefir_ast_translate_expression(mem, default_value_node, builder, context));                     \
            REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,  \
                                                    default_value_node->properties.type,                               \
        ...
        REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_BITINT_BUILTIN_CLZ, (_width)));            \
                                                                                                                       \
        if (default_value_node != NULL) {                                                                              \
            KEFIR_IRBUILDER_BLOCK_INSTR_AT(builder, jmpIndex)->arg.i64 = KEFIR_IRBUILDER_BLOCK_CURRENT_INDEX(builder); \
        }                                                                                                              \
    } while (0)
...
#define CTZ_IMPL(_width)                                                                                               \
    do {                                                                                                               \
        ...
        REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(builder, KEFIR_IR_OPCODE_BITINT_BUILTIN_CTZ, (_width)));            \
                                                                                                                       \
        if (default_value_node != NULL) {                                                                              \
            KEFIR_IRBUILDER_BLOCK_INSTR_AT(builder, jmpIndex)->arg.i64 = KEFIR_IRBUILDER_BLOCK_CURRENT_INDEX(builder); \
        }                                                                                                              \
        ...
        REQUIRE(*(_ir_decl) != NULL,                                                                              \
            ...
            switch (classification) {
                ...
                case KEFIR_AST_TYPE_DATA_MODEL_INT128: {
                    ...
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(
                        builder, KEFIR_IR_OPCODE_BITINT_BUILTIN_CLRSB,
                        context->ast_context->type_traits->data_model->scalar_width.int128_bits));
                } break;
        ...
        REQUIRE(*(_ir_decl) != NULL,                                                                              \
            ...
            switch (classification) {
                ...
                case KEFIR_AST_TYPE_DATA_MODEL_INT128: {
                    ...
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(
                        builder, KEFIR_IR_OPCODE_BITINT_BUILTIN_POPCOUNT,
                        context->ast_context->type_traits->data_model->scalar_width.int128_bits));
                } break;
            ...
            switch (classification) {
                ...
                case KEFIR_AST_TYPE_DATA_MODEL_INT64: {
                    ...
                    const struct kefir_ir_function_decl *ir_decl = kefir_ir_module_new_function_declaration(
                        mem, context->module,
                        context->ast_context->type_traits->data_model->scalar_width.long_bits >= 64
                            ? "__kefir_builtin_parityl"
                            : "__kefir_builtin_parityll",
                        parameters_type_id, false, returns_type_id);
                    REQUIRE(ir_decl != NULL,
                            KEFIR_SET_ERROR(KEFIR_OBJALLOC_FAILURE, "Failed to allocate IR function declaration"));
                ...
                case KEFIR_AST_TYPE_DATA_MODEL_INT128: {
                    ...
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDU64(
                        builder, KEFIR_IR_OPCODE_BITINT_BUILTIN_PARITY,
                        context->ast_context->type_traits->data_model->scalar_width.int128_bits));
                } break;
...
#undef BUILTIN_POPCOUNTG_32BIT
        ...
        case KEFIR_AST_BUILTIN_KEFIR_INT_PRECISION: {
            ...
            switch (classification) {
                ...
                case KEFIR_AST_TYPE_DATA_MODEL_INT16:
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT_CONST, 16));
                    break;

                case KEFIR_AST_TYPE_DATA_MODEL_INT32:
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT_CONST, 32));
                    break;
...
#define DEF_COPYSIGN(_id, _ast_type, _ir_type)                                                                     \
    do {                                                                                                           \
        ...
        kefir_id_t parameters_type_id, returns_type_id;                                                            \
        struct kefir_ir_type *parameters_type =                                                                    \
            kefir_ir_module_new_type(mem, context->module, 2, &parameters_type_id);                                \
        REQUIRE(parameters_type != NULL, KEFIR_SET_ERROR(KEFIR_OBJALLOC_FAILURE, "Failed to allocate IR type"));   \
        struct kefir_ir_type *returns_type = kefir_ir_module_new_type(mem, context->module, 1, &returns_type_id);  \
        REQUIRE(returns_type != NULL, KEFIR_SET_ERROR(KEFIR_OBJALLOC_FAILURE, "Failed to allocate IR type"));      \
                                                                                                                   \
        REQUIRE_OK(kefir_ir_type_append(parameters_type, (_ir_type), 0, 0));                                       \
        REQUIRE_OK(kefir_ir_type_append(parameters_type, (_ir_type), 0, 0));                                       \
        REQUIRE_OK(kefir_ir_type_append(returns_type, (_ir_type), 0, 0));                                          \
        ir_decl = kefir_ir_module_new_function_declaration(mem, context->module, (_id), parameters_type_id, false, \
                                                           returns_type_id);                                       \
        REQUIRE(ir_decl != NULL,                                                                                   \
                KEFIR_SET_ERROR(KEFIR_OBJALLOC_FAILURE, "Failed to allocate IR function declaration"));            \
        ...
        case KEFIR_AST_BUILTIN_KEFIR_COPYSIGNF:
            DEF_COPYSIGN("__kefir_builtin_copysignf", kefir_ast_type_float(), KEFIR_IR_TYPE_FLOAT32);
            break;

        case KEFIR_AST_BUILTIN_KEFIR_COPYSIGN:
            DEF_COPYSIGN("__kefir_builtin_copysign", kefir_ast_type_double(), KEFIR_IR_TYPE_FLOAT64);
            break;

        case KEFIR_AST_BUILTIN_KEFIR_COPYSIGNL:
            DEF_COPYSIGN("__kefir_builtin_copysignl", kefir_ast_type_long_double(), KEFIR_IR_TYPE_LONG_DOUBLE);
            break;
...
#undef DEF_COPYSIGN
        ...
        case KEFIR_AST_BUILTIN_KEFIR_CONSTRUCT_COMPLEX_FLOAT: {
            ...
            REQUIRE_OK(kefir_ast_translate_expression(mem, arg1_node, builder, context));
            REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,
                                                    arg1_type, kefir_ast_type_float()));
            REQUIRE_OK(kefir_ast_translate_expression(mem, arg2_node, builder, context));
            REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,
                                                    arg2_type, kefir_ast_type_float()));
        ...
        case KEFIR_AST_BUILTIN_KEFIR_CONSTRUCT_COMPLEX_DOUBLE: {
            ...
            REQUIRE_OK(kefir_ast_translate_expression(mem, arg1_node, builder, context));
            REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,
                                                    arg1_type, kefir_ast_type_double()));
            REQUIRE_OK(kefir_ast_translate_expression(mem, arg2_node, builder, context));
            REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,
                                                    arg2_type, kefir_ast_type_double()));
        ...
        case KEFIR_AST_BUILTIN_KEFIR_CONSTRUCT_COMPLEX_LONG_DOUBLE: {
            ...
            REQUIRE_OK(kefir_ast_translate_expression(mem, arg1_node, builder, context));
            REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,
                                                    arg1_type, kefir_ast_type_long_double()));
            REQUIRE_OK(kefir_ast_translate_expression(mem, arg2_node, builder, context));
            REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,
