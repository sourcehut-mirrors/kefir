...
static kefir_result_t generate_multiplication_op(const struct generate_op_parameters *parameters) {
    kefir_bool_t common_type_signed;
    kefir_ast_type_data_model_classification_t common_type_classification;
    REQUIRE_OK(kefir_ast_type_data_model_classify(parameters->context->ast_context->type_traits,
                                                  parameters->common_type, &common_type_classification));
    switch (common_type_classification) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_BITINT:
            REQUIRE_OK(kefir_ast_type_is_signed(parameters->context->ast_context->type_traits, parameters->common_type,
                                                &common_type_signed));
            if (common_type_signed) {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(parameters->builder, KEFIR_IR_OPCODE_BITINT_IMUL,
                                                           parameters->common_type->bitprecise.width));
            } else {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(parameters->builder, KEFIR_IR_OPCODE_BITINT_UMUL,
                                                           parameters->common_type->bitprecise.width));
            }
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL32:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(parameters->builder, KEFIR_IR_OPCODE_DECIMAL32_MUL, 0));
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL64:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(parameters->builder, KEFIR_IR_OPCODE_DECIMAL64_MUL, 0));
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL128:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(parameters->builder, KEFIR_IR_OPCODE_DECIMAL128_MUL, 0));
            break;
    ...
    switch (common_type_classification) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_BITINT:
            REQUIRE_OK(kefir_ast_type_is_signed(params->context->ast_context->type_traits, params->common_type,
                                                &common_type_signed));
            if (common_type_signed) {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_BITINT_IDIV,
                                                           params->common_type->bitprecise.width));
            } else {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_BITINT_UDIV,
                                                           params->common_type->bitprecise.width));
            }
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL32:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_DECIMAL32_DIV, 0));
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL64:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_DECIMAL64_DIV, 0));
            break;

        case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL128:
            REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_DECIMAL128_DIV, 0));
            break;
    ...
    switch (common_type_classification) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_BITINT:
            REQUIRE_OK(kefir_ast_type_is_signed(params->context->ast_context->type_traits, params->common_type,
                                                &common_type_signed));
            if (common_type_signed) {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_BITINT_IMOD,
                                                           params->common_type->bitprecise.width));
            } else {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_BITINT_UMOD,
                                                           params->common_type->bitprecise.width));
            }
            break;
    ...
    switch (result_type_classification) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_BITINT:
            ...
            if (result_type_signed) {
                ...
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_BITINT_RSHIFT,
                                                           params->result_normalized_type->bitprecise.width));
            }
            break;
...
static kefir_result_t generate_add(const struct generate_op_parameters *params) {
    if (KEFIR_AST_TYPE_IS_ARITHMETIC_TYPE(params->target_normalized_type) &&
        ...
        switch (common_type_classification) {
            ...
            case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL32:
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_DECIMAL32_ADD, 0));
                break;

            case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL64:
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_DECIMAL64_ADD, 0));
                break;

            case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL128:
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_DECIMAL128_ADD, 0));
                break;
...
static kefir_result_t generate_sub(const struct generate_op_parameters *params) {
    if (KEFIR_AST_TYPE_IS_ARITHMETIC_TYPE(params->target_normalized_type) &&
        ...
        switch (common_type_classification) {
            ...
            case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL32:
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_DECIMAL32_SUB, 0));
                break;

            case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL64:
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_DECIMAL64_SUB, 0));
                break;

            case KEFIR_AST_TYPE_DATA_MODEL_DECIMAL128:
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(params->builder, KEFIR_IR_OPCODE_DECIMAL128_SUB, 0));
                break;
