...
static kefir_result_t binary_prologue(struct kefir_mem *mem, struct kefir_ast_translator_context *context,
    ...
    const kefir_bool_t multiplicative_op =
        node->type == KEFIR_AST_OPERATION_MULTIPLY || node->type == KEFIR_AST_OPERATION_DIVIDE;
    REQUIRE_OK(kefir_ast_translate_expression(mem, node->arg1, builder, context));
    if (multiplicative_op && KEFIR_AST_TYPE_IS_IMAGINARY_TYPE(arg1_normalized_type) &&
    ...
    } else if (multiplicative_op && KEFIR_AST_TYPE_IS_IMAGINARY_TYPE(arg1_normalized_type) &&
               KEFIR_AST_TYPE_IS_IMAGINARY_TYPE(result_normalized_type)) {
        REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,
                                                arg1_normalized_type->imaginary.real_type,
                                                result_normalized_type->imaginary.real_type));
    } else if (multiplicative_op && KEFIR_AST_TYPE_IS_IMAGINARY_TYPE(arg2_normalized_type) &&
               KEFIR_AST_TYPE_IS_IMAGINARY_TYPE(result_normalized_type)) {
        REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,
    ...
    if ((node->type == KEFIR_AST_OPERATION_SHIFT_LEFT || node->type == KEFIR_AST_OPERATION_SHIFT_RIGHT) &&
        (KEFIR_AST_TYPE_IS_BIT_PRECISE_INTEGRAL_TYPE(result_normalized_type) ||
         KEFIR_AST_TYPE_IS_128BIT_INTEGER_TYPE(result_normalized_type))) {
        REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,
    ...
    } else if (multiplicative_op && KEFIR_AST_TYPE_IS_IMAGINARY_TYPE(result_normalized_type) &&
               KEFIR_AST_TYPE_IS_IMAGINARY_TYPE(arg2_normalized_type)) {
        REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,
                                                arg2_normalized_type->imaginary.real_type,
                                                result_normalized_type->imaginary.real_type));
    } else if (multiplicative_op && KEFIR_AST_TYPE_IS_IMAGINARY_TYPE(result_normalized_type) &&
               KEFIR_AST_TYPE_IS_IMAGINARY_TYPE(arg1_normalized_type)) {
        REQUIRE_OK(kefir_ast_translate_typeconv(mem, context->module, builder, context->ast_context->type_traits,
    ...
    if (KEFIR_AST_TYPE_IS_ARITHMETIC_TYPE(arg1_normalized_type) &&
        ...
        switch (result_type_classification) {
            ...
            case KEFIR_AST_TYPE_DATA_MODEL_INT128:
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_ADD, 0));
                break;
...
static kefir_result_t translate_subtraction(struct kefir_mem *mem, struct kefir_ast_translator_context *context,
    ...
    if (KEFIR_AST_TYPE_IS_ARITHMETIC_TYPE(arg1_normalized_type) &&
        ...
        switch (result_type_classification) {
            ...
            case KEFIR_AST_TYPE_DATA_MODEL_INT128:
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_SUB, 0));
                break;
...
static kefir_result_t translate_multiplication(struct kefir_mem *mem, struct kefir_ast_translator_context *context,
    ...
    switch (result_type_classification) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_LONG_DOUBLE:
            ...
            if (KEFIR_AST_TYPE_IS_IMAGINARY_TYPE(arg1_normalized_type) &&
                KEFIR_AST_TYPE_IS_IMAGINARY_TYPE(arg2_normalized_type)) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_INT128:
            REQUIRE_OK(kefir_ast_type_is_signed(context->ast_context->type_traits, result_normalized_type,
                                                &result_type_signedness));
            if (result_type_signedness) {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_IMUL, 0));
            } else {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_UMUL, 0));
            }
            break;
    ...
    switch (result_type_classification) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_INT128:
            REQUIRE_OK(kefir_ast_type_is_signed(context->ast_context->type_traits, result_normalized_type,
                                                &result_type_signedness));
            if (result_type_signedness) {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_IDIV, 0));
            } else {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_UDIV, 0));
            }
            break;
    ...
    switch (result_type_classification) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_INT128:
            REQUIRE_OK(kefir_ast_type_is_signed(context->ast_context->type_traits, result_normalized_type,
                                                &result_type_signedness));
            if (result_type_signedness) {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_IMOD, 0));
            } else {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_UMOD, 0));
            }
            break;
    ...
    switch (node->type) {
        case KEFIR_AST_OPERATION_SHIFT_LEFT:
            switch (result_type_classification) {
                ...
                case KEFIR_AST_TYPE_DATA_MODEL_INT128:
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_LSHIFT, 0));
                    break;
            ...
            switch (result_type_classification) {
                ...
                case KEFIR_AST_TYPE_DATA_MODEL_INT128:
                    REQUIRE_OK(kefir_ast_type_is_signed(context->ast_context->type_traits, result_normalized_type,
                                                        &result_type_signedness));
                    if (result_type_signedness) {
                        REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_ARSHIFT, 0));
                    } else {
                        REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_RSHIFT, 0));
                    }
                    break;
        ...
        case KEFIR_AST_OPERATION_BITWISE_AND:
            switch (result_type_classification) {
                ...
                case KEFIR_AST_TYPE_DATA_MODEL_INT128:
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_AND, 0));
                    break;
        ...
        case KEFIR_AST_OPERATION_BITWISE_OR:
            switch (result_type_classification) {
                ...
                case KEFIR_AST_TYPE_DATA_MODEL_INT128:
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_OR, 0));
                    break;
        ...
        case KEFIR_AST_OPERATION_BITWISE_XOR:
            switch (result_type_classification) {
                ...
                case KEFIR_AST_TYPE_DATA_MODEL_INT128:
                    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_XOR, 0));
                    break;
...
static kefir_result_t translate_relational_less(const struct kefir_ast_type_traits *type_traits,
    ...
    switch (common_type_classification) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_INT128:
            REQUIRE_OK(kefir_ast_type_is_signed(type_traits, common_type, &common_type_signedness));
            if (common_type_signedness) {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_LESS, 0));
            } else {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_BELOW, 0));
            }
            break;
...
static kefir_result_t translate_relational_greater(const struct kefir_ast_type_traits *type_traits,
    ...
    switch (common_type_classification) {
        ...
        case KEFIR_AST_TYPE_DATA_MODEL_INT128:
            REQUIRE_OK(kefir_ast_type_is_signed(type_traits, common_type, &common_type_signedness));
            if (common_type_signedness) {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_GREATER, 0));
            } else {
                REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IR_OPCODE_INT128_ABOVE, 0));
            }
            break;
