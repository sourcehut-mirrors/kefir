...
static kefir_result_t dump_action_impl(struct kefir_mem *mem, const struct kefir_compiler_runner_configuration *options,
    ...
    compiler.parser_configuration.max_errors = options->features.max_parser_errors;
    ...
    compiler.ast_global_context.configuration.analysis.enable_thread_local_common = options->codegen.tls_common;
    ...
    compiler.translator_configuration.cx_limited_range = options->optimizer.cx_limited_range;
    ...
    compiler.optimizer_configuration.position_independent_code =
        compiler.codegen_configuration.position_independent_code;
    ...
    compiler.optimizer_configuration.max_inlines_per_caller = options->optimizer.max_inlines_per_caller;
    compiler.optimizer_configuration.max_inline_caller_blocks = options->optimizer.max_inline_caller_blocks;
    compiler.optimizer_configuration.max_inline_callee_instructions = options->optimizer.max_inline_callee_instructions;
    compiler.optimizer_configuration.max_inline_leaf_callee_instructions = options->optimizer.max_inline_leaf_callee_instructions;
...
static kefir_result_t new_function_macro(struct kefir_mem *mem, struct kefir_compiler_context *compiler,
                                         const char *definition, struct kefir_preprocessor_user_macro **macro) {
    ...
    REQUIRE(macro_name_length > 0 && macro_name_length < sizeof(identifier_buf) - 1,
            KEFIR_SET_ERRORF(KEFIR_UI_ERROR, "Function macro name '%*s' exceeds maximum length", macro_name_length,
                             definition));
    ...
    REQUIRE(macro_name != NULL,
            KEFIR_SET_ERROR(KEFIR_OBJALLOC_FAILURE, "Failed to insert function macro name into string pool"));
...
#define ADD_PARAM                                                                                                  \
    do {                                                                                                           \
        ...
        REQUIRE(param_name_length > 0 && param_name_length < sizeof(identifier_buf) - 1,                           \
                KEFIR_SET_ERRORF(KEFIR_UI_ERROR, "Function macro '%s' parameter '%*s' exceeds maximum length",     \
                                 macro_name, param_name_length, param_begin));                                     \
        ...
        if (param_name_length > 3 && strcmp(identifier_buf + param_name_length - 3, "...") == 0) {                 \
            ...
            const char *parameter_name =                                                                           \
                kefir_string_pool_insert(mem, &compiler->ast_global_context.symbols, identifier_buf, NULL);        \
            REQUIRE(parameter_name != NULL,                                                                        \
                    KEFIR_SET_ERROR(KEFIR_OBJALLOC_FAILURE,                                                        \
                                    "Failed to insert function macro parameter into string pool"));                \
            (*macro)->vararg = true;                                                                               \
            ...
            REQUIRE(parameter_name != NULL,                                                                        \
                    KEFIR_SET_ERROR(KEFIR_OBJALLOC_FAILURE,                                                        \
                                    "Failed to insert function macro parameter into string pool"));                \
            REQUIRE_OK(kefir_list_insert_after(mem, &(*macro)->parameters, kefir_list_tail(&(*macro)->parameters), \
                                               (void *) parameter_name));                                          \
        }                                                                                                          \
                                                                                                                   \
    } while (0)
    for (const char *iter = lparen_ptr + 1;;) {
        if (*iter == ')') {
            REQUIRE(iter[1] == '\0',
                    KEFIR_SET_ERRORF(KEFIR_LEXER_ERROR,
                                     "Expected function macro '%s' name to be terminated by the right parenthese",
                                     macro_name));
            if (param_begin != NULL) {
                ADD_PARAM;
            }
        ...
        if (*iter == ',') {
            REQUIRE(param_begin != NULL,
                    KEFIR_SET_ERRORF(KEFIR_LEXER_ERROR, "Expected function macro '%s' has invalid parameter list",
                                     macro_name));
            ADD_PARAM;
            param_begin = NULL;
...
struct format_macro_definition_payload {
    const struct kefir_preprocessor *preprocessor;
    FILE *output;
};

static kefir_result_t format_macro_definition(const struct kefir_preprocessor_macro *macro, void *payload) {
    REQUIRE(macro != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid preprocessor macro"));
    ASSIGN_DECL_CAST(struct format_macro_definition_payload *, format_payload, payload);
    REQUIRE(format_payload != NULL,
            KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid preprocessor macro formatter payload"));

    if (macro->format != NULL) {
        REQUIRE_OK(macro->format(format_payload->preprocessor, macro, format_payload->output));
    }
    return KEFIR_OK;
}

static kefir_result_t format_macro_definitions(struct kefir_preprocessor *preprocessor, void *payload) {
    REQUIRE(preprocessor != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid preprocessor"));
    ASSIGN_DECL_CAST(FILE *, output, payload);
    REQUIRE(output != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid file output"));

    REQUIRE_OK(preprocessor->macro_overlay.scope.iterate(
        &preprocessor->macro_overlay.scope, format_macro_definition,
        &(struct format_macro_definition_payload) {.preprocessor = preprocessor, .output = output}));
    return KEFIR_OK;
}
...
static kefir_result_t dump_ir_impl(struct kefir_mem *mem, const struct kefir_compiler_runner_configuration *options,
    ...
    REQUIRE_OK(kefir_memory_arena_reset(&compiler->ast_arena));
