...
kefir_result_t kefir_ast_evaluate_builtin_node(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    switch (node->builtin) {
        case KEFIR_AST_BUILTIN_OFFSETOF: {
            ...
            REQUIRE_OK(KEFIR_AST_TARGET_ENVIRONMENT_GET_TYPE(mem, context->target_env, offset_base->properties.type,
                                                             &opaque_type));

            struct kefir_ast_target_environment_object_info objinfo;
            kefir_result_t res =
                KEFIR_AST_TARGET_ENVIRONMENT_OBJECT_INFO(mem, context->target_env, opaque_type, &designator, &objinfo);
