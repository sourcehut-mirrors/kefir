...
kefir_result_t kefir_ast_format_declarator(struct kefir_json_output *json,
    ...
    REQUIRE_OK(kefir_json_output_array_begin(json));
    for (kefir_size_t i = 0; i < kefir_ast_node_attributes_length(&declarator->attributes); i++) {
        struct kefir_ast_attribute_list *attr_list = kefir_ast_node_attributes_at(&declarator->attributes, i);
