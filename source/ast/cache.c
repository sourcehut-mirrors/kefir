...
static kefir_result_t free_cached_type(struct kefir_mem *mem, struct kefir_hashtable *table, kefir_hashtable_key_t key,
...
kefir_result_t kefir_ast_context_type_cache_init(struct kefir_ast_context_type_cache *cache,
    ...
    REQUIRE_OK(kefir_hashtable_init(&cache->types, &kefir_hashtable_uint_ops));
    REQUIRE_OK(kefir_hashtable_on_removal(&cache->types, free_cached_type, (void *) context));
...
kefir_result_t kefir_ast_context_type_cache_free(struct kefir_mem *mem, struct kefir_ast_context_type_cache *cache) {
    ...
    REQUIRE_OK(kefir_hashtable_free(mem, &cache->types));
    return KEFIR_OK;
}
...
kefir_result_t kefir_ast_context_type_cache_reset(struct kefir_mem *mem, struct kefir_ast_context_type_cache *cache) {
    ...
    REQUIRE_OK(kefir_hashtable_clear(mem, &cache->types));
    return KEFIR_OK;
}
...
kefir_result_t kefir_ast_context_type_cache_get_type(struct kefir_mem *mem, struct kefir_ast_context_type_cache *cache,
    ...
    kefir_result_t res = kefir_hashtable_at(&cache->types, (kefir_hashtree_key_t) type, &table_value);
    if (res != KEFIR_NOT_FOUND) {
        REQUIRE_OK(res);
        *opaque_type_ptr = (kefir_ast_target_environment_opaque_type_t) table_value;
    } else {
        ...
        res = kefir_hashtable_insert(mem, &cache->types, (kefir_hashtable_key_t) type,
                                    (kefir_hashtable_value_t) opaque_type);
