...
static struct kefir_ast_alignment *wrap_alignment(struct kefir_mem *mem, kefir_size_t alignment) {
    if (alignment > 0) {
        return kefir_ast_alignment_const_expression(mem, alignment);
    } else {
        return NULL;
    }
}
...
static kefir_result_t process_struct_declaration_entry(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(&entry->declaration.declarators); iter != NULL;
        ...
        REQUIRE_OK(kefir_ast_analyze_declaration_declarator(
            mem, context, &entry->declaration.specifiers, entry_declarator->declarator, &identifier, &field_type,
            &alignment, KEFIR_AST_DECLARATION_ANALYSIS_FORBID_ALIGNMENT_DECREASE, NULL));
        ...
        if (entry_declarator->bitwidth == NULL) {
            ...
            res = kefir_ast_struct_type_bitfield(
                mem, context->symbols, struct_type, identifier, field_type, ast_alignment,
                KEFIR_AST_NODE_CONSTANT_EXPRESSION_VALUE(entry_declarator->bitwidth)->integer);
            REQUIRE_ELSE(res == KEFIR_OK, {
                if (ast_alignment != NULL) {
                    kefir_ast_alignment_free(mem, ast_alignment);
                }
...
static kefir_result_t scan_struct_attributes(struct kefir_mem *mem, const struct kefir_ast_context *context,
                                             const struct kefir_ast_node_attributes *attributes,
                                             struct kefir_ast_struct_type *struct_type,
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(&attributes->attributes); iter != NULL;
        ...
        for (const struct kefir_list_entry *iter2 = kefir_list_head(&attr_list->list); iter2 != NULL;
            ...
            if (attribute->prefix == NULL &&
                ...
                const struct kefir_token *arg = kefir_token_buffer_at(&attribute->unstructured_parameters, 0);
                REQUIRE_OK(
                    multibyte_string_literal_into(mem, context->symbols, arg, &struct_type->flags.no_discard_message));
            } else if (attribute->prefix == NULL &&
                       (strcmp(attribute->name, "deprecated") == 0 || strcmp(attribute->name, "__deprecated__") == 0)) {
                struct_type->flags.deprecated = true;
                const struct kefir_token *arg = kefir_token_buffer_at(&attribute->unstructured_parameters, 0);
                REQUIRE_OK(
                    multibyte_string_literal_into(mem, context->symbols, arg, &struct_type->flags.deprecated_message));
            } else if (attribute->prefix != NULL &&
                ...
                if (strcmp(attribute->name, "packed") == 0 || strcmp(attribute->name, "__packed__") == 0) {
                    ...
                    if (kefir_list_length(&attribute->parameters) == 1) {
                        ...
                        struct_alignment = KEFIR_AST_NODE_CONSTANT_EXPRESSION_VALUE(param)->uinteger;
    ...
    if (struct_alignment > 0) {
        ...
        if (struct_alignment > natural_alignment) {
            struct_type->aggregate_alignment = struct_alignment;
        }
    }
...
static kefir_result_t resolve_struct_type(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    if (specifier->complete) {
        ...
        REQUIRE_OK(
            scan_struct_attributes(mem, context, &decl_specifier->attributes, struct_type, type, source_location));
        REQUIRE_OK(scan_struct_attributes(mem, context, &specifiers->attributes, struct_type, type, source_location));
    } else {
...
#define FITS_FN(_type)                                                                                                 \
    static kefir_result_t enum_constant_range_fits_signed_##_type(                                                     \
        const struct kefir_ast_type_traits *type_traits, kefir_int64_t min_value, kefir_int64_t max_value,             \
        const struct kefir_ast_type *min_value_type, const struct kefir_ast_type *max_value_type,                      \
        ...
        REQUIRE_OK(enum_constant_compare(type_traits, min_value, min_value_type,                                       \
                                         kefir_data_model_descriptor_signed_##_type##_min(type_traits->data_model),    \
                                         kefir_ast_type_signed_##_type(), &min_fits_int));                             \
        REQUIRE_OK(enum_constant_compare(type_traits, max_value, max_value_type,                                       \
                                         kefir_data_model_descriptor_signed_##_type##_max(type_traits->data_model),    \
                                         kefir_ast_type_signed_##_type(), &max_fits_int));                             \
                                                                                                                       \
        *fits_int = min_fits_int >= 0 && max_fits_int <= 0;                                                            \
        return KEFIR_OK;                                                                                               \
    }                                                                                                                  \
                                                                                                                       \
    static kefir_result_t enum_constant_range_fits_unsigned_##_type(                                                   \
        const struct kefir_ast_type_traits *type_traits, kefir_int64_t min_value, kefir_int64_t max_value,             \
        const struct kefir_ast_type *min_value_type, const struct kefir_ast_type *max_value_type,                      \
        kefir_bool_t *fits_int) {                                                                                      \
        kefir_int_t min_fits_int = 0, max_fits_int = 0;                                                                \
        REQUIRE_OK(enum_constant_compare(type_traits, min_value, min_value_type, 0, kefir_ast_type_unsigned_##_type(), \
                                         &min_fits_int));                                                              \
        REQUIRE_OK(enum_constant_compare(type_traits, max_value, max_value_type,                                       \
                                         kefir_data_model_descriptor_unsigned_##_type##_max(type_traits->data_model),  \
                                         kefir_ast_type_unsigned_##_type(), &max_fits_int));                           \
                                                                                                                       \
        *fits_int = min_fits_int >= 0 && max_fits_int <= 0;                                                            \
        return KEFIR_OK;                                                                                               \
    }
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(&attributes->attributes); iter != NULL;
        ...
        for (const struct kefir_list_entry *iter2 = kefir_list_head(&attr_list->list); iter2 != NULL;
            ...
            if (attribute->prefix == NULL &&
                ...
                } else if (strcmp(attribute->name, "packed") == 0 || strcmp(attribute->name, "__packed__") == 0) {
                    *packed = true;
                }
            }
        }
...
static kefir_result_t resolve_enum_type(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    if (specifier->complete) {
        ...
        kefir_bool_t packed = false;
        REQUIRE_OK(scan_enum_attributes(mem, context, &specifiers->attributes, &alignment, &packed, enum_type));
        REQUIRE_OK(scan_enum_attributes(mem, context, &decl_specifier->attributes, &alignment, &packed, enum_type));

        if (fixed_underlying_type == NULL && kefir_list_length(&specifier->entries) > 0) {
            if (has_negative) {
                kefir_bool_t fits_char, fits_short, fits_int;
                REQUIRE_OK(enum_constant_range_fits_signed_char(context->type_traits, min_constant_value,
                                                                max_constant_value, min_constant_type,
                                                                max_constant_type, &fits_char));
                REQUIRE_OK(enum_constant_range_fits_signed_short(context->type_traits, min_constant_value,
                                                                 max_constant_value, min_constant_type,
                                                                 max_constant_type, &fits_short));
                REQUIRE_OK(enum_constant_range_fits_signed_int(context->type_traits, min_constant_value,
                                                               ...
                                                               &fits_int));
                if (fits_char && packed) {
                    enum_type->underlying_type = kefir_ast_type_signed_char();
                } else if (fits_short && packed) {
                    enum_type->underlying_type = kefir_ast_type_signed_short();
                } else if (fits_int) {
                ...
                kefir_bool_t fits_uchar, fits_ushort, fits_uint;
                REQUIRE_OK(enum_constant_range_fits_unsigned_char(context->type_traits, min_constant_value,
                                                                  max_constant_value, min_constant_type,
                                                                  max_constant_type, &fits_uchar));
                REQUIRE_OK(enum_constant_range_fits_unsigned_short(context->type_traits, min_constant_value,
                                                                   max_constant_value, min_constant_type,
                                                                   max_constant_type, &fits_ushort));
                REQUIRE_OK(enum_constant_range_fits_unsigned_int(context->type_traits, min_constant_value,
                                                                 max_constant_value, min_constant_type,
                                                                 max_constant_type, &fits_uint));
                if (fits_uchar && packed) {
                    enum_type->underlying_type = kefir_ast_type_unsigned_char();
                } else if (fits_ushort && packed) {
                    enum_type->underlying_type = kefir_ast_type_unsigned_short();
                } else if (fits_uint) {
...
static kefir_result_t resolve_type(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    switch (specifier->specifier) {
        ...
        case KEFIR_AST_TYPE_SPECIFIER_INT128:
            ...
            REQUIRE(*base_type == NULL && *real_class == REAL_SCALAR,
                    KEFIR_SET_SOURCE_ERROR(KEFIR_ANALYSIS_ERROR, &decl_specifier->source_location,
                                           "Int128 type specifier cannot be combined with complex type specifier"));
            *base_type = kefir_ast_type_signed_int128();
            *seq_state = TYPE_SPECIFIER_SEQUENCE_SPECIFIERS;
            break;
    ...
    if (signedness == SIGNEDNESS_DEFAULT) {
        ...
        if ((*base_type) == NULL) {
            ...
            switch ((*base_type)->tag) {
                ...
                case KEFIR_AST_TYPE_SCALAR_SIGNED_INT128:
                    (*base_type) = kefir_ast_type_unsigned_int128();
                    break;
...
static kefir_result_t analyze_declaration_declarator_attributes(
...
static kefir_result_t analyze_declaration_declarator_impl(
    ...
    REQUIRE_OK(analyze_declaration_declarator_attributes(mem, context, declarator, &declarator->attributes, base_type,
                                                         alignment, flags, attributes));
    return KEFIR_OK;
}
...
kefir_result_t kefir_ast_analyze_declaration(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    REQUIRE_OK(kefir_ast_analyze_declaration_declarator(mem, context, specifiers, declarator, identifier, &base_type,
                                                        alignment, flags, attributes));
    ASSIGN_PTR(type, base_type);
    return KEFIR_OK;
}
