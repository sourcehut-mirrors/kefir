...
VISITOR(attribute_declaration, struct kefir_ast_attribute_declaration)
...
VISITOR(inline_assembly, struct kefir_ast_inline_assembly)
...
kefir_result_t kefir_ast_analyze_node(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    if (base->properties.category == KEFIR_AST_NODE_CATEGORY_EXPRESSION) {
        REQUIRE_OK(kefir_ast_constant_expression_evaluate_node(mem, context, base));
    }
