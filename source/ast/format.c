...
static kefir_result_t visit_constant(const struct kefir_ast_visitor *visitor, const struct kefir_ast_constant *node,
    ...
    switch (node->type) {
        ...
        case KEFIR_AST_BITPRECISE_CONSTANT:
            ...
            REQUIRE_OK(kefir_json_output_object_key(json, "value"));
            REQUIRE_OK(kefir_json_output_bigint(json, &node->value.large->bitprecise));
            REQUIRE_OK(kefir_json_output_object_key(json, "width"));
            REQUIRE_OK(kefir_json_output_integer(json, node->value.large->bitprecise.bitwidth));
            break;
        ...
        case KEFIR_AST_FLOAT64X_CONSTANT:
            ...
            REQUIRE_OK(kefir_json_output_object_key(json, "value"));
            REQUIRE_OK(kefir_json_output_long_double(json, node->value.large->long_double));
            break;
        ...
        case KEFIR_AST_DECIMAL128_CONSTANT: {
            ...
            char buf[128] = {0};
            REQUIRE_OK(kefir_dfp_require_supported(&node->base.source_location));
            kefir_dfp_decimal128_format(buf, sizeof(buf), node->value.large->decimal128);
            REQUIRE_OK(kefir_json_output_string(json, buf));
        } break;
        ...
        case KEFIR_AST_COMPLEX_DOUBLE_CONSTANT:
            ...
            REQUIRE_OK(kefir_json_output_object_key(json, "real_value"));
            REQUIRE_OK(kefir_json_output_float(json, node->value.large->complex_float64.real));
            REQUIRE_OK(kefir_json_output_object_key(json, "imaginary_value"));
            REQUIRE_OK(kefir_json_output_float(json, node->value.large->complex_float64.imaginary));
            break;
        ...
        case KEFIR_AST_COMPLEX_LONG_DOUBLE_CONSTANT:
            ...
            REQUIRE_OK(kefir_json_output_object_key(json, "real_value"));
            REQUIRE_OK(kefir_json_output_float(json, node->value.large->complex_long_double.real));
            REQUIRE_OK(kefir_json_output_object_key(json, "imaginary_value"));
            REQUIRE_OK(kefir_json_output_float(json, node->value.large->complex_long_double.imaginary));
            break;
...
static kefir_result_t visit_string_literal(const struct kefir_ast_visitor *visitor,
    ...
    switch (node->data.type) {
        case KEFIR_AST_STRING_LITERAL_MULTIBYTE:
            ...
            REQUIRE_OK(kefir_json_output_raw_string(json, node->data.literal, node->data.length));
            break;
        ...
        case KEFIR_AST_STRING_LITERAL_UNICODE16:
            ...
            REQUIRE_OK(kefir_json_output_object_key(json, "content"));
            REQUIRE_OK(kefir_json_output_raw_string(json, node->data.literal, node->data.length * sizeof(kefir_char16_t)));
            break;

        case KEFIR_AST_STRING_LITERAL_UNICODE32:
            REQUIRE_OK(kefir_json_output_string(json, "unicode32"));
            REQUIRE_OK(kefir_json_output_object_key(json, "content"));
            REQUIRE_OK(kefir_json_output_raw_string(json, node->data.literal, node->data.length * sizeof(kefir_char32_t)));
            break;

        case KEFIR_AST_STRING_LITERAL_WIDE:
            REQUIRE_OK(kefir_json_output_string(json, "wide"));
            REQUIRE_OK(kefir_json_output_object_key(json, "content"));
            REQUIRE_OK(kefir_json_output_raw_string(json, node->data.literal, node->data.length * sizeof(kefir_wchar_t)));
            break;
    }
    REQUIRE_OK(kefir_json_output_object_key(json, "length"));
    REQUIRE_OK(kefir_json_output_uinteger(json, node->data.length));
    if (param->display_source_location) {
        REQUIRE_OK(format_source_location(json, KEFIR_AST_NODE_BASE(node)));
    }
    REQUIRE_OK(kefir_json_output_object_end(json));
...
static kefir_result_t visit_function_definitions(const struct kefir_ast_visitor *visitor,
    ...
    for (kefir_size_t i = 0; i < node->declarations_length; i++) {
        struct kefir_ast_node_base *declaration = node->declarations[i];
