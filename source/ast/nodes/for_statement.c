...
kefir_result_t ast_for_statement_free(struct kefir_mem *mem, struct kefir_ast_node_base *base) {
    ...
    ASSIGN_DECL_CAST(struct kefir_ast_for_statement *, node, KEFIR_AST_NODE_SELF(base));
...
const struct kefir_ast_node_class AST_FOR_STATEMENT_CLASS = {
    .type = KEFIR_AST_FOR_STATEMENT, .visit = ast_for_statement_visit, .free = ast_for_statement_free};

struct kefir_ast_for_statement *kefir_ast_new_for_statement(struct kefir_mem *mem, struct kefir_memory_arena *arena, struct kefir_ast_node_base *init,
    ...
    struct kefir_ast_for_statement *stmt = KEFIR_AST_NODE_ARENA_ALLOC(mem, arena, struct kefir_ast_for_statement);
