...
kefir_result_t ast_break_statement_free(struct kefir_mem *mem, struct kefir_ast_node_base *base) {
    ...
    ASSIGN_DECL_CAST(struct kefir_ast_break_statement *, node, KEFIR_AST_NODE_SELF(base));
...
struct kefir_ast_break_statement *kefir_ast_new_break_statement(struct kefir_mem *mem, struct kefir_memory_arena *arena) {
    REQUIRE(mem != NULL || arena != NULL, NULL);

    struct kefir_ast_break_statement *stmt = KEFIR_AST_NODE_ARENA_ALLOC(mem, arena, struct kefir_ast_break_statement);
