...
kefir_result_t ast_continue_statement_free(struct kefir_mem *mem, struct kefir_ast_node_base *base) {
    ...
    ASSIGN_DECL_CAST(struct kefir_ast_continue_statement *, node, KEFIR_AST_NODE_SELF(base));
    REQUIRE_OK(kefir_ast_node_attributes_free(mem, &node->attributes));
    KEFIR_AST_NODE_ARENA_FREE(mem, node);
    return KEFIR_OK;
}

const struct kefir_ast_node_class AST_CONTINUE_STATEMENT_CLASS = {
    .type = KEFIR_AST_CONTINUE_STATEMENT, .visit = ast_continue_statement_visit, .free = ast_continue_statement_free};

struct kefir_ast_continue_statement *kefir_ast_new_continue_statement(struct kefir_mem *mem, struct kefir_memory_arena *arena) {
