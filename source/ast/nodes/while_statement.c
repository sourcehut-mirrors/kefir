...
kefir_result_t ast_while_statement_free(struct kefir_mem *mem, struct kefir_ast_node_base *base) {
    ...
    ASSIGN_DECL_CAST(struct kefir_ast_while_statement *, node, KEFIR_AST_NODE_SELF(base));
...
const struct kefir_ast_node_class AST_WHILE_STATEMENT_CLASS = {
    .type = KEFIR_AST_WHILE_STATEMENT, .visit = ast_while_statement_visit, .free = ast_while_statement_free};

struct kefir_ast_while_statement *kefir_ast_new_while_statement(struct kefir_mem *mem, struct kefir_memory_arena *arena,
    ...
    struct kefir_ast_while_statement *stmt = KEFIR_AST_NODE_ARENA_ALLOC(mem, arena, struct kefir_ast_while_statement);
