...
kefir_result_t kefir_ir_inline_assembly_add_parameter(
    ...
    const struct kefir_ir_inline_assembly_parameter_constraints *constraints, const struct kefir_ir_type *param_type,
    kefir_id_t param_type_id, kefir_size_t param_type_idx, kefir_size_t value,
    ...
    REQUIRE(param_class != KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_VALUE_WRITE_LOCATION &&
    ...
    param->location_type.type = param_type;
    param->location_type.type_id = param_type_id;
    param->location_type.index = param_type_idx;
    switch (param_class) {
        case KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_VALUE:
            param->value_index = value;
            param->value_type.type = param_type;
            param->value_type.type_id = param_type_id;
            param->value_type.index = param_type_idx;
            break;
...
kefir_result_t kefir_ir_inline_assembly_parameter_read_from(struct kefir_mem *mem,
                                                            ...
                                                            kefir_size_t type_idx, kefir_size_t read_index) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(inline_asm != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid IR inline assembly"));
    REQUIRE(param != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid IR inline assembly parameter"));
    REQUIRE(param->klass == KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_WRITE_LOCATION &&
    ...
    param->value_index = read_index;
    param->value_type.type = type;
    param->value_type.type_id = type_id;
    param->value_type.index = type_idx;
    param->klass = KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_VALUE_WRITE_LOCATION;
    return KEFIR_OK;
}
