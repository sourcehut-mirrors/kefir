...
kefir_result_t kefir_ir_format_instr_u64_2(struct kefir_json_output *json, const struct kefir_ir_module *module,
                                           const struct kefir_irinstr *instr) {
    UNUSED(module);
    REQUIRE(json != NULL, KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Expected valid json output"));
    REQUIRE(instr != NULL, KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Expected valid IR instruction"));
    ...
    REQUIRE_OK(kefir_json_output_uinteger(json, instr->arg.u64_2[1]));
    REQUIRE_OK(kefir_json_output_array_end(json));
    REQUIRE_OK(kefir_json_output_object_end(json));
    return KEFIR_OK;
}

kefir_result_t kefir_ir_format_instr_boolean(struct kefir_json_output *json, const struct kefir_ir_module *module,
                                             const struct kefir_irinstr *instr) {
    UNUSED(module);
    REQUIRE(json != NULL, KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Expected valid json output"));
    REQUIRE(instr != NULL, KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Expected valid IR instruction"));
...
kefir_result_t kefir_ir_format_instr_immediate(struct kefir_json_output *json, const struct kefir_ir_module *module,
                                               const struct kefir_irinstr *instr) {
    UNUSED(module);
    REQUIRE(json != NULL, KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Expected valid json output"));
    REQUIRE(instr != NULL, KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Expected valid IR instruction"));
    ...
    switch (instr->opcode) {
        ...
        case KEFIR_IR_OPCODE_DECIMAL128_CONST: {
            char buf[128];
            REQUIRE_OK(kefir_dfp_require_supported(NULL));
            kefir_dfp_decimal128_format(buf, sizeof(buf),
                                        (kefir_dfp_decimal128_t) {{instr->arg.u64_2[0], instr->arg.u64_2[1]}});
            REQUIRE_OK(kefir_json_output_string(json, buf));
        } break;
...
kefir_result_t kefir_ir_format_instr_bitint_bitfield(struct kefir_json_output *json,
                                                     const struct kefir_ir_module *module,
                                                     const struct kefir_irinstr *instr) {
    UNUSED(module);
    REQUIRE(json != NULL, KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Expected valid json output"));
...
kefir_result_t kefir_ir_format_instr_tmpobj(struct kefir_json_output *json, const struct kefir_ir_module *module,
                                            const struct kefir_irinstr *instr) {
    UNUSED(module);
    REQUIRE(json != NULL, KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Expected valid json output"));
    REQUIRE(instr != NULL, KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Expected valid IR instruction"));
...
static kefir_result_t kefir_ir_format_function_declaration(struct kefir_json_output *json,
    ...
    REQUIRE_OK(kefir_json_output_object_key(json, "no_return"));
    REQUIRE_OK(kefir_json_output_boolean(json, decl->no_return));
    REQUIRE_OK(kefir_json_output_object_end(json));
    return KEFIR_OK;
}
...
static kefir_result_t kefir_ir_format_function(struct kefir_json_output *json, const struct kefir_ir_module *module,
    ...
    if (func->flags.enable_fenv_access) {
        REQUIRE_OK(kefir_json_output_object_key(json, "enable_fenv_access"));
        REQUIRE_OK(kefir_json_output_boolean(json, true));
    }

    if (func->flags.disallow_fp_contract) {
        REQUIRE_OK(kefir_json_output_object_key(json, "disallow_fp_contract"));
        REQUIRE_OK(kefir_json_output_boolean(json, true));
    }

    if (func->flags.cx_limited_range) {
    ...
    if (debug_info) {
        ...
        if (func->debug_info.subprogram_id != KEFIR_IR_DEBUG_ENTRY_ID_NONE) {
            REQUIRE_OK(kefir_json_output_uinteger(json, func->debug_info.subprogram_id));
        } else {
            REQUIRE_OK(kefir_json_output_null(json));
        }
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(&inline_asm->parameter_list); iter != NULL;
        ...
        switch (param->klass) {
            case KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_DIRECT_INPUT:
                ...
                REQUIRE_OK(kefir_json_output_object_key(json, "type"));
                REQUIRE_OK(kefir_json_output_uinteger(json, param->direct_input_type.type_id));
                REQUIRE_OK(kefir_json_output_object_key(json, "type_index"));
                REQUIRE_OK(kefir_json_output_uinteger(json, param->direct_input_type.index));
                REQUIRE_OK(kefir_json_output_object_key(json, "from"));
                REQUIRE_OK(kefir_json_output_uinteger(json, param->direct_input_index));
                break;

            case KEFIR_IR_INLINE_ASSEMBLY_PARAMETER_INDIRECT_INPUT:
                REQUIRE_OK(kefir_json_output_string(json, "load"));
                REQUIRE_OK(kefir_json_output_object_key(json, "type"));
                REQUIRE_OK(kefir_json_output_uinteger(json, param->indirect_type.type_id));
                REQUIRE_OK(kefir_json_output_object_key(json, "type_index"));
                REQUIRE_OK(kefir_json_output_uinteger(json, param->indirect_type.index));
                REQUIRE_OK(kefir_json_output_object_key(json, "from"));
                REQUIRE_OK(kefir_json_output_uinteger(json, param->indirect_index));
                break;
...
static kefir_result_t format_debug_entry(struct kefir_json_output *json, const struct kefir_ir_module *module,
    ...
    switch (entry->tag) {
        ...
        case KEFIR_IR_DEBUG_ENTRY_LEXICAL_BLOCK:
            REQUIRE_OK(kefir_json_output_string(json, "lexical_block"));
            break;

        case KEFIR_IR_DEBUG_ENTRY_SUBPROGRAM:
            REQUIRE_OK(kefir_json_output_string(json, "subprogram"));
            break;
    ...
    for (res = kefir_ir_debug_entry_attribute_iter(&module->debug_info.entries, entry->identifier, &attr_iter,
        ...
        switch (entry_attr->tag) {
            ...
            case KEFIR_IR_DEBUG_ENTRY_ATTRIBUTE_CODE_BEGIN:
                REQUIRE_OK(kefir_json_output_string(json, "code_begin"));
                REQUIRE_OK(kefir_json_output_object_key(json, "value"));
                REQUIRE_OK(kefir_json_output_uinteger(json, entry_attr->code_index));
                break;

            case KEFIR_IR_DEBUG_ENTRY_ATTRIBUTE_CODE_END:
                REQUIRE_OK(kefir_json_output_string(json, "code_end"));
                REQUIRE_OK(kefir_json_output_object_key(json, "value"));
                REQUIRE_OK(kefir_json_output_uinteger(json, entry_attr->code_index));
                break;
            ...
            case KEFIR_IR_DEBUG_ENTRY_ATTRIBUTE_LOCAL_VARIABLE:
                ...
                REQUIRE_OK(kefir_json_output_object_key(json, "type_index"));
                REQUIRE_OK(kefir_json_output_uinteger(json, entry_attr->local_variable.type_index));
                REQUIRE_OK(kefir_json_output_object_end(json));
                break;
            ...
            case KEFIR_IR_DEBUG_ENTRY_ATTRIBUTE_PARAMETER:
                ...
                REQUIRE_OK(kefir_json_output_object_key(json, "value"));
                REQUIRE_OK(kefir_json_output_uinteger(json, entry_attr->parameter));
                break;
