...
static kefir_result_t destroy_function_decl(struct kefir_mem *mem, struct kefir_hashtable *table,
                                            kefir_hashtable_key_t key, kefir_hashtable_value_t value, void *data) {
    UNUSED(table);
    UNUSED(key);
    UNUSED(data);
...
static kefir_result_t destroy_inline_assembly(struct kefir_mem *mem, struct kefir_hashtable *table,
                                              kefir_hashtable_key_t key, kefir_hashtable_value_t value, void *data) {
    UNUSED(table);
    UNUSED(key);
    UNUSED(data);
...
static kefir_result_t destroy_bigint(struct kefir_mem *mem, struct kefir_hashtable *table, kefir_hashtable_key_t key,
...
kefir_result_t kefir_ir_module_alloc(struct kefir_mem *mem, struct kefir_ir_module *module, kefir_bool_t debug_info) {
    ...
    REQUIRE_OK(kefir_hashtable_init(&module->function_declarations, &kefir_hashtable_uint_ops));
    REQUIRE_OK(kefir_hashtable_on_removal(&module->function_declarations, destroy_function_decl, NULL));
    ...
    REQUIRE_OK(kefir_hashtable_init(&module->named_types, &kefir_hashtable_uint_ops));
    ...
    REQUIRE_OK(kefir_hashtable_init(&module->inline_assembly, &kefir_hashtable_uint_ops));
    REQUIRE_OK(kefir_hashtable_on_removal(&module->inline_assembly, destroy_inline_assembly, NULL));
    REQUIRE_OK(kefir_hashtree_init(&module->global_inline_asm, &kefir_hashtree_uint_ops));
    REQUIRE_OK(kefir_hashtable_init(&module->bigints, &kefir_hashtable_uint_ops));
    REQUIRE_OK(kefir_hashtable_on_removal(&module->bigints, destroy_bigint, NULL));
    REQUIRE_OK(kefir_ir_module_debug_info_init(&module->debug_info));
    REQUIRE_OK(kefir_memory_arena_init(mem, &module->arena));
...
kefir_result_t kefir_ir_module_string_literal(struct kefir_mem *mem, struct kefir_ir_module *module,
    ...
    struct kefir_ir_module_string_literal *literal = kefir_memory_arena_alloc(&module->arena, sizeof(struct kefir_ir_module_string_literal), _Alignof(struct kefir_ir_module_string_literal));
    REQUIRE(literal != NULL, KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate string literal"));

    literal->type = type;
    literal->public = public;
    kefir_size_t sz = 0;
    kefir_size_t alignment = 1;
    switch (literal->type) {
    ...
    literal->content = kefir_memory_arena_alloc(&module->arena, sz, alignment);
    REQUIRE(literal->content != NULL, KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate string literal content"));

    memcpy(literal->content, content, sz);
    literal->length = length;

    REQUIRE_OK(kefir_hashtree_insert(mem, &module->string_literals, (kefir_hashtree_key_t) module->next_string_literal_id,
...
struct kefir_ir_type *kefir_ir_module_new_type(struct kefir_mem *mem, struct kefir_ir_module *module, kefir_size_t size,
    ...
    if (identifier != NULL) {
        *identifier = module->next_type_id++;
        result = kefir_hashtable_insert(mem, &module->named_types, (kefir_hashtable_key_t) *identifier,
                                        (kefir_hashtable_value_t) type);
        REQUIRE_ELSE(result == KEFIR_OK, {
...
struct kefir_ir_function_decl *kefir_ir_module_new_function_declaration(struct kefir_mem *mem,
    ...
    struct kefir_ir_function_decl *decl = kefir_memory_arena_alloc(&module->arena, sizeof(struct kefir_ir_function_decl), _Alignof(struct kefir_ir_function_decl));
    ...
    res = kefir_hashtable_insert(mem, &module->function_declarations, (kefir_hashtable_key_t) decl->id,
                                 (kefir_hashtable_value_t) decl);
    REQUIRE_ELSE(res == KEFIR_OK, {
...
kefir_result_t kefir_ir_module_declare_identifier(struct kefir_mem *mem, struct kefir_ir_module *module,
    ...
    if (res != KEFIR_NOT_FOUND) {
        ...
        REQUIRE(strcmp(identifier_symbol, current_identifier_data->symbol) == 0,
                          KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Mismatch with existing IR module identifier symbol"));
        REQUIRE(identifier->type == current_identifier_data->type,
                          KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Mismatch with existing IR module identifier type"));
        REQUIRE(identifier->scope == current_identifier_data->scope,
                          KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Mismatch with existing IR module identifier scope"));
        REQUIRE(
            identifier->visibility == current_identifier_data->visibility,
            KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Mismatch with existing IR module identifier visibility"));
        REQUIRE((identifier_alias == NULL && current_identifier_data->alias == NULL) ||
                              (identifier_alias != NULL && current_identifier_data->alias != NULL &&
                               strcmp(identifier_alias, current_identifier_data->alias) == 0),
                          KEFIR_SET_ERROR(KEFIR_INVALID_REQUEST, "Mismatch with existing IR module identifier alias"));
    } else {
        struct kefir_ir_identifier *identifier_data = kefir_memory_arena_alloc(&module->arena, sizeof(struct kefir_ir_identifier), _Alignof(struct kefir_ir_identifier));
        ...
        *identifier_data = *identifier;
        ...
        REQUIRE_OK(kefir_hashtree_insert(mem, &module->identifiers, (kefir_hashtree_key_t) symbol,
...
struct kefir_ir_function *kefir_ir_module_new_function(struct kefir_mem *mem, struct kefir_ir_module *module,
                                                       struct kefir_ir_function_decl *decl, kefir_size_t length) {
    REQUIRE(mem != NULL, NULL);
    REQUIRE(module != NULL, NULL);
    REQUIRE(decl != NULL, NULL);
    REQUIRE(decl->name != NULL && strlen(decl->name) != 0, NULL);

    struct kefir_ir_function *func = kefir_memory_arena_alloc(&module->arena, sizeof(struct kefir_ir_function), _Alignof(struct kefir_ir_function));
...
const struct kefir_ir_function_decl *kefir_ir_module_function_declaration_iter(
    const struct kefir_ir_module *module, struct kefir_ir_module_function_declaration_iterator *iter) {
    REQUIRE(module != NULL, NULL);
    REQUIRE(iter != NULL, NULL);

    kefir_hashtable_value_t value;
    kefir_result_t res = kefir_hashtable_iter(&module->function_declarations, &iter->iter, NULL, &value);
    if (res == KEFIR_OK) {
        return (const struct kefir_ir_function_decl *) value;
    } else {
        return NULL;
    }
}
...
const char *kefir_ir_module_identifiers_iter(const struct kefir_ir_module *module,
    ...
    const struct kefir_hashtree_node *node = kefir_hashtree_iter(&module->identifiers, &iter->iter);
    REQUIRE(node != NULL, NULL);
    ASSIGN_PTR(identifier_ptr, (const struct kefir_ir_identifier *) node->value);
    return (const char *) node->key;
}

const char *kefir_ir_module_identifiers_next(struct kefir_ir_module_identifier_iterator *iter,
                                             const struct kefir_ir_identifier **identifier_ptr) {
    REQUIRE(iter != NULL, NULL);
...
kefir_result_t kefir_ir_module_string_literal_iter(const struct kefir_ir_module *module,
                                                   struct kefir_ir_module_string_literal_iterator *iter, kefir_id_t *id,
                                                   kefir_ir_string_literal_type_t *type, kefir_bool_t *public,
                                                   const void **content, kefir_size_t *length) {
    REQUIRE(module != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid IR module"));
    ...
    const struct kefir_hashtree_node *node = kefir_hashtree_iter(&module->string_literals, &iter->iter);
...
kefir_result_t kefir_ir_module_string_literal_next(struct kefir_ir_module_string_literal_iterator *iter, kefir_id_t *id,
                                                   kefir_ir_string_literal_type_t *type, kefir_bool_t *public,
                                                   const void **content, kefir_size_t *length) {
    REQUIRE(iter != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid hash tree iterator pointer"));
    REQUIRE(id != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid string literal identifier pointer"));
...
struct kefir_ir_data *kefir_ir_module_new_named_data(struct kefir_mem *mem, struct kefir_ir_module *module,
    ...
    REQUIRE(symbol != NULL, NULL);
    struct kefir_ir_data *data = kefir_memory_arena_alloc(&module->arena, sizeof(struct kefir_ir_data), _Alignof(struct kefir_ir_data));
    REQUIRE(data != NULL, NULL);
    kefir_result_t res = kefir_ir_data_alloc(mem, &module->arena, storage, type, type_id, data);
    REQUIRE_ELSE(res == KEFIR_OK, {
        return NULL;
    });
...
const struct kefir_ir_data *kefir_ir_module_named_data_iter(const struct kefir_ir_module *module,
    ...
    REQUIRE(module != NULL, NULL);
    REQUIRE(iter != NULL, NULL);
    const struct kefir_hashtree_node *node = kefir_hashtree_iter(&module->named_data, &iter->iter);
    if (node != NULL) {
        if (identifier != NULL) {
...
const struct kefir_ir_data *kefir_ir_module_named_data_next(struct kefir_ir_module_named_data_iterator *iter,
                                                            const char **identifier) {
    REQUIRE(iter != NULL, NULL);
    const struct kefir_hashtree_node *node = kefir_hashtree_next(&iter->iter);
    if (node != NULL) {
...
const struct kefir_ir_type *kefir_ir_module_named_type_iter(const struct kefir_ir_module *module,
    ...
    kefir_hashtable_value_t value;
    kefir_result_t res = kefir_hashtable_iter(&module->named_types, &iter->iter, &key, &value);
    if (res == KEFIR_OK) {
        ASSIGN_PTR(type_id_ptr, (kefir_id_t) key);
        return (const struct kefir_ir_type *) value;
    } else {
        return NULL;
    }
}

const struct kefir_ir_type *kefir_ir_module_named_type_next(struct kefir_ir_module_named_type_iterator *iter,
                                                            kefir_id_t *type_id_ptr) {
    REQUIRE(iter != NULL, NULL);
...
struct kefir_ir_function_decl *kefir_ir_module_get_declaration(const struct kefir_ir_module *module, kefir_id_t id) {
    REQUIRE(module != NULL, NULL);
    kefir_hashtable_value_t value;
    REQUIRE(kefir_hashtable_at(&module->function_declarations, (kefir_hashtree_key_t) id, &value) == KEFIR_OK, NULL);
    return (struct kefir_ir_function_decl *) value;
}

struct kefir_ir_function *kefir_ir_module_get_function(const struct kefir_ir_module *module, const char *identifier) {
    REQUIRE(module != NULL, NULL);
    REQUIRE(identifier != NULL, NULL);
    struct kefir_hashtree_node *node = NULL;
...
struct kefir_ir_type *kefir_ir_module_get_named_type(const struct kefir_ir_module *module, kefir_id_t id) {
    REQUIRE(module != NULL, NULL);
    kefir_hashtable_value_t value;
    REQUIRE_ELSE(kefir_hashtable_at(&module->named_types, (kefir_hashtree_key_t) id, &value) == KEFIR_OK,
                 { return NULL; });
    return (struct kefir_ir_type *) value;
}

kefir_result_t kefir_ir_module_drop_named_types(struct kefir_mem *mem, struct kefir_ir_module *module,
                                                kefir_result_t (*is_garbage)(kefir_id_t, const struct kefir_ir_type *,
                                                                             kefir_bool_t *, void *),
                                                void *payload) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(module != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid IR module"));
    ...
    kefir_result_t res;
    for (res = kefir_hashtable_iter(&module->named_types, &iter, &key, &value); res == KEFIR_OK;
         res = kefir_hashtable_next(&iter, &key, &value)) {
        kefir_bool_t garbage;
        REQUIRE_OK(is_garbage((kefir_id_t) key, (const struct kefir_ir_type *) value, &garbage, payload));
        if (garbage) {
            REQUIRE_OK(kefir_hashtable_delete(mem, &module->named_types, key));
        }
    }
    return KEFIR_OK;
}
...
struct kefir_ir_inline_assembly *kefir_ir_module_new_inline_assembly(struct kefir_mem *mem,
    ...
    kefir_result_t res = kefir_hashtable_insert(mem, &module->inline_assembly, (kefir_hashtable_key_t) id,
                                                (kefir_hashtable_value_t) inline_asm);
    REQUIRE_ELSE(res == KEFIR_OK, {
...
struct kefir_ir_inline_assembly *kefir_ir_module_get_inline_assembly(const struct kefir_ir_module *module,
                                                                     kefir_id_t id) {
    REQUIRE(module != NULL, NULL);

    kefir_hashtable_value_t value;
    kefir_result_t res = kefir_hashtable_at(&module->inline_assembly, id, &value);
    REQUIRE(res == KEFIR_OK, NULL);
    return (struct kefir_ir_inline_assembly *) value;
}

const struct kefir_ir_inline_assembly *kefir_ir_module_inline_assembly_iter(
    const struct kefir_ir_module *module, struct kefir_ir_module_inline_assembly_iterator *iter, kefir_id_t *id_ptr) {
    REQUIRE(module != NULL, NULL);
    REQUIRE(iter != NULL, NULL);

    kefir_hashtable_key_t key;
    kefir_hashtable_value_t value;
    kefir_result_t res = kefir_hashtable_iter(&module->inline_assembly, &iter->iter, &key, &value);
    if (res == KEFIR_OK) {
        if (id_ptr != NULL) {
            *id_ptr = (kefir_id_t) key;
        }
        return (const struct kefir_ir_inline_assembly *) value;
    } else {
        return NULL;
    }
}
...
const struct kefir_ir_inline_assembly *kefir_ir_module_inline_assembly_next(
    struct kefir_ir_module_inline_assembly_iterator *iter, kefir_id_t *id_ptr) {
    REQUIRE(iter != NULL, NULL);
...
kefir_bool_t kefir_ir_module_inline_assembly_has_global(const struct kefir_ir_module *module, kefir_id_t asm_id) {
    REQUIRE(module != NULL, false);

    return kefir_hashtree_has(&module->global_inline_asm, (kefir_hashtree_key_t) asm_id);
}

struct kefir_ir_inline_assembly *kefir_ir_module_global_inline_assembly_iter(
    const struct kefir_ir_module *module, struct kefir_ir_module_global_inline_assembly_iterator *iter,
    kefir_id_t *id_ptr) {
    REQUIRE(module != NULL, NULL);
    REQUIRE(iter != NULL, NULL);
    const struct kefir_hashtree_node *node = kefir_hashtree_iter(&module->global_inline_asm, &iter->iter);
    if (node != NULL) {
        if (id_ptr != NULL) {
...
struct kefir_ir_inline_assembly *kefir_ir_module_global_inline_assembly_next(
    struct kefir_ir_module_global_inline_assembly_iterator *iter, kefir_id_t *id_ptr) {
    REQUIRE(iter != NULL, NULL);
...
kefir_result_t kefir_ir_module_new_bigint(struct kefir_mem *mem, struct kefir_ir_module *module,
    ...
    struct kefir_bigint *big_integer = kefir_memory_arena_alloc(&module->arena, sizeof(struct kefir_bigint), _Alignof(struct kefir_bigint));
    ...
    REQUIRE_CHAIN(&res, kefir_hashtable_insert(mem, &module->bigints, (kefir_hashtable_key_t) module->next_bigint_id,
                                               (kefir_hashtable_value_t) big_integer));
    REQUIRE_ELSE(res == KEFIR_OK, {
...
kefir_result_t kefir_ir_module_get_bigint(const struct kefir_ir_module *module, kefir_id_t bigint_id,
    ...
    kefir_hashtable_value_t value;
    kefir_result_t res = kefir_hashtable_at(&module->bigints, (kefir_hashtable_key_t) bigint_id, &value);
    if (res == KEFIR_NOT_FOUND) {
        res = KEFIR_SET_ERROR(KEFIR_NOT_FOUND, "Unable to find requested big integer in IR module");
    }
    REQUIRE_OK(res);

    *bigint_ptr = (const struct kefir_bigint *) value;
    return KEFIR_OK;
}
