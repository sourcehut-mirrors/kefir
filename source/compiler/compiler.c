...
kefir_result_t kefir_compiler_context_init(struct kefir_mem *mem, struct kefir_compiler_context *context,
    ...
    REQUIRE_OK(kefir_optimizer_configuration_init(&context->optimizer_configuration, &profile->ir_target_platform));
...
kefir_result_t kefir_compiler_parse(struct kefir_mem *mem, struct kefir_compiler_context *context,
                                    const struct kefir_token_cursor_handle *tokens_handle,
                                    struct kefir_ast_translation_unit **unit_ptr) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(context != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid compiler context"));
    REQUIRE(tokens_handle != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid token handle"));
...
kefir_result_t kefir_compiler_parse_source(struct kefir_mem *mem, struct kefir_compiler_context *context,
    ...
    REQUIRE_CHAIN(&res, kefir_compiler_parse(mem, context, &tokens_handle, unit_ptr));
    REQUIRE_ELSE(res == KEFIR_OK, {
...
kefir_result_t kefir_compiler_optimize(struct kefir_mem *mem, struct kefir_compiler_context *context,
    ...
    REQUIRE_OK(kefir_opt_module_construct(mem, context->translator_env.target_platform, opt_module, context->optimizer_configuration.debug_info, consume));
...
kefir_result_t kefir_compiler_codegen(struct kefir_mem *mem, struct kefir_compiler_context *context,
    ...
    kefir_result_t res = KEFIR_CODEGEN_TRANSLATE(mem, codegen, module, false);
    REQUIRE_ELSE(res == KEFIR_OK, {
...
kefir_result_t kefir_compiler_codegen_optimized(struct kefir_mem *mem, struct kefir_compiler_context *context,
                                                struct kefir_opt_module *module, FILE *output, kefir_bool_t no_prologue) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid memory allocator"));
    REQUIRE(context != NULL, KEFIR_SET_ERROR(KEFIR_INVALID_PARAMETER, "Expected valid compiler context"));
    ...
    REQUIRE_OK(
        context->profile->new_codegen(mem, output, &context->codegen_configuration, runtime_hooks_ptr, &codegen));
    kefir_result_t res = KEFIR_CODEGEN_TRANSLATE_OPTIMIZED(mem, codegen, module, no_prologue);
    REQUIRE_ELSE(res == KEFIR_OK, {
        context->profile->free_codegen(mem, codegen);
        return res;
    });
