...
kefir_result_t kefir_compiler_runner_configuration_init(struct kefir_compiler_runner_configuration *options) {
    ...
    *options = (struct kefir_compiler_runner_configuration) {
        ...
        .features = {false, .precise_bitfield_load_store = true, .declare_atomic_support = true, .declare_decimal_support = true, .preprocessor_linemarkers = false, .optimize_stack_frame = true, .imprecise_decimal_bitint_conv = false, .max_parser_errors = 24, .max_analyzer_errors = 24, .error_compiler_ref = true},
