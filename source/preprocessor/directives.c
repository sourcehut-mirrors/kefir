...
kefir_result_t kefir_preprocessor_directive_scanner_init(struct kefir_preprocessor_directive_scanner *directive_scanner,
    ...
    REQUIRE_OK(kefir_preprocessor_tokenizer_context_init(&directive_scanner->tokenizer_context));
...
static kefir_result_t scan_pp_tokens(struct kefir_mem *mem,
    ...
    while (scan_tokens) {
        ...
        if (next_token.klass == KEFIR_TOKEN_SENTINEL ||
            ...
            REQUIRE_OK(kefir_token_allocator_emplace(mem, token_allocator, &next_token, &allocated_token));
...
kefir_result_t kefir_preprocessor_directive_scanner_match(
    ...
    if (chr == '%' && kefir_lexer_source_cursor_at(directive_scanner->lexer->cursor, 1) == ':') {
        ...
        *directive_type = KEFIR_PREPROCESSOR_DIRECTIVE_PP_TOKENS;
        directive_scanner->newline_flag = true;
        return KEFIR_OK;
    }
...
static kefir_result_t next_if(struct kefir_mem *mem, struct kefir_preprocessor_directive_scanner *directive_scanner,
    ...
    kefir_result_t res = scan_pp_tokens(mem, directive_scanner, token_allocator, &directive->if_directive.condition);
...
static kefir_result_t next_define_replacement_list(struct kefir_mem *mem,
    ...
    while (token->klass != KEFIR_TOKEN_SENTINEL &&
        ...
        const struct kefir_token *allocated_token;
        res = kefir_token_allocator_emplace(mem, token_allocator, token, &allocated_token);
        REQUIRE_CHAIN(&res, kefir_token_buffer_emplace(mem, &directive->define_directive.replacement, allocated_token));
        REQUIRE_ELSE(res == KEFIR_OK, {
            kefir_token_free(mem, token);
...
static kefir_result_t next_define_function(struct kefir_mem *mem,
    ...
    REQUIRE_CHAIN(&res, next_define_replacement_list(mem, directive_scanner, token_allocator, directive, token));
    REQUIRE_ELSE(res == KEFIR_OK, {
...
static kefir_result_t next_define(struct kefir_mem *mem, struct kefir_preprocessor_directive_scanner *directive_scanner,
    ...
    if (token.klass == KEFIR_TOKEN_PUNCTUATOR && token.punctuator == KEFIR_PUNCTUATOR_LEFT_PARENTHESE) {
        ...
        REQUIRE_OK(next_define_replacement_list(mem, directive_scanner, token_allocator, directive, &token));
    }
    return KEFIR_OK;
}
...
static kefir_result_t next_error(struct kefir_mem *mem, struct kefir_preprocessor_directive_scanner *directive_scanner,
    ...
    struct kefir_string_buffer str;
    REQUIRE_OK(kefir_string_buffer_init(mem, &str, KEFIR_STRING_BUFFER_MULTIBYTE));
    ...
    kefir_size_t message_length;
    const char *message = kefir_string_buffer_value(&str, &message_length);
    REQUIRE_ELSE(message != NULL, {
        kefir_string_buffer_free(mem, &str);
        return KEFIR_SET_ERROR(KEFIR_OBJALLOC_FAILURE, "Failed to retrieve error preprocessor directive message");
    });

    char *message_copy = KEFIR_MALLOC(mem, sizeof(char) * (message_length + 1));
    REQUIRE_ELSE(message_copy != NULL, {
        kefir_string_buffer_free(mem, &str);
        return KEFIR_SET_ERROR(KEFIR_MEMALLOC_FAILURE, "Failed to allocate error preprocessor directive message");
    });
    strncpy(message_copy, message, message_length);

    directive->type = type;
    directive->error_message.content = message_copy;
    directive->error_message.length = message_length;
    REQUIRE_OK(kefir_string_buffer_free(mem, &str));
...
static kefir_result_t next_pp_token(struct kefir_mem *mem,
    ...
    do {
        ...
        if (token->klass == KEFIR_TOKEN_SENTINEL) {
            ...
            REQUIRE_OK(kefir_token_buffer_emplace(mem, &directive->pp_tokens, token));
            directive_scanner->newline_flag =
                token->klass == KEFIR_TOKEN_PP_WHITESPACE && token->pp_whitespace.newline;
        }
        first_iter = false;
    } while (!directive_scanner->newline_flag);
    return KEFIR_OK;
}
...
kefir_result_t kefir_preprocessor_directive_scanner_next(struct kefir_mem *mem,
    ...
    if (!directive_scanner->newline_flag) {
        REQUIRE_OK(next_pp_token(mem, directive_scanner, token_allocator, directive));
    ...
    switch (directive_type) {
        ...
        case KEFIR_PREPROCESSOR_DIRECTIVE_ELIF:
            REQUIRE_OK(next_elif(mem, directive_scanner, token_allocator, directive));
            break;
        ...
        case KEFIR_PREPROCESSOR_DIRECTIVE_PRAGMA:
            REQUIRE_OK(next_pragma(mem, directive_scanner, token_allocator, directive));
            break;
...
kefir_result_t kefir_preprocessor_directive_free(struct kefir_mem *mem,
    ...
    switch (directive->type) {
        ...
        case KEFIR_PREPROCESSOR_DIRECTIVE_ERROR:
        case KEFIR_PREPROCESSOR_DIRECTIVE_WARNING:
            KEFIR_FREE(mem, (char *) directive->error_message.content);
            break;
