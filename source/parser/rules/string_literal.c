        ...
        case KEFIR_STRING_LITERAL_TOKEN_UNICODE8:
            REQUIRE_ALLOC(result,
                          KEFIR_AST_NODE_BASE(kefir_ast_new_string_literal_multibyte(
                              mem, parser->ast_arena, (const char *) token->string_literal->literal, token->string_literal->length)),
        ...
        case KEFIR_STRING_LITERAL_TOKEN_UNICODE16:
            REQUIRE_ALLOC(result,
                          KEFIR_AST_NODE_BASE(kefir_ast_new_string_literal_unicode16(
                              mem, parser->ast_arena, (kefir_char16_t *) token->string_literal->literal, token->string_literal->length)),
                          "Failed to allocate AST string literal");
            break;
        case KEFIR_STRING_LITERAL_TOKEN_UNICODE32:
            REQUIRE_ALLOC(result,
                          KEFIR_AST_NODE_BASE(kefir_ast_new_string_literal_unicode32(
                              mem, parser->ast_arena, (kefir_char32_t *) token->string_literal->literal, token->string_literal->length)),
                          "Failed to allocate AST string literal");
            break;
        ...
        case KEFIR_STRING_LITERAL_TOKEN_WIDE:
            REQUIRE_ALLOC(result,
                          KEFIR_AST_NODE_BASE(kefir_ast_new_string_literal_wide(
                              mem, parser->ast_arena, (kefir_wchar_t *) token->string_literal->literal, token->string_literal->length)),
                          "Failed to allocate AST string literal");
            break;
    }
