...
static kefir_result_t builder_callback(struct kefir_mem *mem, struct kefir_parser_ast_builder *builder, void *payload) {
    ...
    if (PARSER_TOKEN_IS_PUNCTUATOR(parser, 0, KEFIR_PUNCTUATOR_DOUBLE_PLUS)) {
        ...
        REQUIRE_OK(PARSER_SHIFT(parser));
        kefir_result_t res =
            kefir_parser_ast_builder_scan_impl(mem, builder, KEFIR_PARSER_RULE_FN(parser, unary_expression), NULL);
        if (res == KEFIR_NO_MATCH) {
            ...
            REQUIRE_OK(PARSER_SHIFT(parser));
            REQUIRE_MATCH_OK(
                &res, kefir_parser_ast_builder_scan_impl(mem, builder, KEFIR_PARSER_RULE_FN(parser, type_name), NULL),
        ...
        kefir_result_t res =
            kefir_parser_ast_builder_scan_impl(mem, builder, KEFIR_PARSER_RULE_FN(parser, type_name), NULL);
        if (res == KEFIR_NO_MATCH) {
        ...
        REQUIRE_OK(
            kefir_parser_ast_builder_scan_impl(mem, builder, KEFIR_PARSER_RULE_FN(parser, postfix_expression), NULL));
    }
    return KEFIR_OK;
}
