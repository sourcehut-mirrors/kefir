...
static kefir_result_t parse_compound_statement(struct kefir_mem *mem, struct kefir_parser_ast_builder *builder,
    ...
    while (PARSER_TOKEN_IS_PRAGMA(builder->parser, 0)) {
        ...
        kefir_result_t res = kefir_parser_scan_pragma(mem, &builder->parser->pragmas, &pragmas, token->pragma->pragma,
...
static kefir_result_t builder_callback(struct kefir_mem *mem, struct kefir_parser_ast_builder *builder, void *payload) {
    ...
    REQUIRE_CHAIN(&res, parse_compound_statement(mem, builder, &attributes));
